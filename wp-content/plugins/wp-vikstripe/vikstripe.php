...
add_action('init', function()
{
	if (class_exists('JFactory'))
	{
        ...
        /**
         * Auto-load the VikRestaurants Stripe version to properly register all
         * the custom hooks that might trigger outside the payment process.
         * 
         * @since 2.2
         */
        JLoader::import('vikrestaurants.stripe', dirname(__FILE__));
	}
}, 1);

/**
 * Added support to VikUpdater 2.0
 * 
 * @since 1.3
 */
add_filter('vikupdater_subscribe_plugins', function($subscribers) {
    $subscribers[] = [
        // this is the VirtueMart SKU
        'sku' => 'stripe',
        ...
        'description'  => __('Stripe integration to collect payments through the Vik plugins.', 'vikstripe'),
    ];
...
add_filter('vikupdater_prepare_update_plugin_data', function($update, $manifest, $options) {
    ...
    if ($update->slug === 'vikstripe') {
        // register the plugin icons
        $update->icons = [
            '2x' => plugin_dir_url(__FILE__) . 'Stripe/logo-256x256.png',
            '1x' => plugin_dir_url(__FILE__) . 'Stripe/logo-128x128.png',
        ];
    }
...
/**
 * Fixes the messed up code in VikUpdater 1.4.4 to make sure that it
 * is able to properly obtain the latest version of the plugin.
 */
add_action('plugins_loaded', function() {
	// check whether VikUpdater 1.4.4 or lower is installed
    if (!class_exists('VikUpdaterLibrary')) {
    	// nope, we are probably using the latest version
        return;
    }
    
    // detach the callback that checks the updates in a useless way
    remove_action('init', ['VikUpdaterLibrary', 'checkUpdates']);
    // force a cache in the past to immediately prompt the update
    update_option('vikupdater_update_lastcheck', strtotime('-1 year'));
});
