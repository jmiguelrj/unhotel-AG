...
add_action('payment_after_admin_params_vikappointments', function(&$payment, &$config)
{
...
add_action('payment_after_begin_transaction_vikappointments', function(&$payment, &$html)
{
	// make sure the driver is Stripe
	if (!$payment->isDriver('stripe'))
	{
...
add_action('payment_before_validate_transaction_vikappointments', function($payment)
{
	// make sure the driver is Stripe
	if (!$payment->isDriver('stripe'))
	{
 ...
 * @since 1.0
 */
class VikAppointmentsStripePayment extends AbstractStripePayment
{
	/**
	 * @override
	 * Class constructor.
	...
	public function __construct($alias, $order, $params = array())
	{
		...
		$this->set('btnclass', 'vap-btn blue');
	}
