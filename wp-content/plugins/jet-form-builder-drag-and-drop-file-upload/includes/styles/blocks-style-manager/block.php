...
class Block {
	...
	public function render_block_styles( $block_content, $parsed_block, $wp_block ) {
		...
		if ( ! empty( $parsed_block['attrs'][ Registry::instance()->get_support_name() ] ) ) {
			...
			$force_print = apply_filters( 'jet-styles-manager/block/force-print-styles', false, $this );

			// Check if we're in preview context (JetFormBuilder preview)
			$is_preview = is_singular( 'jet-form-builder' ) && ! is_admin();

			if ( ! $force_print && ! $is_preview ) {
	...
	public function register_children( $parents_trace, $control_id, $args ) {
		...
		foreach ( $parents_trace as $parent ) {
			...
			if ( ! isset( $controls['children'][ $parent ] ) ) {
				_doing_it_wrong(
					'Block::register_children',
					'Parent control `' . esc_html( $parent ) . '` not found in the controls stack.',
					'1.0.0'
				);
				return;
			}
