...
class Mega_Menu_Walker extends \Walker_Nav_Menu {
	...
	public function start_el( &$output, $item, $depth = 0, $args = array(), $id = 0 ) {
		...
		if ( $this->is_mega_enabled( $item->ID ) ) {
			...
			$use_ajax = filter_var( $args->settings[ 'ajax-loading' ], FILTER_VALIDATE_BOOLEAN );
			...
			switch ( $content_type ) {
				case 'default':
					...
					if ( $is_content ) {
						update_post_meta( $mega_template_id, '_is_deps_ready', 'true' );
					}
			...
			$item_output .= sprintf( '<div class="jet-mega-menu-mega-container" data-template-id="%s" data-template-content="%s" data-position="%s"><div class="jet-mega-menu-mega-container__inner">%s</div></div>', $mega_template_id, $content_type, $position, $template_content );
