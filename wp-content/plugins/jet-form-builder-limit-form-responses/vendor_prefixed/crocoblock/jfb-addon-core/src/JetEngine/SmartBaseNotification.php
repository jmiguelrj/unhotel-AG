...
use JFB\LimitResponses\Vendor\JFBCore\SmartNotificationActionTrait;
abstract class SmartBaseNotification extends BaseNotification
{
    ...
    public function getSettingsWithGlobal()
    {
        ...
        return $this->getInstance()->get_settings_with_global($settings, $this->getGlobalOptionName());
    }
    ...
    public function do_action(array $settings, $notifications)
    {
        try {
            ...
            $this->_settings = isset($settings[$this->get_id()]) ? $settings[$this->get_id()] : array();
            $this->run_action();
            $notifications->log[] = \true;
