...
use JFB\LimitResponses\Vendor\JFBCore\BaseRenderField;
trait RenderField
{
    use AttributesTrait;
    use BaseRenderField;
    ...
    {
        return array('class' => array($this->main_field_class()), 'required' => $this->get_required_val(), 'name' => $this->get_field_name(), 'id' => $this->get_field_id(), 'data-field-name' => $this->get_arg('name', 'field_name'), 'value' => $this->get_arg('default'));
    }
    public function include_field_desc($custom_desc = \false)
    {
        $args = \array_merge($this->args, array('desc' => \false !== $custom_desc ? $custom_desc : $this->get_arg('desc')));
        return $this->builder->get_field_desc($args);
    }
    public function include_field_label($custom_label = \false, $custom_mark = \false)
    {
        $args = \array_merge($this->args, array('label' => \false !== $custom_label ? $custom_label : $this->get_arg('label')));
    ...
    {
        return $this->builder->get_field_name($name ? $name : $this->get_arg('name', 'field_name'));
    }
