...
namespace JFB\LimitResponses\Vendor\JFBCore\JetEngine;
...
abstract class NotificationsManager
{
     ...
     * @return array
     */
    public abstract function register_notification();
    ...
    public function on_plugin_init()
    {
        ...
        add_filter('jet-engine/forms/booking/notification-types', array($this, 'register_notifications'));
        add_action('jet-engine/forms/editor/before-assets', array($this, 'register_assets_before'));
        add_action('jet-engine/forms/editor/assets', array($this, 'maybe_localize_notifications'), 0);
        add_action('jet-engine/forms/editor/assets', array($this, 'register_assets'));
    }
    ...
    public function setup_notifications()
    {
        ...
        foreach ($types as $type) {
            ...
            add_action('jet-engine/forms/booking/notifications/fields-after', array($type, 'notification_fields'));
            add_action('jet-engine/forms/booking/notification/' . $type->get_id(), array($type, 'do_action'), 10, 2);
        }
    }
    /**
    ...
    {
        $prepared = array();
        $methods = array(array('name' => 'get_id', 'prop' => 'id'), array('name' => 'editor_labels', 'prop' => '__labels'), array('name' => 'editor_labels_help', 'prop' => '__help_messages'), array('name' => 'visible_attributes_for_gateway_editor', 'prop' => '__gateway_attrs'), array('name' => 'action_data'));
        foreach ($this->notifications as $type) {
    ...
    public function prepare_localize_notification($notification, $methods)
    {
        ...
        foreach ($methods as $method) {
            if (empty($method['prop'])) {
                $response = \array_merge($response, \call_user_func(array($notification, $method['name'])));
                continue;
            }
