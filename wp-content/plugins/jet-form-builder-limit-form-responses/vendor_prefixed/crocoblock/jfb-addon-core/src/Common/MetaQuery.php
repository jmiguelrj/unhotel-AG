...
namespace JFB\LimitResponses\Vendor\JFBCore\Common;
...
class MetaQuery
{
    ...
    {
        $params = \array_merge(array('id' => 0, 'key' => ''), $params);
        $post_id = $params['id'];
        $key = $params['key'];
        if (!$post_id) {
            $post_id = get_the_ID();
        }
        $meta = get_post_meta($post_id, $key, \true);
        if ($meta) {
            /** For compatibility with php 7.0.x */
            if (\defined('JSON_INVALID_UTF8_IGNORE')) {
                return \json_decode($meta, \true, 512, \JSON_INVALID_UTF8_IGNORE);
            }
    ...
    {
        $params = \array_merge(array('id' => 0, 'key' => '', 'value' => array()), $params);
        $post_id = $params['id'];
        $key = $params['key'];
        $value = $params['value'];
        if (!$post_id) {
