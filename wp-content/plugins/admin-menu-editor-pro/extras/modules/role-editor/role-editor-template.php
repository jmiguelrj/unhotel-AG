...
<div id="ame-role-editor-root">
	...
	<div id="rex-main-ui" data-bind="visible: isLoaded, css: {'rex-highlight-changes': highlightUnsavedChangesEnabled}"
	     style="display: none;">
		<div id="rex-content-container">
			...
			<div id="rex-category-sidebar">
				...
				<ul data-bind="template: {name: 'rex-nav-item-template', data: rootCategory}"
				    class="dev-rex-fancy-capability-counts"
		...
		<div id="rex-action-sidebar" class="metabox-holder">
			<div id="rex-main-buttons" class="postbox ws-ame-postbox">
				...
				submit_button(
					'Reset roles',
					'rex-action-button',
					'rex-reset-roles-button',
					false,
					array('data-bind' => 'ameOpenDialog: "#rex-reset-roles-dialog"')
				);
				?>
				<div class="rex-action-separator"></div>
				<?php
				...
				submit_button(
					'User screens',
					'rex-action-button',
					'rex-user-screens-button',
					false,
					array('data-bind' => 'ameOpenDialog: "#rex-user-screens-dialog"')
				);
				?>
				...
				<form action="<?php echo esc_attr(add_query_arg('noheader', '1', $moduleTabUrl)); ?>"
					...
					<input type="hidden" name="addVisibleUsers" value=""
					       data-bind="value: lastVisibleUsersData">
		...
		<div id="rex-category-list-options" class="rex-dropdown" style="display: none;">
			<label class="rex-dropdown-item">
				<input type="checkbox" data-bind="checked: showNumberOfCapsEnabled">
				Show number of capabilities
			</label>
			...
			<label class="rex-dropdown-item rex-dropdown-sub-item">
				<input type="checkbox" data-bind="checked: showTotalCapCountEnabled, enable: showNumberOfCapsEnabled">
				Show total
			</label>
			<label class="rex-dropdown-item rex-dropdown-sub-item">
				<input type="checkbox" data-bind="checked: showZerosEnabled, enable: showNumberOfCapsEnabled">
				Show zeros
			</label>
		</div>
		...
		<div id="rex-general-view-options" class="rex-dropdown" style="display: none;">
			...
			<label class="rex-dropdown-item">
				<input type="checkbox" data-bind="checked: highlightUnsavedChangesEnabled">
		...
		<div id="rex-add-capability-dialog"
			...
			<form data-bind="submit: addCapabilityDialog.onConfirm.bind(addCapabilityDialog)">
				...
				<input type="text" data-bind="textInput: addCapabilityDialog.capabilityName"
				       id="rex-new-capability-name"
				       maxlength="150">
		...
		<div id="rex-add-role-dialog"
			...
			<form data-bind="submit: addRoleDialog.onConfirm.bind(addRoleDialog)">
				<p class="rex-dialog-section">
					...
					<input type="text" data-bind="textInput: addRoleDialog.roleDisplayName"
					       id="rex-new-role-display-name"
		...
		<div id="rex-rename-role-dialog"
			...
			<form data-bind="submit: renameRoleDialog.onConfirm.bind(renameRoleDialog)">
				...
				<p class="rex-dialog-section">
					...
					<input type="text" data-bind="textInput: renameRoleDialog.newDisplayName"
					       id="rex-edited-role-display-name"
					       maxlength="150" placeholder="New Role Name">
				</p>
		...
		<div id="rex-reset-roles-dialog"
		     data-bind="ameDialog: resetRolesDialog, ameEnableDialogButton: resetRolesDialog.isResetButtonEnabled"
		     title="Reset roles to defaults"
		     style="display: none;" class="rex-dialog">

			<!-- ko if: resetRolesDialog.isRendered -->
			<div id="rex-rr-load-overlay" data-bind="visible: !(resetRolesDialog.defaultRoleDataLoaded())">
				<div id="rex-rr-loading-message" data-bind="visible: resetRolesDialog.isLoadingRoleData()">
					<span class="spinner is-active"></span>
					Loading default roles...
				</div>
				<div id="rex-rr-load-error" data-bind="if: resetRolesDialog.roleDataLoadError">
					<h4>Error loading default roles</h4>
					<span data-bind="text: resetRolesDialog.roleDataLoadError"></span>
				</div>
			</div>
			<div id="rex-rr-reset-roles-content">
				<div class="rex-dialog-section">
					<h4>How this works</h4>
					<div id="rex-rr-info-text">
						<ul>
							<li>
								This will reset the default user roles to their default capabilities.
								It can also recreate default roles that have been deleted.
							</li>
							<li>
								<span
									data-bind="class: ('rex-rr-version-description-' + resetRolesDialog.versionMatchType())">
									It uses the default role and capability data for
									<span data-bind="text: resetRolesDialog.selectedVersionText"></span>.
								</span>
								<span data-bind="text: resetRolesDialog.versionMatchNotice,
                                class: ('rex-rr-version-notice-' + resetRolesDialog.versionMatchType())"></span>
							</li>
							<li>
								It will <em>not</em> reset custom roles that have been added by plugins.
							</li>
							<li>
								When you click "<span data-bind="text: resetRolesDialog.resetButtonLabel"></span>",
								the changes will be loaded in the editor. Review the results before saving.
							</li>
						</ul>
					</div>
				</div>
				<div class="rex-dialog-section">
					<h4>Select roles to reset</h4>
					<div id="rex-rr-role-merge-view" data-bind="if: resetRolesDialog.roleMergeVm">
						<!--suppress HtmlUnknownTag -->
						<rex-role-merge-view params="model: resetRolesDialog.roleMergeVm"></rex-role-merge-view>
					</div>
				</div>
			</div>
			<!-- /ko -->
			...
			<!-- ko if: !resetRolesDialog.isRendered() -->
			<div style="height: 500px">(Placeholder.)</div>
			<!-- /ko -->
		</div>
	</div>

	<div id="rex-user-screens-dialog"
	     data-bind="ameDialog: userScreensDialog, ameEnableDialogButton: userScreensDialog.isConfirmButtonEnabled"
	     title="User screens"
	     style="display: none;" class="rex-dialog">

		<!-- ko if: userScreensDialog.isRendered -->
		<form data-bind="submit: userScreensDialog.onConfirm.bind(userScreensDialog)">
			<div class="ws_dialog_subpanel rex-dialog-section">
				<h4>Edit User screen</h4>
				<fieldset>
					<p>
						<label>
							<input type="checkbox" data-bind="checked: userScreensDialog.settings.otherRolesOnEditScreen">
							Show the "Other Roles" field
						</label>
					</p>
					<p>
						<label>
							<input type="checkbox" data-bind="checked: userScreensDialog.settings.editLinkOnEditScreen">
							Add an "Edit" link to the "Additional Capabilities" field
						</label>
					</p>
				</fieldset>
			</div>
			...
			<div class="ws_dialog_subpanel rex-dialog-section">
				<h4>Add User screen</h4>
				<fieldset>
					<label>
						<input type="checkbox" data-bind="checked: userScreensDialog.settings.otherRolesOnAddScreen">
						Show the "Other Roles" field
					</label>
				</fieldset>
			</div>
			...
			<div class="ws_dialog_subpanel rex-dialog-section">
				...
				<fieldset>
					<p>
						<label>
							<input type="checkbox" data-bind="checked: userScreensDialog.settings.capabilitiesLinkInUsersTable">
							Add a "Capabilities" link to each user
						</label>
					</p>
				</fieldset>
			</div>
	...
	<script type="text/html" id="rex-nav-item-template">
		...
		<span class="rex-nav-toggle" data-bind="
			...
			<span class="rex-capability-count"
			      data-bind="css: {
			        'rex-all-capabilities-enabled': areAllPermissionsEnabled,
			        'rex-no-capabilities-enabled': (enabledCapabilityCount() === 0),
			      },
		...
		<table class="widefat rex-permission-table">
			...
			<tr data-bind="visible: isVisible">
				<th scope="row" data-bind="attr: {title: subtitle}">
					<label>
						<input type="checkbox"
						       data-bind="checked: areAllPermissionsEnabled, enable: areAnyPermissionsEditable">
	...
	<script type="text/html" id="rex-permission-template">
		<div class="rex-permission" data-bind="
					...
					css: {
							...
							'rex-is-enabled': capability.isEnabledForSelectedActor,
							'rex-is-disabled': (!capability.isEnabledForSelectedActor()),
							'rex-is-modified': capability.isModifiedForSelectedActor
						}">
