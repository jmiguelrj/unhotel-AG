...
use YahnisElsts\AdminMenuEditor\ImportExport\ameImportResult;
use YahnisElsts\AdminMenuEditor\RoleEditor\ImportExport\RoleDataTransfer;
use YahnisElsts\JsonReferenceHelper\JsonReferenceException;
use YahnisElsts\JsonReferenceHelper\JsonReferenceHelper;
...
class ameRoleEditor extends amePersistentProModule {
	...
	const GET_DEFAULT_ROLES_ACTION = 'ws_ame_rex_get_default_roles';
	...
	public function __construct($menuEditor) {
		...
		add_action('wp_ajax_' . self::GET_DEFAULT_ROLES_ACTION, [$this, 'ajaxGetDefaultRoles']);

		$this->localTabStyles['ame-role-editor-styles'] = 'role-editor.css';
		add_action('admin_menu_editor-enqueue_styles-import', [$this, 'enqueueImportTabStyles']);

		add_action('admin_init', [$this, 'maybeLoadUserUiExtensions']);
		add_action('admin_menu_editor-load_tab-' . $this->tabSlug, [$this, 'addTempVisibleUserFilter']);
	...
	public function enqueueTabScripts() {
		...
		$roleMergeScript = $this->registerRoleMergeScript();
		...
		$moduleSettings = $this->loadSettings();
		...
		$scriptData = [
			...
			'editableRoles'             => ameUtils::get($moduleSettings, 'editableRoles', new stdClass()),
			'userUiSettings'            => ameUtils::get($moduleSettings, 'userUiSettings', new stdClass()),

			'userPreferences'        => $userPreferences,
			'adminAjaxUrl'           => self_admin_url('admin-ajax.php'),
			'updatePreferencesNonce' => wp_create_nonce(self::UPDATE_PREFERENCES_ACTION),
			'getDefaultRolesNonce'   => wp_create_nonce(self::GET_DEFAULT_ROLES_ACTION),
		];
	...
	private function registerRoleMergeScript(): ScriptDependency {
		static $roleMergeScript = null;
		if ( $roleMergeScript !== null ) {
			return $roleMergeScript;
		}

		$roleMergeScript = ScriptDependency::create(
			plugins_url('role-merge.js', __FILE__),
			'ame-role-merge-component',
			__DIR__ . '/role-merge.js',
			[
				'ame-knockout',
				'ame-ko-extensions',
				'ame-actor-manager',
	...
	protected function analyseMenuItem($item, $parent = null) {
		...
		if ( empty($item['custom']) && !empty($item['defaults']) && !empty($capability) && empty($item['separator']) ) {
			...
			$relatedComponents = [];
			if ( !empty($hook) ) {
				$reflections = AmeReflectionCallable::getHookReflections($hook);
	...
	protected function getComponentIdFromPath($absolutePath) {
		$component = ameUtils::getComponentFromPath($absolutePath);
		if ( $component ) {
			return $component['type'] . ':' . $component['path'];
		}
		return null;
	}
	...
	public function handleSettingsForm($post = []) {
		...
		if ( !empty($post['addVisibleUsers']) && is_string($post['addVisibleUsers']) ) {
			$parsedUsers = $this->parseAdditionalVisibleUsers($post['addVisibleUsers']);
			if ( !empty($parsedUsers) ) {
				$redirectParams['addVisibleUsers'] = implode(',', $parsedUsers);
			}
		}

		$validator = $this->makeSettingsValidator($post['settings']);
		list($totalChanges,) = $this->applySettingsChanges($validator, $shouldUpdateNetwork, true);

		if ( $totalChanges <= 0 ) {
			$redirectParams['no-changes-made'] = 1;
			wp_safe_redirect($this->getTabUrl($redirectParams));
			exit;
		}
	...
	/**
	 * @param array|string $incomingSettings
	 * @param int $enabledChangeTypes
	 * @return ameRexSettingsValidator
	 */
	private function makeSettingsValidator(
		$incomingSettings,
		int $enabledChangeTypes = ameRexSettingsValidator::ALL_CHANGES
	): ameRexSettingsValidator {
		$moduleSettings = $this->loadSettings();

		return new ameRexSettingsValidator(
			$incomingSettings,
			$this->getAllCapabilities([wp_get_current_user()]),
			$this->getUserDefinedCaps(),
			ameUtils::get($moduleSettings, 'editableRoles', []),
			ameUtils::get($moduleSettings, 'userUiSettings', []),
	...
	private function applySettingsChanges(
		ameRexSettingsValidator $validator,
		bool                    $shouldUpdateNetwork,
		                        $storeErrors = false
	): array {
		...
		//Save editable roles.
		if ( $validator->hasNewEditableRoleSettings() ) {
			$this->settings['editableRoles'] = $validator->getNewEditableRoleSettings();
			$moduleSettingsChanged = true;
		}

		//Save user UI extension settings.
		if ( $validator->hasNewUserUiSettings() ) {
			$this->settings['userUiSettings'] = $validator->getNewUserUiSettings();
			$moduleSettingsChanged = true;
		}
	...
	private function updateNetworkRoles($roleData) {
		...
		//When a site uses an external object cache (e.g. Redis), we might need to remove the '$prefix_user_roles'
		//option from the cache to force the caching plugin to load the updated roles from the database.
		//This could be slow because it involves switching to each site, so it's off by default.
		$shouldClearCache = is_callable('wp_cache_switch_to_blog')
			&& is_callable('wp_using_ext_object_cache')
	 ...
	 * @return bool
	 */
	public function userCanAccessModule($userId = null) {
	 ...
	 * @return array
	 */
	public function getEffectiveEditableRoles() {
	...
	public function ajaxGetDefaultRoles() {
		check_ajax_referer(self::GET_DEFAULT_ROLES_ACTION);
		...
		$roleDataFile = __DIR__ . '/data/default-role-caps.json';
		if ( !file_exists($roleDataFile) ) {
			wp_send_json(['error' => 'Role data file not found']);
			exit;
		}

		//phpcs:ignore WordPressVIPMinimum.Performance.FetchingRemoteData.FileGetContentsUnknown -- Local file.
		$fileContent = file_get_contents($roleDataFile);
		if ( $fileContent === false ) {
			wp_send_json(['error' => 'Failed to read role data file']);
			exit;
		}

		$roleData = json_decode($fileContent, true);
		if ( !is_array($roleData) ) {
			wp_send_json(['error' => 'Role data is not valid']);
			exit;
		}
		...
		try {
			list($defaultRoles, $versionRange) = $this->findDefaultRolesForWpVersion(
				$roleData,
				$installedWpVersion
			);
		} catch (JsonReferenceException $e) {
			wp_send_json(['error' => 'Role data contains invalid JSON references']);
			exit;
		} catch (InvalidArgumentException $e) {
			wp_send_json(['error' => '(Exception) ' . $e->getMessage()]);
			exit;
		}
		...
		if ( !$defaultRoles ) {
			wp_send_json([
				'error' => 'This plugin does not have the default role data for the installed' .
					' WordPress version, or for any sufficiently close version.',
			]);
			exit;
		}

		//Convert the role collection to the format expected by the script.
		$convertedRoleCollection = [];
		foreach ($defaultRoles as $name => $role) {
			$convertedRoleCollection[] = [
				'name'         => $name,
				'displayName'  => $role['name'],
				'capabilities' => !empty($role['capabilities']) ? $role['capabilities'] : [],
			];
		}
		...
		wp_send_json([
			...
			'defaultRoles'       => $convertedRoleCollection,
			'wpVersionRange'     => $versionRange,
	 ...
	 * @param string $installedWpVersion
	 * @return array [defaultRoles, versionRange]
	 * @throws JsonReferenceException
	 */
	private function findDefaultRolesForWpVersion($roleData, $installedWpVersion) {
		//Load the JSON reference helper. The role data can contain references to other parts of the document.
		require_once AME_ROOT_DIR . '/extras/JsonReferenceHelper.php';

		//The WP version number should *always* be a string, but plugins can change it and break things.
		if ( !is_string($installedWpVersion) ) {
			throw new InvalidArgumentException('Invalid WordPress version: not a string');
		}

		//Do we perchance have role data for this exact version? It's unlikely, but let's check.
		$exactMatch = JsonReferenceHelper::get($roleData, '/versions/' . $installedWpVersion, null);
		if ( $exactMatch !== null ) {
			return [
				$exactMatch,
				['start' => $installedWpVersion, 'end' => $installedWpVersion, 'matchType' => 'exact'],
			];
		}

		//Look for a version range that includes the installed version.
		if ( isset($roleData['ranges']) && is_array($roleData['ranges']) ) {
			foreach ($roleData['ranges'] as $range) {
				$start = $range['start'];
				$end = $range['end'];
				if (
					(version_compare($installedWpVersion, $start) >= 0)
					&& (version_compare($installedWpVersion, $end) <= 0)
				) {
					try {
						$defaultRoles = JsonReferenceHelper::maybeResolveReference($roleData, $range['roles']);
						if ( $defaultRoles !== null ) {
							return [$defaultRoles, ['start' => $start, 'end' => $end, 'matchType' => 'range']];
						}
					} catch (JsonReferenceException $e) {
						//Skip ranges that contain invalid references.
						continue;
					}
				}
			}
		...
		//Look for the most recent version that has the same major.minor numbers as the installed
		//version. Failing that, look for the most recent version that has the same major version.
		if ( !empty($roleData['versions']) && is_array($roleData['versions']) ) {
			$installedVersionParts = explode('.', $installedWpVersion);
			$installedMajorMinor = implode('.', array_slice($installedVersionParts, 0, 2));
			$installedMajor = isset($installedVersionParts[0]) ? $installedVersionParts[0] : '';

			$bestMajorMinorMatch = null;
			$bestMajorMatch = null;

			foreach ($roleData['versions'] as $version => $defaultRoles) {
				$versionParts = explode('.', $version);
				$majorMinor = $versionParts[0] . '.' . $versionParts[1];
				$major = $versionParts[0];

				if ( $majorMinor === $installedMajorMinor ) {
					if (
						($bestMajorMinorMatch === null)
						|| (version_compare($version, $bestMajorMinorMatch) > 0)
					) {
						$bestMajorMinorMatch = $version;
					}
				}
				...
				if ( $major === $installedMajor ) {
					if (
						($bestMajorMatch === null)
						|| (version_compare($version, $bestMajorMatch) > 0)
					) {
						$bestMajorMatch = $version;
					}
				}
			}

			//Prefers the best match with the same major.minor version.
			if ( $bestMajorMinorMatch !== null ) {
				return [
					JsonReferenceHelper::maybeResolveReference(
						$roleData,
						$roleData['versions'][$bestMajorMinorMatch]
					),
					['start' => $bestMajorMinorMatch, 'end' => $bestMajorMinorMatch, 'matchType' => 'majorMinor'],
				];
			}

			if ( $bestMajorMatch !== null ) {
				return [
					JsonReferenceHelper::maybeResolveReference(
						$roleData,
						$roleData['versions'][$bestMajorMatch]
					),
					['start' => $bestMajorMatch, 'end' => $bestMajorMatch, 'matchType' => 'major'],
				];
			}
		}

		return [null, ['start' => $installedWpVersion, 'end' => $installedWpVersion, 'matchType' => 'none']];
	}

	public function getExportOptionLabel() {
		return 'Editable roles & User screens settings';
	}

	public function getExportableComponents(): array {
		return [
			$this->getModuleId()     => ameExportableComponent::builder($this->getExportOptionLabel())
				->description($this->getExportOptionDescription())
				->exportCallback([$this, 'exportSettings'])
				->importCallback([$this, 'importSettings'])
				->build(),
			'roles-and-capabilities' => ameExportableComponent::builder('Roles and capabilities')
				->advanced()
				->exportCallback(function () {
					require_once __DIR__ . '/role-export-feature.php';
					$transfer = RoleDataTransfer::create(ameRoleUtils::get_roles(), $this->menuEditor);
					return $transfer->serialize(true);
				})
				->importCallback(function ($newSettings, $configFieldValue) {
					if ( empty($configFieldValue) || !is_string($configFieldValue) ) {
						return ameImportResult::error('Role import configuration is invalid.');
					}

					//Parse the import configuration.
					$s = new SchemaFactory();
					$configSchema = $s->json(
						$s->struct([
							'roles'                => $s->arr($s->string()->min(1)->max(500))->max(500),
							'localOnlyCapStrategy' => $s->enum(['leaveUnchanged', 'disable']),
					...
					$selectedRoles = $importConfig['roles'];
					$localOnlyCapStrategy = $importConfig['localOnlyCapStrategy'];

					if ( empty($selectedRoles) ) {
						return ameImportResult::nothing('No roles were selected for import.');
					}

					require_once __DIR__ . '/role-export-feature.php';
					try {
						$transfer = RoleDataTransfer::deserialize($newSettings);
					} catch (Exception $e) {
						return ameImportResult::error('Error parsing imported role data: ' . $e->getMessage());
					}

					$incomingCapabilitiesLookup = array_flip($transfer->getAllCapabilities());

					$importedRoles = array_intersect_key($transfer->getRoles(), array_flip($selectedRoles));
					$existingRoles = ameRoleUtils::get_roles();
					$newRoleSettings = [];

					foreach ($importedRoles as $name => $roleDetails) {
						$baseRole = $existingRoles->get_role($name);
						if ( !$baseRole ) {
							$capabilities = $roleDetails->getCapabilities();
						} else {
							$capabilities = $this->computeMergedCapabilities(
								$baseRole->capabilities,
								$roleDetails->getCapabilities(),
								$incomingCapabilitiesLookup,
								$localOnlyCapStrategy
							);
						}

						$newRoleSettings[] = [
							'name'         => $name,
							'displayName'  => $roleDetails->getDisplayName(),
							'capabilities' => $capabilities,
						];
					}

					$validator = $this->makeSettingsValidator(
						['roles' => $newRoleSettings],
						ameRexSettingsValidator::CREATE_ROLES | ameRexSettingsValidator::EDIT_ROLES
					);
					list($totalChanges, $errors) = $this->applySettingsChanges($validator, false, false);

					if ( $totalChanges === 0 ) {
						if ( empty($errors) ) {
							$result = ameImportResult::success('No changes were made');
						} else {
							$result = ameImportResult::error('Error importing roles');
						}
					} else {
						$message = sprintf('%d changes applied', $totalChanges);
						//In Multisite, we only apply the imported role settings to the current site.
						//The user can optionally update other sites via the "Roles" tab.
						if ( is_multisite() ) {
							$message .= '. (Note: Only the current site was updated. If you want to' .
								' apply these changes to all sites, go to the "Roles" tab and click "Update All Sites".)';
						}
						$result = ameImportResult::success($message);
					}
				...
				->importConfigHtmlCallback(function ($pendingNewSettings, $configFieldName) {
					require_once __DIR__ . '/role-export-feature.php';
					try {
						$transfer = RoleDataTransfer::deserialize($pendingNewSettings);
					} catch (Exception $e) {
						return '<p class="ame-error-message">Error parsing imported role data: '
							. esc_html($e->getMessage()) . '</p>';
					}

					$scriptData = [
						'incomingRoleData' => $transfer->serialize(false),
						'coreCapabilities' => array_keys($this->getDefaultCapabilities()),
					];

					ScriptDependency::create(
						plugins_url('import-helper.js', __FILE__),
						'ame-role-import-helper',
						__DIR__ . '/import-helper.js',
						[
							'jquery-qtip',
							'ame-actor-manager',
							'ame-knockout',
							$this->registerRoleMergeScript(),
						]
					)
						->addJsVariable('wsAmeRoleImportHelperData', $scriptData)
						->setInFooter()
						->enqueue();

					$escapedFieldName = esc_attr($configFieldName);

					return <<<EOT
						<div id="ame-rex-import-roles-merge-container">
							<rex-role-merge-view params="model: importMergeVm"></rex-role-merge-view>
							<input type="hidden" name="$escapedFieldName" data-bind="value: configFieldValue" />
						</div>
EOT;
				})
				->build(),
	...
	public function enqueueImportTabStyles() {
		wp_enqueue_auto_versioned_style(
			'ame-role-merge-styles',
			plugins_url('role-merge-styles.css', __FILE__)
		);
	}

	/**
	 * @param array<string,mixed> $baseCaps
	 * @param array<string,boolean> $incomingCaps
	 * @param array<string,mixed> $allIncomingCaps Indexed by capability name. All caps from the incoming data/imported file.
	 * @param string $localOnlyCapStrategy
	 * @return array<string,boolean>
	 */
	private function computeMergedCapabilities(
		array  $baseCaps,
		array  $incomingCaps,
		array  $allIncomingCaps,
		string $localOnlyCapStrategy
	): array {
		$mergedCaps = [];
		$relevantCaps = array_keys(array_merge($baseCaps, $incomingCaps));
		foreach ($relevantCaps as $capName) {
			$baseState = array_key_exists($capName, $baseCaps) ? boolval($baseCaps[$capName]) : null;
			$incomingState = array_key_exists($capName, $incomingCaps) ? boolval($incomingCaps[$capName]) : null;

			$isLocalOnlyCap = (
				array_key_exists($capName, $baseCaps)
				&& !array_key_exists($capName, $allIncomingCaps)
			);
			...
			switch ($action) {
				case 'leaveUnchanged':
					if ( $baseState !== null ) {
						$mergedCaps[$capName] = $baseState;
					}
					break;
				case 'disable':
					//No need to do anything, just leave the capability out of the merged list.
					break;
				case 'acceptIncoming':
				default:
					//Accept the incoming state. This is the default for most capabilities.
					if ( $incomingState !== null ) {
						$mergedCaps[$capName] = $incomingState;
					}
					break;
			}
		}
	...
	public function maybeLoadUserUiExtensions() {
		if ( !$this->userCanAccessModule() ) {
			return;
		}

		$extensionsSettings = ameUtils::get($this->loadSettings(), 'userUiSettings', []);
		new ameRexUserUiExtension($extensionsSettings, $this, $this->menuEditor);
	}

	/**
	 * Add a filter that ensures the selected user and any other temporarily visible users
	 * are included in the actor selector's user list.
	 *
	 * This is needed in two related scenarios:
	 * 1) When editing a specific user via a link from their profile page, that user will be set
	 *    as the "selected_actor" in the URL query string. But they might not be in the saved list
	 *    of visible users, so we need to add them temporarily.
	 * 2) When saving changes after editing a specific user, the user may select a different actor
	 *    before clicking "Save Changes". In that case, the edited user won't be the selected actor
	 *    any more, but they should still be visible in the list after the redirect.
	 */
	public function addTempVisibleUserFilter() {
		$extraUsersToLoad = [];

		$query = $this->menuEditor->get_query_params();
		//Always include the currently selected user, if any.
		if ( isset($query['selected_actor']) ) {
			$actorId = (string)$query['selected_actor'];
			if ( strpos($actorId, 'user:') === 0 ) {
				$userLogin = substr($actorId, strlen('user:'));
				if ( is_string($userLogin) && (strlen($userLogin) > 0) ) {
					$user = get_user_by('login', $userLogin);
					if ( $user ) {
						$extraUsersToLoad[] = $user->user_login;
					}
				}
			}
		}

		//If this request is part of the "save changes -> redirect back" flow, include any users
		//that were visible when the user clicked "Save Changes". This ensures that the user part
		//of the actor list looks the same after the redirect as it did before.
		if ( isset($query['addVisibleUsers']) ) {
			$moreUsers = $this->parseAdditionalVisibleUsers($query['addVisibleUsers']);
			$extraUsersToLoad = array_merge($extraUsersToLoad, $moreUsers);
		}

		//Just a precaution to avoid potential DoS by loading too many users.
		//Each user could potentially be an extra database query.
		$maxExtraUsers = 10;
		if ( count($extraUsersToLoad) > $maxExtraUsers ) {
			$extraUsersToLoad = array_slice($extraUsersToLoad, 0, $maxExtraUsers);
		}

		if ( !empty($extraUsersToLoad) ) {
			add_filter('admin_menu_editor-visible_users', function ($visibleUsers) use ($extraUsersToLoad) {
				return array_unique(array_merge($visibleUsers, $extraUsersToLoad));
			});
		}
	}

	/**
	 * Parse a list of user logins from a comma-separated string.
	 *
	 * Used to pass a list of users that should be visible after a "save changes" redirect.
	 * This typically only matters when editing a specific user via a link from their profile page.
	 * The current user doesn't need to be included in the list because they're always visible.
	 *
	 * Note: JSON would be more convenient here, but a JSON-encoded array of strings doesn't survive
	 * a trip through wp_safe_redirect(). A comma-separated list is more robust in this case.
	 *
	 * @param mixed $possibleUserList
	 * @return array
	 */
	private function parseAdditionalVisibleUsers($possibleUserList): array {
		if ( !is_string($possibleUserList) ) {
			return [];
		}

		$parsedUsers = explode(',', $possibleUserList);
		if ( !is_array($parsedUsers) ) {
			$parsedUsers = [];
		}
		$parsedUsers = array_filter(array_unique(array_map('strval', $parsedUsers)));

		//Remove the current user from the list.
		$currentUser = wp_get_current_user();
		if ( $currentUser ) {
			$parsedUsers = array_diff($parsedUsers, [$currentUser->user_login]);
		}

		//Re-index the array since some entries may have been removed.
