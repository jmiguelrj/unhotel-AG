...
class RexObservableCapabilityMap {
    ...
    getInitialCapabilityState(capabilityName) {
...
class RexBaseActor {
    ...
    isCapabilityStateModified(capability) {
...
class RexCategory {
    ...
    static fromJs(details, editor) {
        ...
        if (!details.variant) {
            ...
            throw new Error('Unknown category data variant: ' + (details.variant ?? 'undefined'));
        }
...
class RexObservableUserUiSettings {
...
var RexResetRolesFeature;
(function (RexResetRolesFeature) {
    const _ = wsAmeLodash;
    class RexResetRolesDialog extends RexBaseDialog {
        constructor(editor, ajaxUrl, ajaxNonce) {
            super();
            this.editor = editor;
            this.ajaxUrl = ajaxUrl;
            this.ajaxNonce = ajaxNonce;
            this.defaultRoleDataLoaded = ko.observable(false);
            this.roleDataLoadError = ko.observable('');
            this.isLoadingRoleData = ko.observable(false);
            this.installedWpVersion = ko.observable('');
            this.roleMergeVm = ko.observable(null);
            this.resetButtonLabel = 'Reset Selected Roles';
            ...
            this.selectedRoleDataVersion = ko.observable({
                defaultRoles: [],
                wpVersionRange: {
                    start: '',
                    end: '',
            ...
            this.versionMatchNotice = ko.pureComputed(() => {
                const range = this.selectedRoleDataVersion().wpVersionRange;
            ...
            this.versionMatchType = ko.pureComputed(() => this.selectedRoleDataVersion().wpVersionRange.matchType);
        }
        populateRoleOptions(roleData) {
            if (roleData.defaultRoles.length < 1) {
                ...
                const existingRoles = {};
                const incomingRoles = [];
                const allIncomingCapabilities = new Set();
                ...
                const vm = new AmeRoleMergeComponent.RoleMergeViewModel(existingRoles, incomingRoles, allIncomingCapabilities, (capName) => {
        ...
        isGetDefaultRolesResponse(data) {
            ...
            if ((typeof rec.installedWpVersion !== 'string')
            ...
            return !((typeof rec.wpVersionRange['start'] !== 'string')
                || (typeof rec.wpVersionRange['end'] !== 'string'));
        }
        onConfirm() {
            const vm = this.roleMergeVm();
            ...
            for (const roleName in selectedChanges) {
                ...
                if (existingRole !== null) {
                    ...
                    this.editor.addRole(roleName, item.displayName, 
                    //Discard null values since those simply mean "capability not set" and
                    //addRole() expects a map of only boolean values.
                    _.pickBy(item.changedCapabilities, (value) => value !== null));
                }
            }
            this.isOpen(false);
        }
        applyCapabilityChanges(role, newCapabilityStates) {
    ...
    RexResetRolesFeature.RexResetRolesDialog = RexResetRolesDialog;
})(RexResetRolesFeature || (RexResetRolesFeature = {}));
//endregion
...
class RexRoleEditor {
    constructor(data) {
        ...
        this.resetRolesDialog = new RexResetRolesFeature.RexResetRolesDialog(this, data.adminAjaxUrl, data.getDefaultRolesNonce);
