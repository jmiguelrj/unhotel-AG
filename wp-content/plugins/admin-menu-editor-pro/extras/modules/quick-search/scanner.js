...
import { condenseWhitespace, getElementTextForItemLabel, getRelativeAdminPageUrl, normalizeUrl, queryAdvancedSelector } from './utils';
...
export function parseAdminMenuItems($root, adminUrl, removableQueryArgs = []) {
    ...
    $root.find('#adminmenu li > a').each((_, element) => {
        ...
        if (url) {
            url = normalizeUrl(url, window.location.href, [], false).toString();
        }
