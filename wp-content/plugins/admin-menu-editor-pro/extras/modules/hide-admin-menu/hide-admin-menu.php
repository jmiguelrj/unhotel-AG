...
class ameAdminMenuHider {
	const HIDEABLE_ITEM_ID = 'hide_admin_menu';
	...
	public function __construct($menuEditor) {
		...
		ameMenu::add_components_with_visibility(['adminMenu']);
	...
	private function should_hide_admin_menu() {
		$grant_access = $this->get_grant_access();
		if ( empty($grant_access) ) {
			return false;
		}
	...
	public function output_option() {
		?>
		<label>
			<input type="checkbox" id="ws_ame_show_admin_menu" data-vis-components="<?php
			echo esc_attr(wp_json_encode(['adminMenu']));
			?>">
			Show the admin menu
