...
class BoxDimensions extends AbstractNumericControl {
	...
	public function __construct($settings = [], $params = [], $children = []) {
		...
		} else if ( $this->mainBinding instanceof BorderRadius ) {
			$this->dimensionNames = [
				'topLeft'     => 'Top left',
				'topRight'    => 'Top right',
	...
	public function renderContent(Renderer $renderer, Context $context) {
		if ( !($this->mainBinding instanceof CssBoxDimensions) ) {
		...
		echo HtmlHelper::tag('fieldset', [
			...
			'style'              => $this->styles,
			'data-ac-setting-id' => $this->mainBinding->getId(),
		...
		foreach ($order as $key => $label) {
			$setting = $this->mainBinding->getChild($key);
			if ( !$setting ) {
				continue;
			}
			...
			echo HtmlHelper::tag(
				'input',
				array_merge(
					$this->getBasicInputAttributes($numberConfig),
					[
						'name'     => $this->getFieldName($context, $key),
		...
		$this->renderUnitDropdown($unitSetting, [
			'name'               => $this->getFieldName($context, 'unit'),
			'id'                 => $unitElementId,
			'class'              => 'ame-box-dimensions-unit-selector',
			'data-slider-ranges' => wp_json_encode($this->getSliderRanges($numberConfig)),
			'disabled'           => !$this->isEnabled($context),
		]);
		...
		$firstSetting = $this->mainBinding->getChild(\ameUtils::getFirstKey($order));
		if ( $firstSetting ) {
			...
			if ( ($firstValue === '') || ($firstValue === null) ) {
				...
				foreach ($order as $side => $label) {
					$setting = $this->mainBinding->getChild($side);
	...
	protected function getKoComponentParams(): array {
		...
		if ( ($this->mainBinding instanceof CssBoxDimensions) ) {
			...
			$params['unitDropdownOptions'] = ChoiceControlOption::generateKoOptions(
				ChoiceControlOption::tryGenerateFromSetting($unitSetting)
			);
		}
