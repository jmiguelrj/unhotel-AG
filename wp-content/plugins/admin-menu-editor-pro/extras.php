...
use YahnisElsts\AdminMenuEditor\ImportExport\wsAmeImportExportFeature;
...
class wsMenuEditorExtras {
	...
	function __construct($wp_menu_editor){
		...
		//Initialize the universal import/export handler.
		...
		add_filter('admin_menu_editor-base_scripts', array($this, 'add_extra_scripts'));
...
wsEditorData.importMenuNonce = "<?php echo esc_js(wp_create_nonce('import_custom_menu'));  ?>";
	...
	public function register_extra_scripts() {
		...
		wp_register_auto_versioned_script(
			...
			array(
				...
				'ame-mini-functional-lib', 'ame-free-ko-extensions'
			)
		);
		wp_register_auto_versioned_script(
			'ame-pro-common-lib',
	...
	public function add_extra_scripts($baseDeps) {
		$koExtensions = ScriptDependency::create(
			plugins_url('extras/ko-extensions.js', $this->wp_menu_editor->plugin_file),
			'ame-ko-extensions',
			__DIR__ . '/extras/ko-extensions.js',
			array(
		...
		$extras = [
			...
			ScriptDependency::create(
				plugins_url('extras/menu-editor-extras.js', $this->wp_menu_editor->plugin_file),
				'ame-menu-editor-extras',
				__DIR__ . '/extras/menu-editor-extras.js',
				array('jquery')
			),
			ScriptDependency::create(
				plugins_url('extras/pro-common-lib.js', $this->wp_menu_editor->plugin_file),
				'ame-pro-common-lib',
				__DIR__ . '/extras/pro-common-lib.js',
				array($baseDeps['ame-knockout'], 'jquery')
			),
			ScriptDependency::create(
				plugins_url('extras/menu-headings/menu-headings.js', $this->wp_menu_editor->plugin_file),
				'ame-menu-heading-settings',
				__DIR__ . '/extras/menu-headings/menu-headings.js',
				array(
					$baseDeps['ame-knockout'],
					$baseDeps['ame-lodash'],
					$baseDeps['ame-mini-functional-lib'],
			...
			ScriptDependency::create(
				plugins_url('extras/choices-js/choices.min.js', $this->wp_menu_editor->plugin_file),
				'ame-choices-js',
				__DIR__ . '/extras/choices-js/choices.min.js'
			)
		];

		foreach ($extras as $extra) {
			$baseDeps[$extra->getHandle()] = $extra;
		}
   ...
   */
	...
	public function enqueue_dashboard_deps() {
		...
		if ( $is_helper_needed ) {
			...
			wp_add_inline_script(
				'ame-pro-admin-helpers',
				sprintf('wsAmeProAdminHelperData = (%s);', wp_json_encode($helper_data)),
				'before'
			);
		}
	}
	...
	public function filter_available_modules($modules) {
		...
		$modules['tweaks'] = array(
			...
			'className'    => 'YahnisElsts\\AdminMenuEditor\\Tweaks\\ameTweakManager',
			'title'        => 'Tweaks',
