...
class Wslm_LicenseServer {
	...
	protected function actionGetLicense($productSlug, $licenseKey = null, $token = null) {
		...
		$inputSiteUrl = isset($this->get['site_url']) ? strval($this->get['site_url']) : '';

		$license = $this->validateLicenseRequest($productSlug, $licenseKey, $token, $this->get);
		$this->outputResponse(array(
			'license' => $this->prepareLicenseForOutput($license, !empty($token), $inputSiteUrl),
		));
	...
	public function loadLicense($licenseKeyOrId, $token = null, $alwaysTreatAsKey = false) {
		...
		if ( !empty($license) ) {
			...
			$license['renewal_url'] = 'https://adminmenueditor.com/renew-license/';
			$license['upgrade_url'] = 'https://adminmenueditor.com/upgrade-license/';
		} else {
			$license = null;
		}
		return $license;
	 ...
	 * @param string $inputSiteUrl
	 * @return array
	 */
	public function prepareLicenseForOutput($license, $usingToken = false, $inputSiteUrl = '') {
		$data = $license->getData();
		...
		//Backwards compatibility: Match the protocol of the input URL for data that contains site URLs.
		if ( !empty($inputSiteUrl) ) {
			if ( !empty($data['site_url']) && is_string($data['site_url']) ) {
				$data['site_url'] = $this->matchValidInputProtocol($data['site_url'], $inputSiteUrl);
			}
			...
			if ( isset($data['sites']) ) {
				...
				if ( is_array($sites) ) {
					foreach ($sites as $key => $site) {
						if ( !empty($site['site_url']) && is_string($site['site_url']) ) {
							$sites[$key]['site_url'] = $this->matchValidInputProtocol($site['site_url'], $inputSiteUrl);
						}
					}
					$data['sites'] = $sites;
				}
	...
	protected function actionLicenseSite($productSlug, $licenseKey) {
		...
		$inputSiteUrl = isset($this->post['site_url']) ? strval($this->post['site_url']) : '';
		if ( !$this->isValidUrl($inputSiteUrl) ) {
			$this->outputError('site_url_expected', "Missing or invalid site URL.", 400);
			return;
		}
		$siteUrl = $this->sanitizeSiteUrl($inputSiteUrl);

		//Maybe the site is already licensed?
		$token = $this->getTokenForLicensedSite($siteUrl, $license['license_id']);
		if ( !empty($token) ) {
			$this->outputResponse(array(
				'site_token' => $token,
				'license' => $this->prepareLicenseForOutput($license, false, $inputSiteUrl),
			));
			return;
		}
		...
		if ( $license['max_sites'] !== null ) {
			...
			if ( intval($licensedSiteCount) >= intval($license['max_sites']) ) {
				...
				$this->outputError(
					...
					400,
					$this->prepareLicenseForOutput($license, false, $inputSiteUrl)
				);
				return;
			}
		}
		...
		if ( !$this->isLocalHost($siteUrl) ) {
			$otherToken = $this->getTokenForSiteExceptLicense($siteUrl, $productSlug, $license['license_id']);
			if ( !empty($otherToken) ) {
	...
	private function getTokenForLicensedSite($sanitizedSiteUrl, $licenseId): ?string {
		if ( empty($sanitizedSiteUrl) ) {
			return null;
		}

		$query = "SELECT token FROM {$this->tablePrefix}tokens WHERE site_url = %s AND license_id = %d";
		$token = $this->wpdb->get_var($this->wpdb->prepare(
			$query,
			$sanitizedSiteUrl, $licenseId
		));

		if ( empty($token) && $this->isUrlFormatTransitionPeriod() ) {
			//Try again but with the old URL format (http:// instead of protocol-relative).
			/** @noinspection HttpUrlsUsage */
			$oldFormatUrl = preg_replace('@^//@', 'http://', $sanitizedSiteUrl);
			$token = $this->wpdb->get_var($this->wpdb->prepare(
				$query,
				$oldFormatUrl, $licenseId
			));
		}
	...
	/**
	 * Find a token for the specified site URL that is associated with a different license.
	 */
	private function getTokenForSiteExceptLicense($sanitizedSiteUrl, $productSlug, $exceptLicenseId): ?string {
		if ( empty($sanitizedSiteUrl) ) {
			return null;
		}

		$query = "SELECT tokens.token
		...
		$otherToken = $this->wpdb->get_var($this->wpdb->prepare(
			$query,
			$sanitizedSiteUrl, $productSlug, $exceptLicenseId
		));

		if ( empty($otherToken) && $this->isUrlFormatTransitionPeriod() ) {
			//Try again but with the old URL format.
			/** @noinspection HttpUrlsUsage */
			$oldFormatUrl = preg_replace('@^//@', 'http://', $sanitizedSiteUrl);
			$otherToken = $this->wpdb->get_var($this->wpdb->prepare(
				$query,
				$oldFormatUrl, $productSlug, $exceptLicenseId
			));
		}
	...
	protected function actionUnlicenseSite($productSlug, $licenseKey = null, $token = null) {
		...
		$response = array( 'license' => $this->prepareLicenseForOutput($license, $usingToken, $inputSiteUrl), );
		...
		if ( empty($token) ) {
			...
			//the desired end state - site not licensed - has been achieved, so treat it as a success.
			...
			$response['license'] = $this->prepareLicenseForOutput($license, $usingToken, $inputSiteUrl);

			$response = array_merge($response, array(
				'site_token_removed' => $token,
				'site_url' => $this->matchValidInputProtocol($siteUrl, $inputSiteUrl)
			));
		}
		$this->outputResponse($response);
	}
	...
	private function sanitizeSiteUrl($url) {
		...
		//Convert to a protocol-relative URL.
	...
	private function isUrlFormatTransitionPeriod(): bool {
		//For a few while after switching to protocol-relative URLs, we need to accept both formats.
		$transitionEnd = mktime(0, 0, 0, 1, 20, 2026);
		return (time() < $transitionEnd);
	}

	/**
	 * Ensure the output URL uses the same protocol as the input URL, provided that
	 * it's either http://, https:// or protocol-relative (//example.com).
	 *
	 * @param string $outputUrl
	 * @param string $inputUrl
	 * @return string
	 */
	private function matchValidInputProtocol(string $outputUrl, string $inputUrl): string {
		$inputUrlProtocol = '';
		if ( preg_match('@^(https?:)?//@i', $inputUrl, $matches) ) {
			$inputUrlProtocol = strtolower($matches[0]);
		}
		$outputUrlProtocol = '';
		if ( preg_match('@^(https?:)?//@i', $outputUrl, $matches) ) {
			$outputUrlProtocol = strtolower($matches[0]);
		}

		if ( $inputUrlProtocol === $outputUrlProtocol ) {
			return $outputUrl; //No changes needed.
		}

		if ( !empty($inputUrlProtocol) ) {
			//Replace the protocol in the output URL with the one from the input URL.
			$outputUrl = $inputUrlProtocol . substr($outputUrl, strlen($outputUrlProtocol));
		} else if ( $outputUrlProtocol !== '//' ) {
			//Convert the output URL to protocol-relative.
			$outputUrl = '//' . substr($outputUrl, strlen($outputUrlProtocol));
		}
