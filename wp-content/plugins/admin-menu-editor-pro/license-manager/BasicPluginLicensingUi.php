...
class Wslm_BasicPluginLicensingUI {
	...
	private function tabManageSites() {
		if ( isset($this->triedLicense, $this->triedLicense->sites) ) {
			?>
			<h3>Sites Associated With License Key "<?php echo esc_html($this->triedLicenseKey); ?>"</h3>
			<?php
			if ( !empty($this->triedLicense->sites) ):
			?>
			<table class="widefat">
				<?php
				foreach($this->triedLicense->sites as $site):
					$displaySiteUrl = preg_replace('#^(https?:)?//#', '', $site->site_url);
					?>
				<tr>
					<td>
						<?php echo esc_html($displaySiteUrl); ?><br>
						Token: <?php echo esc_html($site->token); ?>
					</td>
					<td style="vertical-align: middle; width: 11em;">
						<form method="post" action="<?php echo esc_url($this->getLicensingPageUrl()); ?>">
	...
	public function autoActivateLicense() {
		...
		if ( !empty($this->keyConstant) && defined($this->keyConstant) ) {
			...
			$siteUrl = $this->licenseManager->getSiteUrl();
			$reversedTokenHistory = array_reverse($tokenHistory, true); //Put the most recent first.
			$possibleToken = array_search($siteUrl, $reversedTokenHistory);

			//Backwards compatibility: If we find nothing, try again with "http://" instead of "//".
			//Previous versions of the plugin normalized all site URLs to use "http://".
			if ( empty($possibleToken) ) {
				/** @noinspection HttpUrlsUsage */
				$possibleToken = array_search(preg_replace('#^//#', 'http://', $siteUrl), $reversedTokenHistory);
			}
