...
class VikBookingController extends JControllerVikBooking
{
		...
		if (!VikBooking::userIsLogged()) {
			if (!empty($pname) && !empty($plname) && !empty($pusername) && !empty($pemail) && $ppassword == $pconfpassword) {
				...
				if ($newuserid!=false && strlen($newuserid)) {
					 ...
					 */
					$redirect_to = JRoute::_('index.php?option=com_vikbooking&task=oconfirm&'.$qstring, false);
	...
	public function saveorder()
	{
		...
		if ($cfields) {
			...
			foreach ($cfields as $cf) {
				...
				if ($cf['type'] != 'separator' && $cf['type'] != 'country' && ( $cf['type'] != 'checkbox' || ($cf['type'] == 'checkbox' && intval($cf['required']) != 1) ) ) {
					...
					$arrcustdata[JText::_($cf['name'])] = $def_user_inp_val;
		...
		if (count($rooms) != $proomsnum) {
			...
			$app->redirect(JRoute::_('index.php?option=com_vikbooking'));
			exit;
		}
		...
		if (!empty($ppkg_id)) {
			...
			if (!is_array($pkg) || (is_array($pkg) && !(count($pkg) > 0)) ) {
				...
				$app->redirect(JRoute::_("index.php?option=com_vikbooking&view=packagedetails&pkgid=".$ppkg_id.(!empty($pitemid) ? "&Itemid=".$pitemid : ""), false));
				exit;
			}
		}
		...
		if ($must_payment && empty($payment)) {
			...
			$app->redirect(JRoute::_('index.php?' . http_build_query($redirect_uri_vals), false));
			exit;
		}
		...
		if (!$mod_booking && ((!empty($payment) && intval($payment['setconfirmed']) == 1) || !$must_payment || ($usedcoupon && $isdue <= 0))) {
			...
			$app->redirect(JRoute::_("index.php?option=com_vikbooking&view=booking&sid=" . $sid . "&ts=" . $nowts . (!empty($pnodep) ? "&nodep=".$pnodep : "") . (!empty($pitemid) ? "&Itemid=" . $pitemid : ""), false));
		} elseif ($mod_booking) {
			...
			// VBO 1.11 - push data to tracker for conversion
			$vbo_tracker->pushData('idorder', $mod_booking['id'])->pushMessage(JText::_('VBOBOOKINGMODOK'))->closeTrack();
			$vbo_tracker->resetTrack();

			$app->enqueueMessage(JText::_('VBOBOOKINGMODOK'));
			$app->redirect(JRoute::_("index.php?option=com_vikbooking&view=booking&sid=" . $sid . "&ts=" . $nowts . (!empty($pnodep) ? "&nodep=".$pnodep : "") . (!empty($pitemid) ? "&Itemid=" . $pitemid : ""), false));
		} else {
			...
			$app->redirect(JRoute::_($booking_details_uri, false));
		}
	}
		...
		if (!empty($psid) && !empty($pidorder)) {
			...
			if ($order) {
				...
				if (!empty($pemail) && !empty($preason)) {
					...
					//
					$subject = JText::_('VBCANCREQUESTEMAILSUBJ') . ' #' . $order[0]['id'];
					...
					$mainframe->redirect(JRoute::_("index.php?option=com_vikbooking&view=booking&sid=".$order[0]['sid']."&ts=".$order[0]['ts']."&Itemid=".VikRequest::getString('Itemid', '', 'request'), false));
				} else {
					$mainframe->redirect(JRoute::_("index.php?option=com_vikbooking&view=booking&sid=".$order[0]['sid']."&ts=".$order[0]['ts'], false));
				}
			} else {
		...
		if ($array_result['verified'] == 1) {
			...
			if (isset($array_result['tot_paid'])) {
				...
				if ($shouldpay != $totreceived && $shouldpay_befdep != $totreceived && $shouldpay_befdd != $totreceived && $shouldpay_less_damagedep != $totreceived && $shouldpay_dd != $totreceived && $row['paymcount'] == 0) {
					...
					$vbo_app = VikBooking::getVboApplication();
					$adsendermail = VikBooking::getSenderMail();
					$vbo_app->sendMail($adsendermail, $adsendermail, $recipient_mail, $adsendermail, JText::_('VBTOTPAYMENTINVALID'), JText::sprintf('VBTOTPAYMENTINVALIDTXT', $row['id'], $totreceived." (".$array_result['tot_paid'].")", $shouldpay), false);
				}
		...
		if (!empty($default_cur) && !empty($pprices) && count($pprices) > 0 && !empty($ptocurrency)) {
			...
			if ($default_cur != $ptocurrency) {
				...
				if (count($exchanged) > 0) {
					...
					$response['error'] = !empty($conv_error) ? $conv_error : JText::_('VBERRCURCONVINVALIDDATA');
				}
			} else {
				$session->set('vboLastCurrency', $ptocurrency);
	...
	public function operatorlogin()
	{
		 ...
		 */
		$goto = JRoute::_('index.php?option=com_vikbooking&view=operators&auth=1'.(!empty($pitemid) ? '&Itemid='.$pitemid : ''), false);

		if (empty($pauthcode) || !VikBooking::getOperatorInstance()->authOperator($pauthcode)) {
			// print warning message
			VikError::raiseWarning('', JText::_('VBOOPERINVAUTHCODE'));
		}
	...
	public function operatorlogout()
	{
		$app = JFactory::getApplication();
		$pitemid = VikRequest::getInt('Itemid', '', 'request');
		$goto = JRoute::_('index.php?option=com_vikbooking&view=operators'.(!empty($pitemid) ? '&Itemid='.$pitemid : ''), false);
	...
	public function storeprecheckin()
	{
		...
		$goto = JRoute::_('index.php?option=com_vikbooking&view=booking&sid=' . $order['sid'] . '&ts=' . $order['ts'] . (!empty($pitemid) ? '&Itemid=' . $pitemid : ''), false);
		if (empty($order['sid']) && !empty($order['idorderota'])) {
			// booking details page for OTA bookings
			$goto = JRoute::_('index.php?option=com_vikbooking&view=booking&sid=' . $order['idorderota'] . '&ts=' . $order['ts'] . (!empty($pitemid) ? '&Itemid=' . $pitemid : ''), false);
		}
		...
		$app->enqueueMessage(JText::_('VBOSUBMITPRECHECKINTNKS'));
		$app->redirect($goto);
	}
	...
	public function upsellextras()
	{
		...
		$goto = JRoute::_('index.php?option=com_vikbooking&view=booking&sid=' . (empty($order['sid']) && !empty($order['idorderota']) ? $order['idorderota'] : $order['sid']) . '&ts=' . $order['ts'] . (!empty($pitemid) ? '&Itemid=' . $pitemid : ''), false);
	...
	public function sendreview()
	{
		...
		if ($dbo->insertObject('#__vikchannelmanager_otareviews', $review_record, 'id')) {
			$app->enqueueMessage(JText::_('VBOTHANKSREVIEWLEFT'));
			// Booking History
			VikBooking::getBookingHistoryInstance()->setBid($order['id'])->store('GR');
		} else {
			VikError::raiseWarning('', JText::_('VBOREVIEWGENERROR'));
		}
	...
	public function precheckin_upload_docs()
	{
		...
		$file_prefix = str_replace(' ', '-', JText::_('VBOPRECHECKIN')) . '_';
	...
	public function submit_inquiry()
	{
		...
		if (empty($checkin_dt) || empty($checkout_dt) || empty($checkin_ts) || empty($checkout_ts)) {
			// invalid dates
			VBOHttpDocument::getInstance()->close(400, JText::_('VBINVALIDDATES'));
		}

		// validate guests
		if (!is_array($adults) || !count($adults)) {
			// invalid adults
			VBOHttpDocument::getInstance()->close(400, JText::_('VBINCONGRDATA'));
		}
			...
			foreach ($info_vals as $info_val) {
				...
				if ($info_type == 'nominative') {
					if (empty($t_first_name)) {
						...
						$res_custdata[JText::_('VBNAME')] = $info_val;
					} else {
						$t_last_name = $info_val;
						$res_custdata[JText::_('VBLNAME')] = $info_val;
					}
				} elseif ($info_type == 'email') {
					$guest_email = $info_val;
					$res_custdata[JText::_('ORDER_EMAIL')] = $info_val;
				} elseif ($info_type == 'phone') {
					$guest_phone = $info_val;
					$res_custdata[JText::_('ORDER_PHONE')] = $info_val;
				} elseif ($info_type == 'country') {
					$guest_country = $info_val;
					$res_custdata[JText::_('ORDER_STATE')] = $info_val;
				} elseif ($info_type == 'city') {
					$guest_extras['city'] = $info_val;
					$res_custdata[JText::_('ORDER_CITY')] = $info_val;
				} else {
					...
					if ($info_type == 'special_requests') {
						$res_custdata[JText::_('ORDER_SPREQUESTS')] = $info_val;
					} else {
		...
		$customer->adminnotes .= JText::_('VBPICKUP') . ': ' . $checkin_dt . "\n";
		$customer->adminnotes .= JText::_('VBRETURN') . ': ' . $checkout_dt . "\n";
		$customer->adminnotes .= JText::_('VBFORMADULTS') . ': ' . array_sum($adults) . "\n";
		$customer->adminnotes .= JText::_('VBFORMCHILDREN') . ': ' . array_sum($children) . "\n";
		...
		if (!is_array($room_rates) || $has_av_error) {
			...
			if (count($alternative_dates)) {
				...
				$alt_suggestion = JText::_('VBO_ALT_DATES_INQ');
				...
				$alt_suggestion = JText::_('VBO_ALT_PARTY_INQ');
		...
		if (count($all_rooms)) {
			...
			$inquiry_res_id = $av_helper->createInquiryReservation($room_rplan, $customer);
			$alt_suggestion = JText::_('VBO_ALT_DUMMY_INQ');
	...
	public function upgrade_room()
	{
		...
		$dbo->execute();
		if (!$dbo->getNumRows()) {
			VBOHttpDocument::getInstance()->close(404, JText::_('VBORDERNOTFOUND'));
		}
