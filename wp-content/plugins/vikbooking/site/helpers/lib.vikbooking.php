...
class VikBooking
{
			...
			foreach ($rfeatures as $featname => $featval) {
				...
				if ($tn_featname == $featname) {
					...
					if (VBOPlatformDetection::isWordPress()) {
						...
						$tn_featname = JText::_($ini_constant);
		...
		if (!empty($old_booking['split_stay']) && !empty($room_stay_dates) && count($orooms_map)) {
			...
			foreach ($room_stay_dates as $rs_ind => $room_stay) {
				...
				if (!empty($room_nights)) {
					$split_stay_descr .= ': ' . $room_nights . ' ' . ($room_nights > 1 ? JText::_('VBDAYS') : JText::_('VBDAY'));
				}
				$split_stay_descr .= ', ';
	...
	public static function getGuestsAllowedPolicy($vbo_tn = null)
	{
		...
		if (!$text_record) {
			...
			if (self::isAdmin()) {
				...
				$record->exp 	 = JText::_('VBO_GUESTS_POLICY');
				$record->setting = '';
	...
	public static function parseEmailTemplate($tmpl, $bid, $rooms, $rates, $options, $total = 0, $link = null)
	{
		...
		if ($order_info['status'] == 'cancelled') {
			...
			if (!empty($order_info['type']) && !strcasecmp($order_info['type'], 'Inquiry')) {
				...
				$status_str = JText::_('VBO_INQUIRY_PENDING');
			}
		} else {
		...
		if ($order_info['status'] == 'confirmed' && self::customersPinEnabled()) {
			...
			if (!empty($customer_pin)) {
				$customer_info .= '<h3>'.JText::_('VBYOURPIN').': '.$customer_pin.'</h3>';
			}
		}
		...
		foreach ($rooms as $num => $r) {
			...
			// room info
			$roomstr .= "<strong>".$r['name']."</strong> ".$r['adults']." ".($r['adults'] > 1 ? JText::_('VBMAILADULTS') : JText::_('VBMAILADULT')).($r['children'] > 0 ? ", ".$r['children']." ".($r['children'] > 1 ? JText::_('VBMAILCHILDREN') : JText::_('VBMAILCHILD')) : "")."<br/>";
			// Rooms Distinctive Features
		...
		foreach ($rooms as $num => $r) {
			...
			if ($order_info['split_stay'] && !empty($room_stay_dates) && isset($room_stay_dates[$use_room_ind]) && $room_stay_dates[$use_room_ind]['idroom'] == $room_id) {
				$split_stay_info[] = $room_stay_dates[$use_room_ind]['nights'] . ' ' . ($room_stay_dates[$use_room_ind]['nights'] > 1 ? JText::_('VBDAYS') : JText::_('VBDAY'));
		...
		if (!empty($order_info['coupon'])) {
			...
			$orderdetails .= '<br/><div class="discount"><span>'.JText::_('VBCOUPON').' '.$expcoupon[2].'</span><div class="service-amount" style="float: right;"><span>- ' . self::formatCurrencyNumber(self::numberFormat($expcoupon[1]), $currencyname) . '</span></div></div>';
		}
		...
		if (!in_array($order_info['status'], array('confirmed', 'cancelled')) && !self::payTotal() && self::allowDepositFromRates($tars_info)) {
			...
			if ($percentdeposit > 0 && self::depositAllowedDaysAdv($order_info['checkin'])) {
				...
				if ($deposit_amount > 0) {
					$deposit_str = '<div class="deposit"><span>'.JText::_('VBLEAVEDEPOSIT').'</span><div class="service-amount" style="float: right;"><strong>' . self::formatCurrencyNumber(self::numberFormat($deposit_amount), $currencyname) . '</strong></div></div>';
				}
			}
		}
		...
		if ($order_info['refund'] > 0) {
			$totpaid_str .= '<div class="amountpaid amountrefunded"><span>' . JText::_('VBO_AMOUNT_REFUNDED') . '</span><div class="service-amount" style="float: right;"><strong>' . self::formatCurrencyNumber(self::numberFormat($order_info['refund']), $currencyname) . '</strong></div></div>';
		}
		if ($order_info['status'] != 'cancelled') {
			$tot_paid = $order_info['totpaid'];
			$diff_topay = (float)$total - (float)$tot_paid;
			if ((float)$tot_paid > 0) {
				$totpaid_str .= '<div class="amountpaid"><span>'.JText::_('VBAMOUNTPAID').'</span><div class="service-amount" style="float: right;"><strong>' . self::formatCurrencyNumber(self::numberFormat($tot_paid), $currencyname) . '</strong></div></div>';
				// only in case the remaining balance is greater than 1 to avoid commissions issues
				if ($diff_topay > 1) {
					$totpaid_str .= '<div class="amountpaid"><span>'.JText::_('VBTOTALREMAINING').'</span><div class="service-amount" style="float: right;"><strong>' . self::formatCurrencyNumber(self::numberFormat($diff_topay), $currencyname) . '</strong></div></div>';
				}
			}
		}
		if ($order_info['status'] == 'cancelled' && isset($order_info['canc_fee']) && $order_info['canc_fee'] > 0) {
			$totpaid_str .= '<div class="amountpaid amountcancfee"><span>' . JText::_('VBO_CANC_FEE') . '</span><div class="service-amount" style="float: right;"><strong>' . self::formatCurrencyNumber(self::numberFormat($order_info['canc_fee']), $currencyname) . '</strong></div></div>';
		}
		...
		foreach ($tags as $tkey => $tval) {
			...
			switch ($tval['rule']) {
				...
				case 'conf1':
					...
					if ($booking['status'] == 'confirmed' && (!empty($booking['closure']) || $booking['custdata'] == JText::_('VBDBTEXTROOMCLOSED'))) {
						$tval['fontcolor'] = self::getBestColorContrast($tval['color']);
						return $tval;
					}
					break;
	...
	public static function parseCustomerSMSTemplate($booking, $booking_rooms, $vbo_tn = null, $force_text = null) {
		...
		if (VBOPlatformDetection::isWordPress()) {
			...
			if ($itemid) {
				$book_link = JRoute::_($book_link . "&Itemid={$itemid}", false);
			}
		} else {
			/**
	...
	public static function parseInvoiceTemplate($invoicetpl, $booking, $booking_rooms, $invoice_num, $invoice_suff, $invoice_date, $company_info, $vbo_tn = null, $pdfparams = [])
	{
		...
		foreach ($pricestr as $num => $price_descr) {
			...
			if ($booking['split_stay'] && count($room_stay_dates) && isset($room_stay_dates[$kor])) {
				$split_stay_str .= $room_stay_dates[$kor]['nights'] . ' ' . ($room_stay_dates[$kor]['nights'] > 1 ? JText::_('VBDAYS') : JText::_('VBDAY')) . ', ';
		...
		if ($booking['refund'] > 0) {
			...
			$inv_rows .= '</tr>'."\n";
			$inv_rows .= '<tr>'."\n";
			$inv_rows .= '<td>' . JText::_('VBO_AMOUNT_REFUNDED') . '</td>'."\n";
			$inv_rows .= '<td></td>'."\n";
			$inv_rows .= '<td></td>'."\n";
		...
		foreach ($tax_summary as $tax_rate => $tax_amount) {
			...
			$tax_name = isset($tax_names[$tax_rate]) ? $tax_names[$tax_rate] : JText::_('VBO_INV_VATGST');
			$taxsum_rows .= '<tr>' . "\n";
	...
	public static function generateCustomInvoice($invoice_id)
	{
		...
		$pdf = new TCPDF(constant($pdfparams['pdf_page_orientation']), constant($pdfparams['pdf_unit']), $pdf_page_format, true, 'UTF-8', false);
		$pdf->SetTitle(JText::_('VBOINVNUM').' '.$invoice['number']);
	...
	public static function getPaxFields($precheckin = false, $type = null)
	{
		...
		$precheckin_pax_fields = [
			[
				'first_name'  => JText::_('VBCCFIRSTNAME'),
				'last_name'   => JText::_('VBCCLASTNAME'),
				'date_birth'  => JText::_('ORDER_DBIRTH'),
				'place_birth' => JText::_('VBOCUSTPLACEBIRTH'),
				'country' 	  => JText::_('ORDER_STATE'),
				'city' 		  => JText::_('ORDER_CITY'),
				'zip' 		  => JText::_('ORDER_ZIP'),
				'nationality' => JText::_('VBOCUSTNATIONALITY'),
				'gender' 	  => JText::_('VBOCUSTGENDER'),
				'doctype' 	  => JText::_('VBOCUSTDOCTYPE'),
				'docnum' 	  => JText::_('VBOCUSTDOCNUM'),
				'documents'   => JText::_('VBO_CUSTOMER_UPLOAD_DOCS'),
			],
			[
				'first_name'  => 'text',
				'last_name'   => 'text',
				...
				'nationality' => 'country',
				'gender' 	  => [
					'M' => JText::_('VBOCUSTGENDERM'),
					'F' => JText::_('VBOCUSTGENDERF'),
				],
				'doctype' 	  => 'text',
				'docnum' 	  => 'text',
				'documents'   => 'file',
	...
	public static function sayWeekDay($wd, $short = false)
	{
		switch ($wd) {
			case '6' :
				$ret = $short ? JText::_('VBSAT') : JText::_('VBWEEKDAYSIX');
				break;
			case '5' :
				$ret = $short ? JText::_('VBFRI') : JText::_('VBWEEKDAYFIVE');
				break;
			case '4' :
				$ret = $short ? JText::_('VBTHU') : JText::_('VBWEEKDAYFOUR');
				break;
			case '3' :
				$ret = $short ? JText::_('VBWED') : JText::_('VBWEEKDAYTHREE');
				break;
			case '2' :
				$ret = $short ? JText::_('VBTUE') : JText::_('VBWEEKDAYTWO');
				break;
			case '1' :
				$ret = $short ? JText::_('VBMON') : JText::_('VBWEEKDAYONE');
				break;
			default :
				$ret = $short ? JText::_('VBSUN') : JText::_('VBWEEKDAYZERO');
				break;
		}
		return $ret;
	}
	...
	public static function sayMonth($idm, $short = false)
	{
		switch ($idm) {
			case '12' :
				$ret = $short ? JText::_('VBSHORTMONTHTWELVE') : JText::_('VBMONTHTWELVE');
				break;
			case '11' :
				$ret = $short ? JText::_('VBSHORTMONTHELEVEN') : JText::_('VBMONTHELEVEN');
				break;
			case '10' :
				$ret = $short ? JText::_('VBSHORTMONTHTEN') : JText::_('VBMONTHTEN');
				break;
			case '9' :
				$ret = $short ? JText::_('VBSHORTMONTHNINE') : JText::_('VBMONTHNINE');
				break;
			case '8' :
				$ret = $short ? JText::_('VBSHORTMONTHEIGHT') : JText::_('VBMONTHEIGHT');
				break;
			case '7' :
				$ret = $short ? JText::_('VBSHORTMONTHSEVEN') : JText::_('VBMONTHSEVEN');
				break;
			case '6' :
				$ret = $short ? JText::_('VBSHORTMONTHSIX') : JText::_('VBMONTHSIX');
				break;
			case '5' :
				$ret = $short ? JText::_('VBSHORTMONTHFIVE') : JText::_('VBMONTHFIVE');
				break;
			case '4' :
				$ret = $short ? JText::_('VBSHORTMONTHFOUR') : JText::_('VBMONTHFOUR');
				break;
			case '3' :
				$ret = $short ? JText::_('VBSHORTMONTHTHREE') : JText::_('VBMONTHTHREE');
				break;
			case '2' :
				$ret = $short ? JText::_('VBSHORTMONTHTWO') : JText::_('VBMONTHTWO');
				break;
			default :
				$ret = $short ? JText::_('VBSHORTMONTHONE') : JText::_('VBMONTHONE');
				break;
		}
		return $ret;
	}
			...
			foreach ($pconfig as $value => $cont) {
				...
				switch ($cont['type']) {
					...
					case 'checkbox':
						...
						$html .= self::getVboApplication()->printYesNoButtons('vikpaymentparams['.$value.']', JText::_('VBYES'), JText::_('VBNO'), (isset($arrparams[$value]) ? (int)$arrparams[$value] : (int)$default_paramv), 1, 0);
						break;
					default:
	...
	public static function displaySMSParameters($pfile, $params = null)
	{
		...
		foreach ($config as $value => $cont) {
			...
			switch ($cont['type']) {
				...
				case 'password':
					$html .= '<div class="btn-wrapper input-append">';
					$html .= '<input type="password" name="viksmsparams['.$value.']" value="'.(isset($params[$value]) ? JHtml::_('esc_attr', $params[$value]) : JHtml::_('esc_attr', $default_paramv)).'" size="20"' . $inp_attr . '/>';
				...
				case 'number':
					...
					$html .= '<input type="number" name="viksmsparams['.$value.']" value="'.(isset($params[$value]) ? JHtml::_('esc_attr', $params[$value]) : JHtml::_('esc_attr', $default_paramv)).'" ' . implode(' ', $number_attr) . $inp_attr . '/>';
					break;
				case 'textarea':
					$html .= '<textarea name="viksmsparams['.$value.']"' . $inp_attr . '>'.(isset($params[$value]) ? JHtml::_('esc_textarea', $params[$value]) : JHtml::_('esc_textarea', $default_paramv)).'</textarea>';
					break;
				case 'visual_html':
					...
					break;
				case 'hidden':
					$html .= '<input type="hidden" name="viksmsparams['.$value.']" value="'.(isset($params[$value]) ? JHtml::_('esc_attr', $params[$value]) : JHtml::_('esc_attr', $default_paramv)).'"' . $inp_attr . '/>';
					break;
				case 'checkbox':
					// always display a hidden input value turned off before the actual checkbox to support the "off" (0) status
					$html .= '<input type="hidden" name="viksmsparams['.$value.']" value="0" />';
					$html .= self::getVboApplication()->printYesNoButtons('viksmsparams['.$value.']', JText::_('VBYES'), JText::_('VBNO'), (isset($params[$value]) ? (int)$params[$value] : (int)$default_paramv), 1, 0);
					break;
				default:
					$html .= '<input type="text" name="viksmsparams['.$value.']" value="'.(isset($params[$value]) ? JHtml::_('esc_attr', $params[$value]) : JHtml::_('esc_attr', $default_paramv)).'" size="20"' . $inp_attr . '/>';
					break;
			}
			if (strlen($labelhelp) > 0) {
				$html .= '<span class="vbo-param-setting-comment">'.$labelhelp.'</span>';
	...
	public static function guestReviewsServices()
	{
		...
		$default_services = array(
			JText::_('VBOGREVVALUE'),
			JText::_('VBOGREVLOCATION'),
			JText::_('VBOGREVSTAFF'),
			JText::_('VBOGREVCLEAN'),
			JText::_('VBOGREVCOMFORT'),
			JText::_('VBOGREVFACILITIES'),
		);
	...
	public static function tnCustomerRawDataLabel($label)
	{
		...
		if (isset($known_lbls[$converted])) {
			...
			return JText::_($known_lbls[$converted]);
		}

		// we try to guess the translation string by prepending VBO
		$guessed = JText::_('VBO' . $converted);
