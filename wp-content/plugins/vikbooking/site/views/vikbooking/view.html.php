	...
	function display($tpl = null) {
		...
		if (!empty($pmodify_sid) && !empty($pmodify_id)) {
			...
			if ($dbo->getNumRows() == 1) {
				...
				if (!$mod_allowed) {
					VikError::raiseWarning('', JText::_('VBOERRCANNOTMODBOOK'));
					$mainframe = JFactory::getApplication();
					$mainframe->redirect(JRoute::_("index.php?option=com_vikbooking&view=booking&sid=".$mod_booking['sid']."&ts=".$mod_booking['ts'], false));
					exit;
				}
