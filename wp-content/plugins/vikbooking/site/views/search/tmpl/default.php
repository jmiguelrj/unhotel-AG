...
if (VikBooking::loadJquery()) {
	...
	JHtml::_('script', VBO_SITE_URI.'resources/jquery-1.12.4.min.js');
}
...
JHtml::_('script', VBO_SITE_URI . 'resources/vik-dots-slider.js');
//
...
$change_dates_uri = JRoute::_('index.php?option=com_vikbooking&view=vikbooking&checkin='.$this->checkin.'&checkout='.$this->checkout.'&category_id='.$use_category_filter.(!empty($pitemid) ? '&Itemid='.$pitemid : ''));
	...
	for (val in vboRoomParty[roomnum]) {
		if (vboRoomParty[roomnum][val] != idroom) {
			if (vbhasClass(document.getElementById('vbcontainer'+roomnum+'_'+vboRoomParty[roomnum][val]), 'room_selected')) {
				...
				document.getElementById('vbselector'+roomnum+'_'+vboRoomParty[roomnum][val]).innerHTML = '<?php echo addslashes(JText::_('VBSELECTR')); ?>';
			}
		}
	}
...
<div class="vbstepsbarcont">
	<ol class="vbo-stepbar" data-vbosteps="4">
		<li class="vbo-step vbo-step-complete"><a href="<?php echo $change_dates_uri; ?>"><?php echo JText::_('VBSTEPDATES'); ?></a></li>
...
<div class="vbo-results-wrapper">
	<div class="vbo-results-head">
		...
		<div class="vbo-results-nights">
			...
			<div class="vbo-results-head-det">
				<span class="vbo-results-head-det-lbl"><?php echo ($this->days == 1 ? JText::_('VBSEARCHRESNIGHT') : JText::_('VBSEARCHRESNIGHTS')); ?></span>
	...
	if ($this->roomsnum > 1) {
		...
		<div class="vbo-results-numrooms">
			...
			<div class="vbo-results-head-det">
				<span class="vbo-results-head-det-lbl"><?php echo $this->roomsnum == 1 ? JText::_('VBSEARCHRESROOM') : JText::_('VBSEARCHRESROOMS'); ?></span>
...
<?php
/**
 * @wponly 	if the Itemid is missing, maybe because of a redirect, then using JRoute::_('index.php?option=com_vikbooking')
 * 			generated an empty URL (the home page), by losing the navigation and by rendering an invalid page.
 * 			So we need to use JRoute::_('index.php?option=com_vikbooking&view=vikbooking') like the link above.
 */
?>
<form action="<?php echo JRoute::_('index.php?option=com_vikbooking&view=vikbooking'.(!empty($pcategory_id) ? '&category_id=' . $pcategory_id : '').(!empty($pitemid) ? '&Itemid='.$pitemid : '')); ?>" method="post" id="vbselectroomform">
