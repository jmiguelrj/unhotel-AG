...
class VikbookingViewTableaux extends JViewVikBooking
{
	public function display($tpl = null)
	{
		...
		if ($operator === false) {
			...
			$app->redirect(JRoute::_('index.php?option=com_vikbooking&view=operators'.(!empty($pitemid) ? '&Itemid='.$pitemid : ''), false));
			exit;
		}
		...
		if (!$opobj->checkPermissions($operator, 'tableaux')) {
			...
			VikError::raiseWarning('', JText::_('VBONOTAUTHORIZEDRES'));
			$pitemid = VikRequest::getInt('Itemid', '', 'request');
			$app->redirect(JRoute::_('index.php?option=com_vikbooking&view=operators'.(!empty($pitemid) ? '&Itemid='.$pitemid : ''), false));
			exit;
		}
		...
		if (!$rooms) {
			VikError::raiseWarning('', JText::_('VBONOTAUTHORIZEDRES').' (#2)');
			$pitemid = VikRequest::getInt('Itemid', '', 'request');
			$app->redirect(JRoute::_('index.php?option=com_vikbooking&view=operators'.(!empty($pitemid) ? '&Itemid='.$pitemid : ''), false));
			exit;
		}
