...
<div class="vbo-operator-dashboard">
	...
	<div class="vbo-operator-dashboard-logout">
		<form action="<?php echo JRoute::_('index.php?option=com_vikbooking&task=operatorlogout&Itemid=' . $pitemid); ?>" method="post">
			<input type="submit" name="logout" value="<?php echo JText::_('VBOLOGOUT'); ?>" class="vbo-logout vbo-pref-color-btn-secondary" />
	...
	<div class="vbo-operator-dashboard-links">
		...
		foreach ($this->operator['perms'] as $perm) {
			...
			if (!strcasecmp(($tool_data['rendering_type'] ?? ''), 'view')) {
				...
				$tool_link = JRoute::_('index.php?option=com_vikbooking&view=' . $perm['type'] . (!empty($pitemid) ? '&Itemid=' . $pitemid : ''));
			} else {
				// tool is either native (layout) or custom
				$tool_link = JRoute::_('index.php?option=com_vikbooking&view=operators&tool=' . $perm['type'] . (!empty($pitemid) ? '&Itemid=' . $pitemid : ''));
			}
			...
			<li>
				...
				<div class="vbo-operator-dashboard-link-right">
					<a class="btn vbo-pref-color-btn" href="<?php echo $tool_link; ?>"><?php echo JText::_('VBOOPERVIEWPG'); ?></a>
				</div>
			</li>
			<?php
		}
