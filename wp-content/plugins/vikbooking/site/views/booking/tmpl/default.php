...
$current_booking_uri = JRoute::_('index.php?option=com_vikbooking&view=booking&sid='.(!empty($ord['idorderota']) && !empty($ord['channel']) ? $ord['idorderota'] : $ord['sid']).'&ts='.$ord['ts'].(!empty($bestitemid) ? '&Itemid='.$bestitemid : (!empty($pitemid) ? '&Itemid='.$pitemid : '')));
?>
...
<div class="vbo-booking-print">
	<a class="vbo-booking-print-link" href="<?php echo JRoute::_('index.php?option=com_vikbooking&view=booking&sid='.(!empty($ord['idorderota']) && !empty($ord['channel']) ? $ord['idorderota'] : $ord['sid']).'&ts='.$ord['ts'].'&tmpl=component'.(!empty($bestitemid) ? '&Itemid='.$bestitemid : (!empty($pitemid) ? '&Itemid='.$pitemid : ''))); ?>" target="_blank" title="<?php echo JText::_('VBOPRINT'); ?>"><?php VikBookingIcons::e('print'); ?></a>
</div>
	<?php
}
...
<div class="vbo-booking-details-topcontainer">
	...
	<div class="vbo-booking-details-midcontainer">
		...
		if (!empty($ord['idorderota']) && !empty($ord['channel'])) {
			?>
			<div class="vbo-booking-details-bookinfo">
				<span class="vbo-booking-details-bookinfo-lbl"><?php echo JText::_('VBORDERNUMBER'); ?></span>
		...
		if ($ord['split_stay']) {
			...
			<div class="vbo-booking-details-bookinfo vbo-booking-details-bookinfo-splitstay">
				<span class="vbo-booking-details-bookinfo-lbl"><?php VikBookingIcons::e('random'); ?> <?php echo JText::_('VBO_SPLIT_STAY_RES'); ?></span>
			</div>
			<?php
		}
		?>
		...
		<div class="vbo-booking-details-actions">
				...
				<div class="vbo-booking-mod-container">
					<div class="vbo-booking-mod-inner">
						<div class="vbo-booking-mod-cmd vbo-booking-precheckin-cmd">
							<a href="<?php echo JRoute::_('index.php?option=com_vikbooking&view=precheckin&sid='.(!empty($ord['idorderota']) && !empty($ord['channel']) ? $ord['idorderota'] : $ord['sid']).'&ts='.$ord['ts'].(!empty($pitemid) ? '&Itemid='.$pitemid : (!empty($start_itemid) ? '&Itemid='.$start_itemid : ''))); ?>"><?php VikBookingIcons::e('users'); ?> <?php echo JText::_('VBOPRECHECKIN'); ?></a>
						</div>
					</div>
				</div>
				<?php
				...
				<div class="vbo-booking-mod-container">
					<div class="vbo-booking-mod-inner">
						<div class="vbo-booking-mod-cmd vbo-booking-review-cmd">
							<a href="<?php echo JRoute::_('index.php?option=com_vikbooking&view=revstay&sid='.(!empty($ord['idorderota']) && !empty($ord['channel']) ? $ord['idorderota'] : $ord['sid']).'&ts='.$ord['ts'].(!empty($pitemid) ? '&Itemid='.$pitemid : (!empty($start_itemid) ? '&Itemid='.$start_itemid : ''))); ?>"><?php VikBookingIcons::e('star'); ?> <?php echo JText::_('VBOLEAVEAREVIEW'); ?></a>
						</div>
					</div>
				</div>
				<?php
				...
				<div class="vbo-booking-canc-container">
					<div class="vbo-booking-canc-inner">
						<div class="vbo-booking-canc-cmd">
							<span onclick="document.getElementById('vbo-booking-cancform-container').style.display='block';location.hash='bcancf';"><?php VikBookingIcons::e('times-circle'); ?> <?php echo JText::_('VBOCANCYOURBOOKING'); ?></span>
						</div>
					</div>
				</div>
				<?php
			...
			if ($resmodcanc === 1 && $this->days_to_arrival >= $resmodcancmin && !$isotabooking) {
				...
				<div class="vbo-booking-mod-container">
					<div class="vbo-booking-mod-inner">
						<div class="vbo-booking-mod-cmd">
							<a onclick="vbOpenCancOrdForm();" href="javascript: void(0);"><?php VikBookingIcons::e('envelope'); ?> <?php echo JText::_('VBREQUESTCANCMOD'); ?></a>
						</div>
					</div>
				</div>
				<?php
			...
			if (count($this->upselling)) {
				...
				<div class="vbo-hidein-print vbo-booking-mod-container vbo-booking-upselling-wrap">
					<div class="vbo-booking-mod-inner">
						<div class="vbo-booking-mod-cmd">
							<a onclick="vbGotoUpsell();" href="javascript: void(0);"><?php VikBookingIcons::e('cart-plus'); ?> <?php echo JText::_('VBOADDEXTRASTOBOOK'); ?></a>
						</div>
					</div>
				</div>
				<?php
		...
		if ($ord['split_stay'] && count($room_stay_dates) && isset($room_stay_dates[$kor]) && $room_stay_dates[$kor]['idroom'] == $or['idroom']) {
			...
			<div class="vbo-booking-splitstay-info">
				<div class="vbo-booking-splitstay-info-room">
					<span class="vbo-booking-splitstay-nights"><?php VikBookingIcons::e('moon'); ?> <?php echo $room_stay_nights . ' ' . ($room_stay_nights > 1 ? JText::_('VBDAYS') : JText::_('VBDAY')); ?></span>
			...
			if ($rate_plan_data) {
				...
				if ($rate_plan_data['free_cancellation'] == 1) {
					if ($rate_plan_data['canc_deadline'] > 0) {
						...
						$canc_policy_type = JText::_('VBFREECANCELLATION');
					}
					...
					$canc_policy_type = JText::_('VBONONREFUNDRATE');
				}
				?>
...
if ($usedcoupon === true) {
	...
	<div class="vbo-booking-cost-detail vbo-booking-cost-detail-discount">
		<div class="vbo-booking-cost-lbl">
			<span><?php echo JText::_('VBCOUPON') . ' ' . $expcoupon[2]; ?></span>
		</div>
...
if ($ord['refund'] > 0) {
	...
	<div class="vbo-booking-cost-detail vbo-booking-cost-detail-refund">
		<div class="vbo-booking-cost-lbl">
			<span><?php echo JText::_('VBO_AMOUNT_REFUNDED'); ?></span>
		</div>
		<div class="vbo-booking-cost-val">
			<span class="vbo-booking-cost-val-number">
...
?>
	<div class="vbo-booking-cost-detail vbo-booking-cost-detail-total">
		<div class="vbo-booking-cost-lbl">
			<span><?php echo JText::_('VBTOTAL'); ?></span>
		</div>
		<div class="vbo-booking-cost-val">
			<span class="vbo-booking-cost-val-number">
...
if ($ord['status'] == 'confirmed' && is_array($payment) && VikBooking::multiplePayments() && $ord['total'] > 0 && $payable) {
	...
	if (VBOPlatformDetection::isWordPress()) {
		...
		if ($itemid) {
			...
			$error_url = JRoute::_($error_url . "&Itemid={$itemid}", false);
			$notify_url = JRoute::_($notify_url . "&Itemid={$itemid}", false);
		}
	} else {
	...
	$array_order['transaction_name'] = !empty($transaction_name) ? $transaction_name : (JText::_('VBORDERNUMBER') . ' ' . $ord['id']);
	...
	<div class="vbo-booking-cost-detail vbo-booking-cost-detail-amountpaid">
		<div class="vbo-booking-cost-lbl">
			<span><?php echo JText::_('VBAMOUNTPAID'); ?></span>
		</div>
		<div class="vbo-booking-cost-val">
			<span class="vbo-booking-cost-val-number">
	...
	<div class="vbo-booking-cost-detail vbo-booking-cost-detail-remainingbalance">
		<div class="vbo-booking-cost-lbl">
			<span><?php echo JText::_('VBTOTALREMAINING'); ?></span>
		</div>
		<div class="vbo-booking-cost-val">
			<span class="vbo-booking-cost-val-number">
	...
	<div class="vbo-booking-cost-detail vbo-booking-cost-detail-dd-payable-later">
		<div class="vbo-booking-cost-lbl">
			<span><?php echo JText::_('VBO_DAMAGE_DEPOSIT'); ?></span>
		</div>
		<div class="vbo-booking-cost-val">
			<span class="vbo-booking-cost-val-number">
		...
		<form action="<?php echo JRoute::_('index.php?option=com_vikbooking'.(!empty($pitemid) ? '&Itemid='.$pitemid : '')); ?>" name="vbcanc" method="post" onsubmit="javascript: return vbValidateCancForm();">
			<div class="vbordcancform-inner">
				...
				<div class="vbordcancform-entry-submit">
					<input type="submit" name="sendrequest" value="<?php echo JText::_('VBREQUESTCANCMODSUBMIT'); ?>" class="btn vbo-pref-color-btn"/>
				</div>
			</div>
		<?php
		if (!empty($pitemid)) {
...
if (is_array($payment) && $ord['status'] == 'standby') {
	...
	if (VBOPlatformDetection::isWordPress()) {
		...
		$elapsed_redirect_uri = JRoute::_('index.php?option=com_vikbooking&view=booking&sid='.(!empty($ord['idorderota']) && !empty($ord['channel']) ? $ord['idorderota'] : $ord['sid']).'&ts='.$ord['ts'].(!empty($bestitemid) ? '&Itemid='.$bestitemid : (!empty($pitemid) ? '&Itemid='.$pitemid : '')), false);
	}
	...
	if ($minautoremove > 0 && $minautoremove < 35791) {
		...
		$lbl_hour = strtolower(JText::_('VBHOUR'));
		$lbl_hours = strtolower(JText::_('VBHOURS'));
		$lbl_minute = strtolower(JText::_('VBMINUTE'));
		$lbl_minutes = strtolower(JText::_('VBMINUTES'));
...
if (!$payProcessorInstances && $ord['status'] == 'confirmed' && ($this->damage_deposit_payment['damagedep_gross'] ?? 0) > 0 && ($this->damage_deposit_payment['payment_window']['payable'] ?? false) && !$this->prev_dd_payments) {
	...
	$array_order['rooms_name'] = implode(", ", $roomsnames);
	$array_order['transaction_name'] = !empty($transaction_name) ? $transaction_name : (JText::_('VBORDERNUMBER') . ' ' . $ord['id']);
	$array_order['transaction_name'] .= ' - ' . JText::_('VBO_DAMAGE_DEPOSIT');
	...
	<div class="vbo-booking-guest-review-inner">
		...
		if (isset($review['content']['scoring']) && count($review['content']['scoring']) > 1) {
			...
			<div class="vbo-booking-guest-review-services-score vbo-booking-guest-review-singleservice">
				<div class="vbo-booking-guest-review-service-name">
					<span><?php echo JText::_('VBOREVIEWYOURRATING'); ?></span>
				</div>
				<div class="vbo-booking-guest-review-service-stars">
				<?php
				for ($i = 1; $i <= $stars_count; $i++) {
		...
		if (isset($review['content']['reply']) && ((is_string($review['content']['reply']) && !empty($review['content']['reply'])) || (is_array($review['content']['reply']) && !empty($review['content']['reply']['text'])))) {
			...
			<div class="vbo-booking-guest-review-owner-reply">
				<h5><?php echo JText::_('VBOGREVOWNREPLY'); ?></h5>
...
<div class="vbo-hidein-print vbo-booking-upsell-container">
	<h3><?php echo JText::_('VBOUPSELLTITLE'); ?></h3>
	<form method="post" action="<?php echo JRoute::_('index.php?option=com_vikbooking&task=upsellextras' . (!empty($pitemid) ? '&Itemid='.$pitemid : '')); ?>">
		...
		foreach ($this->upselling as $kor => $uproom) {
			...
			if ($tot_upselling > 1) {
				...
				<h5 class="vbo-booking-upsell-room-details">
					...
					<span class="vbo-booking-upsell-room-adults"><?php echo $uproom->adults; ?> <?php echo ($uproom->adults == 1 ? JText::_('VBSEARCHRESADULT') : JText::_('VBSEARCHRESADULTS')); ?></span>
				<?php
				if ($orderrooms[$kor]['children'] > 0) {
					?>
					<span class="vbo-booking-upsell-room-children"><?php echo $orderrooms[$kor]['children'] . " " . ($orderrooms[$kor]['children'] == 1 ? JText::_('VBSEARCHRESCHILD') : JText::_('VBSEARCHRESCHILDREN')); ?></span>
					<?php
				}
				?>
				</h5>
				...
				foreach ($uproom->upsellable as $o) {
					...
					if (intval($o['hmany']) == 1) {
						...
						$optquanbtn = '<button type="button" class="btn vbo-pref-color-btn" onclick="vboAddExtra(' . $kor . ', ' . $o['id'] . ', -1);"><i class="' . VikBookingIcons::i('cart-plus') . '"></i> ' . JText::_('VBOUPSELLADD') . '</button>';
					} else {
						$optquanbtn = '<button type="button" class="btn vbo-pref-color-btn" onclick="vboAddExtra(' . $kor . ', ' . $o['id'] . ', 1);"><i class="' . VikBookingIcons::i('cart-plus') . '"></i> ' . JText::_('VBOUPSELLADD') . '</button>';
					}
					?>
		...
		<div class="vbo-booking-upsell-confirm" style="display: none;">
			<div class="vbo-booking-upsell-confirm-inner">
				<div class="vbo-booking-upsell-confirm-total">
					<span class="vbo-booking-upsell-confirm-txt"><?php echo JText::_('VBTOTAL'); ?></span>
				...
				<div class="vbo-booking-upsell-confirm-btn">
					<button type="submit" class="btn btn-large vbo-pref-color-btn" onclick="return confirm('<?php echo addslashes(JText::_('VBOUPSELLCONFIRM')); ?>');"><?php VikBookingIcons::e('check-circle'); ?> <?php echo JText::_('VBOUPSELLUPDATE'); ?></button>
				</div>
			</div>
		</div>
	</form>
...
function vboAddExtra(kor, optid, quant) {
	if (quant < 0) {
		...
		if (quant < 1) {
			alert('<?php echo addslashes(JText::_('VBOUPSELLQUANTOPT0')); ?>');
			return false;
		}
	}
