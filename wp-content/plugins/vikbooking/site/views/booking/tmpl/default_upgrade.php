...
<div class="vbo-booking-mod-container">
	<div class="vbo-booking-mod-inner">
		<div class="vbo-booking-mod-cmd vbo-booking-roomupgrade-cmd">
			<a href="JavaScript: void(0);" onclick="vboDisplayUpgradeOptions();"><?php VikBookingIcons::e('gem'); ?> <?php echo JText::_('VBO_UPGRADE_ROOMS'); ?></a>
		</div>
	</div>
</div>
...
<div id="vbo-hidden-roomupgrade-target" style="display: none;">
				...
				foreach ($upg_data['r_costs'] as $rid => $upgrade_sol) {
					...
					<div class="vbo-roomupgrade-solution-cont">
							...
							<div class="vbo-roomupgrade-solution-info">
								...
								?>
									<div class="vbo-roomupgrade-solution-name">
										<a href="<?php echo JRoute::_('index.php?option=com_vikbooking&view=roomdetails&roomid=' . $rid . (!empty($pitemid) ? '&Itemid='.$pitemid : '')); ?>" target="_blank"><?php echo $upgrade_options['rooms'][$rid]['name']; ?></a>
									</div>
								</div>
							<?php
						...
						<div class="vbo-roomupgrade-solution-confirm">
							<button type="button" class="btn vbo-pref-color-btn" onclick="vboDoUpgrade('<?php echo $rid; ?>');"><?php echo JText::_('VBO_UPGRADE_CONFIRM'); ?></button>
						</div>
					</div>
					<?php
				}
	...
	// AJAX upgrade URL
	$ajax_upgrade_url = VikBooking::ajaxUrl(JRoute::_('index.php?option=com_vikbooking&task=upgrade_room' . (!empty($pitemid) ? '&Itemid='.$pitemid : ''), false));
