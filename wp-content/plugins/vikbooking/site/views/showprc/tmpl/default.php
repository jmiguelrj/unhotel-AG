...
// rate plan meals map
$rate_plan_meals_map = [
	'breakfast' => JText::_('VBO_MEAL_BREAKFAST'),
	'lunch'     => JText::_('VBO_MEAL_LUNCH'),
	'dinner'    => JText::_('VBO_MEAL_DINNER'),
];
...
<div class="vbstepsbarcont">
	...
	if ($is_package === true) {
		...
		<li class="vbo-step vbo-step-complete"><a href="<?php echo JRoute::_('index.php?option=com_vikbooking&view=packagedetails&pkgid='.$this->pkg['id'].(!empty($pitemid) ? '&Itemid='.$pitemid : ''), false); ?>"><?php echo JText::_('VBSTEPROOMSELECTION'); ?></a></li>
		<?php
	} else {
		if ($this->roomsnum < 2 && (int)$session->get('vboSearchRoomId', 0) == (int)$this->rooms[1]['id']) {
			...
			$dateslink = JRoute::_('index.php?option=com_vikbooking&view=roomdetails&roomid='.$this->rooms[1]['id'].'&checkin='.$this->checkin.'&checkout='.$this->checkout.(!empty($pitemid) ? '&Itemid='.$pitemid : ''));
		} else {
			// link to general search form
			$dateslink = JRoute::_('index.php?option=com_vikbooking&view=vikbooking&checkin='.$this->checkin.'&checkout='.$this->checkout.(!empty($pitemid) ? '&Itemid='.$pitemid : ''));
		}
		?>
		<li class="vbo-step vbo-step-complete"><a href="<?php echo $dateslink; ?>"><?php echo JText::_('VBSTEPDATES'); ?></a></li>
		<li class="vbo-step vbo-step-complete"><a href="<?php echo JRoute::_('index.php?option=com_vikbooking&task=search&checkindate='.urlencode($checkinforlink).'&checkoutdate='.urlencode($checkoutforlink).'&roomsnum='.$this->roomsnum.$peopleforlink.(!empty($pitemid) ? '&Itemid='.$pitemid : ''), false); ?>"><?php echo JText::_('VBSTEPROOMSELECTION'); ?></a></li>
		<?php
	}
	?>
...
<div class="vbo-showprc-head-wrapper">
	...
	?>
		<div class="vbcheckinroom">
			...
			<div class="vbo-results-head-det">
				<span class="vbo-results-head-det-lbl"><?php echo JText::_('VBCHECKINONTHE'); ?></span>
		...
		<div class="vbcheckoutroom">
			...
			<div class="vbo-results-head-det">
				<span class="vbo-results-head-det-lbl"><?php echo JText::_('VBCHECKOUTONTHE'); ?></span>
	...
	<form action="<?php echo JRoute::_('index.php?option=com_vikbooking&task=oconfirm'.(!empty($pitemid) ? '&Itemid='.$pitemid : '')); ?>" method="post">
	...
	foreach ($this->rooms as $num => $r) {
		...
		<div class="room_container">
			...
			if ($this->roomsnum > 1) {
				...
				?>
					<span class="vbo-showprc-roomnum-num"><?php echo JText::_('VBSEARCHROOMNUM') . ' ' . $num; ?></span>
				</div>
				<?php
			}
			?>
				<div class="vbo-showprc-staydetails">
					<div class="vbo-showprc-staydetails-party">
						...
						<?php echo $this->arrpeople[$num]['adults']; ?> <?php echo ($this->arrpeople[$num]['adults'] == 1 ? JText::_('VBSEARCHRESADULT') : JText::_('VBSEARCHRESADULTS')); ?><?php echo ($showchildren && $this->arrpeople[$num]['children'] > 0 ? ", ".$this->arrpeople[$num]['children']." ".($this->arrpeople[$num]['children'] == 1 ? JText::_('VBSEARCHRESCHILD') : JText::_('VBSEARCHRESCHILDREN')) : ""); ?>
					</div>
				<?php
				if ($is_split_room) {
					?>
					<div class="vbo-showprc-staydetails-splitstay vbo-showprc-staydetails-nights">
						...
						<?php echo $room_nights . ' ' . ($room_nights > 1 ? JText::_('VBDAYS') : JText::_('VBDAY')); ?>
					</div>
			...
			if (!empty($carats)) {
				...
				<div class="room_carats">
					<h4><?php echo JText::_('VBCHARACTERISTICS'); ?></h4>
					<?php echo $carats; ?>
				</div>
				<?php
			}
			...
			foreach ($this->tars[$num] as $k => $t) {
				...
				if ($rate_plan_meals) {
					...
					$priceinfostr .= '<span class="vbprice_breakfast vbprice_all_meals_incl">' . rtrim(JText::_('VBO_MEAL_PLANS_INCL'), ':') . ': ' . implode(', ', $rate_plan_meals_incl) . '</span>';
				}
						...
						<div class="vbageintervals">
							...
							for ($ch = 1; $ch <= $this->arrpeople[$num]['children']; $ch++) {
								...
								<div class="vbo-showprc-child-fee">
									<span><?php echo JText::_('VBSEARCHRESCHILD') . ' #' . $ch; ?></span>
	...
	foreach ($this->arrpeople as $indroom => $aduch) {
		...
		<div class="room_buttons_box">
			...
			<input type="submit" name="goon" value="<?php echo JText::_('VBBOOKNOW'); ?>" class="btn booknow vbo-pref-color-btn"/>
		</div>
	<?php
	/**
