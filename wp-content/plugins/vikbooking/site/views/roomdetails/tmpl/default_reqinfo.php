...
<div class="vbo-reqinfo-cont">
	<span><a href="Javascript: void(0);" onclick="vboShowRequestInfo();" class="vbo-reqinfo-opener vbo-pref-color-btn"><?php echo JText::_('VBOROOMREQINFOBTN'); ?></a></span>
</div>
<div id="vbdialog-overlay" style="display: none;">
	...
	<div class="vbdialog-inner vbdialog-reqinfo">
		...
		<form action="<?php echo JRoute::_('index.php?option=com_vikbooking&task=reqinfo'.(!empty($pitemid) ? '&Itemid='.$pitemid : '')); ?>" method="post">
			...
			<div class="vbdialog-reqinfo-formcont">
				<div class="vbdialog-reqinfo-formentry">
					...
					<input type="text" name="reqname" id="reqname" value="" placeholder="<?php echo JText::_('VBOROOMREQINFONAME'); ?>" required />
				</div>
				<div class="vbdialog-reqinfo-formentry">
					<label for="reqemail"><?php echo JText::_('VBOROOMREQINFOEMAIL'); ?></label>
					<input type="email" name="reqemail" id="reqemail" value="<?php echo $cur_email; ?>" placeholder="<?php echo JText::_('VBOROOMREQINFOEMAIL'); ?>" required />
				</div>
				<div class="vbdialog-reqinfo-formentry">
			...
			if ($this->terms_fields) {
				foreach ($this->terms_fields as $k => $terms_field) {
					if (!empty($terms_field['poplink'])) {
						$fname = "<a href=\"" . $terms_field['poplink'] . "\" id=\"vbof{$k}\" rel=\"{handler: 'iframe', size: {x: 750, y: 600}}\" target=\"_blank\" class=\"vbomodalframe\">" . JText::_($terms_field['name']) . "</a>";
					} else {
						$fname = "<label id=\"vbof{$k}\" for=\"vbof-inp{$k}\" style=\"display: inline-block;\">" . JText::_($terms_field['name']) . "</label>";
					}
					?>
					<div class="vbdialog-reqinfo-formentry vbdialog-reqinfo-formentry-ckbox">
						<?php echo $fname; ?>
						<input type="checkbox" name="vbof" id="vbof-inp<?php echo $k; ?>" value="<?php echo JText::_('VBYES'); ?>" required />
					</div>
					<?php
				}
			} else {
				...
				<div class="vbdialog-reqinfo-formentry vbdialog-reqinfo-formentry-ckbox">
					...
					<input type="checkbox" name="vbof" id="vbof-inp" value="<?php echo JText::_('VBYES'); ?>" required />
				</div>
				<?php
			}
			...
			?>
				<div class="vbdialog-reqinfo-formentry vbdialog-reqinfo-formsubmit">
					<button type="submit" class="btn vbo-pref-color-btn"><?php echo JText::_('VBOROOMREQINFOSEND'); ?></button>
				</div>
			</div>
		</form>
	</div>
