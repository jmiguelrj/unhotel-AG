...
class VikbookingViewOrderslist extends JViewVikBooking
{
	public function display($tpl = null)
	{
		...
		if (!empty($pconfirmnumber)) {
			...
			if ($odata) {
				$app->redirect(JRoute::_('index.php?option=com_vikbooking&view=booking&sid='.(!empty($odata[0]['sid']) ? $odata[0]['sid'] : $odata[0]['idorderota']).'&ts='.$odata[0]['ts'].(!empty($pitemid) ? '&Itemid='.$pitemid : ''), false));
				$app->close();
			}
