...
<form action="<?php echo JRoute::_('index.php?option=com_vikbooking&view=orderslist'.(!empty($pitemid) ? '&Itemid='.$pitemid : '')); ?>" method="post">
	<div class="vbsearchorderdiv">
		...
		<div class="vbo-bookings-list-search">
			<span><?php echo JText::_('VBCONFIRMNUMBORPIN'); ?></span>
			...
			<input type="submit" class="btn vbsearchordersubmit vbo-pref-color-btn" name="vbsearchorder" value="<?php echo JText::_('VBSEARCHCONFIRMNUMBBTN'); ?>"/>
		</div>
	</div>
</form>
...
<div class="vbo-bookings-list-container">
	...
	foreach ($userorders as $ord) {
		...
		<div class="vbo-bookings-list-table-row vbo-bookings-list-table-body-row vbo-bookings-list-table-body-row-<?php echo $ord['status']; ?>">
			...
			<div class="vbo-bookings-list-table-cell">
				<span class="vbo-bookings-list-table-cell-lbl"><?php echo JText::_('VBCONFIRMNUMB'); ?></span>
				<span><a href="<?php echo JRoute::_('index.php?option=com_vikbooking&view=booking&sid='.(!empty($ord['sid']) ? $ord['sid'] : $ord['idorderota']).'&ts='.$ord['ts'].(!empty($pitemid) ? '&Itemid='.$pitemid : '')); ?>"><?php echo (!empty($ord['confirmnumber']) ? $ord['confirmnumber'] : ($ord['status'] == 'standby' ? JText::_('VBINATTESA') : '--------')); ?></a></span>
			</div>
			<div class="vbo-bookings-list-table-cell">
				<span class="vbo-bookings-list-table-cell-lbl"><?php echo JText::_('VBBOOKINGDATE'); ?></span>
				<span><?php echo date(str_replace("/", $datesep, $df).' H:i', $ord['ts']); ?></span>
			</div>
			<div class="vbo-bookings-list-table-cell">
				<span class="vbo-bookings-list-table-cell-lbl"><?php echo JText::_('VBPICKUP'); ?></span>
				<span><?php echo date(str_replace("/", $datesep, $df), $ord['checkin']); ?></span>
			</div>
			<div class="vbo-bookings-list-table-cell">
				<span class="vbo-bookings-list-table-cell-lbl"><?php echo JText::_('VBRETURN'); ?></span>
				<span><?php echo date(str_replace("/", $datesep, $df), $ord['checkout']); ?></span>
			</div>
			<div class="vbo-bookings-list-table-cell">
				<span class="vbo-bookings-list-table-cell-lbl"><?php echo JText::_('VBDAYS'); ?></span>
