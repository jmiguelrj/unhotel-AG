...
<div class="vbo-booking-details-topcontainer vbo-booking-details-precheckin">
	...
	<div class="vbo-booking-details-head vbo-booking-details-head-confirmed">
		<h4><?php echo JText::_('VBOPRECHECKIN'); ?></h4>
	</div>
	...
	<div class="vbo-precheckin-container">
		<div class="info vbo-precheckin-disclaimer"><?php echo JText::_('VBO_PRECHECKIN_DISCLAIMER'); ?></div>
		<form action="<?php echo JRoute::_('index.php?option=com_vikbooking'.(!empty($pitemid) ? '&Itemid='.$pitemid : '')); ?>" method="post" id="vbo-precheckin-main-form">
					...
					for ($g = 1; $g <= $guests_to_register; $g++) {
						...
						<div class="vbo-precheckin-adult-wrap">
							...
							foreach ($pax_fields as $paxk => $paxv) {
								...
								?><div class="vbo-precheckin-guest-detail<?php echo $fieldClasses ? ' ' . implode(' ', $fieldClasses) : ''; ?>" data-rindex="<?php echo $ind; ?>" data-gindex="<?php echo $g; ?>" data-paxk="<?php echo JHtml::_('esc_attr', $paxk); ?>">
								...
								if ($field_rendering_type === 'paxfield') {
									...
									if ($mrzDetectionSupport) {
										...
										<div class="vbo-paxfield-upload-instructions">
											<span><?php VikBookingIcons::e('info-circle'); ?> <?php echo JText::_('VBO_DOCS_UPLOAD_MRZ_HELP'); ?></span>
										</div>
										<?php
									}
									?>
										...
										<div class="vbo-paxfield-upload-container">
											<button type="button" class="btn vbo-pref-color-btn-secondary vbo-precheckin-uploadfile" data-roomi="<?php echo $ind; ?>" data-guesti="<?php echo $g; ?>" data-paxk="<?php echo JHtml::_('esc_attr', $paxk); ?>"><?php VikBookingIcons::e('camera'); ?> <?php echo JText::_('VBOUPSELLADD'); ?></button>
		...
		?>
			<div class="vbo-precheckin-submit">
				<button type="submit" class="btn btn-large vbo-pref-color-btn"><?php echo JText::_('VBOSUBMITPRECHECKIN'); ?></button>
			</div>
...
<script type="text/javascript">
	...
	function vboUploadSetProgress(selector_suffix, progress, mrzSupported) {
		...
		if (progress >= 99 && mrzSupported) {
			...
			progressEl.textContent = <?php echo json_encode(JText::_('VBO_ANALYSING_DOCS')); ?> + '...';
		} else {
	...
	function vboUploadDocuments(files) {
		...
		vboDoAjaxUpload(
			// url
			'<?php echo VikBooking::ajaxUrl(JRoute::_('index.php?option=com_vikbooking&task=precheckin_upload_docs&tmpl=component'.(!empty($pitemid) ? '&Itemid='.$pitemid : ''), false)); ?>',
			// form data
			formData,
			// success callback
			(response) => {
