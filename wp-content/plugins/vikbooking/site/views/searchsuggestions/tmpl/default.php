...
<div id="vbo-splitstay-suggestions-container-<?php echo $this->code; ?>" class="vbo-search-suggestions-container vbo-splitstay-suggestions-container">
	<h4><?php echo JText::_('VBO_SPLIT_STAY_SOLS'); ?></h4>
	<p class="vbo-search-suggestions-intro"><?php echo JText::_('VBO_SPLIT_STAY_SOLS_DESCR'); ?></p>
	...
	foreach ($this->split_stay_solutions as $split_stay_sol) {
		...
		<div class="vbo-booking-solution vbo-splitstay-solution">
			...
			<div class="vbo-booking-solution-rooms">
				<div class="vbo-booking-solution-totrooms"><?php echo $sol_tot_rooms . ' ' . ($sol_tot_rooms > 1 ? JText::_('VBSEARCHRESROOMS') : JText::_('VBSEARCHRESROOM')); ?></div>
			<?php
			foreach ($split_stay_sol as $split_stay) {
				...
				<div class="vbo-booking-solution-room vbo-splitstay-solution-room">
					...
					<div class="vbo-splitstay-solution-details">
						<span class="vbo-splitstay-solution-nights"><?php VikBookingIcons::e('moon'); ?> <?php echo $split_stay['nights'] . ' ' . ($split_stay['nights'] > 1 ? JText::_('VBDAYS') : JText::_('VBDAY')); ?></span>
			...
			<div class="vbo-booking-solution-book vbselectr">
				<a class="btn vbo-pref-color-btn" href="<?php echo JRoute::_('index.php?' . http_build_query($split_stay_url_data)); ?>"><?php echo JText::_('VBBOOKNOW'); ?></a>
			</div>
		</div>
		<?php
		$splitstay_suggested++;
...
<div id="vbo-search-suggestions-container-<?php echo $this->code; ?>" class="vbo-search-suggestions-container">
	<h4><?php echo JText::_('VBOSEARCHSUGGAVNEAR'); ?></h4>
	<p class="vbo-search-suggestions-intro"><?php echo JText::_('VBOSEARCHSUGGINTROCODE' . ($this->code === 1 ? '1' : '2')); ?></p>
	...
	<div class="vbo-search-solutions-container">
		<h4><?php echo JText::sprintf('VBOBOOKSOLSUGGNIGHTS', $this->nights.' '.($this->nights > 1 ? JText::_('VBSEARCHRESNIGHTS') : JText::_('VBSEARCHRESNIGHT'))); ?></h4>
		...
		foreach ($this->solutions as $keyday => $solution) {
			...
			<div class="vbo-search-solution" data-daysol="<?php echo implode(';', $nights_suggested); ?>">
				...
				<div class="vbo-search-solution-book vbselectr">
					<a href="<?php echo JRoute::_('index.php?option=com_vikbooking&task=search&checkindate='.date($df, $day_info[0]).'&checkinh='.$hcheckin.'&checkinm='.$mcheckin.'&checkoutdate='.date($df, $out_info[0]).'&checkouth='.$hcheckout.'&checkoutm='.$mcheckout.'&roomsnum='.count($this->party).$aduchild_str.(!empty($pcategories) ? '&categories='.$pcategories : '').(!empty($pitemid) ? '&Itemid='.$pitemid : '')); ?>" class="btn vbo-pref-color-btn"><?php echo JText::_('VBOBOOKSOLSEARCHROOMS'); ?></a>
				</div>
			</div>
			<?php
			$sug_count++;
		...
		?>
	<div class="vbo-search-solutions-container">
		<p class="vbo-search-no-solutions"><?php echo JText::_('VBOSEARCHNOSOLUTIONS'); ?></p>
	</div>
		<?php
	}
	...
	if (($this->code == 2 || $this->code == 3) && $this->booking_solutions) {
		...
		if ($this->party_guests['adults'] > 0) {
			$guests_strarr[] = $this->party_guests['adults'].' '.($this->party_guests['adults'] > 1 ? JText::_('VBSEARCHRESADULTS') : JText::_('VBSEARCHRESADULT'));
		}
		if ($this->party_guests['children'] > 0) {
			$guests_strarr[] = $this->party_guests['children'].' '.($this->party_guests['children'] > 1 ? JText::_('VBSEARCHRESCHILDREN') : JText::_('VBSEARCHRESCHILD'));
		}
		?>
	<div class="vbo-booking-solutions-container">
			...
			<div class="vbo-booking-solution" data-daysol="<?php echo implode(';', $nights_suggested); ?>">
				...
				foreach ($solution as $roomsol) {
					...
					if ($roomsol['guests_allocation']['adults'] > 0) {
						$room_guests_strarr[] = $roomsol['guests_allocation']['adults'].' '.($roomsol['guests_allocation']['adults'] > 1 ? JText::_('VBSEARCHRESADULTS') : JText::_('VBSEARCHRESADULT'));
					}
					if ($roomsol['guests_allocation']['children'] > 0) {
						$room_guests_strarr[] = $roomsol['guests_allocation']['children'].' '.($roomsol['guests_allocation']['children'] > 1 ? JText::_('VBSEARCHRESCHILDREN') : JText::_('VBSEARCHRESCHILD'));
					}
				...
				<div class="vbo-booking-solution-book vbselectr">
					<a class="btn vbo-pref-color-btn" href="<?php echo JRoute::_('index.php?option=com_vikbooking&task=search&suggestion='.$this->code.'&checkindate='.date($df, $day_info[0]).'&checkinh='.$hcheckin.'&checkinm='.$mcheckin.'&checkoutdate='.date($df, $out_info[0]).'&checkouth='.$hcheckout.'&checkoutm='.$mcheckout.'&roomsnum='.$sol_tot_rooms.$aduchild_str.(!empty($pcategories) ? '&categories='.$pcategories : '').(!empty($pitemid) ? '&Itemid='.$pitemid : '')); ?>"><?php echo JText::_('VBBOOKNOW'); ?></a>
				</div>
			</div>
			<?php
			$sug_count++;
		...
		?>
	<div class="vbo-booking-solutions-container">
		<p class="vbo-search-no-solutions"><?php echo JText::_('VBOSEARCHNOBOOKSOLUTIONS'); ?></p>
	</div>
		<?php
	}
	?>
			...
			?>
				...
				if (paramshowpeople > 0) {
					...
					if (paramshowpeople == 1) {
						daycont += '<div class="vbmaxadultsdet"><span class="vbmaximgdet"><?php VikBookingIcons::e('male'); ?></span><span class="vbmaxlabeldet"><?php echo addslashes(JText::_('VBFORMADULTS')); ?></span><span class="vbmaxnumberdet">'+maxadustr+'</span></div>';
					} else if (paramshowpeople == 2) {
						daycont += '<div class="vbmaxchildrendet"><span class="vbmaximgdet"><?php VikBookingIcons::e('child'); ?></span><span class="vbmaxlabeldet"><?php echo addslashes(JText::_('VBFORMCHILDREN')); ?></span><span class="vbmaxnumberdet">'+maxchistr+'</span></div>';
					} else if (paramshowpeople == 3) {
						daycont += '<div class="vbmaxadultsdet"><span class="vbmaximgdet"><?php VikBookingIcons::e('male'); ?></span><span class="vbmaxlabeldet"><?php echo addslashes(JText::_('VBFORMADULTS')); ?></span><span class="vbmaxnumberdet">'+maxadustr+'</span></div>';
						daycont += '<div class="vbmaxtotdet"><span class="vbmaximgdet"><?php VikBookingIcons::e('users'); ?></span><span class="vbmaxlabeldet"><?php echo addslashes(JText::_('VBMAXTOTPEOPLE')); ?></span><span class="vbmaxnumberdet">'+maxtotstr+'</span></div>';
