...
class VikBookingController extends JControllerVikBooking
{
	...
	public function donut_charts_data() {
		...
		for ($i = 0; $i < $days; $i++) {
			...
			$data_obj->lbl = $data_obj->ymd == $todayymd ? JText::_('VBTODAY') . ', ' . $data_obj->lbl : $data_obj->lbl . ' ' . $monthsmap[($from_info['mon'] - 1)];
			...
			if (!strlen($av_helper->getError())) {
				if (array_key_exists($id_room, $arr_res)) {
					...
					foreach ($arr_res[$id_room] as $rate) {
						...
						$response .= '<span class="vbo-calcrates-pricedet vbo-calcrates-ratenet"><span>'.JText::_('VBCALCRATESNET').'</span>'.$currencysymb.' '.VikBooking::numberFormat($rate['cost']).'</span>';
						$response .= '<span class="vbo-calcrates-pricedet vbo-calcrates-ratetax"><span>'.JText::_('VBCALCRATESTAX').'</span>'.$currencysymb.' '.VikBooking::numberFormat($rate['taxes']).'</span>';
						if (!empty($rate['city_taxes'])) {
							$response .= '<span class="vbo-calcrates-pricedet vbo-calcrates-ratecitytax"><span>'.JText::_('VBCALCRATESCITYTAX').'</span>'.$currencysymb.' '.VikBooking::numberFormat($rate['city_taxes']).'</span>';
						}
						if (!empty($rate['fees'])) {
							$response .= '<span class="vbo-calcrates-pricedet vbo-calcrates-ratefees"><span>'.JText::_('VBCALCRATESFEES').'</span>'.$currencysymb.' '.VikBooking::numberFormat($rate['fees']).'</span>';
						}
						if (array_key_exists('affdays', $rate) && $rate['affdays'] > 0) {
							$extra_response .= '<span class="vbo-calcrates-extrapricedet vbo-calcrates-ratespaffdays"><span>'.JText::_('VBCALCRATESSPAFFDAYS').'</span>'.$rate['affdays'].'</span>';
						}
	...
	private function do_createroom($stay = false) {
		...
		if (empty($pcname)) {
			$app->enqueueMessage(JText::_('VBO_PLEASE_FILL_FIELDS'), 'error');
		...
		if ($stay === true) {
			$app->enqueueMessage(JText::_('VBOROOMSAVEOK').' - <a href="index.php?option=com_vikbooking&task=tariffs&cid[]='.$lid.'">'.JText::_('VBOGOTORATES').'</a>');
	...
	private function do_updateroom($stay = false)
	{
		...
		if (!empty($pcname)) {
			...
			$app->enqueueMessage(JText::_('VBUPDROOMOK'));
		}
	...
	private function do_updatebusy($callback = '')
	{
						...
						foreach (($room_params['features'] ?? []) as $rind => $rfeatures) {
							if ($rind == $invb['roomindex']) {
								foreach ($rfeatures as $fname => $fval) {
									if (strlen($fval)) {
										$subunit_name = '#' . $rind . ' - ' . JText::_($fname) . ': ' . $fval;
										break;
									}
								}
							}
						}
						$adjust_link = '<br/><a class="btn btn-danger" target="_blank" href="index.php?option=com_vikbooking&task=editorder&cid[]=' . $invb['idorder'] . '">' . JText::_('VBOSUBUNITOVERBOOKEDGOTO') . '</a>';
						$app->enqueueMessage(
							JText::sprintf(
		...
		if ($opertwounits === true || $forcebooking) {
			...
			$allow_force = 1;
			$app->enqueueMessage(JText::_('VBO_BOOKING_SHOULDFORCE'), 'notice');
		}
	...
	private function do_updateseason($stay = false)
	{
		...
		if (!$valid || !$prev_record) {
			VikError::raiseWarning('', JText::_('ERRINVDATEROOMSLOCSEASON').($double_records ? ' ('.implode(', ', array_unique($double_records)).')' : ''));
			$app->redirect("index.php?option=com_vikbooking&task=editseason&cid[]=" . $pwhere);
			exit;
		}
		...
		if (!$prev_record['promo'] && $ppromo && $promo_handlers) {
			...
			$ppromo = 0;
			$app->enqueueMessage(JText::_('VBO_NOPROMO_UPD_CHANNELS'), 'warning');
		}
		...
		if ($promo_handlers && $proundmode) {
			 ...
			 */
			$proundmode = '';
			$app->enqueueMessage(sprintf('%s: %s.', JText::_('VBNEWSEASONROUNDCOST'), JText::_('VBPARAMPRICECALENDARDISABLED')), 'warning');
		}
	...
	private function do_createseason($andnew = false)
	{
		...
		if (!$valid && !$ppromo) {
			VikError::raiseWarning('', JText::_('ERRINVDATEROOMSLOCSEASON').(count($double_records) ? ' ('.implode(', ', array_unique($double_records)).')' : ''));
			$app->redirect("index.php?option=com_vikbooking&task=newseason");
			exit;
		}
		...
		if ($ppromo && $pchannels) {
			foreach ($pchannels as $channel_key) {
				...
				if (!$ch_result) {
					...
					$app->enqueueMessage($promo_obj->getName() . ': ' . JText::_('VBOCHPROMOSUCCESS') . (!empty($resp) ? ' (' . str_replace('e4j.ok.', '', $resp) . ')' : ''));
	...
	public function createrestriction()
	{
		...
		if (!empty($lid)) {
			...
			//
			$mainframe->enqueueMessage(JText::_('VBRESTRICTIONSAVED'));
	...
	public function savetmplfile()
	{
		...
		if (!empty($fpath)) {
			...
			if ($byt > 0) {
				...
				$result = [
					'status'  => 1,
					'message' => JText::_('VBOUPDTMPLFILEOK'),
				];
				...
				$result = [
					'status'  => 0,
					'message' => JText::_('VBOUPDTMPLFILENOBYTES'),
				];
			}
		} else {
			// error
			$result = [
				'status'  => 0,
				'message' => JText::_('VBOUPDTMPLFILEERR'),
			];
		}
	...
	private function do_storemaninvoice($action, $invid = 0) {
		...
		if (empty($pidcustomer)) {
			VikError::raiseWarning('', JText::_('VBNOCUSTOMERS'));
		...
		if (strpos($action, 'save') !== false) {
			...
			$dbo->execute();
			if (!$dbo->getNumRows()) {
				VikError::raiseWarning('', JText::_('VBNOINVOICESFOUND'));
				$mainframe->redirect($error_uri);
				exit;
			}
	...
	private function do_updateoptional($stay = false) {
		...
		if ($minguests > 0 && $minguestsnum > 0 && $maxguestsnum > 0) {
			if ($minguestsnum >= $maxguestsnum) {
				$minguests_conflict = JText::_('VBOMINMAXGUESTSOPTCONFL1');
			} elseif (($maxguestsnum - $minguestsnum) < 2) {
				$minguests_conflict = JText::_('VBOMINMAXGUESTSOPTCONFL2');
			}
		}
	...
	public function invoke_vcm()
	{
		...
		if ($result === true) {
			$app->enqueueMessage(JText::_('VBCHANNELMANAGERRESULTOK'));
		} else {
	...
	public function getbookingsinfo()
	{
		...
		if (!$bookings) {
			...
			// output the error
			VBOHttpDocument::getInstance()->close(500, '1 - ' . JText::_('VBOVWGETBKERRMISSDATA'));
		}
		...
		foreach ($booking_infos as $k => $row) {
			...
			if ($row['status'] == 'confirmed') {
				$booking_infos[$k]['status_lbl'] = JText::_('VBCONFIRMED');
				if ($row['checkout'] < $current_ts) {
					$booking_infos[$k]['status_lbl'] = JText::_('VBOCHECKEDSTATUSOUT');
				} elseif ($row['checkin'] < $current_ts && $row['checkout'] > $current_ts) {
					if ($row['checked'] == 1) {
						$booking_infos[$k]['status_lbl'] = JText::_('VBOCHECKEDSTATUSIN');
					} elseif ($row['checked'] == -1) {
						$booking_infos[$k]['status_lbl'] = JText::_('VBOCHECKEDSTATUSNOS');
					}
				}
			} elseif ($row['status'] == 'standby') {
				$booking_infos[$k]['status_lbl'] = JText::_('VBSTANDBY');
			} elseif ($row['status'] == 'cancelled') {
				$booking_infos[$k]['status_lbl'] = JText::_('VBCANCELLED');
			} else {
			...
			foreach ($rooms as $kor => $or) {
				...
				if (!strlen($or['roomindex'])) {
					...
					foreach ($room_params['features'] as $rind => $rfeatures) {
						...
						foreach ($rfeatures as $fname => $fval) {
							if ($fval) {
								$av_indexes[$rind] = '#' . $rind . ' - ' . JText::_($fname) . ': ' . $fval;
								break;
							}
						}
					}
			...
			$booking_infos[$k]['closure_txt'] = $row['closure'] ? JText::_('VBDBTEXTROOMCLOSED') : null;
		...
		if (!$booking_infos) {
			...
			// output the error
			VBOHttpDocument::getInstance()->close(500, '2 - ' . JText::_('VBOVWGETBKERRMISSDATA'));
		}
	...
	public function switchRoomIndex()
	{
		...
		VikBooking::getBookingHistoryInstance($bid)
			...
			->store(
				...
				sprintf(
					'%s [%d → %d]',
					JText::_('VBODEFAULTDISTFEATUREONE'),
...
jQuery(".' . $selector . '").hover(function() {
		...
		if (!empty($pbid) && !empty($phow) && !empty($pto) && count($customer_info) > 0) {
			...
			if ($dbo->getNumRows() > 0) {
				...
				if (VBOPlatformDetection::isWordPress()) {
					...
					if (count($itemid)) {
						$share_link = JRoute::_($share_link . "&Itemid={$itemid[0]->post_id}", false);
					}
				} else {
					/**
					 * @joomlaonly
	...
	public function updatebookingcheckin()
	{
		...
		$app->enqueueMessage(JText::_('VBOCHECKINSTATUSUPDATED'));
	...
	public function csvexportlaunch()
	{
		...
		$orders = $dbo->loadAssocList();
		if (!$orders) {
			$app->enqueueMessage(JText::_('VBCSVEXPNORECORDS'), 'error');
		...
		$columns = [
			[
				'label' => JText::_('VBDASHBOOKINGID'),
			],
			[
				'label' => JText::_('VBPVIEWORDERSONE'),
			],
			[
				'label' => JText::_('VBCSVCHECKIN'),
			],
			[
				'label' => JText::_('VBCSVCHECKOUT'),
			],
			[
				'label' => JText::_('VBCSVNIGHTS'),
			],
			[
				'label' => JText::_('VBCSVROOM'),
			],
			[
				'label' => JText::_('VBCSVPEOPLE'),
			],
			[
				'label' => JText::_('VBCSVCUSTINFO'),
			],
			[
				'label' => JText::_('ORDER_SPREQUESTS'),
			],
			[
				'label' => JText::_('ORDER_NOTES'),
			],
			[
				'label' => JText::_('VBCSVCREATEDBY'),
			],
			[
				'label' => JText::_('VBCSVCUSTMAIL'),
			],
			[
				'label' => JText::_('ORDER_PHONE'),
			],
			[
				'label' => JText::_('VBCSVOPTIONS'),
			],
			[
				'label' => JText::_('VBCSVPAYMENTMETHOD'),
			],
			[
				'label' => JText::_('VBCSVORDIDCONFNUMB'),
			],
			[
				'label' => JText::_('VBOCHANNEL'),
			],
			[
				'label' => JText::_('VBCSVEXPFILTBSTATUS'),
			],
			[
				'label' => JText::_('VBCSVTOTAL'),
			],
			[
				'label' => JText::_('VBCSVTOTPAID'),
			],
			[
				'label' => JText::_('VBCSVTOTTAXES'),
			],
		];
		...
		if (empty($filterstatus) || $filterstatus === 'cancelled') {
			...
			array_splice($columns, 2, 0, [['label' => JText::_('VBO_CANC_DATE')]]);
			...
			if ($cancellation_ids && $cancHistoryEvents = VikBooking::getBookingHistoryInstance(0)->getBookingEventsType('cancelled')) {
                ...
                foreach ($dbo->loadAssocList() as $cancRecord) {
                    if (!($cancBidsProcessed[$cancRecord['idorder']] ?? 0)) {
                        ...
                        $cancellation_timestamps[$cancRecord['idorder']] = JHtml::_('date', $cancRecord['dt'], 'U');
                    }
                }
			}
		}
		...
		foreach ($orders as $kord => $ord) {
			...
			if (preg_match("/(?:special requests:\s*)(.*?)$/is", $ord['custdata'], $match)) {
				...
				$special_requests = $match[1];
			} elseif (preg_match("/(?:" . JText::_('ORDER_SPREQUESTS') . ":\s*)(.*?)$/is", $ord['custdata'], $match)) {
				$special_requests = $match[1];
			}
			$paystr = '';
			...
			$bookingSource = JText::_('VBORDFROMSITE');
	...
	public function exportcustomerslaunch() {
		...
		$csvheadline = [
			'ID',
			JText::_('VBCUSTOMERLASTNAME'),
			JText::_('VBCUSTOMERFIRSTNAME'),
			JText::_('VBCUSTOMEREMAIL'),
			JText::_('VBCUSTOMERPHONE'),
			JText::_('VBCUSTOMERADDRESS'),
			JText::_('VBCUSTOMERCITY'),
			JText::_('VBCUSTOMERZIP'),
			JText::_('VBCUSTOMERCOUNTRY'),
			JText::_('VBCUSTOMERGENDER'),
			JText::_('ORDER_DBIRTH'),
			JText::_('VBCUSTOMERTOTBOOKINGS'),
		];
		if ($ppin > 0) {
	...
	private function do_updateoperator($stay = false)
	{
		...
		if (!empty($pfirst_name) && !empty($pemail) && !empty($pcode)) {
			...
			if (!$ex_operator) {
				...
				$dbo->setQuery($q);
				$dbo->execute();
				$app->enqueueMessage(JText::_('VBOPERATORSAVED'));
			} else {
				//email already exists
				VikError::raiseWarning('', JText::_('VBERROPERATOREXISTS').'<br/><a href="index.php?option=com_vikbooking&task=editoperator&cid[]='.$ex_operator['id'].'" target="_blank">'.$ex_operator['first_name'].' '.$ex_operator['last_name'].'</a>');
				$app->redirect("index.php?option=com_vikbooking&task=editoperator&cid[]=".$pwhere);
				exit;
			}
		} else {
			VikError::raiseWarning('', JText::_('VBERROPERATORDATA'));
		}
	...
	public function get_report_data()
	{
		...
		if ($direction == 'prev' || $direction == 'next') {
			...
			if ($step == 'weekend') {
				...
				// month
				$period_name = JText::_('VBPVIEWRESTRICTIONSTWO');
				if ($direction == 'next') {
		...
		if (!count($report_values)) {
			$error = strlen($report->getError()) ? $report->getError() : JText::_('VBNOTRACKINGS');
		} else {
	...
	private function navigateToBooking($direction = 'next')
	{
		...
		$dbo->execute();
		if (!$dbo->getNumRows()) {
			VikError::raiseWarning('', JText::_('VBPEDITBUSYONE'));
			$app->redirect("index.php?option=com_vikbooking&task=orders");
			exit;
		}
	...
	public function do_refundtn()
	{
		...
		if (empty($bid) || $amount <= 0) {
			VikError::raiseWarning('', JText::_('VBO_PLEASE_FILL_FIELDS'));
		...
		$app->enqueueMessage(JText::_('VBO_REFUND_SUCCESS'));
