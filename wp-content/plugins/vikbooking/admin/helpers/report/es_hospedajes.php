...
class VikBookingReportEsHospedajes extends VikBookingReport
{
	...
	public function getFilters()
	{
		...
		// export type filter
		$types = [
			'checkin' 	  => sprintf('%s (%s)', JText::_('VBPICKUPAT'), 'Partes de viajeros'),
			'reservation' => sprintf('%s (%s)', JText::_('VBPCHOOSEBUSYORDATE'), 'Reservas de hospedaje'),
		];
		...
		$filter_opt = array(
			...
			'html' => '<button type="button" class="btn vbo-config-btn vbo-report-hospedajes-manualsave" style="display: none;" onclick="vboHospedajesSaveData();"><i class="' . VikBookingIcons::i('save') . '"></i> ' . JText::_('VBSAVE') . '</button>',
		);
		array_push($this->reportFilters, $filter_opt);
	...
	public function getReportData()
	{
		...
		$this->cols = array(
			...
			array(
				...
				'label' => JText::_('VBPCHOOSEBUSYORDATE'),
				'ignore_view' => 1,
			),
			// number of guests
			array(
