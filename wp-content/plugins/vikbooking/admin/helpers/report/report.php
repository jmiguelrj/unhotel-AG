...
abstract class VikBookingReport
{
     ...
     * @uses    getReportData()
     */
    public function getReportValues(?int $depth = null)
    {
     ...
     * 
     * @param   bool    $registry           True to wrap options into a registry.
     * @param   bool    $profileListings    True to merge report profile listings setting.
     *
     * @return  JObject|array
     * 
     * @since   1.15.0 (J) - 1.5.0 (WP)
     * @since   1.18.7 (J) - 1.8.7 (WP) added argument $profileListings.
     */
    public function getReportOptions(bool $registry = true, bool $profileListings = true)
    {
        if ($profileListings && $this->allowsProfileListings()) {
            // load report settings
            $reportSettings = $this->loadSettings();
            if ($reportSettings['_listings'] ?? null) {
                // options should force profile listings
                $this->options['listings'] = (array) $reportSettings['_listings'];
            }
        }
    ...
    /**
     * Returns the optional report custom setting fields at profile level.
     * This method allows to load the report setting fields by injecting
     * profile-level (protected) preferences, such as listings filtered.
     * 
     * @return  array
     * 
     * @since   1.18.7 (J) - 1.8.7 (WP)
     */
    public function getProfileSettingFields()
    {
        // load all report setting fields
        $reportSettingFields = $this->getSettingFields();

        if ($reportSettingFields && $this->allowsProfileListings()) {
            // set protected listing setting field
            $reportSettingFields['_listings'] = [
                'type'     => 'listings',
        ...
        // return the full list of setting fields
        return $reportSettingFields;
    }
    ...
    /**
     * Tells whether the current report allows to save
     * the listings across multiple profile identifiers.
     * 
     * @return  bool
     * 
     * @since   1.18.7 (J) - 1.8.7 (WP)
     */
    public function allowsProfileListings()
    {
        // report profile listings enabled as long as multiple profiles are enabled
        return $this->allowsProfileSettings();
    }
