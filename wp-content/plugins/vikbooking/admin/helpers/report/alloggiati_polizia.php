...
class VikBookingReportAlloggiatiPolizia extends VikBookingReport
{
	...
	public function getFilters()
	{
		...
		$filter_opt = array(
			...
			'html' => '<button type="button" class="btn vbo-config-btn vbo-report-alloggiati-manualsave" style="display: none;" onclick="vboAlloggiatiSaveData();"><i class="' . VikBookingIcons::i('save') . '"></i> ' . JText::_('VBSAVE') . '</button>',
		);
		array_push($this->reportFilters, $filter_opt);
				...
				*/
				...
				array_push($insert_row, array(
					...
					'callback' => function($val) use ($booking_id, $line_number) {
						...
						return '<span class="vbo-toggle-mini">' . VikBooking::getVboApplication()->printYesNoButtons($booking_guest_signature, JText::_('VBYES'), JText::_('VBNO'), $val , 1, 0, 'vboAlloggiatiExcludeGuestLine(this);', ['red']) . '</span>';
					},
					'ignore_export' => 1,
