...
class VikBookingReportPtSef extends VikBookingReport
{
	...
	public function getFilters()
	{
		...
		// From Date Filter
		$filter_opt = array(
			'label' => '<label for="fromdate">'.JText::_('VBOREPORTREVENUEDAY').'</label>',
		...
		$hidden_vals .= '		<button type="button" class="btn vbo-config-btn" onclick="vboReportChosenDocnum(document.getElementById(\'choose-docnum\').value);">' . JText::_('VBAPPLY') . '</button>';
		$hidden_vals .= '	</div>';
		...
		$hidden_vals .= '		<button type="button" class="btn vbo-config-btn" onclick="vboReportChosenDbirth(document.getElementById(\'choose-dbirth\').value);">' . JText::_('VBAPPLY') . '</button>';
		$hidden_vals .= '	</div>';
...
HTML;
		...
		$filter_opt = array(
			...
			'html' => '<button type="button" class="btn vbo-config-btn vbo-report-ptsef-mngsettings" onclick="vboPtSefManageSettings();"><i class="' . VikBookingIcons::i('cogs') . '"></i> ' . JText::_('VBOADMINLEGENDSETTINGS') . '</button>' . $hidden_vals . $html_helper,
		);
		array_push($this->reportFilters, $filter_opt);

		// append button to save the data when creating manual values
		$filter_opt = array(
			'label' => '<label class="vbo-report-ptsef-manualsave" style="display: none;">' . JText::_('VBOGUESTSDETAILS') . '</label>',
			'html' => '<button type="button" class="btn vbo-config-btn vbo-report-ptsef-manualsave" style="display: none;" onclick="vboPtSefSaveData();"><i class="' . VikBookingIcons::i('save') . '"></i> ' . JText::_('VBSAVE') . '</button>',
		);
		array_push($this->reportFilters, $filter_opt);
	...
	public function getReportData()
	{
		...
		$this->cols = array(
			...
			array(
				'key' => 'lastname',
				'sortable' => 1,
				'label' => JText::_('ORDER_LNAME')
			),
			// name
			array(
				'key' => 'name',
				'sortable' => 1,
				'label' => JText::_('ORDER_NAME')
			),
			// nationality
			array(
				'key' => 'nationality',
				...
				'label' => JText::_('VBCUSTOMERNATION')
			),
			// birthplace
			array(
				'key' => 'place_birth',
				...
				'tip' => ucwords(JText::_('VBOFILTEISROPTIONAL')),
				'label' => JText::_('VBOCUSTPLACEBIRTH')
			),
			// birth date
			array(
				'key' => 'date_birth',
		...
		foreach ($bookings as $gbook) {
			...
			foreach ($guests_rows as $ind => $guests) {
				...
				if (date('Y-m-d', $guests['checkin']) == date('Y-m-d', $from_ts)) {
					$stay_type = JText::_('VBOTYPEARRIVAL');
				} elseif (date('Y-m-d', $guests['checkout']) == date('Y-m-d', $from_ts)) {
					$stay_type = JText::_('VBOTYPEDEPARTURE');
				} else {
					$stay_type = JText::_('VBOTYPESTAYOVER');
				}
