...
class VikBookingReportIstatVitAlbergatori extends VikBookingReport
{
	...
	public function getFilters()
	{
		...
		// export type filter
		$types = [
			'presenze'    => sprintf('%s (%s)', JText::_('VBPICKUPAT'), 'Presenze turistiche'),
			'occupazione' => sprintf('%s (%s)', JText::_('VBOGRAPHTOTOCCUPANCYLBL'), 'Occupazione camere'),
		];
		...
		$filter_opt = array(
			...
			'html' => '<button type="button" class="btn vbo-config-btn vbo-report-vitalbergatori-manualsave" style="display: none;" onclick="vboVitAlbergatoriSaveData();"><i class="' . VikBookingIcons::i('save') . '"></i> ' . JText::_('VBSAVE') . '</button>',
		);
		array_push($this->reportFilters, $filter_opt);
		...
		if ($ptype === 'occupazione') {
			...
			$this->cols = [
				...
				[
					...
					'label' => JText::_('VBOGRAPHTOTOCCUPANCYLBL'),
				],
			];
		}
		...
		foreach ($bookings as $gbook) {
			...
			foreach ($guests_rows as $ind => $guests) {
				...
				array_push($insert_row, [
					...
					'callback' => function($val) {
						$lbl_name  = !strcasecmp($val, 'confirmed') ? JText::_('VBCONFIRMED') : JText::_('VBCANCELLED');
