...
class VikBookingReportOptionsExtras extends VikBookingReport
{
	...
	public function getFilters()
	{
		...
		if (count($options)) {
			$opts_sel_html = $vbo_app->getNiceSelect($options, $pidoptional, 'idoptional', JText::_('VBOREPORTFILTALLOPTS'), JText::_('VBOREPORTFILTALLOPTS'), '', '', 'idoptional');
			$filter_opt = array(
				'label' => '<label for="idoptional">'.JText::_('VBOREPORTOPTIONSEXTRAS').'</label>',
				'html' => $opts_sel_html,
				'type' => 'select',
				'name' => 'idoptional'
			);
