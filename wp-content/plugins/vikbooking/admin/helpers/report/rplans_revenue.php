...
class VikBookingReportRplansRevenue extends VikBookingReport
{
	...
	public function getFilters()
	{
		...
		$fullsc_sel_html = $vbo_app->getNiceSelect([JText::_('VBO_STAYED_NIGHTS'), JText::_('VBO_FULL_STAY_VALUES')], $pfull_stay_count, 'full_stay_count', '', '', '', '', 'full_stay_count');
		$filter_opt = array(
			'label' => '<label for="full_stay_count">'.JText::_('VBO_DATA_CALCULATION').'</label>',
	...
	public function getReportData()
	{
		...
		$this->cols = array(
			...
			array(
				...
				'sortable' => 1,
				'label' => JText::_('VBTOTALCOMMISSIONS')
			),
			// Cancellation fees
			array(
				'key' => 'canc_fees',
			...
			array(
				...
				'tip' => JText::_('VBO_DATA_CALCULATION_HELP'),
				'tip_pos' => 'left',
			)
		);
	...
	private function registerExportCSVFileName()
	{
		...
		if (strlen($pchannel)) {
			...
			if ($pchannel == '-1') {
				$report_extraname = JText::_('VBORDFROMSITE');
			} else {
