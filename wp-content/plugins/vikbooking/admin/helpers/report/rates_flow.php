...
class VikBookingReportRatesFlow extends VikBookingReport
{
	...
	public function getFilters()
	{
		...
		if (!$rflow_handler) {
			...
			$this->setError(JText::_('VBCONFIGVCMAUTOUPDMISS'));
			return $this->reportFilters;
		}
		...
		$dt_types = array(
			'night'    => JText::_('VBDAY'),
			'creation' => JText::_('VBOINVCREATIONDATE'),
		);
		$dt_type_sel_html = $vbo_app->getNiceSelect($dt_types, $pdt_type, 'dt_type', '', '', '', '', 'dt_type');
		$filter_opt = array(
			'label' => '<label for="dt_type">' . JText::_('VBODASHSEARCHKEYS') . '</label>',
		...
		if (count($prices)) {
			$prices_sel_html = $vbo_app->getNiceSelect($prices, $pidprice, 'idprice', JText::_('VBAFFANYPRICE'), JText::_('VBAFFANYPRICE'), '', '', 'idprice');
			$filter_opt = array(
				'label' => '<label for="idprice">'.JText::_('VBOROVWSELRPLAN').'</label>',
		...
		$all_channels['-1'] = JText::_('VBORDFROMSITE');
		try {
		...
		$filter_opt = array(
			'label' => '<label for="channel">'.JText::_('VBCHANNELFILTER').'</label>',
	...
	public function getReportData()
	{
		...
		$this->cols = array(
			...
			array(
				'key' => 'created_on',
				'sortable' => 1,
				'label' => JText::_('VBOINVCREATIONDATE'),
			),
			// channel
			array(
				'key' => 'channel_id',
			...
			array(
				'key' => 'day_from',
				'sortable' => 1,
				'label' => JText::_('VBNEWRESTRICTIONDFROMRANGE'),
			),
			// to night (date) updated
			array(
				'key' => 'day_to',
				'sortable' => 1,
				'label' => JText::_('VBNEWRESTRICTIONDTORANGE'),
			),
			// VBO room id
			array(
				'key' => 'vbo_room_id',
	...
	private function sayCreatedBy($created_by)
	{
		...
		if (!strcasecmp($created_by, 'setNewRate') || !strcasecmp($created_by, 'VCM')) {
			// VCM Custom Rates
			return JText::_('VBMENUCHANNELMANAGER');
		}
