...
class VikBookingReportEsGuardiaCivil extends VikBookingReport
{
	...
	public function getFilters()
	{
		...
		$hidden_vals .= '		<input type="text" size="40" id="choose-dbirth" placeholder="'.JText::_('VBCUSTOMERBDATE').'" value="" /><br/>';
		...
		$hidden_vals .= '		<button type="button" class="btn" onclick="vboReportChosenDocissuedt(document.getElementById(\'choose-docissuedt\').value);">'.JText::_('VBAPPLY').'</button>';
		$hidden_vals .= '	</div>';
		...
		$filter_opt = array(
			...
			'html' => '<button type="button" class="btn vbo-config-btn vbo-report-guardiacivil-manualsave" style="display: none;" onclick="vboGuardiacivilSaveData();"><i class="' . VikBookingIcons::i('save') . '"></i> ' . JText::_('VBSAVE') . '</button>',
		);
		array_push($this->reportFilters, $filter_opt);
	...
	public function getReportData()
	{
		...
		if (empty($photelcode) || empty($photelname)) {
			$this->setError(JText::_('VBOREPORTERRNODATA'));
			return false;
		}
		...
		$this->cols = array(
			...
			array(
				...
				'label' => JText::_('VBCUSTOMERDOCISSUE')
			),
			//id booking
			array(
				'key' => 'idbooking',
