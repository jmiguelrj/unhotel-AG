...
class VikBookingReportDailyRoomReport extends VikBookingReport
{
	...
	public function getFilters()
	{
		...
		$filter_opt = array(
			...
			'html' => '<input type="text" id="todate" name="todate" value="" placeholder="'.addslashes(JText::_('VBOFILTEISROPTIONAL')).'" class="vbo-report-datepicker vbo-report-datepicker-to" />',
			'type' => 'calendar',
			'name' => 'todate'
		);
		array_push($this->reportFilters, $filter_opt);
		...
		// type filter
		$types = [
			'arrivals' 	 => JText::_('VBOARRIVING'),
			'departures' => JText::_('VBODEPARTING'),
			'stayover' 	 => JText::_('VBOTYPESTAYOVER'),
		];
		$ptype = VikRequest::getString('type', '', 'request');
		$types_sel_html = $vbo_app->getNiceSelect($types, $ptype, 'type', JText::_('VBANYTHING'), JText::_('VBANYTHING'), '', '', 'type');
		$filter_opt = array(
	...
	public function getReportData()
	{
		...
		$this->cols = array(
			...
			array(
				...
				'sortable' => 1,
				'label' => JText::_('VBCUSTOMERNOMINATIVE')
			),
			//guests
			array(
				'key' => 'guests',
				...
				'sortable' => 1,
				'label' => JText::_('VBPVIEWORDERSPEOPLE')
			),
			//rate plan
			array(
				'key' => 'rplan',
			...
			array(
				...
				'sortable' => 1,
				'label' => JText::_('VBPSHOWPAYMENTSTHREE')
			),
			//ID
			array(
				'key' => 'id',
		...
		$website_provenience = JText::_('VBORDFROMSITE');
		...
		foreach ($bookings as $gbook) {
			...
			$type = JText::_('VBOTYPESTAYOVER');
			$raw_type = 'stayover';
			if (date('Y-m-d', $gbook[0]['checkin']) == $arrivedt) {
				$type = JText::_('VBOTYPEARRIVAL');
				$raw_type = 'arriving';
			} elseif (date('Y-m-d', $gbook[0]['checkout']) == $departdt) {
				$type = JText::_('VBOTYPEDEPARTURE');
				$raw_type = 'departing';
			}
			...
			foreach ($gbook as $roomres) {
				...
				$pets_str = '';
				if ($roomres['pets'] > 0) {
					$pets_str = $roomres['pets'] > 1 ? "{$roomres['pets']} " . JText::_('VBO_PETS') : '1 ' . JText::_('VBO_PET');
				}
		...
		$stay_type_tots = [
			JText::_('VBOARRIVING') . ': ' . $guests_stay_counter['arriving'],
			JText::_('VBODEPARTING') . ': ' . $guests_stay_counter['departing'],
			JText::_('VBOTYPESTAYOVER') . ': ' . $guests_stay_counter['stayover'],
		];
	...
	private function getCountriesMap($countries)
	{
		...
		if (in_array('unknown', $countries)) {
			$map['unknown'] = JText::_('VBOREPORTTOPCUNKNC');
	...
	{
		if (empty($idtar)) {
			return [JText::_('VBOROOMCUSTRATEPLAN'), 0];
		}
