				...
				foreach ($hotelInventoryListingIds as $hotelInventoryListingId) {
					foreach ($elements as $elIndex => $element) {
						...
						if (!($options['subunits']['entire_listing'] ?? true)) {
							...
							$nameSuffix = (string) ($options['subunits']['listing_suffix'] ?? sprintf(' (%s)', strtolower(JText::_('VBNEWCOUPONEIGHT'))));
...
JAVASCRIPT;
	...
	public function renderSearchElementsDropDown(array $options = [])
	{
		...
		$lang_error_loading = json_encode($options['language']['error'] ?? JText::_('VBO_ERR_LOAD_RESULTS'));
		$lang_no_results    = json_encode($options['language']['noresults'] ?? JText::_('VBO_NO_RECORDS_FOUND'));
		$lang_searching     = json_encode($options['language']['searching'] ?? JText::_('VBO_SEARCHING'));
	...
	public function loadVisualEditorAssets()
	{
		...
		$css_font_decl .= '
			...
			.ql-picker.ql-specialtags .ql-picker-label:before {
				content: "' . htmlspecialchars(JText::_('VBO_CONDTEXT_TKN')) . '";
			}
			.ql-formats .ql-genai {
		...
		$mail_wrapper_icn = '<i class="' . VikBookingIcons::i('minus-square') . '" title="' . htmlspecialchars(JText::_('VBO_INSERT_CONT_WRAPPER')) . '"></i>';

		// icon for mail preview
		$mail_preview_icn = '<i class="' . VikBookingIcons::i('eye') . '" title="' . htmlspecialchars(JText::_('VBOPREVIEW')) . '"></i>';

		// icon for property logo (home icon)
		$mail_homelogo_icn = '<i class="' . VikBookingIcons::i('hotel') . '" title="' . htmlspecialchars(JText::_('VBCONFIGFOURLOGO'), ENT_QUOTES | ENT_SUBSTITUTE | ENT_HTML401) . '"></i>';

		// text for generating content through AI
		$mail_genai_icn = htmlspecialchars(JText::_('VBO_AI_LABEL_DEF'), ENT_QUOTES | ENT_SUBSTITUTE | ENT_HTML401);
...
JS
	...
	public function renderVisualEditor($name, $value, array $attrs = [], array $opts = [], array $btns = [])
	{
		...
		$text_html_lbl   = JText::_('VBO_MODE_TEXTHTML');
		$visual_mode_lbl = JText::_('VBO_MODE_VISUAL');
		...
		if (!$gen_ai_use_prompt && !strcasecmp(($opts['gen_ai']['environment'] ?? ''), 'cron')) {
			...
			$gen_ai_use_prompt = JText::_('VBO_AITOOL_WRITER_CRON_DEF_PROMPT');
		} elseif (!$gen_ai_use_prompt && !strcasecmp(($opts['gen_ai']['environment'] ?? ''), 'taskmanager')) {
			// use default prompt for the task manager
			$gen_ai_use_prompt = JText::_('VBO_AITOOL_WRITER_TM_DEF_PROMPT');
		}
		...
		if ($gen_ai_use_prompt && ($opts['gen_ai']['placeholders'] ?? 0) && $btns) {
			...
			if ($placeholders) {
				$gen_ai_use_prompt .= ' ' . JText::_('VBO_AITOOL_WRITER_USE_PLACEHOLDERS') . "\n" . implode(', ', $placeholders);
			}
		}
		...
		$preview_lbl = htmlspecialchars(JText::_('VBOPREVIEW'));
		$booking_lbl = htmlspecialchars(JText::_('VBDASHBOOKINGID'));
		$editor .= <<<HTML
<script>
jQuery(function() {
