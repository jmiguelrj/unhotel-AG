...
class VikBookingAdminWidgetVirtualTerminal extends VikBookingAdminWidget
{
	...
	public function __construct()
	{
		...
		$this->widgetName = JText::_('VBO_W_VIRTUALTERMINAL_TITLE');
		$this->widgetDescr = JText::_('VBO_W_VIRTUALTERMINAL_DESCR');
	...
	public function loadTerminalForm()
	{
		...
		if (!($processor = $this->getPaymentProcessor($booking_info))) {
			...
			return [
				'html' => '<p class="err">' . JText::_('VBO_PAY_PROCESS_NO_DIRECT_CHARGE') . '</p>',
			];
		}
		...
		if ($this->vcm_exists && !JFactory::getUser()->authorise('core.admin', 'com_vikchannelmanager') && !$off_session_tn_data) {
			...
			return [
				'html' => '<p class="err">' . JText::_('JERROR_ALERTNOAUTHOR') . '</p>',
			];
		}
		...
		<div class="vbo-vterminal-cc-container">
			...
			<div class="vbo-vterminal-cc-row-group vbo-vterminal-cc-row-group-amount">
				<div class="vbo-vterminal-cc-row vbo-vterminal-cc-row-currency">
					<div class="vbo-vterminal-cc-lbl"><?php echo JText::_('VBOCPARAMCURRENCY'); ?></div>
					<div class="vbo-vterminal-cc-val">
						<input type="text" autocomplete="off" value="<?php echo JHtml::_('esc_attr', $currency_code); ?>" data-vt-cc-field="currency" />
					</div>
				</div>
				<div class="vbo-vterminal-cc-row vbo-vterminal-cc-row-amount">
					<div class="vbo-vterminal-cc-lbl"><?php echo JText::_('VBO_CC_AMOUNT'); ?></div>
					<div class="vbo-vterminal-cc-val">
		...
		if ($off_session_tn_data) {
			...
			<div class="vbo-vterminal-cc-row-group vbo-vterminal-cc-row-group-submit" data-tn-type="off-session">
				<div class="vbo-vterminal-cc-row vbo-vterminal-cc-row-submit" data-tn-type="off-session">
					<div class="vbo-vterminal-cc-val">
						<button type="button" class="btn vbo-config-btn" onclick="vboWidgetVTerminalOffSessionCharge('<?php echo $wrapper; ?>');"><?php VikBookingIcons::e('credit-card'); ?> <?php echo JText::_('VBO_CHARGE_AUTHORIZED_CARD'); ?></button>
					</div>
				</div>
			</div>
			<?php
			...
			if ($off_session_tn_data) {
				...
				<div class="vbo-vterminal-cc-row-group vbo-vterminal-cc-row-group-usecard">
					<div class="vbo-vterminal-cc-row vbo-vterminal-cc-row-usecard">
						<div class="vbo-vterminal-cc-lbl"><?php echo JText::_('VBO_CREDIT_CARD') . ($only_off_session ? ' - ' . JText::_('VBMAINPAYMENTSNEW') : ''); ?></div>
					</div>
				</div>
				<?php
			}
			?>
				...
				<div class="vbo-vterminal-cc-row-group vbo-vterminal-cc-row-group-cardholder">
					<div class="vbo-vterminal-cc-row vbo-vterminal-cc-row-cardholder">
						<div class="vbo-vterminal-cc-lbl"><?php echo JText::_('VBISNOMINATIVE'); ?></div>
						<div class="vbo-vterminal-cc-val">
							<input type="text" autocomplete="off" value="<?php echo isset($known_tn_vals['name']) ? JHtml::_('esc_attr', $cc_value_pairs['name']) : ''; ?>" data-vt-cc-field="cardholder" />
						</div>
					</div>
				</div>
				...
				<div class="vbo-vterminal-cc-row-group vbo-vterminal-cc-row-group-ccpan">
					<div class="vbo-vterminal-cc-row vbo-vterminal-cc-row-ccpan">
						<div class="vbo-vterminal-cc-lbl"><?php echo JText::_('VBO_CC_NUMBER'); ?></div>
						<div class="vbo-vterminal-cc-val vbo-vterminal-cc-val-withlogo">
							<input type="text" autocomplete="off" value="<?php echo isset($known_tn_vals['card_number']) ? JHtml::_('esc_attr', $cc_value_pairs['card_number']) : ''; ?>" data-vt-cc-field="card_number" />
				...
				<div class="vbo-vterminal-cc-row-group vbo-vterminal-cc-row-group-ccextra">
					<div class="vbo-vterminal-cc-row vbo-vterminal-cc-row-ccexpiry">
						<div class="vbo-vterminal-cc-lbl"><?php echo JText::_('VBO_CC_EXPIRY_DT'); ?></div>
						<div class="vbo-vterminal-cc-val">
							<input type="text" autocomplete="off" placeholder="MM/YYYY" value="<?php echo isset($known_tn_vals['expiration_date']) ? JHtml::_('esc_attr', $cc_value_pairs['expiration_date']) : ''; ?>" data-vt-cc-field="expiry" />
						</div>
					</div>
					<div class="vbo-vterminal-cc-row vbo-vterminal-cc-row-cvc">
						<div class="vbo-vterminal-cc-lbl"><?php echo JText::_('VBO_CC_CVV'); ?></div>
						<div class="vbo-vterminal-cc-val">
							<input type="text" autocomplete="off" value="<?php echo isset($known_tn_vals['cvv']) ? JHtml::_('esc_attr', $cc_value_pairs['cvv']) : ''; ?>" data-vt-cc-field="cvv" />
						</div>
					</div>
				</div>
				...
				<div class="vbo-vterminal-cc-row-group vbo-vterminal-cc-row-group-submit" data-tn-type="direct-charge">
					<div class="vbo-vterminal-cc-row vbo-vterminal-cc-row-submit" data-tn-type="direct-charge">
						<div class="vbo-vterminal-cc-val">
							<button type="button" class="btn vbo-config-btn" onclick="vboWidgetVTerminalChargeCard('<?php echo $wrapper; ?>');"><?php VikBookingIcons::e('credit-card'); ?> <?php echo JText::_('VBO_CC_DOCHARGE'); ?></button>
						</div>
					</div>
				</div>
		...
		foreach ($hidden_tn_vals as $key => $value) {
			?>
			<input type="hidden" value="<?php echo JHtml::_('esc_attr', $value); ?>" data-vt-cc-field="<?php echo JHtml::_('esc_attr', $key); ?>" />
			<?php
		}
		?>
	...
	public function doDirectCharge()
	{
		...
		if (!$processor) {
			VBOHttpDocument::getInstance()->close(500, JText::_('VBO_PAY_PROCESS_NO_DIRECT_CHARGE'));
		}
		...
		try {
			...
			// set error message
			$array_result['log'] = sprintf(JText::_('VBO_CC_TN_ERROR') . " \n%s", $e->getMessage());
		}
		...
		// Booking History
		$ev_descr = JText::_('VBO_W_VIRTUALTERMINAL_TITLE') . " - {$pay_process_name} ({$now_user->name})";
	...
	public function render(?VBOMultitaskData $data = null)
	{
		...
		if (!$page_bid) {
			...
			?>
		<p class="warn"><?php echo JText::_('VBPEDITBUSYONE'); ?></p>
			<?php
			return;
		}
