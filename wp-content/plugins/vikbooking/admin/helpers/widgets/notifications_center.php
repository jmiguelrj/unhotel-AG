...
class VikBookingAdminWidgetNotificationsCenter extends VikBookingAdminWidget
{
	...
	public function __construct()
	{
		...
		$this->widgetName = JText::_('VBO_W_NOTIFSCENTER_TITLE');
		$this->widgetDescr = JText::_('VBO_W_NOTIFSCENTER_DESCR');
	...
	public function render(?VBOMultitaskData $data = null)
	{
					...
					<div class="vbo-admin-widget-head-commands">
						...
						?>
							<div class="vbo-reportwidget-commands-main" style="display: none;">
								<div class="vbo-reportwidget-command-dates">
									<div class="vbo-reportwidget-period-name"><?php echo JText::_('VBNEWRESTRICTIONDATERANGE'); ?></div>
						...
						<div class="vbo-reportwidget-filters">
							<div class="vbo-reportwidget-filter vbo-widget-notifscenter-markread">
								<span class="vbo-widget-notifscenter-read-all"><?php echo JText::_('VBO_MARK_ALL_READ'); ?></span>
							</div>
							<div class="vbo-reportwidget-filter vbo-widget-notifscenter-onlyunread">
								<span class="vbo-widget-notifscenter-filter-lbl"><?php echo JText::_('VBO_ONLY_UNREAD'); ?></span>
								<span class="vbo-widget-notifscenter-filter-val"><?php echo $this->vbo_app->printYesNoButtons('onlyunread', JText::_('VBYES'), JText::_('VBNO'), 0, 1, 0); ?></span>
							</div>
							<div class="vbo-reportwidget-filter">
								<span class="vbo-reportwidget-datepicker">
									...
									<input type="text" class="vbo-notifscenter-dtpicker-from" value="" placeholder="<?php echo JHtml::_('esc_attr', JText::_('VBNEWRESTRICTIONDFROMRANGE')); ?>" />
								</span>
							</div>
							<div class="vbo-reportwidget-filter">
								<span class="vbo-reportwidget-datepicker">
									<?php VikBookingIcons::e('calendar', 'vbo-widget-notifscenter-caltrigger'); ?>
									<input type="text" class="vbo-notifscenter-dtpicker-to" value="" placeholder="<?php echo JHtml::_('esc_attr', JText::_('VBNEWRESTRICTIONDTORANGE')); ?>" />
								</span>
							</div>
							<div class="vbo-reportwidget-filter vbo-reportwidget-filter-confirm vbo-widget-notifscenter-filter-confirm">
								<button type="button" class="btn btn-secondary" onclick="vboWidgetNotifsCenterClearFilters('<?php echo $wrapper_id; ?>');" title="<?php echo JHtml::_('esc_attr', JText::_('VBOSIGNATURECLEAR')); ?>"><?php VikBookingIcons::e('broom'); ?></button>
								<button type="button" class="btn vbo-config-btn" onclick="vboWidgetNotifsCenterApplyFilters('<?php echo $wrapper_id; ?>');"><?php echo JText::_('VBADMINNOTESUPD'); ?></button>
							</div>
						</div>
		...
		if (!$notifications) {
			...
			<div class="vbo-widget-notifscenter-loadmore-info">
				<p><?php echo JText::_('VBO_NO_NOTIFS'); ?></p>
			</div>
			<?php
		}
		?>
	...
	protected function buildNotificationsHTML(array $notifications, int $page_num = 0)
	{
		...
		foreach ($notifications as $index => $notif) {
			...
			$date_info = VBORemindersHelper::getInstance()->relativeDatesDiff(JHtml::_('date', $notif->createdon, 'Y-m-d H:i:s'));
			...
			if ($date_info['past'] && ($date_info['today'] || ($date_info['yesterday'] && !$date_info['days']))) {
				...
				if ($date_info['hours']) {
					$rel_time_str = $date_info['hours'] . ' ' . JText::_(($date_info['hours'] == 1 ? 'VBO_HOUR' : 'VBCONFIGONETENEIGHT'));
				} elseif ($date_info['minutes']) {
					$rel_time_str = $date_info['minutes'] . ' ' . JText::_(($date_info['minutes'] == 1 ? 'VBO_MINUTE' : 'VBTRKDIFFMINS'));
				} else {
					$rel_time_str = $date_info['seconds'] . ' ' . JText::_(($date_info['seconds'] == 1 ? 'VBO_SECOND' : 'VBTRKDIFFSECS'));
				}
				...
				$rel_time_str = ($date_info['hours'] + 24) . ' ' . JText::_('VBCONFIGONETENEIGHT');
				$relative_dt = JText::sprintf('VBO_REL_EXP_PAST', strtolower($rel_time_str));
			}

			// build a human-readable date and time
			if ($date_info['today']) {
				$human_dtime = JHtml::_('date', $notif->createdon, 'H:i:s');
			} else {
				$human_dtime = JHtml::_('date', $notif->createdon, str_replace("/", $this->datesep, $this->df) . ' H:i:s');
			}
			...
			>
					...
					<div class="vbo-customer-info-box">
						...
						if (!empty($notif->customer_pic)) {
							...
							<span class="vbo-widget-notifscenter-cpic-zoom">
								<img src="<?php echo strpos($notif->customer_pic, 'http') === 0 ? $notif->customer_pic : VBO_SITE_URI . 'resources/uploads/' . $notif->customer_pic; ?>" data-caption="<?php echo JHtml::_('esc_attr', (string) $notif->customer_name); ?>" decoding="async" loading="lazy" />
							</span>
							<?php
				...
				if ($notif->title) {
					if (preg_match("/^VBO/", $notif->title)) {
						 ...
						 */
						$notif->title = JText::_($notif->title);
					}
					?>
				...
				?>
					...
					?>
						<span class="vbo-tooltip vbo-tooltip-<?php echo !$index && !$page_num ? 'bottom' : 'top'; ?>" data-tooltiptext="<?php echo JHtml::_('esc_attr', $human_dtime); ?>"><?php echo $relative_dt; ?></span>
					</div>
				<?php
				...
				if (is_object($notif->cta_data) && (!empty($notif->cta_data->url) || !empty($notif->cta_data->widget))) {
					...
					$cta_btn_lbl = !empty($notif->cta_data->label) ? $notif->cta_data->label : JText::_('VBO_TAKE_ACTION');
					?>
					...
					if (!empty($notif->cta_data->url)) {
						// open "remote" URL
						?>
						<a class="btn btn-small vbo-notifscenter-cta-btn" href="<?php echo JHtml::_('esc_attr', $notif->cta_data->url); ?>" target="_blank"><?php echo $cta_btn_lbl; ?></a>
						<?php
						...
						?>
						<button type="button" class="btn btn-small vbo-notifscenter-cta-btn" onclick="<?php echo JHtml::_('esc_attr', $js_command); ?>"><?php echo $cta_btn_lbl; ?></button>
						<?php
					}
					?>
					</div>
	...
	protected function getMinDatesNotifications()
	{
		...
		if ($info_dates && !empty($info_dates->mindate)) {
			$mindate = strtotime(JHtml::_('date', $info_dates->mindate, 'Y-m-d'));
			$maxdate = strtotime(JHtml::_('date', $info_dates->maxdate, 'Y-m-d'));
		}
