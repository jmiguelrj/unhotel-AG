...
class VikBookingAdminWidgetArrivingToday extends VikBookingAdminWidget
{
	...
	public function __construct()
	{
		...
		$this->widgetName = JText::_('VBO_W_ARRIVETOD_TITLE');
		$this->widgetDescr = JText::_('VBO_W_ARRIVETOD_DESCR');
			...
			<div class="vbo-admin-widget-head vbo-dashboard-today-checkin-head">
				<div class="vbo-admin-widget-head-inline">
					<h4>
						...
						<span class="arrivals-when"><?php echo JText::_('VBOARRIVING'); ?></span> 
	...
	protected function buildArrivalRows($arrivals, $when = 'today')
	{
		...
		if (!$arrivals) {
			...
			$no_arrivals_when = JText::_('VBONOCHECKINSTODAY');
			if ($when == 'yesterday') {
				$no_arrivals_when = JText::_('VBONOCHECKINSYESTERDAY');
			} elseif ($when == 'tomorrow') {
				$no_arrivals_when = JText::_('VBONOCHECKINSTOMORROW');
			}
