...
class VikBookingAdminWidgetGuestMessages extends VikBookingAdminWidget
{
	...
	public function __construct()
	{
		...
		$this->widgetName = JText::_('VBO_W_GUESTMESSAGES_TITLE');
		$this->widgetDescr = JText::_('VBO_W_GUESTMESSAGES_DESCR');
	...
	public function loadMessages()
	{
		...
		if (!$latest_messages) {
			?>
			<p class="info" data-no-messages="1" style="<?php echo $offset > 0 ? 'text-align: center;' : ''; ?>"><?php echo $offset > 0 ? JText::_('VBO_NO_MORE_MESSAGES') : JText::_('VBO_NO_RECORDS_FOUND'); ?></p>
			<?php
		}
			...
			>
				...
				if (!empty($gmessage->ai_priority)) {
					...
					if (!strcasecmp(trim((string) $gmessage->ai_priority), 'high')) {
						$priority_value = JText::_('VBO_PRIORITY_HIGH');
					} elseif (!strcasecmp(trim((string) $gmessage->ai_priority), 'medium')) {
						$priority_value = JText::_('VBO_PRIORITY_MEDIUM');
					} elseif (!strcasecmp(trim((string) $gmessage->ai_priority), 'low')) {
						$priority_value = JText::_('VBO_PRIORITY_LOW');
					}
					?>
					<div class="vbo-w-guestmessages-message-aipriority">
						<span class="vbo-w-guestmessages-message-aipriority-icn <?php echo $priority_class; ?> vbo-tooltip vbo-tooltip-right" data-tooltiptext="<?php echo JHtml::_('esc_attr', $priority_value); ?>"><?php VikBookingIcons::e($priority_icon, $priority_class); ?></span>
					</div>
					<?php
				}
				?>
				...
				<div class="vbo-dashboard-guest-activity-content">
					<div class="vbo-dashboard-guest-activity-content-head">
							...
							if (!empty($gmessage->b_status)) {
								switch ($gmessage->b_status) {
									case 'standby':
										...
										$badge_text  = JText::_('VBSTANDBY');
										break;
									case 'cancelled':
										$badge_class = 'badge-danger';
										$badge_text  = JText::_('VBCANCELLED');
										break;
									default:
										$badge_class = 'badge-success';
										$badge_text  = JText::_('VBCONFIRMED');
										if (!empty($gmessage->b_checkout) && $gmessage->b_checkout < $current_ts) {
											$badge_text  = JText::_('VBOCHECKEDSTATUSOUT');
										}
										break;
								}
								?>
						...
						<div class="vbo-dashboard-guest-activity-content-info-date">
							<span><?php echo JHtml::_('date', $gmessage->last_updated, 'H:i'); ?></span>
						<?php
						if (JHtml::_('date', $gmessage->last_updated, 'Y-m-d') != $this->today_ymd) {
							// format and print the date
							?>
							<span><?php echo JHtml::_('date', $gmessage->last_updated, str_replace('/', $this->datesep, $this->df)); ?></span>
							<?php
						} else {
							// print "today"
							?>
							<span><?php echo JText::_('VBTODAY'); ?></span>
							<?php
						}
						?>
						</div>
						...
						<div class="vbo-reportwidget-commands">
							<div class="vbo-reportwidget-commands-main">
								<div class="vbo-reportwidget-command-dates">
									<div id="<?php echo $wrapper_id; ?>-filters" class="vbo-reportwidget-period-name" style="<?php echo !$contains_filter && !$unread_filter ? 'display: none;' : ''; ?>"><?php VikBookingIcons::e('filter'); ?> <?php echo JText::_('VBO_FILTERS_APPLIED'); ?></div>
								</div>
								<div id="<?php echo $wrapper_id; ?>-aipowered" class="vbo-admin-widget-head-ai-powered" style="display: none;">
									<span class="label label-info"><?php echo JText::_('VBO_AI_LABEL_DEF'); ?></span>
								</div>
							</div>
									...
									<div class="vbo-params-block">
										...
										<div class="vbo-param-container">
											...
											<div class="vbo-param-setting">
												...
												<span class="vbo-param-setting-comment"><?php echo JText::_('VBO_FIRST_NAME_ACCURATE_HELP'); ?></span>
											</div>
										</div>

										<div class="vbo-param-container">
											<div class="vbo-param-label"><?php echo JText::_('VBSENDEMAILCUSTCONT'); ?></div>
											<div class="vbo-param-setting">
												<input type="text" class="vbo-widget-guest-messages-messcontains" value="<?php echo JHtml::_('esc_attr', $contains_filter); ?>" autocomplete="off" />
												<span class="vbo-param-setting-comment"><?php echo JText::_('VBO_MESSAGE_CONTAINS_HELP'); ?></span>
											</div>
										</div>

										<div class="vbo-param-container vbo-toggle-small">
											<div class="vbo-param-label"><?php echo JText::_('VBO_UNREAD_MESSAGES'); ?></div>
											<div class="vbo-param-setting">
												<?php echo $this->vbo_app->printYesNoButtons('unread', JText::_('VBYES'), JText::_('VBNO'), (int) $unread_filter, 1, 0, '', ['blue']); ?>
											</div>
										</div>

										<div class="vbo-param-container vbo-toggle-small">
											<div class="vbo-param-label"><?php echo JText::_('VBO_AI_LABEL_DEF') . ' ' . JText::_('VBO_SORT_BY_PRIORITY'); ?></div>
											<div class="vbo-param-setting">
												<?php echo $this->vbo_app->printYesNoButtons('ai_sort', JText::_('VBYES'), JText::_('VBNO'), (int) $sort_by_ai, 1, 0, '', ['gold']); ?>
											</div>
										</div>

										<div class="vbo-param-container vbo-toggle-small">
											<div class="vbo-param-label"><?php echo JText::_('VBO_SENDER'); ?></div>
											<div class="vbo-param-setting">
										...
										<div class="vbo-param-container">
											...
											<div class="vbo-param-setting">
												...
												<span class="vbo-param-setting-comment"><?php echo JText::_('VBO_FROM_DT_HELP'); ?></span>
											</div>
										</div>
										...
										<div class="vbo-param-container">
											...
											<div class="vbo-param-setting">
												...
												<span class="vbo-param-setting-comment"><?php echo JText::_('VBO_TO_DT_HELP'); ?></span>
											</div>
										</div>

										<div class="vbo-param-container">
											<div class="vbo-param-label"><?php echo JText::_('VBO_MESS_PER_PAGE'); ?></div>
											<div class="vbo-param-setting">
		...
		<script type="text/javascript">
			...
			function vboWidgetGuestMessagesOpenSettings(wrapper) {
				...
				var search_modal_body = VBOCore.displayModal({
					...
					title: '<?php echo JHtml::_('esc_attr', JText::_('VBO_W_GUESTMESSAGES_TITLE')); ?> - ' + Joomla.JText._('VBODASHSEARCHKEYS'),
					body_prepend: true,
					draggable: true,
		...
		<script type="text/javascript">
			...
			jQuery(function() {
				...
				if (jQuery('#vbmessagingdiv').length) {
					...
					jQuery('#vbmessagingdiv').html('<p class="info"><?php echo JHtml::_('esc_attr', JText::_('VBO_W_GUESTMESSAGES_TITLE')); ?> - <?php echo JHtml::_('esc_attr', JText::_('VBO_MULTITASK_PANEL')); ?></p>');
				}
