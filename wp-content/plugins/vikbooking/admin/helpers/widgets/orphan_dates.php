...
class VikBookingAdminWidgetOrphanDates extends VikBookingAdminWidget
{
	...
	public function __construct()
	{
		...
		$this->widgetName = JText::_('VBO_W_ORPHDATES_TITLE');
		$this->widgetDescr = JText::_('VBO_W_ORPHDATES_DESCR');
	...
	public function render(?VBOMultitaskData $data = null)
	{
		...
		<div class="vbo-admin-widget-wrapper vbo-admin-widget-wrapper-orphandates" style="<?php echo !$is_multitask ? 'display: none;' : ''; ?>">
			<div class="vbo-admin-widget-head">
				<h4><?php echo $this->vbo_app->createPopover(array('title' => JText::_('VBORPHANSFOUND'), 'content' => JText::_('VBORPHANSFOUNDSHELP'), 'icon_class' => VikBookingIcons::i('exclamation-triangle'))); ?> <span><?php echo JText::_('VBORPHANSFOUND'); ?></span></h4>
			</div>
			...
			if (!hideorphans) {
				...
				var jqxhr = jQuery.ajax({
					...
					for (var rid in obj_res) {
						...
						orphans_list += '		<a href="index.php?option=com_vikbooking&task=ratesoverv&cid[]='+rid+'&startdate='+obj_res[rid]['linkd']+'" class="btn btn-primary" target="_blank"><?php echo addslashes(JText::_('VBORPHANSCHECKBTN')); ?></a>';
						orphans_list += '	</div>';
						orphans_list += '</div>';
					}
					// populate content
