...
class VikBookingAdminWidgetLatestEvents extends VikBookingAdminWidget
{
	...
	public function __construct()
	{
		...
		$this->widgetName = JText::_('VBO_W_LATESTEVS_TITLE');
		$this->widgetDescr = JText::_('VBO_W_LATESTEVS_DESCR');
	...
	public function loadHistoryEvents()
	{
		...
		foreach ($events as $history) {
			...
			if (!empty($history->booking_type) && !strcasecmp($history->booking_type, 'overbooking')) {
				$ord_status .= ' <span class="label label-error vbo-status-label vbo-label-small">' . JText::_('VBO_BTYPE_OVERBOOKING') . '</span>';
			}
			...
			$nights_lbl = $history->days > 1 ? JText::_('VBDAYS') : JText::_('VBDAY');
	...
	public function render(?VBOMultitaskData $data = null)
	{
		...
		if (!empty($instance_settings->evtype)) {
			if (isset($history_types[$instance_settings->evtype])) {
				...
				$current_evtype = $instance_settings->evtype;
				$current_evname = JText::_('VBO_HISTORY_GROUPS') . ' - ' . $history_groups[$current_evtype]['name'];
			}
		}
						...
						<div class="vbo-reportwidget-commands">
							<div class="vbo-reportwidget-commands-main">
								<div class="vbo-reportwidget-command-dates">
									<div class="vbo-reportwidget-period-name"><?php echo JText::_('VBOBOOKHISTORYLBLTYPE'); ?></div>
									<div class="vbo-widget-latestevents-filttype-name"><?php echo !empty($current_evname) ? $current_evname : JText::_('VBANYTHING'); ?></div>
								</div>
							</div>
						...
						<div class="vbo-reportwidget-filters">
							<div class="vbo-reportwidget-filter">
								<select class="vbo-latestevents-filttype-event">
									<option value=""></option>
									<optgroup label="<?php echo htmlspecialchars(JText::_('VBO_HISTORY_GROUPS')); ?>">
									<?php
									...
									<optgroup label="<?php echo htmlspecialchars(JText::_('VBOBOOKHISTORYTAB')); ?>">
									<?php
							...
							<div class="vbo-reportwidget-filter vbo-reportwidget-filter-confirm">
								<button type="button" class="btn vbo-config-btn" onclick="vboWidgetLatestEventsFilterType('<?php echo $wrapper_id; ?>');"><?php echo JText::_('VBADMINNOTESUPD'); ?></button>
							</div>
						</div>
