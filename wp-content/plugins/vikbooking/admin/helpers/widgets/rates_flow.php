...
class VikBookingAdminWidgetRatesFlow extends VikBookingAdminWidget
{
	...
	public function __construct()
	{
		...
		$this->widgetName = JText::_('VBOREPORTRATESFLOW');
		$this->widgetDescr = JText::_('VBO_W_RATESFLOW_DESCR');
		...
		foreach ($alterations as $rflow) {
			...
			if (is_object($rflow['data']['value']) && isset($rflow['data']['value']->Restrictions)) {
				...
				if (isset($restr_obj->minLOS) && (int)$restr_obj->minLOS > 1) {
					array_push($restr_infos, JText::_('VBOMINIMUMSTAY') . ': ' . $restr_obj->minLOS);
				}
						...
						<div class="vbo-reportwidget-filters">
							...
							<div class="vbo-reportwidget-filter">
								<select class="vbo-ratesflow-period-nav" onchange="vboWidgetRatesFlowChangePeriod(this.value);">
									<option value="weekend"<?php echo $cookie_step == 'weekend' ? ' selected="selected"' : ''; ?>><?php echo JText::_('VBOWEEKND'); ?></option>
							...
							<div class="vbo-reportwidget-filter vbo-reportwidget-filter-confirm">
								<button type="button" class="btn vbo-config-btn" onclick="vboWidgetRatesFlowChangeDates('<?php echo $wrapper_id; ?>');"><?php echo JText::_('VBADMINNOTESUPD'); ?></button>
							</div>
						</div>
	...
	protected function calcDatesNavigation($from_ts, $to_ts, $step, $date_dir)
	{
		...
		if ($step == 'weekend') {
			...
			return array($from_ts, $to_ts, JText::_('VBOWEEKND'));
		}
		...
		if ($step == 'month') {
			...
			return array($from_ts, $to_ts, JText::_('VBPVIEWRESTRICTIONSTWO'));
		}
		...
		return array($from_ts, $to_ts, JText::_('VBO_QUARTER'));
	}
