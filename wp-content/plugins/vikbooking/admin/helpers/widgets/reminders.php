...
class VikBookingAdminWidgetReminders extends VikBookingAdminWidget
{
	...
	public function __construct()
	{
		...
		$this->widgetName = JText::_('VBO_W_REMINDERS_TITLE');
		$this->widgetDescr = JText::_('VBO_W_REMINDERS_DESCR');
	...
	public function loadReminders()
	{
		...
		if (!$reminders) {
			?>
		<p class="info"><?php echo JText::_('VBONORESULTS'); ?></p>
			<?php
		} else {
					...
					<div class="vbo-admin-widget-head-commands">
						...
						<div class="vbo-reportwidget-filters">
							<div class="vbo-reportwidget-filter">
								<label for="show_completed<?php echo $wrapper_instance; ?>-on"><?php echo JText::_('VBO_SHOW_COMPLETED'); ?></label>
								<?php echo $this->vbo_app->printYesNoButtons('show_completed' . $wrapper_instance, JText::_('VBYES'), JText::_('VBNO'), $def_completed, 1, 0); ?>
							</div>
							<div class="vbo-reportwidget-filter">
								<label for="show_expired<?php echo $wrapper_instance; ?>-on"><?php echo JText::_('VBO_SHOW_EXPIRED'); ?></label>
								<?php echo $this->vbo_app->printYesNoButtons('show_expired' . $wrapper_instance, JText::_('VBYES'), JText::_('VBNO'), $def_expired, 1, 0); ?>
							</div>
							<div class="vbo-reportwidget-filter vbo-reportwidget-filter-confirm">
								<button type="button" class="btn vbo-config-btn" onclick="vboWidgetRemindersChangeParams('<?php echo $wrapper_id; ?>');"><?php echo JText::_('VBADMINNOTESUPD'); ?></button>
							</div>
						</div>
				...
				<div class="vbo-widget-reminders-inner">
					<div class="vbo-widget-reminders-manage" style="display: none;">
						...
						<div class="vbo-widget-reminders-goback">
							<a href="JavaScript: void(0);" onclick="vboWidgetRemindersToggleManage('<?php echo $wrapper_id; ?>');"><?php VikBookingIcons::e('chevron-left'); ?> <?php echo JText::_('VBBACK'); ?></a>
						</div>

						<div class="vbo-widget-reminders-manage-wrap">
							<div class="vbo-widget-reminders-filter vbo-widget-reminders-filter-title">
								<input type="text" class="vbo-widget-reminders-ftitle" value="" placeholder="<?php echo htmlspecialchars(JText::_('VBO_REMINDER_TITLE')); ?>" />
							</div>
							<div class="vbo-widget-reminders-filter vbo-widget-reminders-filter-descr">
								<textarea class="vbo-widget-reminders-fdescr" placeholder="<?php echo htmlspecialchars(JText::_('VBPVIEWOPTIONALSTWO')); ?>" maxlength="2000"></textarea>
							</div>
							<div class="vbo-widget-reminders-filter vbo-widget-reminders-filter-booking vbo-toggle-small" style="<?php echo empty($page_bid) ? 'display: none;' : ''; ?>">
								<span class="vbo-widget-reminders-filter-tobooking">
									<label for="assign_booking<?php echo $wrapper_instance; ?>-on"><span class="label label-info vbo-widget-reminders-filter-booking-id"><?php echo $page_bid; ?></span> <?php echo JText::_('VBO_ASSIGN_TO_BOOKING'); ?></label>
									<?php echo $this->vbo_app->printYesNoButtons('assign_booking' . $wrapper_instance, JText::_('VBYES'), JText::_('VBNO'), (!empty($page_bid) ? 1 : 0), 1, 0); ?>
								</span>
								<span class="vbo-widget-reminders-filter-importance">
									<label for="is_important<?php echo $wrapper_instance; ?>-on"><?php VikBookingIcons::e('exclamation-triangle'); ?> <?php echo JText::_('VBO_IMPORTANT'); ?></label>
									<?php echo $this->vbo_app->printYesNoButtons('is_important' . $wrapper_instance, JText::_('VBYES'), JText::_('VBNO'), 0, 1, 0); ?>
								</span>
							</div>
							<div class="vbo-widget-reminders-filter-datetime">
								<div class="vbo-widget-reminders-filter vbo-widget-reminders-filter-date">
									<div class="vbo-field-calendar">
										<div class="input-append">
											<input type="text" class="vbo-reminders-dtpicker" value="<?php echo date($this->df, $def_duedate); ?>" placeholder="<?php echo htmlspecialchars(JText::_('VBO_REMINDER_DUE_DATE')); ?>" />
								...
								<div class="vbo-widget-reminders-filter vbo-widget-reminders-filter-time">
									<select class="vbo-widget-reminders-fh">
										<option value=""><?php echo ucfirst(JText::_('VBCONFIGONETENEIGHT')); ?></option>
									<?php
									for ($i = 0; $i < 24; $i++) { 
										?>
									...
									<select class="vbo-widget-reminders-fm">
										<option value=""><?php echo ucfirst(JText::_('VBTRKDIFFMINS')); ?></option>
									<?php
									for ($i = 0; $i < 60; $i++) { 
										?>
							...
							<div class="vbo-widget-reminders-filter vbo-widget-reminders-filter-save">
								...
								<button type="button" class="btn btn-success vbo-widget-reminder-confirm" onclick="vboWidgetRemindersSaveNew('<?php echo $wrapper_id; ?>');"><?php VikBookingIcons::e('bell'); ?> <span><?php echo JText::_('VBCONFIGCLOSINGDATEADD'); ?></span></button>
								<button type="button" class="btn btn-danger vbo-widget-reminder-delete" onclick="vboWidgetRemindersDelete('<?php echo $wrapper_id; ?>');" style="display: none;"><?php VikBookingIcons::e('trash'); ?> <?php echo JText::_('VBELIMINA'); ?></button>
							</div>
						</div>
	...
	protected function scheduleChannelManagerReminders()
	{
		...
		foreach ($airbnb_checkouts as $airbnb_res) {
			...
			if (!$helper->bookingHasReminder($airbnb_res['id'], $payload) && VikChannelManager::hostToGuestReviewSupported($airbnb_res)) {
				...
				$reminder = new stdClass;
				$reminder->title 	 = JText::_('VBO_REVIEW_YOUR_GUEST');
				$reminder->descr 	 = 'Airbnb - ' . JText::_('VBDASHBOOKINGID') . ' ' . $airbnb_res['id'];
