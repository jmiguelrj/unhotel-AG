...
class VikBookingAdminWidgetCheckAvailability extends VikBookingAdminWidget
{
	...
	public function __construct()
	{
		...
		$this->widgetName = JText::_('VBO_W_CHECKAV_TITLE');
		$this->widgetDescr = JText::_('VBO_W_CHECKAV_DESCR');
	...
	public function loadRoomRates()
	{
		...
		// nights information
		$nights_lbl = $num_nights > 1 ? JText::_('VBDAYS') : JText::_('VBDAY');
		$party_info_parts[] = $num_nights . ' ' . $nights_lbl;
		// adults information
		$adults_lbl = $num_adults === 1 ? JText::_('VBMAILADULT') : JText::_('VBEDITORDERADULTS');
		$party_info_parts[] = $num_adults . ' ' . $adults_lbl;
		// children information
		$children_lbl = $num_children === 1 ? JText::_('VBMAILCHILD') : JText::_('VBEDITORDERCHILDREN');
		if ($num_children > 0) {
		...
		if (!is_array($room_rates) || !$room_rates || $has_av_error) {
			...
			$say_error 	 = empty($say_error) ? JText::_('VBOVERVIEWLEGRED') : $say_error;
			?>
					...
					<div class="vbo-widget-checkav-result-room-rate">
						...
						if ($net_price != $rplan['cost']) {
							...
							<span class="vbo-widget-checkav-result-rate-price vbo-widget-checkav-result-rate-net">
								<span><?php echo JText::_('VBCALCRATESNET'); ?></span>
							...
							<span class="vbo-widget-checkav-result-rate-price vbo-widget-checkav-result-rate-tax">
								<span><?php echo JText::_('VBCALCRATESTAX'); ?></span>
						...
						?>
							<span class="vbo-widget-checkav-result-rate-price vbo-widget-checkav-result-rate-total">
								<span><?php echo JText::_('VBCALCRATESTOT'); ?></span>
							...
							<span class="vbo-widget-checkav-result-rate-price vbo-widget-checkav-result-rate-booknow">
								<button type="button" class="btn btn-primary" onclick="vboWidgetCheckAvailabilityBook(<?php echo $js_book_args; ?>);"><?php echo JText::_('VBO_BOOKNOW'); ?></button>
							</span>
						</div>
					</div>
					<?php
		...
		if ($split_stay_sols) {
			...
			?>
			<p class="info"><?php VikBookingIcons::e('random'); ?> <?php echo JText::_('VBO_SPLIT_STAYS'); ?></p>
			...
			foreach ($split_stay_sols as $split_stay_sol) {
				...
				<div class="vbo-widget-checkav-splitstay-rooms">
						...
						<div class="vbo-widget-checkav-splitstay-room">
							...
							<div class="vbo-widget-checkav-result-altdates-info">
								<div class="vbo-widget-checkav-result-alt-date">
									<span class="vbo-widget-checkav-result-split-nights"><?php VikBookingIcons::e('moon'); ?> <?php echo $split_stay_room['nights']; ?> <?php echo $split_stay_room['nights'] > 1 ? JText::_('VBDAYS') : JText::_('VBDAY'); ?></span>
					...
					<div class="vbo-widget-checkav-splitstay-rooms-book">
						<button type="button" class="btn btn-primary" onclick="vboWidgetCheckAvailabilityBookSplitStay(<?php echo $js_book_args; ?>);"><?php echo JText::_('VBO_BOOKNOW'); ?></button>
					</div>
				</div>
				<?php
			}
		...
		if ($alternative_dates) {
			...
			?>
			<p class="info"><?php echo JText::_('VBO_ALT_STAY_DATES'); ?></p>
				...
				<div class="vbo-widget-checkav-result-room-rates vbo-widget-checkav-result-altdates">
					...
					<div class="vbo-widget-checkav-result-altdates-info">
						...
						<div class="vbo-widget-checkav-result-alt-choose">
							<button type="button" class="btn btn-primary" onclick="vboWidgetCheckAvailabilityPickRoom(<?php echo $js_pick_args; ?>);"><?php echo JText::_('VBO_SELECT'); ?></button>
						</div>
					</div>
				</div>
					<?php
		...
		if ($alternative_parties) {
			...
			?>
			<p class="info"><?php echo JText::_('VBO_ALT_ROOM_PARTIES'); ?></p>
			...
			foreach ($alternative_parties as $arrive_dt => $alt_parties) {
				...
				foreach ($alt_parties as $alt_party) {
					...
					$party_info_parts = array();
					// adults information
					$adults_lbl = $alt_party['guests_allocation']['adults'] == 1 ? JText::_('VBMAILADULT') : JText::_('VBEDITORDERADULTS');
					$party_info_parts[] = $alt_party['guests_allocation']['adults'] . ' ' . $adults_lbl;
					// children information
					$children_lbl = isset($alt_party['guests_allocation']['children']) && $alt_party['guests_allocation']['children'] == 1 ? JText::_('VBMAILCHILD') : JText::_('VBEDITORDERCHILDREN');
				...
				<div class="vbo-widget-checkav-filters">
					<div class="vbo-widget-checkav-filters-main">
						<div class="vbo-widget-checkav-filter vbo-widget-checkav-filter-dpicker">
							<div class="vbo-field-calendar">
								<div class="input-append">
									<input type="text" class="vbo-widget-checkav-checkindt" value="" placeholder="<?php echo htmlspecialchars(JText::_('VBPICKUPAT')); ?>" />
						...
						<div class="vbo-widget-checkav-filter vbo-widget-checkav-filter-nights">
							<label for="vbo-widget-checkav-nights-<?php echo $wrapper_instance; ?>"><?php echo JText::_('VBDAYS'); ?></label>
					...
					<div class="vbo-widget-checkav-filters-secondary">
						<div class="vbo-widget-checkav-filter vbo-widget-checkav-filter-adults">
							<label for="vbo-widget-checkav-adults-<?php echo $wrapper_instance; ?>"><?php echo JText::_('VBEDITORDERADULTS'); ?></label>
						...
						<div class="vbo-widget-checkav-filter vbo-widget-checkav-filter-children">
							<label for="vbo-widget-checkav-children-<?php echo $wrapper_instance; ?>"><?php echo JText::_('VBEDITORDERCHILDREN'); ?></label>
					...
					<div class="vbo-widget-checkav-filters-submit">
						<div class="vbo-widget-checkav-filter vbo-widget-checkav-filter-submit">
							<button type="button" class="btn vbo-config-btn" onclick="vboWidgetCheckAvailabilityCalc('<?php echo $wrapper_id; ?>');"><?php VikBookingIcons::e('bed'); ?><?php echo JText::_('VBRATESOVWRATESCALCULATORCALC'); ?></button>
						</div>
					</div>
				</div>
