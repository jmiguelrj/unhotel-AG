...
class VikBookingAdminWidgetDepartingToday extends VikBookingAdminWidget
{
	...
	public function __construct()
	{
		...
		$this->widgetName = JText::_('VBO_W_DEPARTTOD_TITLE');
		$this->widgetDescr = JText::_('VBO_W_DEPARTTOD_DESCR');
	...
	public function render(?VBOMultitaskData $data = null)
	{
		...
		if (!defined('VBO_JMODAL_CHECKIN_BOOKING')) {
			...
			echo $this->vbo_app->getJmodalHtml('vbo-checkin-booking', JText::_('VBOMANAGECHECKSINOUT'));
		}
		...
		$multistate_lbls = array(
			array(
				'value' => JText::_('VBTODAY'),
			),
			array(
				'value' => JText::_('VBOTOMORROW'),
			),
			array(
				'value' => JText::_('VBOYESTERDAY'),
			),
		);
			...
			<div class="vbo-admin-widget-head vbo-dashboard-today-checkout-head">
				<div class="vbo-admin-widget-head-inline">
					<h4>
						...
						<span class="departures-when"><?php echo JText::_('VBODEPARTING'); ?></span> 
	...
	protected function buildDepartureRows($departures, $when = 'today')
	{
		...
		foreach ($departures as $outk => $outtoday) {
			...
			if ($outtoday['status'] == 'confirmed') {
				switch ($outtoday['checked']) {
					case -1:
						$ord_status = '<span class="label label-error vbo-status-label">'.JText::_('VBOCHECKEDSTATUSNOS').'</span>';
						break;
					case 1:
						$ord_status = '<span class="label label-success vbo-status-label">'.JText::_('VBOCHECKEDSTATUSIN').'</span>';
						break;
					case 2:
						$ord_status = '<span class="label label-success vbo-status-label">'.JText::_('VBOCHECKEDSTATUSOUT').'</span>';
						break;
					default:
						$ord_status = '<span class="label label-success vbo-status-label">'.JText::_('VBCONFIRMED').'</span>';
						break;
				}
				if (!strcasecmp((string)$outtoday['type'], 'overbooking')) {
					$ord_status .= '<div class="vbo-orders-substatus"><span class="label label-error">' . JText::_('VBO_BTYPE_OVERBOOKING') . '</span></div>';
				}
				...
				$ord_status = '<span class="label label-error vbo-status-label">'.JText::_('VBCANCELLED').'</span>';
			}
		...
		if (!count($departures)) {
			...
			$no_departures_when = JText::_('VBONOCHECKOUTSTODAY');
			if ($when == 'yesterday') {
				$no_departures_when = JText::_('VBONOCHECKOUTSYESTERDAY');
			} elseif ($when == 'tomorrow') {
				$no_departures_when = JText::_('VBONOCHECKOUTSTOMORROW');
			}
