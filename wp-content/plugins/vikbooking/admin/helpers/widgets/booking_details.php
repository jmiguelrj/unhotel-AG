...
class VikBookingAdminWidgetBookingDetails extends VikBookingAdminWidget
{
	...
	public function __construct()
	{
		...
		$this->widgetName = JText::_('VBMAINORDEREDIT');
		$this->widgetDescr = JText::_('JSEARCH_TOOLS');
	...
	public function loadBookingDetails()
	{
		...
		if (empty($booking_key) && empty($booking_id)) {
			// no bookings found
			VBOHttpDocument::getInstance()->close(400, JText::_('VBOREPORTSERRNORESERV'));
		}
		...
		if (!$details) {
			// no bookings found
			VBOHttpDocument::getInstance()->close(404, JText::_('VBOREPORTSERRNORESERV'));
		}
		...
		$otachannel_name = JText::_('VBORDFROMSITE');
		$otachannel_bid  = '';
		...
		<div class="vbo-admin-container vbo-admin-container-full vbo-admin-container-compact" data-bookingid="<?php echo $details['id']; ?>">
			<div class="vbo-params-wrap">
				<div class="vbo-params-container">
					...
					<div class="vbo-params-block">
						<div class="vbo-param-container">
							...
							?>
								<div class="vbo-customer-info-box">
									<div class="vbo-customer-info-box-avatar vbo-customer-avatar-medium">
										...
										if (!empty($channel_logo)) {
											...
											?>
											<span class="vbo-tooltip vbo-tooltip-top" data-tooltiptext="<?php echo JHtml::_('esc_attr', $otachannel_name); ?>">
											...
											?>
											<span class="vbo-tooltip vbo-tooltip-top" data-tooltiptext="<?php echo JHtml::_('esc_attr', $otachannel_name); ?>" onclick="vboWidgetBookDetsOpenBooking('<?php echo $details['id']; ?>');"><?php VikBookingIcons::e('hotel', 'vbo-dashboard-guest-activity-avatar-icon'); ?></span>
											<?php
										}
										?>
										</span>
								...
								<span class="label label-info"><?php echo JText::_('VBDASHUPRESONE') . ' ' . $details['id']; ?></span>
							<?php
							...
							if (!empty($details['type'])) {
								$status_type = JText::_('VBO_BTYPE_' . strtoupper($details['type']));
							...
							$extra_status = $details['refund'] > 0 ? ' / ' . JText::_('VBO_STATUS_REFUNDED') : '';
							if ($details['status'] == "confirmed") {
							...
							if ($details['closure']) {
								?>
								<span class="label label-error"><?php VikBookingIcons::e('ban'); ?> <?php echo JText::_('VBDBTEXTROOMCLOSED'); ?></span>
								<?php
							}
							?>
							</div>
					...
					?>
						...
						<div class="vbo-param-container">
							<div class="vbo-param-label">
								<div class="vbo-customer-info-box">
									<div class="vbo-customer-info-box-name">
										<?php echo (isset($customer['country_img']) ? $customer['country_img'] . ' ' : '') . ($customer ? ltrim($customer['first_name'] . ' ' . $customer['last_name']) : JText::_('VBPVIEWORDERSPEOPLE')); ?>
									</div>
								<?php
								if (!$custpic_used && !empty($customer['pic'])) {
									...
									<div class="vbo-customer-info-box-avatar vbo-customer-avatar-small vbo-widget-bookdets-cpic-zoom">
										<span>
											<img src="<?php echo strpos($customer['pic'], 'http') === 0 ? $customer['pic'] : VBO_SITE_URI . 'resources/uploads/' . $customer['pic']; ?>" data-caption="<?php echo JHtml::_('esc_attr', $customer_name); ?>" />
										</span>
									</div>
									<?php
								}
							...
							$guest_counters = [];
							if ($tot_adults) {
								$guest_counters[] = $tot_adults . ' ' . JText::_(($tot_adults > 1 ? 'VBMAILADULTS' : 'VBMAILADULT'));
							}
							if ($tot_children) {
								$guest_counters[] = $tot_children . ' ' . JText::_(($tot_children > 1 ? 'VBMAILCHILDREN' : 'VBMAILCHILD'));
							}
							if ($tot_pets) {
								$guest_counters[] = $tot_pets . ' ' . JText::_(($tot_pets > 1 ? 'VBO_PETS' : 'VBO_PET'));
							}
						...
						<div class="vbo-param-container">
							<div class="vbo-param-label"><?php echo JText::_('VBPICKUPAT'); ?></div>
							<div class="vbo-param-setting">
								<span class="vbo-tooltip vbo-tooltip-top" data-tooltiptext="<?php echo JHtml::_('esc_attr', $checkin_read_dt); ?>"><?php echo date(str_replace("/", $this->datesep, $this->df), $details['checkin']); ?></span>
							</div>
						</div>

						<div class="vbo-param-container">
							<div class="vbo-param-label"><?php echo JText::_('VBRELEASEAT'); ?></div>
							<div class="vbo-param-setting">
								<span class="vbo-tooltip vbo-tooltip-top" data-tooltiptext="<?php echo JHtml::_('esc_attr', $checkout_read_dt); ?>"><?php echo date(str_replace("/", $this->datesep, $this->df), $details['checkout']); ?></span>
								<span>(<?php echo $details['days'] . ' ' . ($details['days'] > 1 ? JText::_('VBDAYS') : JText::_('VBDAY')); ?>)</span>
							<?php
							if ($details['split_stay']) {
								?>
								<span class="vbo-tooltip vbo-tooltip-top" data-tooltiptext="<?php echo JHtml::_('esc_attr', JText::_('VBO_SPLIT_STAY')); ?>"><?php VikBookingIcons::e('random'); ?></span>
								<?php
							}
							?>
							</div>
						...
						<div class="vbo-param-container">
							<div class="vbo-param-label"><?php echo JText::_('VBOINVCREATIONDATE'); ?></div>
							<div class="vbo-param-setting">
					...
					?>
						<div class="vbo-param-container">
							<div class="vbo-param-label"><?php echo JText::_('VBO_CRONJOB_WEBHOOK_TYPE_URL_OPTION'); ?></div>
							<div class="vbo-param-setting">
					...
					if ($tot_unread_messages) {
						...
						<div class="vbo-param-container">
							...
							<div class="vbo-param-setting">
								<button type="button" class="btn vbo-config-btn vbo-btn-icon-right" data-hasguestmessages="1">
									<span><?php echo JText::_('VBO_GUEST_MESSAGING'); ?></span>
						...
						<div class="vbo-param-container">
							...
							<div class="vbo-param-setting">
								<button type="button" class="btn vbo-config-btn vbo-btn-icon-right" data-hasguestmessages="1">
									<span><?php echo JText::_('VBO_GUEST_MESSAGING'); ?> <?php VikBookingIcons::e('comment-dots'); ?></span>
								</button>
							</div>
						</div>
						<?php
						...
						<div class="vbo-param-container">
							...
							if ($details['roomsnum'] > 1 && !$details['split_stay'] && isset($guests[$ind])) {
								...
								if ($guests[$ind]['adults']) {
									$room_guest_counters[] = $guests[$ind]['adults'] . ' ' . JText::_(($guests[$ind]['adults'] > 1 ? 'VBMAILADULTS' : 'VBMAILADULT'));
								}
								if ($guests[$ind]['children']) {
									$room_guest_counters[] = $guests[$ind]['children'] . ' ' . JText::_(($guests[$ind]['children'] > 1 ? 'VBMAILCHILDREN' : 'VBMAILCHILD'));
								}
								if ($guests[$ind]['pets']) {
									$room_guest_counters[] = $guests[$ind]['pets'] . ' ' . JText::_(($guests[$ind]['pets'] > 1 ? 'VBO_PETS' : 'VBO_PET'));
								}
								?>
								<div>
				...
				if (!$details['closure']) {
					...
					<div class="vbo-params-block">
						<div class="vbo-param-container">
							<div class="vbo-param-label"><?php echo JText::_('VBEDITORDERNINE'); ?></div>
							<div class="vbo-param-setting">
					...
					if ($details['totpaid'] > 0) {
						?>
						<div class="vbo-param-container">
							<div class="vbo-param-label"><?php echo JText::_('VBPEDITBUSYTOTPAID'); ?></div>
							<div class="vbo-param-setting">
					...
					<div class="vbo-params-fieldset">
						<div class="vbo-params-fieldset-label"><?php echo JText::_('VBADMINNOTESTOGGLE'); ?></div>
						<div class="vbo-params-block">
							<div class="vbo-param-container">
								<div class="vbo-param-setting">
					...
					<div class="vbo-params-fieldset vbo-widget-bookdets-tasks-list">
						<div class="vbo-params-fieldset-label"><?php echo JText::_('VBO_TASK_MANAGER'); ?></div>
						...
						foreach ($bookingTasks as $taskRecord) {
							...
							if ($taskManager->statusTypeExists($task->getStatus())) {
								...
								<div class="vbo-bookingdet-task-detail" data-type="status">
									<span class="vbo-tm-task-status-badge vbo-tm-color <?php echo $taskStatus->getColor(); ?>" data-status="<?php echo JHtml::_('esc_attr', $taskStatus->getEnum()); ?>"><?php echo $taskStatus->getName(); ?></span>
								</div>
								<?php
							}
							?>
					...
					?>
					<div class="vbo-params-fieldset">
						<div class="vbo-params-fieldset-label"><?php echo JText::_('VBO_W_REMINDERS_TITLE'); ?></div>
						<div class="vbo-params-block">
						<?php
						...
						?>
							<div class="vbo-param-container">
								...
								if ($has_reminders) {
									?>
									<button type="button" class="btn vbo-config-btn" data-hasreminders="1"><?php VikBookingIcons::e('bell'); ?> <?php echo JText::_('VBO_SEE_ALL'); ?></button>
									<?php
								} else {
									?>
									<button type="button" class="btn btn-success" data-hasreminders="0"><?php VikBookingIcons::e('plus-circle'); ?> <?php echo JText::_('VBO_ADD_NEW'); ?></button>
									<?php
								}
								?>
								</div>
					...
					<div class="vbo-params-fieldset vbo-widget-bookdets-customer-docs">
						<div class="vbo-params-fieldset-label"><?php echo JText::_('VBOCUSTOMERDOCUMENTS'); ?></div>
						<div class="vbo-params-block">
							<?php
							...
							<div class="vbo-param-container">
								<div class="vbo-param-label">
									<span class="label label-warning"><?php VikBookingIcons::e('certificate'); ?> <?php echo JText::_('VBO_CONDTEXT_RULE_RETCUST'); ?></span>
								</div>
							...
							if ($checked_status) {
								...
								?>
							<div class="vbo-param-container">
								<div class="vbo-param-label"><?php echo JText::_('VBOCHECKEDSTATUS'); ?></div>
								<div class="vbo-param-setting">
					...
					<div class="vbo-params-fieldset vbo-widget-bookdets-history-list">
						<div class="vbo-params-fieldset-label"><?php echo JText::_('VBOBOOKHISTORYTAB'); ?></div>
						<div class="vbo-params-block">
						<?php
						...
						foreach ($history_list as $hind => $hist) {
							...
							$tip_info = [
								JText::_('VBOBOOKHISTORYLBLTOT') . ': ' . VikBooking::formatCurrencyNumber(VikBooking::numberFormat($hist['total']), $currencysymb),
								JText::_('VBOBOOKHISTORYLBLTPAID') . ': ' . VikBooking::formatCurrencyNumber(VikBooking::numberFormat($hist['totpaid']), $currencysymb),
							];
							?>
							<div class="vbo-param-container vbo-widget-bookdets-history-record" style="<?php echo $hind >= $max_display_records ? 'display: none;' : ''; ?>">
								<div class="vbo-param-label">
									<div>
										<span class="vbo-tooltip vbo-tooltip-top vbo-widget-bookdets-history-evname" data-tooltiptext="<?php echo JHtml::_('esc_attr', implode(', ', $tip_info)); ?>"><?php echo $history_obj->validType($hist['type'], true); ?></span>
									</div>
									<div>
										<span class="vbo-widget-bookdets-history-evdate"><?php echo JHtml::_('date', $hist['dt'], 'Y-m-d H:i:s'); ?></span>
									</div>
								</div>
								...
								if ($read_more) {
									...
									<div class="vbo-widget-bookdets-history-descr-txt">
										...
										<div>
											<span class="vbo-tooltip vbo-tooltip-top vbo-widget-bookdets-history-descr-more" data-tooltiptext="<?php echo JHtml::_('esc_attr', JText::_('VBO_READ_MORE')); ?>" onclick="vboWidgetBookDetsReadFullHistory(this);">
						...
						if (count($history_list) > $max_display_records) {
							...
							<div class="vbo-param-container vbo-widget-bookdets-history-showall">
								<div class="vbo-param-label">&nbsp;</div>
								<div class="vbo-param-setting">
									<button type="button" class="btn vbo-config-btn" onclick="vboWidgetBookDetsHistoryAll('<?php echo $wrapper; ?>');"><?php VikBookingIcons::e('history'); ?> <?php echo JText::_('VBO_SEE_ALL'); ?></button>
								</div>
							</div>
							<?php
						}
					...
					if ($cm_notifications) {
						?>
					<div class="vbo-params-fieldset">
						<div class="vbo-params-fieldset-label"><?php echo JText::_('VBOBOOKHISTORYTCM'); ?></div>
						<div class="vbo-params-block">
						<?php
										...
										<div class="vbo-param-label">
											<div class="vbo-customer-info-box">
												<div class="vbo-customer-info-box-avatar vbo-customer-avatar-small">
													<span class="vbo-tooltip vbo-tooltip-top" data-tooltiptext="<?php echo JHtml::_('esc_attr', $channel_name); ?>">
											...
											if ($ruid_codes) {
												...
												<span class="vbo-widget-bookdets-cm-ruids">
													<button class="btn btn-small vbo-btn-icon-right" type="button" onclick="vboWidgetBookDetsCopyRuids(this);">
														<span class="vbo-tooltip vbo-tooltip-top" data-tooltiptext="<?php echo JHtml::_('esc_attr', JText::_('VBO_COPY')); ?>">RUID <?php VikBookingIcons::e('copy'); ?></span>
													</button>
												</span>
						...
						if (count($cm_notifications) > $max_display_notifications) {
							...
							<div class="vbo-param-container vbo-widget-bookdets-cmnotifs-showall">
								<div class="vbo-param-label">&nbsp;</div>
								<div class="vbo-param-setting">
									<button type="button" class="btn vbo-config-btn" onclick="vboWidgetBookDetsCmnotifsAll('<?php echo $wrapper; ?>');"><?php VikBookingIcons::e('network-wired'); ?> <?php echo JText::_('VBO_SEE_ALL'); ?></button>
								</div>
							</div>
							<?php
						}
					...
					<div class="vbo-param-container">
						<div class="vbo-param-setting">
							<button type="button" class="btn vbo-config-btn" onclick="vboWidgetBookDetsOpenBooking('<?php echo $details['id']; ?>');"><?php echo JText::_('VBOVIEWBOOKINGDET'); ?></button>
							<button type="button" class="btn" onclick="vboWidgetBookDetsOpenBooking('<?php echo $details['id']; ?>', 'edit');"><?php echo JText::_('VBMODRES'); ?></button>
						</div>
					</div>
				...
				<div class="vbo-widget-bookdets-inner">
					...
					<div class="vbo-widget-element-filter-top">
						<div class="btn-group-inline">
							...
							<input type="text" name="booking_key" value="<?php echo $load_bid ? "id: {$load_bid}" : ''; ?>" placeholder="<?php echo htmlspecialchars(JText::_('VBOFILTCONFNUMCUST')); ?>" autocomplete="off" />
							<button type="button" class="btn vbo-config-btn" onclick="vboWidgetBookDetsSearch('<?php echo $wrapper_id; ?>');"><?php VikBookingIcons::e('search'); ?> <span class="vbo-widget-bookdets-searchbtn"><?php echo JText::_('VBODASHSEARCHKEYS'); ?></span></button>
						<?php
						if ($selected_event) {
							// display button to select the booking found
							?>
							<button type="button" class="btn btn-success vbo-wbookingdet-select-booking" style="margin-left: 4px;" disabled><?php VikBookingIcons::e('check'); ?> <?php echo JText::_('VBO_SELECT'); ?></button>
							<?php
						}
						?>
						</div>
				...
				if ($from_push) {
					...
					if ($push_action && !strcasecmp($push_from, 'airbnb')) {
						...
						<div class="vbo-widget-push-notification-action" style="display: none;">
							<a class="btn vbo-config-btn vbo-widget-bookdets-actionbtn" href="<?php echo VBOFactory::getPlatform()->getUri()->admin('index.php?option=com_vikbooking&task=editorder&cid[]=%d&notif_action=airbnb_host_guest_review', $xhtml = false); ?>" target="_blank"><?php VikBookingIcons::e('star'); ?> <?php echo JText::_('VBO_REVIEW_YOUR_GUEST'); ?></a>
						</div>
							<?php
							...
							?>
						<div class="vbo-widget-push-notification-action" style="display: none;">
							<a class="btn vbo-config-btn vbo-widget-bookdets-actionbtn" href="<?php echo VBOFactory::getPlatform()->getUri()->admin('index.php?option=com_vikbooking&task=editorder&cid[]=%d&notif_action=see_guest_review', $xhtml = false); ?>" target="_blank"><?php VikBookingIcons::e('star'); ?> <?php echo JText::_('VBOSEEGUESTREVIEW'); ?></a>
						</div>
							<?php
						}
					}
