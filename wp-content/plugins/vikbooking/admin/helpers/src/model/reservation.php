...
class VBOModelReservation extends JObject
{
	...
	public function extractSpecialRequests()
	{
		...
		if (preg_match("/(?:special_?requests:\s*)(.*?)$/is", $raw_cust_data, $match)) {
			...
			$special_requests = $match[1];
		} elseif (preg_match("/(?:" . JText::_('ORDER_SPREQUESTS') . ":\s*)(.*?)$/is", $raw_cust_data, $match)) {
			$special_requests = $match[1];
		}
	...
	public function create()
	{
		...
		if (!$this->get('force_booking', 0) && !$this->get('set_closed', 0) && !$room_available) {
			...
			$this->setError(JText::_('VBBOOKNOTMADE'));
			return false;
		}
		...
		if (in_array('vcm_sync', $trigger_operations)) {
			...
			if ($vcm_autosync > 0) {
				...
				if ($sync_result === false) {
					...
					$this->setError(JText::_('VBCHANNELMANAGERRESULTKO') . (!empty($vcm_err) ? ' - ' . $vcm_err : ''));
				}
				...
				$this->setChannelManagerAction(
					JText::_('VBCHANNELMANAGERINVOKEASK') . ' ' .
					...
					'<button type="submit" class="btn btn-primary">' . JText::_('VBCHANNELMANAGERSENDRQ') . '</button>' .
					'</form>'
				);
			}
		}
	...
	public function setConfirmed(array $options)
	{
		...
		if ($vcm_autosync > 0) {
			...
			$this->setChannelManagerAction(JText::_('VBCHANNELMANAGERINVOKEASK') . ' <button type="button" class="btn btn-primary" onclick="document.location.href=\'' . $vcm_sync_url . '\';">' . JText::_('VBCHANNELMANAGERSENDRQ') . '</button>');
		}
	...
	protected function detectForcedReason($room_available = true)
	{
		...
		if (empty($split_stay_data)) {
			...
			if (($force_booking || $set_closed) && !$room_available) {
				$forced_reason = JText::_('VBO_FORCED_BOOKDATES');
			}
		} else {
			$all_rooms = VikBooking::getAvailabilityInstance(true)->loadRooms();
			// set "split stay" as the description of the history event
			$forced_reason = JText::_('VBO_SPLIT_STAY') . "\n";
			foreach ($split_stay_data as $sps_k => $split_stay) {
				...
				$forced_reason .= $all_rooms[$split_stay['idroom']]['name'] . ': ' . $room_stay_nights . ' ' . ($room_stay_nights > 1 ? JText::_('VBDAYS') : JText::_('VBDAY')) . ', ';
	...
	protected function storeReservationRecords(array $rooms_pool)
	{
		...
		if ($set_closed) {
			...
			$customer_data = JText::_('VBDBTEXTROOMCLOSED');
