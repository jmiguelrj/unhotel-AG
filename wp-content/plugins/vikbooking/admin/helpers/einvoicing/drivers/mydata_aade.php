...
class VikBookingEInvoicingMydataAade extends VikBookingEInvoicing
{
	...
	public function getFilters()
	{
		...
		if ($minmax) {
			if (!empty($minmax['mindate']) && !empty($minmax['maxdate'])) {
				...
				$filter_opt = array(
					'label' => '<label for="monyear">' . JText::_('VBPVIEWRESTRICTIONSTWO') . '</label>',
		...
		// date type filter
		$filter_opt = array(
			'label' => '<label for="datetype">' . JText::_('VBPVIEWORDERSONE') . '</label>',
			'html' => '<select name="datetype" id="datetype">
							<option value="ts"'.($pdatetype == 'ts' ? ' selected="selected"' : '').'>' . JText::_('VBRENTALORD') . '</option>
							<option value="checkin"'.($pdatetype == 'checkin' ? ' selected="selected"' : '').'>' . JText::_('VBPICKUPAT') . '</option>
							<option value="checkout"'.($pdatetype == 'checkout' ? ' selected="selected"' : '').'>' . JText::_('VBRELEASEAT') . '</option>
						</select>',
			'type' => 'select',
			'name' => 'datetype'
		);
		...
		$filter_opt = array(
			'label' => '<label for="einvkword">' . JText::_('VBODASHSEARCHKEYS') . '</label>',
	...
	public function getButtons()
	{
		...
		array_push($this->driverButtons, '
			<a href="JavaScript: void(0);" onclick="vboDriverDoAction(\'generateEInvoices\', false);" class="vbcsvexport"><i class="vboicn-file-text2 icn-nomargin"></i> <span>'.JText::_('VBODRIVERGENERATEINVS').'</span></a>
		');
	...
	protected function prepareSavingSettings()
	{
		...
		foreach ($mandatory as $field) {
			if (empty($field)) {
				$this->setError(JText::_('VBO_PLEASE_FILL_FIELDS'));
				return false;
			}
		}
	...
	public function getBookingsData()
	{
		...
		$this->cols = array(
			...
			array(
				'key' => 'customer',
				'sortable' => 1,
				'label' => JText::_('VBOCUSTOMER')
			),
			// country
			array(
				'key' => 'country',
				'sortable' => 1,
				'label' => JText::_('ORDER_STATE')
			),
			// city
			array(
				'key' => 'city',
			...
			array(
				...
				'sortable' => 1,
				'label' => JText::_('VBCUSTOMERCOMPANYVAT')
			),
			// counterpart company name
			array(
				'key' => 'company',
				'sortable' => 1,
				'label' => JText::_('VBCUSTOMERCOMPANY')
			),
			// total
			array(
				'key' => 'tot',
				...
				'sortable' => 1,
				'label' => JText::_('VBPVIEWORDERSSEVEN')
			),
			// commands
			array(
				'key' => 'commands',
			...
			array(
				...
				'sortable' => 1,
				'label' => JText::_('VBO_BACKUP_ACTION_LABEL')
			),
		);
					...
					'callback' => function ($val) use ($analog_id, $multi_analog_ids) {
						if ($val < 0 && !empty($analog_id)) {
							...
							if (count($multi_analog_ids) < 2) {
								...
								return '<a href="index.php?option=com_vikbooking&task=editmaninvoice&cid[]='.$analog_id.'&goto='.$returi.'"><i class="'.VikBookingIcons::i('external-link').'"></i> '.JText::_('VBOMANUALINVOICE').'</a>';
							}
							/**
							...
							foreach ($multi_analog_ids as $analog_info) {
								array_push($all_links, '<a href="index.php?option=com_vikbooking&task=editmaninvoice&cid[]='.$analog_info['invid'].'&goto='.$returi.'" onclick="alert(\'Use date filters to not list manual invoices with the same number\'); return true;"><i class="'.VikBookingIcons::i('external-link').'"></i> '.JText::_('VBOMANUALINVOICE').' (' . $analog_info['for_date'] . ')</a>');
							}
				...
				array(
					...
					'callback' => function ($val) use ($customer, $bid) {
						...
						if (!empty($val)) {
							...
							$cont = '<a class="btn btn-danger" href="index.php?option=com_vikbooking&task=newcustomer&bid='.$bid.'&goto='.$goto.'">' . JText::_('VBOCREATENEWCUST') . '</a>';
						}
						return $cont;
					},
					...
					'callback' => function ($val) use ($customer) {
						...
						if (empty($val)) {
							if (count($customer) && !empty($customer['id'])) {
								...
								$cont = '<a class="btn" href="index.php?option=com_vikbooking&task=editcustomer&cid[]='.$customer['id'].'&goto='.$goto.'">' . JText::_('VBCONFIGCLOSINGDATEADD') . '</a>';
							} else {
								$cont = '-----';
							}
							return $cont;
					...
					'callback' => function ($val) use ($customer, $bid) {
						if (!empty($val)) {
							...
							if (count($customer) && !empty($customer['id'])) {
								...
								// if empty customer ($val) print danger button to assign a customer to this booking ID
								$cont = '<a class="btn btn-danger" href="index.php?option=com_vikbooking&task=newcustomer&bid='.$bid.'&goto='.$goto.'">' . JText::_('VBCONFIGCLOSINGDATEADD') . '</a>';
							}
						}
						return $cont;
					},
