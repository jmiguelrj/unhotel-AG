...
<div class="vbo-admin-wizard-container">
	...
	if (!empty($this->rows)) {
		?>
		<a class="btn vbo-wizard-btn" href="index.php?option=com_vikbooking"><?php VikBookingIcons::e('home'); ?> <?php echo JText::_('VBMENUDASHBOARD'); ?></a>
		<?php
	}
	?>
		<a class="btn vbo-wizard-btn" href="javascript: void(0);" onclick="showVboWizard();"><?php VikBookingIcons::e('magic'); ?> <?php echo JText::_('VBOTOGGLEWIZARD'); ?></a>
	</div>
</div>
<?php
}
			...
			<div class="vbo-params-wrap">
				<legend class="adminlegend">
					<div class="vbo-quickres-head">
						<span><?php echo $name . " - " . JText::_('VBINSERTFEE'); ?></span>
				...
				<div class="vbo-params-container vbo-tariffs-params-container">
					<div class="vbo-param-container">
						...
						<div class="vbo-param-setting">
							<h4><?php echo JText::_('VBDAILYFARES'); ?></h4>
						<?php
						if (empty($prices)) {
							?>
							<p class="err">
								<span><?php echo JText::_('VBMSGONE'); ?></span>
								<a href="index.php?option=com_vikbooking&task=newprice"><?php echo JText::_('VBHERE'); ?></a>
							</p>
							<?php
						}
						?>
							<form name="newd" method="post" action="index.php?option=com_vikbooking" onsubmit="javascript: if (!document.newd.ddaysfrom.value.match(/\S/)){alert('<?php echo addslashes(JText::_('VBMSGTWO')); ?>'); return false;} else {return true;}">
								<div class="vbo-insertrates-cont">
									<div class="vbo-insertrates-top">
										<div class="vbo-ratestable-lbl"><?php echo JText::_('VBDAYS'); ?></div>
										<div class="vbo-ratestable-nights">
											<div class="vbo-ratestable-night-from">
												<span><?php echo JText::_('VBDAYSFROM'); ?></span>
												<input type="number" name="ddaysfrom" id="ddaysfrom" value="<?php echo !is_array($prices) ? '1' : ''; ?>" min="1" />
											</div>
											<div class="vbo-ratestable-night-to">
												<span><?php echo JText::_('VBDAYSTO'); ?></span>
									...
									<div class="vbo-insertrates-bottom">
										<div class="vbo-ratestable-lbl"><?php echo JText::_('VBDAILYPRICES'); ?></div>
								...
								<div class="vbo-insertrates-save">
									<input type="submit" class="btn vbo-config-btn" name="newdispcost" value="<?php echo JText::_('VBINSERT'); ?>"/>
				...
				if (empty($rows)) {
					...
					foreach ($allpr as $kap => $ap) {
						...
						if ($rplan_haslos) {
							...
							$los_descr .= "<span class=\"badge badge-info vbo-tariffs-los-badge\">LOS &gt;= $rplan_haslos " . JText::_('VBDAYS') . "</span>\n";
							$los_descr .= "</div>\n";
						}
					...
					<form action="index.php?option=com_vikbooking" method="post" name="adminForm" id="adminForm" class="vbo-list-form">
						<div class="vbo-tariffs-updaterates-cont">
							<input type="submit" name="modtar" value="<?php echo JText::_( 'VBPVIEWTARTWO' ); ?>" onclick="vbRateSetTask(event);" class="btn vbo-config-btn" />
						</div>
