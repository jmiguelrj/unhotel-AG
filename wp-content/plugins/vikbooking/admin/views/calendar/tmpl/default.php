...
JHtml::_('script', VBO_ADMIN_URI.'resources/jquery.highlighttextarea.min.js');
...
if ($this->new_res_id) {
	?>
<p class="successmade"><?php echo JText::_('VBBOOKMADE'); ?> &nbsp;&nbsp;&nbsp; <a href="index.php?option=com_vikbooking&task=editorder&cid[]=<?php echo $this->new_res_id; ?>" target="_blank" class="btn"><i class="vboicn-eye"></i> <?php echo JText::_('VBOVIEWBOOKINGDET'); ?></a></p>
	<?php
...
if (count($allc) > 1) {
	...
	$closeotherrooms .= "<option value=\"-1\">- ".JText::_('VBOCALCLOSEALLROOMSDT')."</option>\n";
	foreach ($allc as $cc) {
		if ($cc['id'] == $room['id']) {
			continue;
		}
...
<div class="vbo-admin-container">
			...
			<div class="vbo-params-wrap">
				<legend class="adminlegend">
					<div class="vbo-quickres-head">
						<span><?php echo $room['name'] . " - " . JText::_('VBQUICKBOOK') . (!empty($split_stay) ? ' (' . JText::_('VBO_SPLIT_STAY') . ')' : ''); ?></span>
				...
				<form name="newb" method="post" action="index.php?option=com_vikbooking" id="vbo-calendar-newb-form">
					<div class="vbo-params-container">
						<div class="vbo-param-container">
							<div class="vbo-param-label"><?php echo JText::_('VBDATEPICKUP'); ?></div>
							<div class="vbo-param-setting">
								...
								<span class="vbo-calendar-time-inline"><?php echo JText::_('VBAT')." ".($hcheckin < 10 ? '0'.$hcheckin : $hcheckin).":".($mcheckin < 10 ? '0'.$mcheckin : $mcheckin); ?></span>
								<input type="hidden" name="checkinh" value="<?php echo $hcheckin; ?>"/>
								<input type="hidden" name="checkinm" value="<?php echo $mcheckin; ?>"/>
							</div>
						</div>
						<div class="vbo-param-container">
							<div class="vbo-param-label"><?php echo JText::_('VBDATERELEASE'); ?></div>
							<div class="vbo-param-setting">
								<div class="input-append">
								...
								<span class="vbo-calendar-time-inline"><?php echo JText::_('VBAT')." ".($hcheckout < 10 ? '0'.$hcheckout : $hcheckout).":".($mcheckout < 10 ? '0'.$mcheckout : $mcheckout); ?></span>
								<span style="display: none; margin-left: 25px; vertical-align: top;" id="vbjstotnights">
									<span style="font-weight: bold;"><?php echo JText::_('VBDAYS'); ?></span>
						...
						<div class="vbo-param-container" style="<?php echo !empty($split_stay) ? 'display: none;' : ''; ?>">
							<div class="vbo-param-label">
								<span class="vbcloseroomsp">
									<label for="setclosed-on"><?php echo JText::_('VBCLOSEROOM'); ?> <i class="<?php echo VikBookingIcons::i('ban'); ?>" style="float: none;"></i></label>
								</span>
							</div>
							<div class="vbo-param-setting">
								<?php echo $vbo_app->printYesNoButtons('setclosed', JText::_('VBYES'), JText::_('VBNO'), 0, 1, 0, 'vbCloseRoom();'); ?>
					...
					if ($room['units'] > 1 && empty($split_stay)) {
						...
						<div class="vbo-param-container" id="vbo-row-numrooms">
							<div class="vbo-param-label"><?php echo JText::_('VBPVIEWROOMSEVEN'); ?></div>
							<div class="vbo-param-setting">
					...
					?>
						<div class="vbo-param-container" id="vbo-row-people">
							<div class="vbo-param-label"><?php echo JText::_('VBQUICKRESGUESTS'); ?></div>
							<div class="vbo-param-setting">
								<span class="vbo-quickres-aduchi-wrap">
									<span class="vbo-quickres-aduchi-inlbl"><?php echo JText::_('VBQUICKADULTS'); ?></span>
									<?php echo $seladults; ?>
								</span>
								<span class="vbo-quickres-aduchi-wrap">
									<span class="vbo-quickres-aduchi-inlbl"><?php echo JText::_('VBQUICKCHILDREN'); ?></span>
				...
				if (!empty($split_stay)) {
					foreach ($split_stay as $sps_k => $sps_v) {
						?>
						<div class="vbo-param-container">
							<div class="vbo-param-label"><?php echo $sps_k < 1 ? '<i class="' . VikBookingIcons::i('random') . '"></i> ' . JText::_('VBO_SPLIT_STAY') : ' '; ?></div>
							<div class="vbo-param-setting">
								<div class="vbo-cal-splitstay-details">
									<div class="vbo-cal-splitstay-room">
										...
										<span class="vbo-cal-splitstay-room-nights"><?php VikBookingIcons::e('moon'); ?> <?php echo $sps_v['nights']; ?> <?php echo $sps_v['nights'] > 1 ? JText::_('VBDAYS') : JText::_('VBDAY'); ?></span>
									</div>
				...
				?>
						<div class="vbo-param-container"<?php echo ($poverview > 0 ? ' style="display: none;"' : ''); ?> id="vbo-row-bstat">
							<div class="vbo-param-label"><?php echo JText::_('VBCALBOOKINGSTATUS'); ?></div>
							<div class="vbo-param-setting">
								<select name="newstatus">
									<option value="confirmed"><?php echo JText::_('VBCONFIRMED'); ?></option>
									<option value="standby"><?php echo JText::_('VBSTANDBY'); ?></option>
								</select>
							</div>
						</div>
						<div class="vbo-param-container" id="vbo-row-bpay">
							<div class="vbo-param-label"><?php echo JText::_('VBCALBOOKINGPAYMENT'); ?></div>
							<div class="vbo-param-setting">
						...
						<div class="vbo-param-container" id="vbo-row-fillcustfields">
							...
							<div class="vbo-param-setting">
								<span class="vbo-assign-customer" id="vbfillcustfields">
									...
									<span><?php echo JText::_('VBFILLCUSTFIELDS'); ?></span>
								</span>
							</div>
						</div>
						<div class="vbo-param-container" id="vbo-row-cmail">
							<div class="vbo-param-label"><?php echo JText::_('VBCUSTEMAIL'); ?></div>
							<div class="vbo-param-setting">
								<input type="text" name="custmail" id="custmailfield" value="" size="25"/>
							</div>
						</div>
						<div class="vbo-param-container">
							<div class="vbo-param-label" id="vbcustdatalbl"><?php echo JText::_('VBCUSTINFO'); ?></div>
							<div class="vbo-param-setting">
						...
						<div class="vbo-param-container" id="vbo-website-rates-row" style="display: none;">
							<div class="vbo-param-label"><?php echo JText::_('VBOWEBSITERATES'); ?></div>
							<div class="vbo-param-setting" id="vbo-website-rates-cont"></div>
						</div>
						<div class="vbo-param-container" id="vbo-row-custcost">
							<div class="vbo-param-label"><?php echo JText::_('VBOROOMCUSTRATEPLANADD'); ?></div>
							<div class="vbo-param-setting">
						...
						<div id="vbo-force-bookingdates" class="vbo-param-container" style="display: none;">
							<div class="vbo-param-label"><?php echo $vbo_app->createPopover(array('title' => JText::_('VBO_FORCE_BOOKDATES'), 'content' => JText::_('VBO_FORCE_BOOKDATES_HELP'))); ?> <?php echo JText::_('VBO_FORCE_BOOKDATES'); ?></div>
							<div class="vbo-param-setting">
						...
						<div class="vbo-param-container">
							<div class="vbo-param-label">&nbsp;</div>
							<div class="vbo-param-setting">
								<button type="submit" id="quickbsubmit" class="btn btn-success btn-large"><i class="icon-save"></i> <span><?php echo JText::_('VBMAKERESERV'); ?></span></button>
							</div>
						</div>
					</div>
					<?php
					...
					if (!empty($split_stay)) {
						...
						foreach ($split_stay as $spsk => $sps_room) {
							foreach ($sps_room as $sps_room_k => $sps_room_v) {
								?>
						<input type="hidden" name="split_stay_data[<?php echo $spsk; ?>][<?php echo $sps_room_k; ?>]" value="<?php echo JHtml::_('esc_attr', $sps_room_v); ?>" />
								<?php
							}
						}
					}
	...
	<div class="vbo-config-maintab-right">
		...
		if (empty($busy)) {
			?>
			<p class="warn"><?php echo JText::_('VBNOFUTURERES'); ?></p>
			<?php
		} else {
			$check = true;
			...
			?>
			<div>
				<a class="vbmodelink<?php echo $vmode == 3 ? ' vbmodelink-active' : ''; ?>" href="index.php?<?php echo http_build_query(array_merge($base_uri_data, ['vmode' => 3])); ?>"><?php VikBookingIcons::e('calendar'); ?> <span><?php echo JText::_('VBTHREEMONTHS'); ?></span></a>
				<a class="vbmodelink<?php echo $vmode == 6 ? ' vbmodelink-active' : ''; ?>" href="index.php?<?php echo http_build_query(array_merge($base_uri_data, ['vmode' => 6])); ?>"><?php VikBookingIcons::e('calendar'); ?> <span><?php echo JText::_('VBSIXMONTHS'); ?></span></a>
				<a class="vbmodelink<?php echo $vmode == 12 ? ' vbmodelink-active' : ''; ?>" href="index.php?<?php echo http_build_query(array_merge($base_uri_data, ['vmode' => 12])); ?>"><?php VikBookingIcons::e('calendar'); ?> <span><?php echo JText::_('VBTWELVEMONTHS'); ?></span></a>
				<a class="vbmodelink<?php echo $vmode == 24 ? ' vbmodelink-active' : ''; ?>" href="index.php?<?php echo http_build_query(array_merge($base_uri_data, ['vmode' => 24])); ?>"><?php VikBookingIcons::e('calendar'); ?> <span><?php echo JText::_('VBTWOYEARS'); ?></span></a>
			</div>
			<?php
		}
		?>
						...
						if ($check) {
							foreach ($busy as $b) {
								...
								if ($newarr[0] >= $ritts && $newarr[0] < $conts) {
									...
									if ((int)$b['closure']) {
										$dclass .= " busy-closure";
										$dalt = JText::_('VBDBTEXTROOMCLOSED');
		...
		<div class="vbo-calendar-cfields-topcont">
			<div class="vbo-calendar-cfields-search">
				...
				echo $vbo_app->renderSearchElementsDropDown([
					...
					'elements' => 'customers',
					'placeholder' => JText::_('VBOSEARCHEXISTCUST'),
					'allow_clear' => true,
					'attributes'  => [
