...
if (count($row) && $row['toadult'] > 1 && $row['fromadult'] < $row['toadult']) {
	...
	for ($adi = $startadind; $adi <= $row['toadult']; $adi++) {
		$stradultsdiff .= "<p>";
		$stradultsdiff .= '<span class="vbo-adults-usage">' . JText::sprintf('VBADULTSDIFFNUM', $adi) . "</span><select name=\"adultsdiffchdisc[]\"><option value=\"1\"".(array_key_exists($adi, $parseadultsdiff) && $parseadultsdiff[$adi]['chdisc'] == 1 ? " selected=\"selected\"" : "").">".JText::_('VBADULTSDIFFCHDISCONE')."</option><option value=\"2\"".(array_key_exists($adi, $parseadultsdiff) && $parseadultsdiff[$adi]['chdisc'] == 2 ? " selected=\"selected\"" : "").">".JText::_('VBADULTSDIFFCHDISCTWO')."</option></select>\n";
		...
		$stradultsdiff .= "<select name=\"adultsdiffpernight[]\"><option value=\"1\"".(array_key_exists($adi, $parseadultsdiff) && $parseadultsdiff[$adi]['pernight'] == 1 ? " selected=\"selected\"" : "").">".JText::_('VBADULTSDIFFONPERNIGHT')."</option><option value=\"0\"".(array_key_exists($adi, $parseadultsdiff) && $parseadultsdiff[$adi]['pernight'] == 0 ? " selected=\"selected\"" : "").">".JText::_('VBADULTSDIFFONTOTAL')."</option></select>\n";
		$stradultsdiff .= "</p>\n";
	}
}
//
...
function addMoreImages() {
	...
	newdiv.innerHTML = '<input type=\'file\' name=\'cimgmore[]\' size=\'35\'/><div class=\'vbo-imgup-caption\'><span><?php echo addslashes(JText::_('VBIMGCAPTION')); ?></span> <input type=\'text\' name=\'cimgcaption[]\' size=\'30\' value=\'\'/></div>';
		...
		if (to_units > cur_units) {
			...
			for (var i = 1; i <= diff_units; i++) {
				var unit_html = "<div class=\"vbo-runit-features-cont\" id=\"runit-features-"+(i + cur_units)+"\">"+
								...
								"		</div>"+
								"		<span class=\"vbo-feature-add btn vbo-config-btn\"><i class=\"icon-new\"></i><?php echo addslashes(JText::_('VBODISTFEATUREADD')); ?></span>"+
								"	</div>"+
								"</div>";
...
?>
	<div class="vbo-admin-container">
				...
				<div class="vbo-params-wrap">
					...
					<div class="vbo-params-container">
						<div class="vbo-param-container">
							<div class="vbo-param-label"><?php echo JText::_('VBNEWROOMFIVE'); ?></div>
							<div class="vbo-param-setting"><input type="text" name="cname" value="<?php echo $row ? htmlspecialchars($row['name']) : ''; ?>" size="40"/></div>
						</div>
						<div class="vbo-param-container">
							<div class="vbo-param-label"><?php echo JText::_('VBNEWROOMEIGHT'); ?></div>
							<div class="vbo-param-setting"><?php echo $vbo_app->printYesNoButtons('cavail', JText::_('VBYES'), JText::_('VBNO'), ((count($row) && intval($row['avail']) == 1) || !$row ? 'yes' : 0), 'yes', 0); ?></div>
						</div>
						<div class="vbo-param-container">
							<div class="vbo-param-label"><?php echo JText::_('VBNEWROOMNINE'); ?></div>
						...
						<div class="vbo-param-container vbo-distfeature-row" style="display: <?php echo count($row) && $row['units'] > 1 ? 'flex' : 'none'; ?>;">
							<div class="vbo-param-label"><?php echo JText::_('VBOROOMUNITSDISTFEAT'); ?></div>
							<div class="vbo-param-setting">
								<div class="vbo-distfeatures-toggle-cont">
									<span id="vbo-distfeatures-toggle" class="btn vbo-config-btn"><?php VikBookingIcons::e('binoculars'); ?><?php echo JText::_('VBOROOMUNITSDISTFEATTOGGLE'); ?></span>
								</div>
								...
								for ($i=1; $i <= $unitslim; $i++) {
									...
									if (array_key_exists($i, $room_features)) {
										foreach ($room_features[$i] as $fkey => $fval) {
											?>
											<div class="vbo-runit-feature">
												<input type="text" name="feature-name<?php echo $i; ?>[]" value="<?php echo htmlspecialchars(JText::_($fkey)); ?>" size="20"/>
									...
									?>
											<span class="vbo-feature-add btn vbo-config-btn"><?php VikBookingIcons::e('plus-circle'); ?> <?php echo JText::_('VBODISTFEATUREADD'); ?></span>
										</div>
									</div>
									<?php
								}
						...
						<div class="vbo-param-container">
							<div class="vbo-param-label"><?php echo JText::_('VBNEWROOMADULTS'); ?></div>
							<div class="vbo-param-setting">
								<div class="vbo-param-setting-group">
									<div class="vbplusminuscont">
										<span><?php echo JText::_('VBNEWROOMMIN'); ?></span>
								...
								<div class="vbo-param-setting-group">
									<div class="vbplusminuscont">
										<span><?php echo JText::_('VBNEWROOMMAX'); ?></span>
					...
					if ($writeadultsdiff == true) {
						?>
						<div class="vbo-param-container">
							<div class="vbo-param-label"><?php echo $vbo_app->createPopover(array('title' => JText::_('VBNEWROOMADULTSDIFF'), 'content' => JText::_('VBNEWROOMADULTSDIFFHELP'))); ?> <?php echo JText::_('VBNEWROOMADULTSDIFF'); ?></div>
							<div class="vbo-param-setting">
								<div id="vbadultsdiffsavemess" style="display: none;">
									<span class="vbo-param-setting-comment"><?php echo JText::_('VBNEWROOMNOTCHANGENUMMESS'); ?></span>
								</div>
						...
						<div class="vbo-param-container">
							<div class="vbo-param-label"><?php echo $vbo_app->createPopover(array('title' => JText::_('VBNEWROOMADULTSDIFF'), 'content' => JText::_('VBNEWROOMADULTSDIFFHELP'))); ?> <?php echo JText::_('VBNEWROOMADULTSDIFF'); ?></div>
							<div class="vbo-param-setting">
								<span class="vbo-param-setting-comment"><?php echo JText::_('VBNEWROOMADULTSDIFFBEFSAVE'); ?></span>
							</div>
						</div>
						<?php
					}
					?>
						<div class="vbo-param-container">
							<div class="vbo-param-label"><?php echo JText::_('VBNEWROOMCHILDREN'); ?></div>
							<div class="vbo-param-setting">
								<div class="vbo-param-setting-group">
									<div class="vbplusminuscont">
										<span><?php echo JText::_('VBNEWROOMMIN'); ?></span>
						...
						<div class="vbo-param-container">
							<div class="vbo-param-label"><?php echo JText::_('VBMAXTOTPEOPLE'); ?></div>
							<div class="vbo-param-setting">
								<input type="number" min="1" name="totpeople" id="totpeople" value="<?php echo $row ? $row['totpeople'] : '1'; ?>" size="3" style="width: 40px;"/>
								<span class="vbo-param-setting-comment-inline"><?php echo JText::_('VBMAXTOTPEOPLEDESC'); ?></span>
							</div>
						</div>
						<div class="vbo-param-container">
							<div class="vbo-param-label"><?php echo JText::_('VBMINTOTPEOPLE'); ?></div>
							<div class="vbo-param-setting">
								<input type="number" min="1" name="mintotpeople" id="mintotpeople" value="<?php echo $row ? $row['mintotpeople'] : '1'; ?>" size="3" style="width: 40px;"/>
								<span class="vbo-param-setting-comment-inline"><?php echo JText::_('VBMINTOTPEOPLEDESC'); ?></span>
							</div>
						</div>
					</div>
				</div>
			...
			<fieldset class="adminform">
				<div class="vbo-params-wrap">
					...
					if (!empty($wcats)) {
						?>
						<div class="vbo-param-container">
							<div class="vbo-param-label"><?php echo JText::_('VBNEWROOMONE'); ?></div>
					...
					if (!empty($wcarats)) {
						?>
						<div class="vbo-param-container">
							<div class="vbo-param-label"><?php echo JText::_('VBNEWROOMTHREE'); ?></div>
					...
					if (!empty($woptionals)) {
						?>
						<div class="vbo-param-container">
							<div class="vbo-param-label"><?php echo JText::_('VBNEWROOMFOUR'); ?></div>
			...
			<fieldset class="adminform">
				<div class="vbo-params-wrap">
					...
					<div class="vbo-params-container">
						...
						?>
						<div class="vbo-param-container">
							<div class="vbo-param-label"><label for="layout_style"><?php echo JText::_('COM_VIKBOOKING_LAYOUT_STYLE'); ?></label></div>
							<div class="vbo-param-setting">
								<select name="layout_style" id="layout_style">
									<option value="classic"><?php echo JText::_('VBOCONFIGSEARCHRESTPLCL'); ?></option>
									<option value="listing"<?php echo $layout_style == 'listing' ? ' selected="selected"' : ''; ?>><?php echo JText::_('VBO_LISTING'); ?></option>
								</select>
							</div>
						</div>
						<?php
						...
						<div class="vbo-param-container param-multiunits" style="display: <?php echo (count($row) && $row['units'] > 0 ? 'flex' : 'none'); ?>;">
							<div class="vbo-param-label"><label for="multi_units"><?php echo JText::_('VBPARAMROOMMULTIUNITS'); ?></label></div>
							<div class="vbo-param-setting">
								<select name="multi_units" id="multi_units">
									<option value="0"><?php echo JText::_('VBPARAMROOMMULTIUNITSDISABLE'); ?></option>
									<option value="1"<?php echo intval($multi_units) == 1 ? ' selected="selected"' : ''; ?>><?php echo JText::_('VBPARAMROOMMULTIUNITSENABLE'); ?></option>
								</select>
								<span class="vbo-param-setting-comment"><?php echo JText::_('VBPARAMROOMMULTIUNITSHELP'); ?></span>
							</div>
						</div>
						<div class="vbo-param-container">
							<div class="vbo-param-label"><label for="lastavail"><?php echo JText::_('VBPARAMLASTAVAIL'); ?></label></div>
							<div class="vbo-param-setting">
								...
								<span class="vbo-param-setting-comment"><?php echo JText::_('VBPARAMLASTAVAILHELP'); ?></span>
							</div>
						</div>
						<div class="vbo-param-container">
							<div class="vbo-param-label"><label for="suggocc"><?php echo JText::_('VBOPARAMSUGGOCC'); ?></label></div>
							<div class="vbo-param-setting">
								<input type="number" min="0" name="suggocc" id="suggocc" value="<?php echo $row ? (int)VikBooking::getRoomParam('suggocc', $row['params']) : '1'; ?>"/>
							</div>
						</div>
						<div class="vbo-param-container">
							<div class="vbo-param-label"><label for="custprice"><?php echo JText::_('VBPARAMCUSTPRICE'); ?></label></div>
							<div class="vbo-param-setting">
								<input type="text" name="custprice" id="custprice" value="<?php echo $row ? JHtml::_('esc_attr', VikBooking::getRoomParam('custprice', $row['params'])) : ''; ?>" size="5"/>
								<span class="vbo-param-setting-comment"><?php echo JText::_('VBPARAMCUSTPRICEHELP'); ?></span>
							</div>
						</div>
						<div class="vbo-param-container">
							<div class="vbo-param-label"><label for="custpricetxt"><?php echo JText::_('VBPARAMCUSTPRICETEXT'); ?></label></div>
							<div class="vbo-param-setting">
								<input type="text" name="custpricetxt" id="custpricetxt" value="<?php echo $row ? JHtml::_('esc_attr', VikBooking::getRoomParam('custpricetxt', $row['params'])) : ''; ?>" size="9"/>
								<span class="vbo-param-setting-comment"><?php echo JText::_('VBPARAMCUSTPRICETEXTHELP'); ?></span>
							</div>
						</div>
						<div class="vbo-param-container">
							<div class="vbo-param-label"><label for="custpricesubtxt"><?php echo JText::_('VBPARAMCUSTPRICESUBTEXT'); ?></label></div>
							<div class="vbo-param-setting">
								...
								<span class="vbo-param-setting-comment"><?php echo JText::_('VBPARAMCUSTPRICESUBTEXTHELP'); ?></span>
							</div>
						</div>
						<div class="vbo-param-container">
							<div class="vbo-param-label"><label for="reqinfo"><?php echo JText::_('VBORPARAMREQINFO'); ?></label></div>
							<div class="vbo-param-setting"><?php echo $vbo_app->printYesNoButtons('reqinfo', JText::_('VBYES'), JText::_('VBNO'), (count($row) && intval(VikBooking::getRoomParam('reqinfo', $row['params'])) == 1 ? 1 : 0), 1, 0); ?></div>
						</div>
						<?php
						$paramshowpeople = $row ? VikBooking::getRoomParam('maxminpeople', $row['params']) : '';
						?>
						<div class="vbo-param-container">
							<div class="vbo-param-label"><label for="maxminpeople"><?php echo JText::_('VBPARAMSHOWPEOPLE'); ?></label></div>
							<div class="vbo-param-setting">
								<select name="maxminpeople" id="maxminpeople">
									<option value="0"><?php echo JText::_('VBPARAMSHOWPEOPLENO'); ?></option>
									<option value="1"<?php echo ($paramshowpeople == "1" ? ' selected="selected"' : ''); ?>><?php echo JText::_('VBPARAMSHOWPEOPLEADU'); ?></option>
									<option value="2"<?php echo ($paramshowpeople == "2" ? ' selected="selected"' : ''); ?>><?php echo JText::_('VBPARAMSHOWPEOPLECHI'); ?></option>
									<option value="3"<?php echo ($paramshowpeople == "3" ? ' selected="selected"' : ''); ?>><?php echo JText::_('VBPARAMSHOWPEOPLEADUTOT'); ?></option>
									<option value="4"<?php echo ($paramshowpeople == "4" ? ' selected="selected"' : ''); ?>><?php echo JText::_('VBPARAMSHOWPEOPLECHITOT'); ?></option>
									<option value="5"<?php echo ($paramshowpeople == "5" ? ' selected="selected"' : ''); ?>><?php echo JText::_('VBPARAMSHOWPEOPLEALLTOT'); ?></option>
								</select>
							</div>
						</div>
						<div class="vbo-param-container">
							<div class="vbo-param-label"><label for="pricecal"><?php echo JText::_('VBPARAMPRICECALENDAR'); ?></label></div>
							<div class="vbo-param-setting">
								<select name="pricecal" id="pricecal" onchange="togglePriceCalendarParam();">
									<option value="0"><?php echo JText::_('VBPARAMPRICECALENDARDISABLED'); ?></option>
									<option value="1"<?php echo (count($row) && intval(VikBooking::getRoomParam('pricecal', $row['params'])) == 1 ? ' selected="selected"' : ''); ?>><?php echo JText::_('VBPARAMPRICECALENDARENABLED'); ?></option>
								</select>
								<span class="vbo-param-setting-comment"><?php echo JText::_('VBPARAMPRICECALENDARHELP'); ?></span>
							</div>
						</div>
						...
						if ($row && $room_rate_plans) {
							...
							?>
							<div class="vbo-param-label"><label for="defrplan"><?php echo JText::_('VBOROVWSELRPLAN'); ?></label></div>
							<div class="vbo-param-setting">
								...
								<span class="vbo-param-setting-comment"><?php echo JText::_('VBPARAMDEFCALCOSTHELP'); ?></span>
							</div>
							<?php
						} else {
							// default to the old calendar default (custom) cost
							?>
							<div class="vbo-param-label"><label for="defcalcost"><?php echo JText::_('VBPARAMDEFCALCOST'); ?></label></div>
							<div class="vbo-param-setting">
						...
						<div class="vbo-param-container param-seasoncal">
							<div class="vbo-param-label"><label for="seasoncal"><?php echo JText::_('VBPARAMSEASONCALENDAR'); ?></label></div>
							<div class="vbo-param-setting">
								<select name="seasoncal" id="seasoncal" onchange="toggleSeasonalCalendarParam();">
									<option value="0"><?php echo JText::_('VBPARAMSEASONCALENDARDISABLED'); ?></option>
									<optgroup label="<?php echo JText::_('VBPARAMSEASONCALENDARENABLED'); ?>">
										<option value="1"<?php echo intval($season_cal) == 1 ? ' selected="selected"' : ''; ?>><?php echo JText::_('VBPARAMSEASONCALENDARENABLEDALL'); ?></option>
										<option value="2"<?php echo intval($season_cal) == 2 ? ' selected="selected"' : ''; ?>><?php echo JText::_('VBPARAMSEASONCALENDARENABLEDCHARGEDISC'); ?></option>
										<option value="3"<?php echo intval($season_cal) == 3 ? ' selected="selected"' : ''; ?>><?php echo JText::_('VBPARAMSEASONCALENDARENABLEDCHARGE'); ?></option>
									</optgroup>
								</select>
							</div>
						</div>
						<div class="vbo-param-container vbo-param-nested param-seasoncal" style="display: <?php echo (intval($season_cal) > 0 ? 'flex' : 'none'); ?>;">
							<div class="vbo-param-label"><label for="seasoncal_nights"><?php echo JText::_('VBPARAMSEASONCALNIGHTS'); ?></label></div>
							<div class="vbo-param-setting">
								<input type="text" name="seasoncal_nights" id="seasoncal_nights" size="10" value="<?php echo $row ? JHtml::_('esc_attr', VikBooking::getRoomParam('seasoncal_nights', $row['params'])) : ''; ?>" placeholder="1, 3, 7, 14"/>
								<span class="vbo-param-setting-comment"><?php echo JText::_('VBPARAMSEASONCALNIGHTSHELP'); ?></span>
							</div>
						</div>
						<div class="vbo-param-container vbo-param-nested param-seasoncal" style="display: <?php echo (intval($season_cal) > 0 ? 'flex' : 'none'); ?>;">
							<div class="vbo-param-label"><label for="seasoncal_prices"><?php echo JText::_('VBPARAMSEASONCALENDARPRICES'); ?></label></div>
							<div class="vbo-param-setting">
								<select name="seasoncal_prices" id="seasoncal_prices">
									<option value="0"><?php echo JText::_('VBPARAMSEASONCALENDARPRICESANY'); ?></option>
									<option value="1"<?php echo intval($season_cal_prices) == 1 ? ' selected="selected"' : ''; ?>><?php echo JText::_('VBPARAMSEASONCALENDARPRICESLOW'); ?></option>
								</select>
							</div>
						</div>
						<div class="vbo-param-container vbo-param-nested param-seasoncal" style="display: <?php echo (intval($season_cal) > 0 ? 'flex' : 'none'); ?>;">
							<div class="vbo-param-label"><label for="seasoncal_restr"><?php echo JText::_('VBPARAMSEASONCALENDARLOS'); ?></label></div>
							<div class="vbo-param-setting">
								<select name="seasoncal_restr" id="seasoncal_restr">
									<option value="0"><?php echo JText::_('VBPARAMSEASONCALENDARLOSHIDE'); ?></option>
									<option value="1"<?php echo intval($season_cal_restr) == 1 ? ' selected="selected"' : ''; ?>><?php echo JText::_('VBPARAMSEASONCALENDARLOSSHOW'); ?></option>
								</select>
							</div>
						</div>
						...
						if ($row) {
							...
							?>
						<div class="vbo-param-container">
							<div class="vbo-param-label"><?php echo JText::_('VBO_MIN_ADV_BOOK_NOTICE'); ?></div>
							<div class="vbo-param-setting">
								<div>
									<?php
									echo $vbo_app->printYesNoButtons('min_adv_notice_room', JText::_('VBYES'), JText::_('VBNO'), (empty($mindate_val) ? 0 : 1), 1, 0, 'vboToggleMinAdvNotice(this.checked);');
									?>
								</div>
								<div class="vbo-room-level-min_adv-notice" style="<?php echo empty($mindate_val) ? 'display: none;' : ''; ?>">
									...
									<select name="mindateinterval">
										<option value="h"><?php echo ucfirst(JText::_('VBCONFIGONETENEIGHT')); ?></option>
									</select>
								</div>
							</div>
						</div>
						...
						if ($row) {
							...
							?>
						<div class="vbo-param-container">
							<div class="vbo-param-label"><?php echo JText::_('VBO_MAX_ADV_BOOK_NOTICE'); ?></div>
							<div class="vbo-param-setting">
								<div>
									<?php
									echo $vbo_app->printYesNoButtons('max_adv_notice_room', JText::_('VBYES'), JText::_('VBNO'), (empty($room_level_max_adv_notice) || empty($maxdate_val) ? 0 : 1), 1, 0, 'vboToggleMaxAdvNotice(this.checked);');
									?>
								</div>
								<div class="vbo-room-level-max_adv-notice" style="<?php echo empty($room_level_max_adv_notice) || empty($maxdate_val) ? 'display: none;' : ''; ?>">
									...
									<select name="maxdateinterval">
										<option value="d"<?php echo $maxdate_interval == 'd' ? ' selected="selected"' : ''; ?>><?php echo JText::_('VBCONFIGSEARCHPMAXDATEDAYS'); ?></option>
										<option value="w"<?php echo $maxdate_interval == 'w' ? ' selected="selected"' : ''; ?>><?php echo JText::_('VBCONFIGSEARCHPMAXDATEWEEKS'); ?></option>
										<option value="m"<?php echo $maxdate_interval == 'm' ? ' selected="selected"' : ''; ?>><?php echo JText::_('VBCONFIGSEARCHPMAXDATEMONTHS'); ?></option>
										<option value="y"<?php echo $maxdate_interval == 'y' ? ' selected="selected"' : ''; ?>><?php echo JText::_('VBCONFIGSEARCHPMAXDATEYEARS'); ?></option>
									</select>
								</div>
								<span class="vbo-param-setting-comment"><?php echo JText::_('VBO_MAX_ADV_BOOK_NOTICE_HELP'); ?></span>
							</div>
						</div>
							<?php
						}
						...
						?>
						<div class="vbo-param-container">
							<div class="vbo-param-label"><label for="listing_checkin"><?php echo JText::_('VBCONFIGONESEVEN'); ?></label></div>
							<div class="vbo-param-setting">
								<select name="listing_checkin" id="listing_checkin">
									<option value=""><?php echo JText::_('VBO_USE_DEFAULT'); ?></option>
									<optgroup label="<?php echo JText::_('VBO_CUSTOM'); ?>">
								<?php
								for ($h = 0; $h < 24; $h++) {
									for ($m = 0; $m < 60; $m += 15) {
										$display_h = ($h < 10 ? '0' : '') . $h;
						...
						<div class="vbo-param-container">
							<div class="vbo-param-label"><label for="listing_checkout"><?php echo JText::_('VBCONFIGONETHREE'); ?></label></div>
							<div class="vbo-param-setting">
								...
								<span class="vbo-param-setting-comment"><?php echo JText::_('VBO_TURNOVER_TIME_WARNING'); ?></span>
							</div>
						</div>
					</div>
				</div>
		...
		<div class="vbo-config-maintab-right">
			<fieldset class="adminform">
				<div class="vbo-params-wrap">
					...
					<div class="vbo-params-container">
						<div class="vbo-param-container">
							<div class="vbo-param-label"><?php echo JText::_('VBNEWROOMSIX'); ?></div>
							<div class="vbo-param-setting">
						...
						<div class="vbo-param-container">
								...
								<div class="vbo-bulkupload-cont">
									<div class="vbo-bulkupload-inner">
										<a href="javascript: void(0);" onclick="showBulkUpload();" class="btn vbo-config-btn">
											<i class="icon-image" style="float: none;"></i><?php echo JText::_('VBOBULKUPLOAD'); ?>
										</a>
									</div>
								</div>
							</div>
							<div class="vbo-param-setting">
								...
								<div class="vbo-rmphotos-cont">
									<a class="btn btn-danger" href="index.php?option=com_vikbooking&amp;task=removemoreimgs&amp;roomid=<?php echo $row['id']; ?>&amp;imgind=-1" onclick="return confirm('<?php echo addslashes(JText::_('VBORMALLPHOTOS')); ?>?');">
										<i class="icon-cancel"></i><?php echo JText::_('VBORMALLPHOTOS'); ?>
									</a>
								</div>
								<div class="vbo-first-imgup">
									<input type="file" name="cimgmore[]" size="35"/> 
									<div class="vbo-imgup-caption">
										<span><?php echo JText::_('VBIMGCAPTION'); ?></span> 
								...
								<div class="vbo-param-setting-block">
									<span class="vbo-resize-lb-cont">
										<label style="display: inline;" for="autoresizemore"><?php echo JText::_('VBRESIZEIMAGES'); ?></label> 
										<input type="checkbox" id="autoresizemore" name="autoresizemore" value="1" onclick="showResizeSelMore();"/> 
									</span>
									<span id="resizeselmore" style="display: none;"><span><?php echo JText::_('VBNEWOPTTEN'); ?></span><input type="number" name="resizetomore" value="600" min="0" class="vbo-medium-input"/> px</span>
								</div>
							</div>
						</div>
					<?php
							...
							<div class="vbo-param-label">
								<div class="vbo-param-label-top">
									...
									<a class="btn vbo-config-btn" href="javascript: void(0);" onclick="addMoreImages();">
										<?php VikBookingIcons::e('plus-circle'); ?> <?php echo JText::_('VBADDIMAGES'); ?>
									</a>
								</div>
								<p class="vbo-small-p-info"><?php echo JText::_('VBOBULKUPLOADAFTERSAVE'); ?></p>
							</div>
							<div class="vbo-param-setting">
								<div class="vbo-first-imgup">
									<input type="file" name="cimgmore[]" size="35"/>
								...
								<div class="vbo-param-setting-block">
									<label style="display: inline;" for="autoresizemore"><?php echo JText::_('VBRESIZEIMAGES'); ?></label> 
									<input type="checkbox" id="autoresizemore" name="autoresizemore" value="1" onclick="showResizeSelMore();"/> 
									<span id="resizeselmore" style="display: none;"><span><?php echo JText::_('VBNEWOPTTEN'); ?></span><input type="text" name="resizetomore" value="600" size="3"/> px</span>
								</div>
							</div>
						</div>
						<?php
					...
					?>
						<div class="vbo-param-container">
							<div class="vbo-param-label with-buttons">
								<span class="vbo-param-label-main"><?php echo JText::_('VBNEWROOMSMALLDESC'); ?></span>
							<?php
							if ($has_vcm) {
								// Gen-AI small description
								?>
								<div class="vbo-room-genai-btn-wrap">
									<button type="button" class="btn btn-small vbo-content-genai vbo-tooltip vbo-tooltip-top" data-tooltiptext="<?php echo JHtml::_('esc_attr', JText::_('VBO_GEN_CONTENT')); ?>" data-type="short"><?php echo JText::_('VBO_AI_LABEL_DEF'); ?></button>
								</div>
								<?php
							}
							?>
						...
						<div class="vbo-param-container vbo-param-container-full">
							<div class="vbo-param-label with-buttons">
								<span class="vbo-param-label-main"><?php echo JText::_('VBNEWROOMSEVEN'); ?></span>
							<?php
							if ($has_vcm) {
								// Gen-AI small description
								?>
								<span class="vbo-room-genai-btn-wrap inline">
									<button type="button" class="btn btn-small vbo-content-genai vbo-tooltip vbo-tooltip-top" data-tooltiptext="<?php echo JHtml::_('esc_attr', JText::_('VBO_GEN_CONTENT')); ?>" data-type="long"><?php echo JText::_('VBO_AI_LABEL_DEF'); ?></button>
								</span>
								<?php
							}
							?>
		...
		?>
			<fieldset class="adminform">
				<div class="vbo-params-wrap">
					<legend class="adminlegend"><?php VikBookingIcons::e('calendar'); ?> <?php echo JText::_('VBOROOMCALXREFSETTINGS'); ?></legend>
					<div class="vbo-params-container">
						<div class="vbo-param-container">
							<div class="vbo-param-label"><?php echo $vbo_app->createPopover(array('title' => JText::_('VBOROOMSHARECALENDAR') . '...', 'content' => JText::_('VBOROOMSHARECALENDARHELP'))); ?> <?php echo JText::_('VBOROOMSHARECALENDAR'); ?></div>
							<div class="vbo-param-setting">
					...
					?>
						<div class="vbo-param-container">
							<div class="vbo-param-label"><?php echo JText::_('VBOROOMCALENDARSHAREDBY'); ?></div>
							<div class="vbo-param-setting">
		...
		if (count($this->rooms_map)) {
			...
			<fieldset class="adminform">
				<div class="vbo-params-wrap">
					<legend class="adminlegend"><?php VikBookingIcons::e('gem'); ?> <?php echo JText::_('VBO_ROOM_UPGRADE'); ?></legend>
					<div class="vbo-params-container">
						<div class="vbo-param-container">
							<div class="vbo-param-label"><?php echo $vbo_app->createPopover(array('title' => JText::_('VBO_ROOM_UPGRADE'), 'content' => JText::_('VBO_ROOM_UPGRADE_HELP'))); ?> <?php echo JText::_('VBPARAMPRICECALENDARENABLED'); ?></div>
							<div class="vbo-param-setting">
								<?php
								echo $vbo_app->printYesNoButtons('room_upgrade', JText::_('VBYES'), JText::_('VBNO'), $room_upgrade_enabled, 1, 0, 'vboToggleRoomUpgrade();');
								?>
							</div>
						</div>
					</div>
					<div class="vbo-params-container vbo-roomupgrade-param" style="<?php echo !$room_upgrade_enabled ? 'display: none;' : ''; ?>">
						<div class="vbo-param-container">
							<div class="vbo-param-label"><?php echo JText::_('VBO_ELIGIBLE_ROOMS'); ?></div>
							<div class="vbo-param-setting">
					...
					<div class="vbo-params-container vbo-roomupgrade-param" style="<?php echo !$room_upgrade_enabled ? 'display: none;' : ''; ?>">
						<div class="vbo-param-container">
							<div class="vbo-param-label"><?php echo JText::_('VBAPPLYDISCOUNT'); ?></div>
							<div class="vbo-param-setting">
				...
				<div class="vbo-params-wrap">
					...
					<div class="vbo-params-container">
						...
						if (VBOPlatformDetection::isJoomla()) {
							...
							?>
						<div class="vbo-param-container">
							<div class="vbo-param-label"><label for="sefalias"><?php echo JText::_('VBROOMSEFALIAS'); ?></label></div>
							<div class="vbo-param-setting">
								<input type="text" id="sefalias" name="sefalias" value="<?php echo $row ? JHtml::_('esc_attr', $row['alias']) : ''; ?>" placeholder="double-room-superior"/>
							</div>
						</div>
							<?php
						}
						?>
						<div class="vbo-param-container">
							<div class="vbo-param-label"><label for="custptitle"><?php echo JText::_('VBPARAMPAGETITLE'); ?></label></div>
							<div class="vbo-param-setting">
								<input type="text" id="custptitle" name="custptitle" value="<?php echo JHtml::_('esc_attr', $custptitle); ?>"/> 
							</div>
						</div>
						<div class="vbo-param-container vbo-param-child">
							...
							<div class="vbo-param-setting">
								<select name="custptitlew">
									<option value="before"<?php echo $custptitlew == 'before' ? ' selected="selected"' : ''; ?>><?php echo JText::_('VBPARAMPAGETITLEBEFORECUR'); ?></option>
									<option value="after"<?php echo $custptitlew == 'after' ? ' selected="selected"' : ''; ?>><?php echo JText::_('VBPARAMPAGETITLEAFTERCUR'); ?></option>
									<option value="replace"<?php echo $custptitlew == 'replace' ? ' selected="selected"' : ''; ?>><?php echo JText::_('VBPARAMPAGETITLEREPLACECUR'); ?></option>
								</select>
							</div>
						</div>
						<div class="vbo-param-container">
							<div class="vbo-param-label"><label for="metakeywords"><?php echo JText::_('VBPARAMKEYWORDSMETATAG'); ?></label></div>
							<div class="vbo-param-setting">
								<textarea name="metakeywords" id="metakeywords" rows="3" cols="40"><?php echo $metakeywords; ?></textarea>
							</div>
						</div>
						<div class="vbo-param-container">
							<div class="vbo-param-label"><label for="metadescription"><?php echo JText::_('VBPARAMDESCRIPTIONMETATAG'); ?></label></div>
							<div class="vbo-param-setting">
...
<div class="vbo-info-overlay-block">
	...
	<div class="vbo-info-overlay-content">
		...
		<div class="vbo-upload-done">
			<button type="button" class="btn btn-success" onclick="vboCloseModal();"><i class="icon-save"></i><?php echo JText::_('VBOUPLOADFILEDONE'); ?></button>
		</div>
	</div>
</div>
