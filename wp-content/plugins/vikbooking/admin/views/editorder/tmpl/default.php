...
JHtml::_('stylesheet', VBO_SITE_URI.'resources/jquery-ui.min.css');
...
$status_type = '';
if (!empty($row['type'])) {
	$status_type = JText::_('VBO_BTYPE_' . strtoupper($row['type']));
...
$extra_status = $row['refund'] > 0 ? ' / ' . JText::_('VBO_STATUS_REFUNDED') : '';
if ($row['status'] == "confirmed") {
	$saystaus = '<span class="label label-success">' . $status_type . JText::_('VBCONFIRMED') . $extra_status . '</span>';
} elseif ($row['status'] == "standby") {
	$saystaus = '<span class="label label-warning">' . $status_type . JText::_('VBSTANDBY') . $extra_status . '</span>';
} else {
	$saystaus = '<span class="label label-error">' . $status_type . JText::_('VBCANCELLED') . $extra_status . '</span>';
}
	...
	?>
		...
		?>
			<div class="vbo-bookdet-wrap">
				...
				if ($row['split_stay']) {
					?>
					<span class="hasTooltip" title="<?php echo JHtml::_('esc_attr', JText::_('VBO_SPLIT_STAY')); ?>"><?php VikBookingIcons::e('random'); ?></span>
					<?php
				}
				?>
				</div>
			...
			switch ($row['checked']) {
				...
				default:
					...
					if ($customer && isset($customer['pax_data']) && !empty($customer['pax_data'])) {
						...
						$checked_status = JText::_('VBOCHECKEDSTATUSPRECHECKIN');
					}
					break;
			}
			?>
		...
		if ($off_session_capturing) {
			...
			<div class="vbo-bookdet-wrap">
				<div class="vbo-bookdet-head">
					<span><?php echo JText::_('VBO_CREDIT_CARD'); ?></span>
				</div>
				<div class="vbo-bookdet-foot">
					<button type="button" class="btn btn-small vbo-config-btn vbo-offsessioncapture-btn" onclick="VBOCore.handleDisplayWidgetNotification({widget_id: 'virtual_terminal'}, {bid: <?php echo $row['id']; ?>});">
						...
						<?php echo JText::_('VBO_W_VIRTUALTERMINAL_TITLE'); ?>
					</button>
				</div>
			</div>
			<?php
		...
		if ($refundable) {
			...
			array_push($vbo_modals_html, $vbo_app->getJmodalHtml('vbo-refund-tn', JText::_('VBO_ISSUE_REFUND')));
			...
			<div class="vbo-bookdet-wrap">
				<div class="vbo-bookdet-head">
					<span><?php echo JText::_('VBO_ISSUE_REFUND'); ?></span>
				</div>
				<div class="vbo-bookdet-foot">
...
JS
			...
			<div class="vbo-bookdet-wrap">
				...
				<div class="vbo-bookdet-foot">
					<select name="vcm_assign_channel" class="vbo-select-small" onchange="return vboConfirmAssignChannel(this.value, this.options[this.selectedIndex].text);">
						<option value=""><?php echo JText::_('VBORDFROMSITE'); ?></option>
					<?php
					foreach ($assignable_channels as $assign_ch_key => $assign_ch_name) {
						?>
						<option value="<?php echo JHtml::_('esc_attr', $assign_ch_key); ?>"><?php echo $assign_ch_name; ?></option>
						<?php
					}
					?>
					</select>
		...
		?>
			...
			<div class="vbo-bookdet-wrap">
				...
				<div class="vbo-bookdet-foot">
					...
					if (empty($this->vcm_pre_approval[2]) || !empty($this->vcm_pre_approval[3])) {
						...
						?>
						<a href="index.php?option=com_vikchannelmanager&task=sendSpecialOffer&vbo_oid=<?php echo $row['id']; ?>&spo_type=preapproval&spo_action=send&ota_thread_id=<?php echo $this->vcm_pre_approval[0]; ?>" class="btn btn-small btn-primary vcm-btn-channel-<?php echo strtolower($this->vcm_pre_approval[1]); ?>"><?php VikBookingIcons::e('thumbs-up'); ?> <?php echo JText::_('VBO_PREAPPROVE_INQUIRY'); ?></a>
						<?php
					} else {
						// pre-approved before, pass the ID
						?>
						<a href="index.php?option=com_vikchannelmanager&task=sendSpecialOffer&vbo_oid=<?php echo $row['id']; ?>&spo_type=preapproval&spo_action=withdraw&ota_thread_id=<?php echo $this->vcm_pre_approval[0]; ?>&spo_id=<?php echo $this->vcm_pre_approval[2]; ?>" class="btn btn-small btn-danger"><?php VikBookingIcons::e('thumbs-down'); ?> <?php echo JText::_('VBO_WITHDRAW_PREAPPROVAL'); ?></a>
						<?php
					}
					?>
					</span>
			...
			<div class="vbo-bookdet-wrap">
				<div class="vbo-bookdet-head">
					<span><?php echo JText::_('VBOGUESTMISCONDUCT'); ?></span>
				</div>
				<div class="vbo-bookdet-foot">
				<?php
				if ($misconduct_reported === false) {
					...
					?>
					<button type="button" class="btn btn-small btn-danger" onclick="vboRenderGuestMisconduct();"><?php VikBookingIcons::e('user-slash'); ?> <?php echo JText::_('VBOREPORTMISCONDUCT'); ?></button>
					<?php
				} else {
						...
						<div class="vbo-modal-overlay-content-body">
							<div class="vbo-modal-guestmisconduct-addnew" data-bid="<?php echo $row['id']; ?>" data-otabid="<?php echo $row['idorderota']; ?>">
								...
								<div class="vbo-modal-guestmisconduct-addnew-save">
									<button type="button" id="vbo-guest-misconduct-submit-btn" class="btn btn-success" onclick="vboSubmitGuestMisconduct();" style="display: none;"><?php echo JText::_('VBCHANNELMANAGERSENDRQ'); ?></button>
									<button type="button" class="btn btn-secondary" onclick="hideVboDialogGuestMisconduct();"><?php echo JText::_('VBOCLOSE'); ?></button>
								</div>
							</div>
						</div>
					</div>
				...
				function vboRenderGuestMisconduct() {
					// empty content
					jQuery('.vbo-modal-guestmisconduct-report-elems').html('<?php echo $this->escape(JText::_('VIKLOADING') . '...'); ?>');
			...
			<div class="vbo-bookdet-wrap">
				<div class="vbo-bookdet-head">
					<span><?php echo JText::_('VBO_REVIEW_YOUR_GUEST'); ?></span>
				</div>
				<div class="vbo-bookdet-foot">
					<button type="button" class="btn btn-small btn-primary vcm-btn-channel-airbnbapi" onclick="vboOpenJModal('vbo-vcm-host-to-guest-review', 'index.php?option=com_vikchannelmanager&task=hostguestreview&cid[]=<?php echo $row['id']; ?>&tmpl=component');">
						...
						<?php echo JText::_('VBO_WRITE_REVIEW'); ?>
					</button>
				</div>
			</div>
				<?php
				// prepare modal (host-to-guest review)
				array_push($vbo_modals_html, $vbo_app->getJmodalHtml('vbo-vcm-host-to-guest-review', JText::_('VBO_REVIEW_YOUR_GUEST')));
...
JS
		...
		<div class="vbo-bookdet-container vbo-bookdet-conv-currency">
			<div class="vbo-bookdet-conv-currency-inner">
				...
				<div class="vbo-bookdet-conv-currency-apply">
					<a class="btn vbo-config-btn" onclick="return confirm(Joomla.JText._('VBO_CONV_RES_OTA_CURRENCY_APPLY') + '?');" href="index.php?option=com_vikbooking&task=editorder&cid[]=<?php echo $row['id']; ?>&do_ota_curr_conv=1"><?php VikBookingIcons::e('exchange-alt'); ?> <?php echo JText::_('VBO_CONV_RES_OTA_CURRENCY_APPLY'); ?></a>
				</div>
			</div>
		</div>
			<?php
		...
		<div class="vbo-bookdet-container vbo-bookdet-inquiry-alert">
			<div class="vbo-bookdet-inquiry-alert-dismiss">
				<button type="button" class="btn btn-warning" onclick="jQuery('.vbo-bookdet-inquiry-alert').remove();"><?php VikBookingIcons::e('times-circle'); ?> <?php echo JText::_('VBOBTNKEEPREMIND'); ?></button>
			</div>
			<div class="vbo-bookdet-inquiry-alert-message">
				<div class="vbo-bookdet-inquiry-alert-top">
					<span><?php echo JText::_('VBO_WEB_INQUIRY_ALERT'); ?></span>
				</div>
				<div class="vbo-bookdet-inquiry-alert-avtype">
					...
					if ($prev_ir_data[0]->av_type === 1) {
						echo JText::_('VBO_WEB_INQUIRY_AVTYPE_1');
					} elseif ($prev_ir_data[0]->av_type === 2) {
						echo JText::_('VBO_WEB_INQUIRY_AVTYPE_2');
					} elseif ($prev_ir_data[0]->av_type === 3) {
						echo JText::_('VBO_WEB_INQUIRY_AVTYPE_3');
					} else {
						echo JText::_('VBO_WEB_INQUIRY_AVTYPE_4');
					}
					?></span>
					<span class="vbo-bookdet-inquiry-alert-mess-origrq">
						<strong><?php echo JText::_('VBO_WEB_INQUIRY_ORIGRQ') . ':'; ?></strong>
						<span class="badge"><?php echo JText::_('VBPICKUPAT') . ': ' . $orig_checkin_dt_obj->format('D, d M Y', true); ?></span>
						<span class="badge"><?php echo JText::_('VBRELEASEAT') . ': ' . $orig_checkout_dt_obj->format('D, d M Y', true); ?></span>
						<span class="badge"><?php echo JText::_('VBEDITORDERADULTS') . ': ' . $prev_ir_data[0]->adults; ?></span>
					<?php
					if (!empty($prev_ir_data[0]->children)) {
						?>
						<span class="badge"><?php echo JText::_('VBEDITORDERCHILDREN') . ': ' . $prev_ir_data[0]->children; ?></span>
						<?php
					}
					?>
					</span>
				...
				<div class="vbo-bookdet-inquiry-alert-bottom">
					<span><?php echo JText::_('VBO_WEB_INQUIRY_SUGG'); ?></span>
				</div>
			</div>
		</div>
				<?php
		...
		<div class="vbo-bookingdet-innertop">
			...
			if ($busy || $row['status'] == "standby") {
				...
				<div class="vbo-bookingdet-command">
					<button onclick="document.location.href='index.php?option=com_vikbooking&task=editbusy&cid[]=<?php echo $row['id']; ?>';" class="btn vbo-config-btn" type="button"><i class="icon-pencil"></i> <?php echo JText::_('VBMODRES'); ?></button>
				</div>
				<?php
			}
			if ((array_key_exists(1, $tars) && $tars[1]) || ($is_package || $is_cust_cost)) {
				...
				?>
				<div class="vbo-bookingdet-command">
					<button onclick="window.open('<?php echo VikBooking::externalroute('index.php?option=com_vikbooking&view=booking&sid='.(!empty($row['sid']) ? $row['sid'] : $row['idorderota']).'&ts='.$row['ts'], false); ?>', '_blank');" type="button" class="btn vbo-config-btn"><?php VikBookingIcons::e('external-link'); ?> <?php echo JText::_('VBVIEWORDFRONT'); ?></button>
				</div>
				<?php
			}
			if (($row['status'] == "confirmed" || ($row['status'] == "standby" && !empty($row['custmail']))) && ((array_key_exists(1, $tars) && $tars[1]) || ($is_package || $is_cust_cost))) {
				?>
				<div class="vbo-bookingdet-command">
					<button class="btn vbo-config-btn" type="button" onclick="document.location.href='index.php?option=com_vikbooking&task=resendordemail&cid[]=<?php echo $row['id']; ?>';"><?php VikBookingIcons::e('envelope'); ?> <?php echo JText::_('VBRESENDORDEMAIL'); ?></button>
				</div>
				<?php
			}
			...
			if ($row['status'] == "standby" || ($row['status'] == "cancelled" && $row['checkout'] >= $current_ts)) {
				?>
				<div class="vbo-bookingdet-command">
					<button class="btn btn-success" type="button" onclick="if (confirm('<?php echo htmlspecialchars(JText::_('VBSETORDCONFIRMED')); ?> ?')) {document.location.href='index.php?option=com_vikbooking&task=setordconfirmed&cid[]=<?php echo $row['id'] . ($row['status'] == "cancelled" ? '&skip_notification=1' : ''); ?>';}"><?php VikBookingIcons::e('check'); ?> <?php echo JText::_('VBSETORDCONFIRMED'); ?></button>
				</div>
				<?php
			}
			...
			if ($row['status'] == 'confirmed' && $row['closure'] < 1) {
				...
				<div class="vbo-bookingdet-command pro-feature">
					<button type="button" class="btn vbo-config-btn" onclick="document.getElementById('invnotes-hid').value=document.getElementById('invnotes').value;document.getElementById('vbo-gen-invoice').submit();"><?php VikBookingIcons::e('file-text-o'); ?> <?php echo JText::_('VBOGENBOOKINGINVOICE'); ?></button>
				</div>
				<?php
			}
			if ($row['status'] == 'confirmed' && $row['closure'] < 1) {
				?>
				<div class="vbo-bookingdet-command pro-feature">
					<button type="button" class="btn vbo-config-btn" onclick="window.open('index.php?option=com_vikbooking&task=editorder&cid[]=<?php echo $row['id']; ?>&print=1&tmpl=component', '_blank');"><?php VikBookingIcons::e('print'); ?> <?php echo JText::_('VBOPRINTRECEIPT'); ?></button>
				</div>
				<?php
			}
			if ($row['status'] != 'confirmed' || $row['closure'] > 0 || $this->vcm_cancel_active_res) {
				if ($this->vcm_decline_actions) {
					...
					?>
				<div class="vbo-bookingdet-command">
					<button type="button" class="btn btn-danger" onclick="vboOpenJModal('vbo-vcm-decline-booking', 'index.php?option=com_vikchannelmanager&task=declinebooking&cid[]=<?php echo $row['id']; ?>&tmpl=component');"><?php VikBookingIcons::e('trash'); ?> <?php echo JText::_('VBMAINEBUSYDEL'); ?></button>
				</div>
					<?php
					// prepare modal (VCM decline booking)
					array_push($vbo_modals_html, $vbo_app->getJmodalHtml('vbo-vcm-decline-booking', JText::_('VBMAINEBUSYDEL')));
					// end prepare modal
				} elseif ($this->vcm_cancel_active_res) {
					// the active OTA booking can be cancelled
					?>
				<div class="vbo-bookingdet-command">
					<button type="button" class="btn btn-danger" onclick="vboOpenJModal('vbo-vcm-cancel-reservation', 'index.php?option=com_vikchannelmanager&task=cancelreservation&vbo_oid=<?php echo $row['id']; ?>&tmpl=component');"><?php VikBookingIcons::e('trash'); ?> <?php echo JText::_('VBMAINEBUSYDEL'); ?></button>
				</div>
					<?php
					// prepare modal (VCM cancel reservation)
					array_push($vbo_modals_html, $vbo_app->getJmodalHtml('vbo-vcm-cancel-reservation', JText::_('VBMAINEBUSYDEL')));
					// end prepare modal
				} else {
					// regular remove button with confirmation
					?>
				<div class="vbo-bookingdet-command">
					<button type="button" class="btn btn-danger" onclick="if (confirm(Joomla.JText._('VBDELCONFIRM'))){document.location.href='index.php?option=com_vikbooking&task=removeorders&cid[]=<?php echo $row['id']; ?>&goto=<?php echo $row['id']; ?>';}"><?php VikBookingIcons::e('trash'); ?> <?php echo $row['status'] == 'cancelled' ? JText::_('VBO_PURGERM_RESERVATION') : JText::_('VBMAINEBUSYDEL'); ?></button>
				</div>
					<?php
				}
			}
			...
			<div class="vbo-bookingdet-innercontainer">
				...
				if ($bookingTasks) {
					...
					<div class="vbo-bookingdet-detcont vbo-bookingdet-detcont-tm vbo-hidein-print">
						<div class="vbo-bookingdet-lblcont">
							<label><?php echo JText::_('VBO_TASK_MANAGER'); ?></label>
						</div>
				...
				$invoice_details = [
					[
						...
						'label' => JText::_('VBOCOLORTAGRULEINVONE'),
						'btn'   => JText::_('VBOINVDOWNLOAD'),
					]
				];
				...
				if (!$printreceipt && ((!empty($row['channel']) && !empty($row['idorderota'])) || strlen((string)$row['confirmnumber']) || $invoiced)) {
					...
					if (!empty($row['channel']) && !empty($row['idorderota'])) {
						...
						<div class="vbo-bookingdet-detcont-label vbo-bookingdet-detcont-label-idorderota <?php echo strtolower(preg_replace('/[^a-z0-9]/i', '', $channelparts[0])); ?>">
							<span class="label label-info">
								...
								<span class="badge<?php echo $displayOtaBid ? ' vbo-tooltip vbo-tooltip-top vbo-reservation-copy-otabid' : ''; ?>" data-tooltiptext="<?php echo JHtml::_('esc_attr', JText::_('VBO_COPY')); ?>"><?php echo $displayOtaBid ? $row['idorderota'] : '-----'; ?></span>
							</span>
					...
					if (strlen((string)$row['confirmnumber'])) {
						...
						<div class="vbo-bookingdet-detcont-label vbo-bookingdet-detcont-label-confirmnumber">
							<span class="label label-success">
								<span><?php echo JText::_('VBCONFIRMNUMB'); ?></span>
					...
					if (class_exists('VikChannelManager') && VikChannelManager::hostToGuestReviewSupported($row, $willbe = true)) {
						...
						<div class="vbo-bookingdet-detcont-ai-info">
							<p class="vbo-bookingdet-detcont-ai-info-status">
								<span class="vbo-bookingdet-detcont-ai-info-status-on" style="<?php echo ($ai_auto_guest_review_opt['ignore'] ?? 0) ? 'display:  none;' : ''; ?>"><?php VikBookingIcons::e('check-circle'); ?> <span><?php echo JText::_('VBO_AI_AUTO_GUEST_REV_WILLRUN'); ?></span></span>
								<span class="vbo-bookingdet-detcont-ai-info-status-off" style="<?php echo !($ai_auto_guest_review_opt['ignore'] ?? 0) ? 'display:  none;' : ''; ?>"><?php VikBookingIcons::e('ban'); ?> <span><?php echo JText::_('VBO_AI_AUTO_GUEST_REV_RESIGNORE'); ?></span></span>
							</p>
					...
					if ($invoiced) {
						...
						<div class="vbo-bookingdet-detcont-label vbo-bookingdet-detcont-label-invoice">
							<span class="label label-success">
								<span><?php echo $invoice_detail['label'] ?? JText::_('VBOCOLORTAGRULEINVONE'); ?></span>
								<span class="badge"><a href="<?php echo $invoice_detail['uri'] . (strpos($invoice_detail['uri'], '?') === false ? '?' . rand() . '=1' : ''); ?>" target="_blank"><?php VikBookingIcons::e('download'); ?> <?php echo $invoice_detail['btn'] ?? JText::_('VBOINVDOWNLOAD'); ?></a></span>
							</span>
						</div>
							<?php
						}
					...
					if ($this->vcm_review) {
						...
						<div class="vbo-bookingdet-detcont-label vbo-bookingdet-detcont-label-review">
							<span class="label label-warning" onclick="VBOCore.handleDisplayWidgetNotification({widget_id: 'guest_reviews'}, {review_id: '<?php echo $this->vcm_review['id']; ?>'});">
								<a href="javascript: void(0);" style="color: #fff;"><i class="vboicn-star-full"></i> <?php echo JText::_('VBOSEEGUESTREVIEW'); ?></a>
							</span>
						</div>
						<?php
						// check if a notification was clicked
				...
				?>
						...
						<div class="vbo-bookingdet-inpwrap">
							<div class="vbo-bookingdet-inpcont">
								...
								<div class="vbo-bookingdet-btncont vbo-bookingdet-save-email" style="display: none;">
									<button type="button" class="btn btn-success" onclick="document.adminForm.submit();"><?php VikBookingIcons::e('save'); ?> <?php echo JText::_('VBSAVE'); ?></button>
								</div>
							</div>
						<?php if (!empty($row['custmail'])) : ?>
							<div class="vbo-bookingdet-btncont">
								<button type="button" class="btn vbo-config-btn" onclick="vboDisplaySendEmail();" style="vertical-align: top;"><?php VikBookingIcons::e('envelope'); ?> <?php echo JText::_('VBSENDEMAILACTION'); ?></button>
							</div>
						<?php endif; ?>
						</div>
					...
					<div class="vbo-bookingdet-detcont vbo-hidein-print">
						...
						<?php if (!empty($row['phone'])) : ?>
							<div class="vbo-bookingdet-btncont">
								<button type="button" class="btn vbo-config-btn" onclick="vboDisplaySendSMS();" style="vertical-align: top;"><?php VikBookingIcons::e('comment-dots'); ?> <?php echo JText::_('VBSENDSMSACTION'); ?></button>
							</div>
						<?php endif; ?>
						</div>
					...
					<div class="table-responsive">
						...
						foreach ($rooms as $ind => $or) {
							...
							<tr class="vbo-bookingdet-summary-room">
								<td class="vbo-bookingdet-summary-room-firstcell">
									<div class="vbo-bookingdet-summary-room-wrap">
										<div class="vbo-bookingdet-summary-roomnum"><?php VikBookingIcons::e($room_icon); ?> <?php echo JText::_('VBEDITORDERTHREE').' '.$num; ?></div>
									<?php
									// Room Specific Unit
									...
									if ($arrpeople[$num]['pets'] > 0) {
										...
										<div class="vbo-bookingdet-summary-roompets">
											<span><?php VikBookingIcons::e('dog'); ?> <?php echo JText::_('VBO_PETS'); ?>:</span> <?php echo $arrpeople[$num]['pets']; ?>
										</div>
										<?php
									}
									?>
								...
								if ($row['split_stay'] && $room_stay_dates && isset($room_stay_dates[$ind]) && $room_stay_dates[$ind]['idroom'] == $or['idroom']) {
									...
									<div class="vbo-cal-splitstay-details vbo-bookdet-splitstay-details">
										<div class="vbo-cal-splitstay-dates">
											<span class="vbo-cal-splitstay-room-nights"><?php VikBookingIcons::e('moon'); ?> <?php echo $room_stay_nights . ' ' . ($room_stay_nights > 1 ? JText::_('VBDAYS') : JText::_('VBDAY')); ?></span>
							...
							<tr class="vbo-bookingdet-summary-custcosts">
								...
								if (!empty($ecv['type'])) {
									?>
									<span class="vbo-tooltip vbo-tooltip-top" data-tooltiptext="<?php echo JHtml::_('esc_attr', JText::_('VBPSHOWSEASONSTHREE') . ': ' . $ecv['type']); ?>">
						...
						if ($row['refund'] > 0) {
							...
							<tr class="vbo-bookingdet-summary-totpaid vbo-bookingdet-summary-totrefunded">
								<td>
									<strong><?php echo JText::_('VBOBOOKHISTORYTRF'); ?></strong>
								</td>
								<td>
									<span class="vbo-amount-refunded-lbl"><?php echo JText::_('VBO_AMOUNT_REFUNDED'); ?></span>
								</td>
								<td>
									...
									<div id="vbo-amountrefunded-modcont" style="display: none;">
										...
										<span id="vbo-amountrefunded-save"><button type="submit" class="btn btn-success"><?php echo JText::_('VBAPPLYDISCOUNTSAVE'); ?></button></span>
									</div>
								</td>
							</tr>
							<?php
						...
						if ($printreceipt && $row['tot_taxes'] > 0) {
							...
							<tr class="vbo-bookingdet-summary-total vbo-bookingdet-summary-totaltax">
								...
								<td>
									<span class="vbo-bookingdet-summary-lbl"><?php echo JText::_('VBTOTALVAT'); ?></span>
								</td>
								<td>
							...
							<tr class="vbo-bookingdet-summary-total">
								...
								if (!$printreceipt) {
									...
									<span class="vbapplydiscsp" onclick="toggleDiscount(this);">
										<i class="<?php echo VikBookingIcons::i('chevron-down'); ?>" title="<?php echo htmlspecialchars(JText::_('VBAPPLYDISCOUNT')); ?>"></i>
									</span>
									<?php
								}
								?>
								...
								<td>
									...
									<div class="vbdiscenter" id="vbdiscenter" style="display: none;">
										<div class="vbdiscenter-entry">
											<span class="vbdiscenter-label"><?php echo JText::_('VBTOTALVAT'); ?>:</span><span class="vbdiscenter-value"><?php echo $currencyname; ?> <input type="number" step="any" name="tot_taxes" value="<?php echo $row['tot_taxes']; ?>" size="4"/></span>
										</div>
									<?php
									if ($tax_breakdown) {
									...
									if ($booking_has_damage_dep) {
										...
										?>
										<div class="vbdiscenter-entry">
											<span class="vbdiscenter-label"><?php echo JText::_('VBO_DAMAGE_DEPOSIT'); ?>:</span><span class="vbdiscenter-value"><?php echo $currencyname; ?> <input type="number" step="any" name="tot_damage_dep" value="<?php echo $row['tot_damage_dep']; ?>" min="0" size="4"/></span>
										</div>
										<?php
									}
									?>
										<div class="vbdiscenter-entry">
											<span class="vbdiscenter-label"><?php echo JText::_('VBAPPLYDISCOUNT'); ?>:</span><span class="vbdiscenter-value"><?php echo $currencyname; ?> <input type="number" step="any" name="admindisc" value="<?php echo isset($expcoupon) ? (float)$expcoupon[1] : ''; ?>" size="4"/></span>
										</div>
								...
								if ($row['status'] == 'confirmed' && !empty($row['channel']) && !empty($row['idorderota']) && $row['checkout'] >= $current_ts && !empty($ota_type_data['expected_payout'])) {
									...
									<div class="vbo-bookingdet-exp-payout">
										<span class="vbo-bookingdet-exp-payout-lbl"><?php echo JText::_('VBO_EXPECTED_PAYOUT'); ?></span>
						...
						if ($row['total'] > 0 && !$row['closure']) {
							...
							<tr class="vbo-bookingdet-summary-totpaid">
								...
								<td>
									...
									if ($payments) {
										...
										<span id="vbo-amountpaid-paymeth" style="margin: 0 5px; max-width: 160px;">
											<select name="newamountpaymeth" style="margin: 0;">
												<option value=""><?php echo JText::_('VBPAYMENTMETHOD'); ?></option>
											<?php
											foreach ($payments as $pay) {
												?>
												<option value="<?php echo JHtml::_('esc_attr', $pay['id'] . '_' . $pay['name']); ?>"><?php echo $pay['name']; ?></option>
												<?php
											}
											?>
											</select>
									...
									?>
										<span id="vbo-amountpaid-save"><button type="submit" class="btn btn-success"><?php echo JText::_('VBAPPLYDISCOUNTSAVE'); ?></button></span>
									</div>
								</td>
							</tr>
							<?php
								...
								<td>
									...
									if (!$printreceipt && $row['status'] == 'confirmed' && !($row['paymcount'] > 0) && VikBooking::multiplePayments() && is_array($payment) && !empty($payment['id'])) {
										...
										<div style="margin-top: 5px;">
											<a href="index.php?option=com_vikbooking&amp;task=editorder&amp;makepay=1&amp;cid[]=<?php echo $row['id']; ?>" class="vbo-makepayable-link"><?php VikBookingIcons::e('credit-card'); ?> <?php echo JText::_('VBMAKEORDERPAYABLE'); ?></a>
										</div>
										<?php
									}
									?>
						...
						if (!$printreceipt && $row['status'] == 'confirmed' && VikBooking::multiplePayments() && is_array($payment) && !empty($payment['id']) && $row['checkout'] > strtotime('-10 days')) {
							...
							<tr class="vbo-bookingdet-summary-totpaid vbo-bookingdet-summary-totpayable">
								<td>&nbsp;</td>
								<td>
									<span class="vbo-amount-payable-lbl<?php echo $row['payable'] > 0 ? ' vbo-amount-payable-lbl-requested' : ''; ?>"><?php echo $row['payable'] > 0 ? JText::_('VBO_AMOUNT_PAYABLE') : JText::_('VBO_AMOUNT_PAYABLE_RQ'); ?></span>
								</td>
								<td>
									...
									<div id="vbo-amountpayable-modcont" style="display: none;">
										...
										<span id="vbo-amountpayable-save"><button type="submit" class="btn btn-success"><?php echo JText::_('VBAPPLYDISCOUNTSAVE'); ?></button></span>
									</div>
								</td>
							</tr>
							<?php
						...
						if ($row['status'] == 'cancelled' && !$row['closure'] && $row['total'] > 0) {
							...
							<tr class="vbo-bookingdet-summary-cancfee">
								<td>&nbsp;</td>
								<td><?php echo JText::_('VBO_CANC_FEE'); ?></td>
								<td>
									...
									<div id="vbo-cancfee-modcont" style="display: none;">
										...
										<span id="vbo-cancfee-save"><button type="submit" class="btn btn-success"><?php echo JText::_('VBAPPLYDISCOUNTSAVE'); ?></button></span>
									</div>
								</td>
							</tr>
							<?php
			...
			<div class="vbo-bookingdet-innercontainer">
				...
				if (!$row['closure']) {
					...
					<div class="vbo-bookingdet-admin-entry">
						<label for="vbo-searchcust"><?php echo JText::_($customer ? 'VBOASSIGNNEWCUST' : 'VBFILLCUSTFIELDS'); ?></label>
						<span style="display: block;"><?php echo JText::_('VBOSEARCHEXISTCUST'); ?></span>
						<span class="vbo-eorder-assigncust" style="margin-bottom: 1px;">
							<input type="text" id="vbo-searchcust" autocomplete="off" value="" placeholder="<?php echo JText::_('VBOSEARCHCUSTBY'); ?>" size="30" style="margin-bottom: 0;" />
						</span>
				...
				<div class="vbo-bookingdet-noteslogs">
					...
					<div class="vbo-bookingdet-noteslogs-btns">
						<div class="vbo-bookingdet-noteslogs-btn vbo-bookingdet-noteslogs-btn-active" data-id="notes">
							<a href="javascript: void(0);" id="vbo-trig-notes" onclick="javascript: vbToggleNotes(this);">
								...
								<span class="vbo-bookingdet-admintab-lbl"><?php echo JText::_('VBADMINNOTESTOGGLE'); ?></span>
							</a>
						</div>
					<?php
					/**
					...
					if (!$printreceipt && !is_null($messaging)) {
						...
						<div class="vbo-bookingdet-noteslogs-btn" data-id="messaging">
							<a name="messaging" href="javascript: void(0);" id="vbo-trig-messaging" onclick="javascript: vbToggleMessaging(this);">
								...
								<span class="vbo-bookingdet-admintab-lbl"><?php echo JText::_('VBO_GUEST_MESSAGING'); ?></span>
							</a>
						</div>
						<?php
					}
						...
						if ($takings) {
							...
							<div class="vbo-bookingdet-noteslogs-btn pro-feature" data-id="takings">
								<a href="javascript: void(0);" id="vbo-trig-bookhistory" onclick="javascript: vbToggleDynamicPanel(this, '#vboextrapanel-takings');">
									...
									<span class="vbo-bookingdet-admintab-lbl"><?php echo JText::_('VBO_TAKINGS'); ?></span>
								</a>
							</div>
							<?php
						}
					...
					if ($history) {
						...
						<div class="vbo-bookingdet-noteslogs-btn" data-id="history">
							<a href="javascript: void(0);" id="vbo-trig-bookhistory" onclick="javascript: vbToggleHistory(this);">
								...
								<span class="vbo-bookingdet-admintab-lbl"><?php echo JText::_('VBOBOOKHISTORYTAB'); ?></span>
							</a>
						</div>
						<script type="text/javascript">
					...
					?>
						<div class="vbo-bookingdet-noteslogs-btn pro-feature" data-id="invnotes">
							<a href="javascript: void(0);" class="hasTooltip" id="vbo-trig-invnotes" onclick="javascript: vbToggleInvNotes(this);" title="<?php echo addslashes(JText::_('VBBOOKINGINVNOTESHELP')); ?>">
								<?php VikBookingIcons::e('file-invoice'); ?>
								<span class="vbo-bookingdet-admintab-lbl"><?php echo JText::_('VBBOOKINGINVNOTES'); ?></span>
							</a>
						</div>
						...
						<div class="vbo-bookingdet-noteslogs-btn" data-id="<?php echo $legendId; ?>">
							<a href="javascript: void(0);" class="hasTooltip" id="vbo-trig-<?php echo $legendId; ?>" onclick="javascript: vbToggleDynamicPanel(this, '#vboextrapanel-<?php echo $legendId; ?>');">
								...
								<span class="vbo-bookingdet-admintab-lbl"><?php echo JText::_($legend); ?></span>
							</a>
						</div>
							<?php
						}
						...
						if (!empty($row['paymentlog'])) {
							$paylogs_lbl = JText::_('VBPAYMENTLOGTOGGLE');
							if ($may_have_cc_details) {
								$paylogs_lbl = JText::_('VBO_CREDIT_CARD');
							}
							?>
					...
					<div class="vbo-bookingdet-noteslogs-cont">
						<div id="vbadminnotesdiv" class="vbo-extra-panel" style="display: block;">
							...
							<input type="submit" name="updadmnotes" value="<?php echo JText::_('VBADMINNOTESUPD'); ?>" class="btn btn-success" />
						</div>
					<?php
					if ($takings) {
						?>
								...
								<table class="table">
									<thead>
										<tr class="vbo-booking-takings-firstrow">
											<td class="vbo-booking-takings-td-totpaid"><?php echo JText::_('VBO_TAKINGS'); ?></td>
											<td class="vbo-booking-takings-td-date"><?php echo JText::_('VBOBOOKHISTORYLBLDATE'); ?></td>
											<td class="vbo-booking-takings-td-type"><?php echo JText::_('VBOBOOKHISTORYLBLTYPE'); ?></td>
											<td class="vbo-booking-takings-td-fees"><?php echo JText::_('VBO_PROCESSING_FEES'); ?></td>
											<td class="vbo-booking-takings-td-descr"><?php echo JText::_('VBOBOOKHISTORYLBLDESC'); ?></td>
										</tr>
									</thead>
									<tbody>
									<?php
										...
										<tr class="vbo-booking-takings-row">
											...
											if (!strcasecmp($hist['type'], 'PU')) {
												...
												<div class="vbo-booking-taking-cmds">
													<button type="button" class="btn btn-small btn-primary vbo-booking-taking-edit vbo-tooltip vbo-tooltip-top" data-tooltiptext="<?php echo $this->escape(JText::_('VBMAINPAYMENTSEDIT')); ?>" data-historyid="<?php echo $hist['id']; ?>" data-amount="<?php echo $tn_amount_paid; ?>"><?php VikBookingIcons::e('edit'); ?></button>
													<button type="button" class="btn btn-small btn-danger vbo-booking-taking-delete vbo-tooltip vbo-tooltip-top" data-tooltiptext="<?php echo $this->escape(JText::_('VBELIMINA')); ?>" data-historyid="<?php echo $hist['id']; ?>"><?php VikBookingIcons::e('times-circle'); ?></button>
												</div>
												<?php
											}
											?>
											</td>
											<td><?php echo JHtml::_('date', $hist['dt'], 'Y-m-d H:i:s'); ?></td>
									...
									<tfoot>
										<tr>
											...
											<td>
												<button type="button" class="btn btn-small btn-success vbo-takings-addnew"><?php VikBookingIcons::e('plus-circle'); ?> <?php echo JText::_('VBO_REG_NEW_PAYMENT'); ?></button>
											</td>
										</tr>
									</tfoot>
								</table>
												...
												<div class="vbo-params-block">
													<div class="vbo-param-container">
														<div class="vbo-param-label">
															<label for="new_taking_amount"><?php echo JText::_('VBO_CC_AMOUNT'); ?></label>
														</div>
														<div class="vbo-param-setting">
															...
															<span class="vbo-param-setting-comment">
																<?php echo JText::_('VBOCUSTOMERCMMSONTOTAL') . ' ' . VikBooking::formatCurrencyNumber(VikBooking::numberFormat($row['total']), $currencysymb) . ' - ' . JText::_('VBAMOUNTPAID') . ' ' . VikBooking::formatCurrencyNumber(VikBooking::numberFormat($row['totpaid']), $currencysymb); ?>
															</span>
														</div>
													</div>
													<div class="vbo-param-container">
														<div class="vbo-param-label">
															<label for="new_taking_descr"><?php echo JText::_('VBLIBPAYNAME'); ?></label>
														</div>
														<div class="vbo-param-setting">
													...
													<div class="vbo-param-container vbo-param-nested" id="vbo-new-taking-custom-wrap">
														...
														<div class="vbo-param-setting">
															<input id="new_taking_descr" type="text" value="" maxlength="200" placeholder="<?php echo $this->escape(JText::_('VBPVIEWOPTIONALSTWO')); ?>" />
														</div>
													</div>
												</div>
											</div>
					...
					?>
						<div id="vbinvnotesdiv" class="vbo-extra-panel" style="display: none;">
							...
							<input type="submit" name="updinvnotes" value="<?php echo JText::_('VBADMINNOTESUPD'); ?>" class="btn btn-success" />
							<button type="button" class="btn vbo-config-btn btn-secondary pull-right" onclick="document.getElementById('invnotes-hid').value=document.getElementById('invnotes').value;document.getElementById('vbo-gen-invoice').submit();"><?php VikBookingIcons::e('file-invoice'); ?> <?php echo JText::_('VBOGENBOOKINGINVOICE'); ?></button>
						</div>
					<?php
					if (!empty($row['paymentlog'])) {
						?>
						...
						if (!empty($row['idorderota']) && !empty($row['channel'])) {
							...
							if (!$canDo->authorise('core.admin', 'com_vikchannelmanager')) {
								...
								$cc_acl_denied = true;
								?>
								<p class="err"><?php echo JText::_('JERROR_ALERTNOAUTHOR'); ?></p>
								<?php
							} else {
								/**
								...
								<div class="vbo-eorder-payschedules-wrap">
									<button type="button" class="btn btn-<?php echo $active_payschedules || $automatic_payment_collection ? 'success' : 'primary'; ?> vbo-context-menu-btn vbo-context-menu-payschedules">
										<span class="vbo-context-menu-lbl"><?php echo JText::_('VBO_AUTOPAY_SCHEDULED'); ?></span>
										<span class="vbo-context-menu-ico"><?php VikBookingIcons::e('sort-down'); ?></span>
									</button>
								</div>
								...
								<div class="vbo-eorder-newpayschedule-helper-wrap" style="display: none;">
									<div class="vbo-eorder-newpayschedule-helper-cont" data-auto-pay-schedules="<?php echo (int) $automatic_payment_collection; ?>">
										<p class="info" data-auto-pay-schedules="help" style="display: none;"><?php echo JText::_('VBO_AUTO_PAY_COLLECT_HELP'); ?></p>
										<div class="vbo-admin-container vbo-admin-container-full vbo-admin-container-compact">
											<div class="vbo-params-wrap">
												<div class="vbo-params-container">
													<div class="vbo-params-block">
														<div class="vbo-param-container">
															<div class="vbo-param-label">
																<label for="new_payschedule_amount"><?php echo JText::_('VBO_CC_AMOUNT'); ?></label>
															</div>
															<div class="vbo-param-setting">
																<div class="vbo-input-currency-wrap">
																	<span><?php echo $currencysymb; ?></span>
																...
																<span class="vbo-param-setting-comment"><?php echo JText::_('VBEDITORDERNINE') . ' ' . VikBooking::formatCurrencyNumber(VikBooking::numberFormat($row['total']), $currencysymb); ?></span>
															</div>
														</div>
														<div class="vbo-param-container">
															<div class="vbo-param-label">
																<label for="new_payschedule_fordt"><?php echo JText::_('VBPVIEWORDERSONE'); ?></label>
															</div>
															<div class="vbo-param-setting">
														...
														<div class="vbo-param-container">
															<div class="vbo-param-label">
																<label for="new_payschedule_fordtime"><?php echo JText::_('VBO_REMINDER_TIME'); ?></label>
															</div>
															<div class="vbo-param-setting">
								...
								<script type="text/javascript">
									...
									var vbo_payschedule_btns = [
										// toggle automatic payment collection scheduling for all bookings
										{
											...
											text: <?php echo json_encode(JText::_('VBO_ENABLED_FOR_ALL_BOOKINGS')); ?>,
											separator: true,
											icon: function() {
											...
											action: function(root, event) {
												...
												let confirm_enable = <?php echo json_encode(JText::_('VBO_AUTO_PAY_COLLECT_ENABLE_CONF')); ?>;
												let confirm_disable = <?php echo json_encode(JText::_('VBO_AUTO_PAY_COLLECT_DISABLE_CONF')); ?>;
							...
							?>
								<div class="vbo-eorder-emptyccdata">
									<a class="btn btn-warning" href="index.php?option=com_vikbooking&task=editorder&cid[]=<?php echo $row['id']; ?>&unset_cc=1" onclick="return confirm(Joomla.JText._('VBDELCONFIRM'));"><?php VikBookingIcons::e('broom'); ?> <?php echo JText::_('VBO_EMPTY_DATA'); ?></a>
								</div>
							<?php
						}
...
<div class="vbo-editorder-sms-email-send-helper" style="display: none;">
	...
	<div id="vbo-modal-footer-sms" style="display: none;">
		<button type="submit" class="btn vbo-config-btn" onclick="document.getElementById('vbo-modal-form-sms').submit();"><?php VikBookingIcons::e('comment'); ?> <?php echo JText::_('VBSENDSMSACTION'); ?></button>
	</div>
	<div id="vbo-modal-footer-email" style="display: none;">
		<button type="submit" class="btn vbo-config-btn" onclick="document.getElementById('vbo-modal-form-email').submit();"><?php VikBookingIcons::e('envelope'); ?> <?php echo JText::_('VBSENDEMAILACTION'); ?></button>
	</div>
</div>
...
jQuery(function() {
	...
	jQuery("body").on("click", ".vbo-custsearchres-entry", function() {
		...
		if (confirm('<?php echo addslashes(JText::_('VBOASSIGNNEWCUSTCONF')); ?>')) {
	...
	if (otaBidEl) {
		otaBidEl.addEventListener('click', () => {
			...
			VBOCore.copyToClipboard(copyEl).then((success) => {
				otaBidEl.setAttribute('data-tooltiptext', <?php echo json_encode(JText::_('VBO_COPIED') . '!'); ?>);
			}).catch((err) => {
				// do nothing
			});
		});
