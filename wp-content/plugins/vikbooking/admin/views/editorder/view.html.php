...
class VikBookingViewEditorder extends JViewVikBooking
{
	public function display($tpl = null)
	{
		...
		if (($pnewcancfee > 0 || ($pnewcancfee == 0 && is_float($pnewcancfee))) && (float)$row['canc_fee'] != $pnewcancfee) {
			...
			VikBooking::getBookingHistoryInstance()->setBid($row['id'])->store('MB', sprintf(JText::_('VBO_CANC_FEE') . ': %s', VikBooking::numberFormat($pnewcancfee)));
		}
							...
							foreach ($room_params['features'] as $origrind => $rfeatures) {
								if ($orr['roomindex'] == $origrind) {
									foreach ($rfeatures as $fname => $fval) {
										if (strlen($fval)) {
											$prev_subindex = '#' . $origrind . ' - ' . JText::_($fname) . ': ' . $fval;
											break;
										}
									}
								}
								if ($rind == $origrind) {
									foreach ($rfeatures as $fname => $fval) {
										if (strlen($fval)) {
											$new_subindex = '#' . $origrind . ' - ' . JText::_($fname) . ': ' . $fval;
											break;
										}
									}
								}
	...
	protected function addToolBar() {
		...
		JToolBarHelper::custom( 'prev_booking', 'backward', 'backward', JText::_('VBJQCALPREV'), false);
		JToolBarHelper::spacer();
		JToolBarHelper::custom( 'next_booking', 'forward', 'forward', JText::_('VBJQCALNEXT'), false);
		JToolBarHelper::spacer();
	}
}
