...
<div class="vbo-ratesoverview-top-outer">
	<div class="vbo-ratesoverview-top-outer-inner vbo-toggle-small">
		<?php echo $vbo_app->printYesNoButtons('vbo_collapse', JText::_('VBYES'), JText::_('VBNO'), (!$collapse_status ? 1 : 0), 1, 0, "vboToggleCollapse();"); ?>
		<span onclick="jQuery(this).parent().find('input').trigger('click');"><?php echo JText::_('VBO_EXPAND'); ?></span>
	</div>
</div>
	...
	<div class="vbo-ratesoverview-roomsel-block">
		<form method="get" action="index.php?option=com_vikbooking" name="vboratesovwform">
			...
			<div class="vbo-ratesoverview-roomsel-entry vbo-ratesoverview-roomsel-entry-chrooms">
				<label for="roomsel"><?php VikBookingIcons::e('bed'); ?> <?php echo JText::_('VBPVIEWORDERSTHREE'); ?></label>
		...
		if (count($all_rooms) > 1 && count($categories) > 1) {
			?>
			<optgroup label="<?php echo addslashes(JText::_('VBOCATEGORYFILTER')); ?>">
			<?php
		...
		?>
				...
				<button type="button" class="btn vbo-config-btn" title="<?php echo htmlspecialchars(JText::_('VBODRIVERLOAD')); ?>" onclick="document.vboratesovwform.submit();"><i class="vboicn-loop2"></i></button>
			<?php
			if (count($all_rooms) > 1 && count($all_rooms) < 50 && count($this->req_room_ids) < count($all_rooms)) {
				...
				<div class="vbo-roverview-selall-rooms">
					<button type="button" class="btn vbo-config-btn" onclick="vboSelectAllRooms(this);"><?php VikBookingIcons::e('layer-group'); ?> <?php echo JText::_('VBOSELECTALL'); ?></button>
				</div>
				<?php
			}
			?>
			...
			<div class="vbo-ratesoverview-roomsel-entry vbo-ratesoverview-roomsel-entry-calc">
				<label for="checkindate" onclick="vboCheckCollapse();"><?php VikBookingIcons::e('calculator'); ?> <?php echo JText::_('VBRATESOVWRATESCALCULATOR'); ?></label>
				<div class="vbo-ratesoverview-roomsel-entry-calc-inner"<?php echo $collapse_status ? ' style="display: none;"' : ''; ?>>
					...
					<span class="vbo-ratesoverview-entryinline">
						<div class="vbo-field-calendar">
							<div class="input-append">
								<input type="text" autocomplete="off" value="" class="vbo-roverv-rcalc-cal" size="10" id="checkindate" name="checkindate" placeholder="<?php echo JText::_('VBPICKUPAT'); ?>" />
					...
					<span class="vbo-ratesoverview-entryinline">
						<div class="vbo-field-calendar">
							<div class="input-append">
								<input type="text" autocomplete="off" value="" class="vbo-roverv-rcalc-cal" size="10" id="checkoutdate" name="checkoutdate" placeholder="<?php echo JText::_('VBRELEASEAT'); ?>" />
								<button type="button" class="btn btn-secondary" id="checkoutdate-trig"><?php VikBookingIcons::e('calendar', 'icn-nomargin'); ?></button>
							</div>
						</div>
					</span>
					<span class="vbo-ratesoverview-entryinline"><span><?php echo JText::_('VBRATESOVWRATESCALCNUMNIGHTS'); ?></span> <input type="number" id="vbo-numnights" value="1" min="1" max="365" step="1" /></span>
					<span class="vbo-ratesoverview-entryinline"><span><?php echo JText::_('VBRATESOVWRATESCALCNUMADULTS'); ?></span> <input type="number" id="vbo-numadults" value="<?php echo ($roomrows[$this->firstroom]['fromadult'] ?? ''); ?>" step="1"/></span>
					<span class="vbo-ratesoverview-entryinline"><span><?php echo JText::_('VBRATESOVWRATESCALCNUMCHILDREN'); ?></span> <input type="number" id="vbo-numchildren" value="<?php echo ($roomrows[$this->firstroom]['fromchild'] ?? ''); ?>" step="1"/></span>
					<span class="vbo-ratesoverview-entryinline"><button type="button" class="btn vbo-config-btn" id="vbo-ratesoverview-calculate"><?php echo JText::_('VBRATESOVWRATESCALCULATORCALC'); ?></button></span>
				</div>
			...
			<div class="vbo-ratesoverview-roomsel-entry vbo-ratesoverview-roomsel-entry-forecast"<?php echo (!empty($cookie_tab) && $cookie_tab != 'cal' ? ' style="display: none;"' : ''); ?>>
				<label onclick="vboCheckCollapse();"><?php VikBookingIcons::e('cloud-sun-rain'); ?> <?php echo JText::_('VBOFORECAST'); ?></label>
		...
		?>
				...
				<button type="button" class="btn vbo-config-btn vbo-apply-los-btn" onclick="document.vboratesovwform.submit();"><?php echo JText::_('VBRATESOVWAPPLYLOS'); ?></button>
			</div>
		</form>
	</div>
				...
				<table class="vboverviewtable vbratesoverviewtable vbo-table" data-idroom="<?php echo $rid; ?>">
					...
					foreach ($this->roomrates[$rid] as $roomrate) {
						...
						<tr class="vbo-roverviewtablerow" id="vbo-roverw-<?php echo $roomrate['id'].'-'.$roomrate['idroom']; ?>" data-idprice="<?php echo $roomrate['idprice']; ?>">
							...
							if ($rplan_minlos || $rplan_haslos) {
								...
								if ($rplan_minlos) {
									?>
									<span class="vbo-roverv-rplan-restrictions-lbl"><?php echo JText::_('VBOMINIMUMSTAY'); ?></span>
						...
						<tr class="vbo-roverviewtablerow-occupancy">
							<td>
								<span class="vbo-rplan-name vbo-rplan-name-occ">
									<span><?php echo JText::_('VBPVIEWOPTIONALSTHREE'); ?></span> 
					...
					?>
						<tr class="vbo-roverviewtableavrow">
							<td><span class="vbo-roverview-roomunits"><?php echo $roomrow['units']; ?></span><span class="vbo-roverview-uleftlbl"><?php echo JText::_('VBPCHOOSEBUSYCAVAIL'); ?></span></td>
						<?php
						$nowts = getdate($tsstart);
						$cell_count = 0;
						$last_booking_info = [];
					...
					?>
						<tr class="vbo-roverviewtablerow-restrs">
							<td><span class="vbo-roverview-restrs"><?php echo JText::_('VBOMINIMUMSTAY'); ?></span></td>
						<?php
						$nowts = getdate($tsstart);
						$cell_count = 0;
			...
			<div class="vbo-ratesoverview-period-container" style="<?php echo $collapse_status ? 'display: none;' : ''; ?>">
						...
						<div class="vbo-ratesoverview-period-box-cals" style="display: none;">
							<div class="vbo-ratesoverview-period-box-cals-inner">
								<div class="vbo-ratesoverview-period-cal-left">
									<h4><?php echo JText::_('VBOROVWSELPERIODFROM') . ' / ' . JText::_('VBOROVWSELPERIODTO'); ?></h4>
				...
				<div class="vbo-ratesoverview-orphans-wrapper" id="vbo-ratesoverview-orphans-wrapper-<?php echo $rid; ?>" style="display: none;">
					<div class="vbo-ratesoverview-orphans-lbl">
						<span><?php echo $vbo_app->createPopover(array('title' => JText::_('VBORPHANSFOUND'), 'content' => JText::_('VBORPHANSFOUNDSHELP'), 'icon_class' => VikBookingIcons::i('exclamation-triangle'))); ?> <?php echo JText::_('VBORPHANSFOUND'); ?></span>
					</div>
...
<div class="vbo-ratesoverview-newratesrestr-helper" style="display: none;">
	...
	<div class="vbo-ratesoverview-newratesrestr-wrap">
		...
		<div class="vbo-roverw-setnewrate">
			<div class="vbo-roverw-setnewrate-title">
				...
				<div class="vbo-roverw-setnewrate-skip-derived vbo-toggle-small vbo-toggle-mini" style="display: none;">
					<label for="roverw-skip-derived-on" class="vbo-roverw-setnewrate-skip-derived-lbl"><?php echo JText::_('VBO_SKIP_DERIVED_RPLANS'); ?></label>
					<?php echo $vbo_app->printYesNoButtons('roverw-skip-derived', JText::_('VBYES'), JText::_('VBNO'), 0, 1, 0); ?>
				</div>
			</div>
					...
					<h4>
						<button type="button" class="vbo-btn-transparent vbo-context-menu-rate-type">
							<span class="vbo-transparent-wrap">
								...
								<span class="vbo-roverw-action-rates-title"><?php echo JText::_('VBRATESOVWSETNEWRATE'); ?></span>
			...
			<div class="vbo-roverw-setnewrate-inner">
				<div class="vbo-roverw-setnewrate-vcm">
					...
					<div class="vbo-roverw-setnewrate-vcm-body vbo-toggle-small">
						<?php
						echo $vbo_app->printYesNoButtons('roverw-newrate-vcm', JText::_('VBYES'), JText::_('VBNO'), ($vcm_enabled > 0 ? 1 : 0), 1, 0, 'vboCheckVcmRestrictions();', ['blue']);
		...
		<div class="vbo-roverw-closeopenrp">
			<h4><i class="vboicn-switch"></i><?php echo JText::_('VBRATESOVWCLOSEOPENRRP'); ?> <span id="rovervw-closeopen-rplan"></span> <span class="vbo-roverw-closeopenrp-arrow"><?php VikBookingIcons::e('chevron-down'); ?></span></h4>
			<div class="vbo-roverw-closeopenrp-btns">
				<button type="button" class="btn btn-danger" onclick="modRoomRatePlan('close');"><?php VikBookingIcons::e('lock'); ?> <?php echo JText::_('VBRATESOVWCLOSERRP'); ?></button>
				<button type="button" class="btn btn-success" onclick="modRoomRatePlan('open');"><?php VikBookingIcons::e('lock-open'); ?> <?php echo JText::_('VBRATESOVWOPENRRP'); ?></button>
			</div>
		</div>
	</div>
...
var roverw_messages = {
	...
	"notesForRoom": "<?php echo addslashes(JText::_('VBONOTESFORROOM')); ?>"
};
...
jQuery(function() {
	...
	btnOtaRatesEl.innerHTML = '<span>' + <?php echo json_encode(JText::_('VBO_SHOW_OTA_RATES')); ?> + '</span>';
	...
	icnOtaRatesWrapEl.setAttribute('data-tooltiptext', <?php echo json_encode(JText::_('VBO_REMEMBER_PREF')); ?>);
		...
		buttons: [
			{
				...
				text: <?php echo json_encode(JText::_('VBOROVERVOBP')); ?>,
				separator: true,
				icon: function() {
						...
						(res) => {
							...
							try {
								...
								if (!obj_res?.records) {
									...
									throw new Error(<?php echo json_encode(JText::_('VBO_NO_RECORDS_FOUND')); ?>);
								}
	...
	const fixedRateLbl = <?php echo json_encode(JText::_('VBRATESOVWSETNEWRATE')); ?>;
		...
		VBOCore.doAjax(
			...
			(res) => {
				...
				if (res.indexOf('e4j.error') >= 0 ) {
					...
					var titlecont = '<span class="vbo-ratesoverview-calculation-response-room-name">'+jQuery("#roomselcalc option:selected").text() + '</span> ' + checkindate + ' - ' + jQuery("#checkoutdate").val() + ', ' + nights + ' <?php echo addslashes(JText::_('VBRATESOVWRATESCALCNUMNIGHTS')); ?>, ' + adults + ' <?php echo addslashes(JText::_('VBRATESOVWRATESCALCNUMADULTS')); ?>';
