...
<form action="index.php?option=com_vikbooking" method="post" name="adminForm" id="adminForm">
	<div class="vbo-bookdet-container">
		...
		<div class="vbo-bookdet-wrap">
			...
			if ($missing_customer_det) {
				echo $vbo_app->createPopover(array('title' => JText::_('VBCUSTOMERMISSIMPDET'), 'content' => JText::_('VBCUSTOMERMISSIMPDETHELP'), 'icon_class' => VikBookingIcons::i('exclamation-triangle'), 'placement' => 'bottom'));
		...
		<div class="vbo-bookdet-wrap">
			<div class="vbo-bookdet-head">
				<span><?php echo JText::_('VBO_PREVIOUS_CHECKINS'); ?></span>
			</div>
			<div class="vbo-bookdet-foot">
				<button type="button" class="btn btn-small btn-primary vbo-context-menu-btn vbo-context-menu-prevchkins">
					<span class="vbo-context-menu-lbl"><?php echo JText::_('VBO_SELECT'); ?></span>
						...
						?>
							<div class="vbo-roomdet-guests-toggle-cont">
								<span tabindex="0" class="vbo-roomdet-guests-toggle <?php echo $count_pax_data >= $guests_to_register ? 'vbo-guestscount-complete' : 'vbo-guestscount-incomplete'; ?>" data-roomind="<?php echo $ind; ?>">
									...
									<span class="vbo-roomdet-guests-toggleword"><?php echo JText::_('VBOGUESTSDETAILS'); ?> (<span id="vbo-guestscount-<?php echo $ind; ?>"><?php echo $count_pax_data; ?></span>/<?php echo $guests_to_register; ?>)</span>
								</span>
							</div>
						</div>
					</div>
	...
	<div class="vbo-checkin-update-wrap">
		<div class="vbo-checkin-update-inner">
			<div>
				<button type="button" class="btn btn-primary" onclick="jQuery('#adminForm').submit();"><i class="vboicn-checkmark"></i> <?php echo JText::_('VBOCHECKINUPDATEBTN'); ?></button>
			</div>
		</div>
	</div>
		...
		if ($today_midnight < $order['checkout'] || $grace_ts < $order['checkout']) {
			...
			<div class="vbo-checkin-commands-btn">
				<button type="button" onclick="vboSetCheckinAction(-1);" class="btn btn-large btn-danger"><i class="vboicn-blocked"></i> <?php echo JText::_('VBOSETCHECKEDSTATUSNOS'); ?></button>
					...
					case 0:
						?>
			<div class="vbo-checkin-commands-btn">
				<button type="button" onclick="vboSetCheckinAction(0);" class="btn btn-large btn-secondary"><i class="vboicn-cancel-circle"></i> <?php echo JText::_('VBOSETCHECKEDSTATUSZERO'); ?></button>
			</div>
						<?php
						break;
					case 1:
			...
			<div class="vbo-checkin-commands-btn">
				<button type="button" class="btn btn-large btn-danger vbo-context-menu-btn vbo-context-menu-otanoshow">
					<span class="vbo-context-menu-lbl"><?php VikBookingIcons::e('ban'); ?> <?php echo JText::_('VBOBCOMREPORTNOSHOW'); ?></span>
					<span class="vbo-context-menu-ico"><?php VikBookingIcons::e('sort-down'); ?></span>
				</button>
			</div>
			...
			<script type="text/javascript">
				jQuery(function() {
					...
					const notify_noshow = function() {
						if (confirm('<?php echo addslashes(JText::_('VBOBCOMREPORTNOSHOWCONF')); ?>')) {
							// show inner modal
						...
						buttons: [
							{
								...
								text: '<?php echo addslashes(JText::_('VBO_DONT_WAIVE_FEES')); ?>',
							...
							{
								icon: '<?php echo VikBookingIcons::i('ban'); ?>',
								text: '<?php echo addslashes(JText::_('VBO_DO_WAIVE_FEES')); ?>',
	...
	vbo_overlay_data['paymentlog'] = '<div class="vcm-notif-pcidrq-container">'+
			'<button type="button" class="btn vbo-config-btn vcm-pcid-launch" onclick="vboUpdateModal(\'<?php echo addslashes(JText::_('VBPAYMENTLOGTOGGLE')); ?>\', pci_vcm_frame, false);">'+
	...
	if (count($customer)) {
		 ...
		 */
		?>
	vbo_overlay_data['customer_info'] = '<a class="btn btn-primary pull-right" href="index.php?option=com_vikbooking&task=editcustomer&cid[]=<?php echo $customer['id'].'&checkin=1&bid='.$order['id'].($tmpl == 'component' ? '&tmpl=component' : ''); ?>"><i class="vboicn-profile"></i> <?php echo addslashes(JText::_('VBMAINCUSTOMEREDIT')); ?></a>'+
