...
<form action="index.php?option=com_vikbooking" method="post" name="adminForm" id="adminForm">
		...
		if ($tmpl == 'component') {
			//back button
			?>
	<a class="btn pull-left" href="index.php?option=com_vikbooking&task=bookingcheckin&cid[]=<?php echo $order['id']; ?>&tmpl=component"><i class="vboicn-undo"></i> <?php echo JText::_('VBBACK'); ?></a>
			<?php
		}
		?>
	<div class="vbo-bookdet-container">
		...
		<div class="vbo-bookdet-wrap">
			...
			if (!empty($customer['comments'])) {
				?>
				<i class="vboicn-info icn-bigger icn-nomargin icn-float-left icn-clickable" onclick="vboUpdateModal('<?php echo addslashes(JText::_('VBOTOGGLECHECKINNOTES')); ?>', '.comments', true);"></i>
				<?php
			}
			?>
			</div>
		...
		<div class="vbo-bookdet-wrap">
			<div class="vbo-bookdet-head">
				<span><?php echo JText::_('VBEDITORDERNINE'); ?></span>
			</div>
			<div class="vbo-bookdet-foot">
			<?php
		...
		<div class="vbo-bookdet-wrap vbo-bookdet-wrap-special">
			...
			if ($order['checked'] < 0) {
				//no show
				$checked_status = '<span class="label label-error">'.JText::_('VBOCHECKEDSTATUSNOS').'</span>';
	...
	<div class="vbo-signature-container" id="fake-signature-container">
		<div class="vbo-signature-pad">
			<div class="vbo-signature-pad-head">
				<p class="vbo-current-signature-p"><?php echo JText::_('VBOCURRENTSIGNATURE'); ?></p>
			</div>
			...
			<div class="vbo-signature-pad-footer">
				...
				<div class="vbo-signature-cmds">
					<div class="vbo-signature-cmd">
						<button type="button" class="btn btn-large btn-success" onclick="vboConfirmGenerate(-1);"><i class="vboicn-checkmark"></i> <?php echo JText::_('VBOGENCHECKINDOC'); ?></button>
					</div>
					<div class="vbo-signature-cmd">
						<button type="button" class="btn btn-large btn-success" onclick="vboShowSignPad();"><i class="vboicn-quill"></i> <?php echo JText::_('VBOSIGNATUREAGAIN'); ?></button>
					</div>
				</div>
			</div>
		</div>
	...
	<div class="vbo-signature-container" id="real-signature-container" style="<?php echo $signpad_style; ?>">
		<div id="vbo-signature-pad" class="vbo-signature-pad">
			<div class="vbo-signature-pad-head">
				<div class="pull-left">
					<button type="button" id="signature_link" class="btn <?php echo $prefreshing > 0 ? 'btn-success' : 'btn-primary'; ?>" onclick="vboStopRefresh();vboUpdateModal('<?php echo addslashes(JText::_('VBOSIGNATURESHARE')); ?>', '.signature_link', true);"><i class="vboicn-link"></i> <?php echo JText::_('VBOSIGNATURESHARE'); ?></button>
				</div>
				<div class="pull-right">
					<button type="button" class="btn" onclick="vboConfirmGenerate(0);"><i class="vboicn-cross"></i> <?php echo JText::_('VBOSIGNATURESKIP'); ?></button>
				</div>
			<?php
			if (!$empty_termsconds) {
			?>
				<div class="vbo-signature-pad-head-terms">
					<a href="javascript: void(0);" onclick="vboUpdateModal('<?php echo addslashes(JText::_('VBOTERMSCONDS')); ?>', '.termsconds', true);"><i class="vboicn-link"></i> <?php echo JText::_('VBOTERMSCONDSIACCEPT'); ?></a>
			...
			<div class="vbo-signature-pad-footer">
				<div class="vbo-signature-signabove">
					<span><i class="vboicn-quill"></i> <?php echo JText::_('VBOSIGNATURESIGNABOVE'); ?></span>
				</div>
				<div class="vbo-signature-cmds">
					<div class="vbo-signature-cmd">
						<button type="button" class="btn btn-large btn-success" onclick="vboConfirmGenerate(1);"><i class="vboicn-checkmark"></i> <?php echo JText::_('VBOSIGNATURESAVE'); ?></button>
					</div>
					<div class="vbo-signature-cmd">
						<button type="button" class="btn btn-large btn-danger" onclick="vboClearSignPad();"><i class="vboicn-bin"></i> <?php echo JText::_('VBOSIGNATURECLEAR'); ?></button>
					</div>
				</div>
			</div>
		</div>
...
<script type="text/javascript">
	...
	vbo_overlay_data['signature_link'] = '<div class="vbo-sign-share-meth-cont">'+
		'<div class="vbo-sign-share-meth">'+
			'<label for="vbo-share-email"><?php echo addslashes(JText::_('VBOSIGNSHAREEMAIL')); ?></label>'+
			...
			'<button type="button" class="btn btn-primary" onclick="vboSendSignLink(\'email\');"><i class="vboicn-envelop"></i> <?php echo addslashes(JText::_('VBOSIGNSENDLINK')); ?></button>'+
		'</div>'+
		'<div class="vbo-sign-share-meth">'+
			'<label for="vbo-share-sms"><?php echo addslashes(JText::_('VBOSIGNSHARESMS')); ?></label>'+
			...
			'<button type="button" class="btn btn-primary" onclick="vboSendSignLink(\'sms\');"><i class="vboicn-mobile"></i> <?php echo addslashes(JText::_('VBOSIGNSENDLINK')); ?></button>'+
		'</div>'+
		'<div class="vbo-sign-share-meth">'+
		...
		'<div class="vbo-sign-share-meth vbo-sign-share-meth-close">'+
			'<span><button type="button" class="btn btn-secondary" onclick="vboCloseModal();"><?php echo addslashes(JText::_('VBANNULLA')); ?></button></span>'+
		'</div>'+
	'</div>';
	<?php
	if (!$empty_termsconds) {
		$termsconds = VikBooking::strTrimLiteral($termsconds);
		$terms_acc_close = VikBooking::strTrimLiteral(JText::_('VBOTERMSCONDSACCCLOSE'));
		?>
	...
	refreshing = setInterval(function() {
		jQuery('#signature_link').html(jQuery('#signature_link').html().replace('<?php echo addslashes(JText::_('VBOSIGNATURESHARE')); ?>', '<?php echo addslashes(JText::_('VBORELOADING')); ?>'));
		setTimeout(function() {
...
<script type="text/javascript">
	...
	function vboSendSignLink(method) {
		...
		if (!sendto || !sendto.value.length) {
			alert('<?php echo addslashes(JText::_('VBOSIGNSHAREEMPTY')); ?>');
			return false;
		}
		jQuery.ajax({
			type: 'POST',
	...
	function vboConfirmGenerate(action) {
		if (action > 0) {
			if (signaturePad.isEmpty()) {
				alert('<?php echo addslashes(JText::_('VBOSIGNATUREISEMPTY')); ?>');
				return false;
			}
			if (document.getElementById('termsconds') && !document.getElementById('termsconds').checked) {
				alert('<?php echo addslashes(JText::_('VBOSIGNMUSTACCEPT')); ?>');
				return false;
			}
			...
			var proceed_txt = '<?php echo addslashes(JText::_('VBONOSIGNATURECONF')); ?>';
