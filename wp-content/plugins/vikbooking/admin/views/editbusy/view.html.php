...
class VikBookingViewEditbusy extends JViewVikBooking
{
	...
	protected function addToolBar()
	{
		...
		if ($pfrominv > 0 && JFactory::getUser()->authorise('core.edit', 'com_vikbooking')) {
			JToolBarHelper::apply( 'updatebusydoinv', JText::_('VBSAVEANDDOINV'));
		...
		if ($pgoto == 'overv') {
			JToolBarHelper::custom( 'cancelbusy', 'back', 'back', JText::_('VBOVIEWBOOKINGDET'), false, false);
		}
		if ($pgoto == 'tableaux') {
			JToolBarHelper::custom( 'canceltableaux', 'back', 'back', JText::_('VBMENUTABLEAUX'), false, false);
		}
		JToolBarHelper::cancel( ($pgoto == 'overv' ? 'canceloverv' : 'cancelbusy'), JText::_('VBBACK'));
