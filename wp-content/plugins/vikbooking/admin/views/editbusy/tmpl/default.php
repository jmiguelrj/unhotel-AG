...
$status_type = '';
if (!empty($ord['type'])) {
	$status_type = JText::_('VBO_BTYPE_' . strtoupper($ord['type']));
...
if ($ord['status'] == "confirmed") {
	$saystaus = '<span class="label label-success">' . $status_type . JText::_('VBCONFIRMED') . '</span>';
} elseif ($ord['status']=="standby") {
	$saystaus = '<span class="label label-warning">' . $status_type . JText::_('VBSTANDBY') . '</span>';
} else {
	$saystaus = '<span class="label label-error">' . $status_type . JText::_('VBCANCELLED') . '</span>';
}
...
if ($ivas) {
	$wiva = "<select class=\"vbo-editbusy-custom-rate-element vbo-editbusy-custom-rate-element-vat\" name=\"aliq%s\"><option value=\"\">".JText::_('VBNEWOPTFOUR')."</option>\n";
...
if ($all_rate_plans) {
	...
	$wcancpolicyid .= '<option value="">- ' . JText::_('VBNEWPRICECANCPOLICY') . ' -</option>' . "\n";
...
Joomla.submitbutton = function(task) {
	if ( task == 'removebusy' ) {
		if (confirm('<?php echo htmlspecialchars(JText::_('VBDELCONFIRM'), ENT_QUOTES | ENT_SUBSTITUTE | ENT_HTML401); ?>')) {
...
var vboMessages = {
	...
	extracnameph: "<?php echo htmlspecialchars(JText::_('VBPEDITBUSYEXTRACNAME')); ?>",
	taxoptions : "<?php echo $jstaxopts; ?>",
	cantaddroom: "<?php echo htmlspecialchars(JText::_('VBOBOOKCANTADDROOM')); ?>"
};
	...
	<div class="vbo-modal-servsearch-form-wrap">
		<div class="vbo-modal-servsearch-input">
			<div class="input-append">
				...
				<button type="button" class="btn vbo-config-btn"><?php VikBookingIcons::e('search'); ?> <?php echo JText::_('VBODASHSEARCHKEYS'); ?></button>
			</div>
		</div>
...
<div class="vbo-bookingdet-topcontainer vbo-editbooking-topcontainer">
	<form name="adminForm" id="adminForm" action="index.php" method="post">
		...
		if (!empty($ord['channel']) && !empty($ord['idorderota'])) {
			?>
		<p class="warn"><?php echo JText::_('VBO_WARN_CM_RES_MOD'); ?></p>
			<?php
		}
		?>
		...
		<div class="vbo-bookingdet-innertop">
			<div class="vbo-bookingdet-commands">
				<div class="vbo-bookingdet-command">
					<button type="button" class="btn btn-danger" onclick="if (confirm('<?php echo addslashes(JText::_('VBDELCONFIRM')); ?>')){document.location.href='index.php?option=com_vikbooking&task=removeorders&goto=<?php echo $ord['id']; ?>&cid[]=<?php echo $ord['id']; ?>';}"><?php VikBookingIcons::e('trash'); ?> <?php echo $ord['status'] == 'cancelled' ? JText::_('VBO_PURGERM_RESERVATION') : JText::_('VBMAINEBUSYDEL'); ?></button>
				</div>
			</div>
				...
				<div class="vbo-bookingdet-customer">
					...
					<div class="vbo-bookingdet-detcont">
						...
						<div class="vbo-bookingdet-checkdt vbo-editbook-force-av" style="<?php echo !$canforce ? 'display: none;' : ''; ?>">
							<label for="forcebooking-on">
								<?php echo $vbo_app->createPopover(array('title' => JText::_('VBO_FORCE_BOOKDATES'), 'content' => JText::_('VBO_FORCE_BOOKDATES_HELP'))); ?>
								<?php echo JText::_('VBO_FORCE_BOOKDATES'); ?>
								<span class="label label-danger" data-alert="force-no-availability" style="display: none;"><?php echo JText::_('VBOVERVIEWLEGRED'); ?></span>
							</label>
							<div>
								<?php echo $vbo_app->printYesNoButtons('forcebooking', JText::_('VBYES'), JText::_('VBNO'), 0, 1, 0); ?>
							</div>
						</div>
					...
					if (!empty($otachannel_name) && class_exists('VCMOtaReporting') && VCMOtaReporting::getInstance($ord)->stayChangeAllowed()) {
						...
						<div class="vbo-bookingdet-checkdt vbo-editbook-report-stay-change-ota">
							...
							<div>
								<?php echo $vbo_app->printYesNoButtons('ota_stay_change_all', JText::_('VBYES'), JText::_('VBNO'), 0, 1, 0, "jQuery('#vbo-report-stay-change-ota-allr').toggle();"); ?>
							</div>
					...
					foreach ($ordersrooms as $kor => $or) {
						...
						<div class="vbo-bookingdet-summary-room vbo-editbooking-summary-room">
							...
							if ($ord['roomsnum'] > 1) {
								...
								<div class="vbo-editbooking-room-remove pro-feature">
									<button type="button" class="btn btn-danger" onclick="vboConfirmRmRoom('<?php echo $or['id']; ?>');"><?php VikBookingIcons::e('times-circle'); ?> <?php echo JText::_('VBOREMOVEROOM'); ?></button>
								</div>
								<?php
							}
							$switch_code = '';
						...
						if ($ord['split_stay'] && count($room_stay_dates) && isset($room_stay_dates[$kor]) && $room_stay_dates[$kor]['idroom'] == $or['idroom']) {
							...
							<div class="vbo-editbooking-room-splitstay">
								<h4>
									<span class="vbo-editbooking-room-splitstay-first"><?php VikBookingIcons::e('random'); ?> <?php echo JText::_('VBO_SPLIT_STAY'); ?></span>
									<span class="vbo-editbooking-room-splitstay-second"><?php VikBookingIcons::e('moon'); ?> <?php echo $room_stay_nights . ' ' . ($room_stay_nights > 1 ? JText::_('VBDAYS') : JText::_('VBDAY')); ?></span>
								</h4>
								<div class="vbo-editbooking-room-traveler-guestsinfo vbo-editbooking-room-splitstay-details">
									<div class="vbo-editbooking-room-splitstay-date vbo-editbooking-room-splitstay-date-in">
										<label for="room-splitstay-checkin<?php echo $num; ?>"><?php echo JText::_('VBPEDITBUSYFOUR'); ?></label>
									...
									<div class="vbo-editbooking-room-splitstay-date vbo-editbooking-room-splitstay-date-out">
										<label for="room-splitstay-checkout<?php echo $num; ?>"><?php echo JText::_('VBPEDITBUSYSIX'); ?></label>
						...
						if (!$ord['split_stay'] && !$ord['closure'] && $ord['roomsnum'] > 1 && $ord['days'] > 1 && $ord['status'] == 'confirmed') {
							...
							<div class="vbo-editbooking-room-nights-info">
								<div class="vbo-editbooking-room-nights-info-top">
									<h4>
										<span class="vbo-editbooking-room-nights-txt"><?php VikBookingIcons::e('moon'); ?> <?php echo $room_now_nights . ' ' . ($room_now_nights > 1 ? JText::_('VBDAYS') : JText::_('VBDAY')); ?></span>
									</h4>
									<div class="vbo-editbooking-room-nights-modify">
										<label for="room_modify_dates<?php echo $kor; ?>-on"><?php echo JText::_('VBO_MODIFY_DATES'); ?></label>
										<span><?php echo $vbo_app->printYesNoButtons('room_modify_dates' . $kor, JText::_('VBYES'), JText::_('VBNO'), $room_mod_dates_state, 1, 0, 'vboEditbusyModifyRoomDates(this.checked, ' . $kor . ')'); ?></span>
									</div>
								</div>
								<div class="vbo-editbooking-room-traveler-guestsinfo vbo-editbooking-room-nights-modify-details" data-roomind="<?php echo $kor; ?>" style="display: none;">
									<div class="vbo-editbooking-room-modify-date vbo-editbooking-room-modify-date-in">
										<label for="room-modify-checkin<?php echo $num; ?>"><?php echo JText::_('VBPEDITBUSYFOUR'); ?></label>
									...
									<div class="vbo-editbooking-room-modify-date vbo-editbooking-room-modify-date-out">
										<label for="room-modify-checkout<?php echo $num; ?>"><?php echo JText::_('VBPEDITBUSYSIX'); ?></label>
										<div class="vbo-input-calendar-box">
									...
									<div class="vbo-editbooking-room-modify-date vbo-editbooking-room-modify-date-help">
										<?php echo $vbo_app->createPopover(array('title' => JText::_('VBO_MODIFY_DATES'), 'content' => JText::_('VBO_MODIFY_DATES_HELP'))); ?>
									</div>
								<?php
								/**
								...
								if (!empty($otachannel_name) && class_exists('VCMOtaReporting') && VCMOtaReporting::getInstance($ord)->stayChangeAllowed()) {
									...
									<div class="vbo-editbooking-room-modify-date vbo-editbook-report-stay-change-ota">
										...
										<div>
											<?php echo $vbo_app->printYesNoButtons('ota_stay_change_room_' . $kor, JText::_('VBYES'), JText::_('VBNO'), 0, 1, 0, "jQuery('#vbo-report-stay-change-ota-oner{$kor}').toggle();"); ?>
										</div>
						...
						?>
							<div class="vbo-editbooking-room-traveler">
								...
								<div class="vbo-editbooking-room-traveler-guestsinfo">
									...
									<div class="vbo-editbooking-room-traveler-guestnum">
										<label for="pets<?php echo $num; ?>"><?php VikBookingIcons::e('dog'); ?> <?php echo JText::_('VBO_PETS'); ?></label>
							...
							if ($is_package || $is_cust_cost) {
									...
									if ($t['id'] == $or['idtar'] && !empty($or['room_cost']) && VikBooking::numberFormat($or['room_cost']) != $format_cost) {
										...
										<div class="vbo-editbooking-room-pricetype-older">
											<div class="vbo-editbooking-room-pricetype-older-inner">
												<label for="olderpid<?php echo $num.$t['idprice']; ?>"><?php echo JText::_('VBOBOOKEDATPRICE') . ' ' . $vbo_app->createPopover(array('title' => JText::_('VBOBOOKEDATPRICE'), 'content' => JText::_('VBOBOOKEDATPRICEHELP'))); ?></label>
												<div class="vbo-editbooking-room-pricetype-cost">
								...
								?>
									<div class="vbo-editbooking-room-pricetype">
										<div class="vbo-editbooking-room-pricetype-inner">
											<label for="cust_cost<?php echo $num; ?>" class="vbo-custrate-lbl-add"><?php echo JText::_('VBOROOMCUSTRATEPLANADD'); ?></label>
											<div class="vbo-editbooking-room-pricetype-cost">
						...
						?>
							<div class="vbo-editbooking-room-services vbo-editbooking-room-meal-plans">
								<h4><?php echo JText::_('VBO_MEAL_PLANS_INCL'); ?></h4>
						...
						?>
							<div class="vbo-editbooking-room-extracosts pro-feature" id="vbo-ebusy-extracosts-<?php echo $num; ?>">
								<h4>
									...
									<button class="btn vbo-ebusy-addextracost" type="button" onclick="vboAddExtraCost('<?php echo $num; ?>');"><?php VikBookingIcons::e('plus-circle'); ?> <?php echo JText::_('VBPEDITBUSYADDEXTRAC'); ?></button>
								</h4>
							...
							if (!empty($or['extracosts'])) {
								...
								foreach ($cur_extra_costs as $eck => $ecv) {
									...
									<div class="vbo-editbooking-room-extracost">
										<div class="vbo-ebusy-extracosts-cellname">
											<input type="text" name="extracn[<?php echo $num; ?>][]" value="<?php echo JHtml::_('esc_attr', $ecv['name']); ?>" placeholder="<?php echo JHtml::_('esc_attr', JText::_('VBPEDITBUSYEXTRACNAME')); ?>" size="25" />
											<input type="hidden" name="extractype[<?php echo $num; ?>][]" value="<?php echo isset($ecv['type']) ? JHtml::_('esc_attr', $ecv['type']) : ''; ?>" />
											<input type="hidden" name="extracfk[<?php echo $num; ?>][]" value="<?php echo isset($ecv['fk']) ? (string)$ecv['fk'] : ''; ?>" />
											<input type="hidden" name="extracdata[<?php echo $num; ?>][]" value="<?php echo isset($ecv['data']) ? JHtml::_('esc_attr', json_encode($ecv['data'])) : ''; ?>" />
										</div>
						...
						<div class="vbo-bookingdet-summary-room vbo-editbooking-summary-room vbo-editbooking-summary-totpaid">
							<div class="vbo-editbooking-summary-room-head">
								<div class="vbo-editbooking-addroom pro-feature">
									<button class="btn btn-success" type="button" id="vbo-add-room"><?php VikBookingIcons::e('plus-circle'); ?> <?php echo JText::_('VBOBOOKADDROOM'); ?></button>
								</div>
								...
								<div class="vbo-editbooking-totpaid vbo-editbooking-totrefund">
									<label for="refund"><?php echo JText::_('VBO_AMOUNT_REFUNDED'); ?></label>
				...
				if ($proceedtars) {
					...
					foreach ($ordersrooms as $kor => $or) {
						...
						<div class="vbo-bookingdet-summary-room vbo-editbooking-summary-room">
							...
							<div class="vbo-editbooking-room-pricetypes">
								<h4><?php echo JText::_('VBPEDITBUSYSEVEN'); ?><?php echo $ord['closure'] < 1 && $ord['status'] != 'cancelled' ? '&nbsp;&nbsp; '.$vbo_app->createPopover(array('title' => JText::_('VBPEDITBUSYSEVEN'), 'content' => JText::_('VBOMISSPRTYPEROOMH'))) : ''; ?></h4>
								<div class="vbo-editbooking-room-pricetypes-wrap">
								<?php
