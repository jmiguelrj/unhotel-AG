	...
	>
		...
		<template slot="items">
			<template
				...
				<div class="list-table-item" v-if="breakByDays && ( ! itemsList[ index - 1 ] || itemsList[ index - 1 ].date !== item.date )" style="background:#f8f8f8;">
					<div class="list-table-item__cell" style="padding: 0 25px;">
						<h4 style="margin: 0; font-size: 15px; color: var(--wc-secondary-text)">{{ item.date }}</h4>
					</div>
				</div>
				<div
				...
				<div :class="classItem( item.group_ID )">
					<div
						v-for="column in columnsIDs"
						:class="[ 'list-table-item__cell', 'cell--' + column ]"
						@click="openPopupOnColumnClick( column, item )"
					>
						<a v-if="column === 'order_id' && getItemValue( item, column )" :href="getOrderLink( item[ column ] )" target="_blank">#{{ getItemValue( item, column ) }}</a>
						<span
