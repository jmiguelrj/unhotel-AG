	...
	>
		...
		<label>
			<input
				...
				v-model="breakByDays"
			>
			<?php esc_html_e( 'Group items by days', 'jet-appointments-booking' ); ?>
		</label><br>
		<label>
			<input
				type="checkbox"
				...
				v-model="groupView"
			>
			<?php esc_html_e( 'Group items by parents', 'jet-appointments-booking' ); ?>
		</label><br>
		<div class="jet-apb-date-format">
			<label>
				<?php 
				esc_html_e( 'Date Format', 'jet-appointments-booking' ); 
				?>
				<input
					type="text"
					name="listDateFormat"
					v-model="listDateFormat"
					@change="dateFormat"
				>
				<?php 
				echo sprintf(
					'<a href="https://wordpress.org/documentation/article/customize-date-and-time-format">' . esc_html__( 'Allowed format options', 'jet-appointments-booking' ) . '</a>'
				); 
				?>
			</label>
		</div>
		...
		<ul>
			<li v-for="(filter, index) in filters" v-if="'search' !== index">
				<label :key="index">
					<input
						type="checkbox"
						name="index"
						v-model="filters[index].visibility"
						@change="maybeRemoveFilter( filter.visibility, index )"
					>
					{{filter.label}}
				</label>
			</li>
