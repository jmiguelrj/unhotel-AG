...
/**
 * Google VR handler for the Promotions.
 ...
 * The promotions for Google VR are supposed to be synced automatically
 * with the website promotions, as long as this channel/handler is selected
 * during the creation of a new promotion ID on VBO.
 * 
 * @since 	1.9.4
 */
class VikChannelManagerPromoGooglevr extends VikChannelManagerPromo
{
	/**
	...
	public function __construct()
	{
		...
		$this->handler_name = 'Google VR';
		$this->handler_key = basename(__FILE__, '.php');
		$this->handler_logo = VCM_ADMIN_URI . 'assets/css/channels/googlevr.png';
	}
	...
	{
		$channel_info = VikChannelManager::getChannel(VikChannelManagerConfig::GOOGLEVR);
		if (!is_array($channel_info) || !$channel_info || empty($channel_info['params'])) {
			return false;
		}
	 ...
	 * Google VR also supports live updates of website promotions, in this case the
	 ...
	 * @param 	string 	$method 	"new", "update" or "delete" ("edit" not supported)
	 * 
	...
	public function createPromotion($data, $method)
	{
		...
		// get the PM account ID
		$pm_account_id = VCMFactory::getConfig()->get('account_key_' . VikChannelManagerConfig::GOOGLEVR);
		if (empty($pm_account_id)) {
			$this->setError('No account ID for ' . $this->getName());
			return false;
		}
		 ...
		 * For Google VR no multi accounts should be supported.
		 */
		$accounts_involved = [];
		if (count($mapped_rooms_acc) > 1) {
			...
			// just one account
			$accounts_involved[$pm_account_id] = $rooms;
		}

		// currently and previously involved listing IDs, if any
		$current_listing_ids = [];
		$previous_listing_ids = [];

		if ($method == 'update' || $method == 'delete') {
			/**
			 * Check if the previous actions were made on multiple accounts (should not happen with Google VR)
			 * and we should gather the list of nested Google VR promotion IDs (VBO_PROMO_ID-LISTING_ID).
			 * 
			 * Note that with Google VR it is not really possible to include/exclude a listing from an existing promotion
			 * that was previously created, because on Google VR we will create one promotion ID for each listing.
			 * In this case it would be required to delete the promotion and to re-create it just for the wanted listings.
			 */
			...
			if ($prev_data) {
				...
				foreach ($prev_data as $prev_promo) {
					...
					if (!empty($prev_promo['data']['listing_ids']) && is_array($prev_promo['data']['listing_ids'])) {
						$previous_listing_ids = array_merge($previous_listing_ids, $prev_promo['data']['listing_ids']);
					}
		...
		foreach ($accounts_involved as $use_hinv_id => $use_rooms) {
			...
			if ($method == 'delete' && $previous_listing_ids) {
				// we use the "auth" attribute to pass along the previously involved listing IDs
				$filters[] = 'auth="' . implode(',', array_map('intval', array_unique($previous_listing_ids))) . '"';
			}
			...
			if ($method != 'delete') {
				...
				foreach ($use_rooms as $r) {
					...
					// push current listing ID
					$current_listing_ids[] = $r;
				}
				$rooms_rates .= '</rooms>'."\n";
			...
			$e4jc_url = "https://slave.e4jconnect.com/channelmanager/?r=wprom&c=googlevr";
			
			$xml = '<?xml version="1.0" encoding="UTF-8"?>
<!-- VikChannelManager WPROM Request e4jConnect.com - Google VR -->
<WritePromotionRQ xmlns="http://www.e4jconnect.com/channels/wpromrq">
	...
	' . $write_promo_node . '
			...
			if ($method == 'new' && $current_listing_ids) {
				// make sure to store the involved listing IDs for later update or delete actions
				$data['listing_ids'] = $current_listing_ids;
			}
