...
if (count($rows) == 0) {
	...
	<div class="table-responsive vcmrelationshema">
		...
		foreach ($schema as $otak => $relval) {
			...
			if (count($room_info) > 0 && array_key_exists('RoomInfo', $room_info)) {
				$room_info_str .= '<p class="vcm-roominfo-paragraph">'.JText::_('VCMROOMSRELDETAILS').':</p>';
			...
			if (count($room_info) > 0 && array_key_exists('RatePlan', $room_info)) {
				$room_info_str .= '<p class="vcm-roominfo-paragraph">'.JText::_('VCMROOMSRELRPLANS').':</p>';
				foreach ($room_info['RatePlan'] as $rplan_k => $rplan_v) {
					...
					foreach ($rplan_v as $rinfo_k => $rinfo_v) {
						...
						$room_info_str .= '<div class="vcm-roominfo-entry"><span>'.ucwords(str_replace('_', ' ', $rinfo_k)).':</span>'.$rinfo_v.'</div>';
					}
					$room_info_str .= '</div>';
				}
			}
			...
			foreach ($relval as $relk => $rel) {
				...
				<td><img src="<?php echo VCM_ADMIN_URI; ?>assets/css/images/<?php echo $relimg; ?>"/></td>
	...
	function startRarRq() {
		...
		if(fromdate.length == 0) {
			jQuery("#fromdate").val(def_date).attr('data-alt-value', def_date);
			fromdate = def_date;
		}
		if(todate.length == 0) {
			jQuery("#todate").val(fromdate).attr('data-alt-value', fromdate);
			todate = fromdate;
		}
		var df_obj = new Date(fromdate);
		var td_obj = new Date('<?php echo $limit_date; ?>');
		if(df_obj < td_obj) {
			jQuery("#fromdate").val('<?php echo date('Y-m-d'); ?>').attr('data-alt-value', '<?php echo date('Y-m-d'); ?>');
	...
	if($fill_dates) {
		?>
		jQuery("#fromdate").val(def_date).attr('data-alt-value', def_date);
		jQuery("#todate").val(def_todate).attr('data-alt-value', def_todate);
	...
	<br clear="all"/>
	<span class="vcminlinedate"><?php echo JText::_('VCMFROMDATE'); ?> <?php echo JHTML::_('calendar', '', 'fromdate', 'fromdate', '%Y-%m-%d', array('class'=>'', 'size'=>'10',  'maxlength'=>'19', 'todayBtn' => 'true')); ?></span>
	<span class="vcminlinedate"><?php echo JText::_('VCMTODATE'); ?> <?php echo JHTML::_('calendar', '', 'todate', 'todate', '%Y-%m-%d', array('class'=>'', 'size'=>'10',  'maxlength'=>'19', 'todayBtn' => 'true')); ?></span>
<?php
if ($channel['uniquekey'] == VikChannelManagerConfig::AGODA || $channel['uniquekey'] == VikChannelManagerConfig::YCS50) {
	...
	foreach ($channel_pricing as $idroom => $rateplans) {
		...
		foreach ($rateplans['RatePlan'] as $rateplan) {
			...
			if (array_key_exists('vcm_default', $rateplan)) {
				$rateplan_info['vcm_default'] = 1;
			}
			$rooms_rate_plans[$idroom][$rateplan['id']] = $rateplan_info;
			if (!$rate_plans_caught) {
				$rate_plans_sel .= '<option value="'.$rateplan['id'].'"'.(array_key_exists('vcm_default', $rateplan) ? ' selected="selected"' : '').'>'.$rateplan['name'].' '.(array_key_exists('rate_type', $rateplan) ? '('.$rateplan['rate_type'].')' : '').'</option>'."\n";
			}
		}
		...
		if(rooms_rate_plans.hasOwnProperty(rtype_id)) {
			...
			for(var i in rooms_rate_plans[rtype_id]) {
				if(rooms_rate_plans[rtype_id].hasOwnProperty(i)) {
					allrateplans += "<option value=\""+rooms_rate_plans[rtype_id][i].id+"\" title=\"Tax Included: "+rooms_rate_plans[rtype_id][i].tax_included+"\""+(rooms_rate_plans[rtype_id][i].hasOwnProperty('vcm_default') ? " selected" : "")+">"+rooms_rate_plans[rtype_id][i].name+rooms_rate_plans[rtype_id][i].rate_type+"</option>";
				}
			}
	...
	function vcmSetRoomRatePlans(rtype_id) {
		...
		if(rooms_rate_plans.hasOwnProperty(rtype_id)) {
			...
			for(var i in rooms_rate_plans[rtype_id]) {
				if(rooms_rate_plans[rtype_id].hasOwnProperty(i)) {
					allrateplans += "<option value=\""+rooms_rate_plans[rtype_id][i].id+"\" title=\""+rooms_rate_plans[rtype_id][i].pricingModel+"\""+(rooms_rate_plans[rtype_id][i].hasOwnProperty('vcm_default') ? " selected" : "")+">- "+rooms_rate_plans[rtype_id][i].name+" ("+rooms_rate_plans[rtype_id][i].distributionModel+")</option>";
				}
			}
			jQuery("#ch_rateplanid").append(allrateplans);
		}
	...
	foreach ($channel_pricing as $idroom => $rateplans) {
		$room_types_sel .= '<option value="'.$idroom.'">'.$channel_rooms[$idroom].'</option>'."\n";
		if (!isset($rateplans['RatePlan'])) {
			continue;
		}
		foreach ($rateplans['RatePlan'] as $rateplan) {
			if (isset($rateplan['ChargeTypeCode'])) {
				if ((int)$rateplan['ChargeTypeCode'] == 19) {
					$rateplan['ChargeTypeCode'] = 'PerRoomPerNight';
				} elseif ((int)$rateplan['ChargeTypeCode'] == 21) {
					$rateplan['ChargeTypeCode'] = 'PerPersonPerNight';
				}
			}
			$rateplan_info = array(
				'id' => $rateplan['id'],
				'name' => $rateplan['name'],
				'charge_type' => (array_key_exists('ChargeTypeCode', $rateplan) ? ' ('.$rateplan['ChargeTypeCode'].')' : ''),
				'currencycode' => (array_key_exists('CurrencyCode', $rateplan) ? $rateplan['CurrencyCode'] : '')
			);
			if(array_key_exists('vcm_default', $rateplan)) {
				$rateplan_info['vcm_default'] = 1;
			}
			$rooms_rate_plans[$idroom][$rateplan['id']] = $rateplan_info;
			if(!$rate_plans_caught) {
				$rate_plans_sel .= '<option value="'.$rateplan['id'].'"'.(array_key_exists('vcm_default', $rateplan) ? ' selected="selected"' : '').' title="ID: '.$rateplan['id'].' - Currency: '.$rateplan_info['currencycode'].'">'.$rateplan['name'].(array_key_exists('charge_type', $rateplan_info) ? $rateplan_info['charge_type'] : '').'</option>'."\n";
			}
		}
		$rate_plans_caught = true;
	}
	...
	function vcmSetRoomRatePlans(rtype_id) {
		...
		if(rooms_rate_plans.hasOwnProperty(rtype_id)) {
			...
			for(var i in rooms_rate_plans[rtype_id]) {
				if(rooms_rate_plans[rtype_id].hasOwnProperty(i)) {
					allrateplans += "<option value=\""+rooms_rate_plans[rtype_id][i].id+"\" title=\"ID: "+rooms_rate_plans[rtype_id][i].id+" - Currency: "+rooms_rate_plans[rtype_id][i].currencycode+"\""+(rooms_rate_plans[rtype_id][i].hasOwnProperty('vcm_default') ? " selected" : "")+">"+rooms_rate_plans[rtype_id][i].name+rooms_rate_plans[rtype_id][i].charge_type+"</option>";
				}
			}
			jQuery("#ch_rateplanid").append(allrateplans);
		}
	...
	foreach ($channel_pricing as $idroom => $rateplans) {
		...
		foreach ($rateplans['RatePlan'] as $rateplan) {
			...
			$rooms_rate_plans[$idroom][$rateplan['id']] = $rateplan_info;
			if (!$rate_plans_caught) {
				$rate_plans_sel .= '<option value="'.$rateplan['id'].'"'.(array_key_exists('vcm_default', $rateplan) ? ' selected="selected"' : '').' title="ID: '.$rateplan['id'].'">'.$rateplan['name'].'</option>'."\n";
			}
		}
		$rate_plans_caught = true;
	}
	...
	function vcmSetRoomRatePlans(rtype_id) {
		...
		if(rooms_rate_plans.hasOwnProperty(rtype_id)) {
			...
			for(var i in rooms_rate_plans[rtype_id]) {
				if(rooms_rate_plans[rtype_id].hasOwnProperty(i)) {
					allrateplans += "<option value=\""+rooms_rate_plans[rtype_id][i].id+"\" title=\"ID: "+rooms_rate_plans[rtype_id][i].id+"\""+(rooms_rate_plans[rtype_id][i].hasOwnProperty('vcm_default') ? " selected" : "")+">"+rooms_rate_plans[rtype_id][i].name+"</option>";
				}
			}
			jQuery("#ch_rateplanid").append(allrateplans);
		}
	...
	function vcmSetRoomRatePlans(rtype_id) {
		...
		if(rooms_rate_plans.hasOwnProperty(rtype_id)) {
			...
			for(var i in rooms_rate_plans[rtype_id]) {
				if(rooms_rate_plans[rtype_id].hasOwnProperty(i)) {
					allrateplans += "<option value=\""+rooms_rate_plans[rtype_id][i].id+"\" title=\"Policy: "+rooms_rate_plans[rtype_id][i].policy+", Max Persons: "+rooms_rate_plans[rtype_id][i].max_persons+rooms_rate_plans[rtype_id][i].is_child_rate+"\""+(rooms_rate_plans[rtype_id][i].hasOwnProperty('vcm_default') ? " selected" : "")+">- "+rooms_rate_plans[rtype_id][i].name+rooms_rate_plans[rtype_id][i].is_child_rate+"</option>";
				}
			}
			jQuery("#ch_rateplanid").append(allrateplans);
		}
	...
	function vcmSetRoomRatePlans(rtype_id) {
		...
		if(rooms_rate_plans.hasOwnProperty(rtype_id)) {
			...
			for(var i in rooms_rate_plans[rtype_id]) {
				if(rooms_rate_plans[rtype_id].hasOwnProperty(i)) {
					allrateplans += "<option value=\""+rooms_rate_plans[rtype_id][i].id+"\" title=\"Breakfast: "+rooms_rate_plans[rtype_id][i].breakfast+", Max Persons: "+rooms_rate_plans[rtype_id][i].max_persons+"\">- "+rooms_rate_plans[rtype_id][i].name+"</option>";
				}
			}
			jQuery("#ch_rateplanid").append(allrateplans);
		}
	...
	foreach ($channel_pricing as $idroom => $rateplans) {
		...
		foreach ($rateplans['RatePlan'] as $rateplan) {
			$rateplan_info = array(
				'id' => $rateplan['id'],
				'name' => $rateplan['name'],
				'standard_occupation' => $rateplan['standard_occupation'],
				'max_occupation' => $rateplan['max_occupation']
			);
			$rooms_rate_plans[$idroom][$rateplan['id']] = $rateplan_info;
		}
	}
	...
	function vcmSetRoomRatePlans(rtype_id) {
		...
		if(rooms_rate_plans.hasOwnProperty(rtype_id)) {
			...
			for(var i in rooms_rate_plans[rtype_id]) {
				if(rooms_rate_plans[rtype_id].hasOwnProperty(i)) {
					allrateplans += "<option value=\""+rooms_rate_plans[rtype_id][i].id+"\" title=\"Standard Occupation: "+rooms_rate_plans[rtype_id][i].standard_occupation+", Max Occupation: "+rooms_rate_plans[rtype_id][i].max_occupation+"\">- "+rooms_rate_plans[rtype_id][i].name+"</option>";
				}
			}
			jQuery("#ch_rateplanid").append(allrateplans);
		}
	...
	foreach ($channel_pricing as $idroom => $rateplans) {
		...
		foreach ($rateplans['RatePlan'] as $rateplan) {
			$rateplan_info = array(
				'id' => $rateplan['id'],
				'name' => $rateplan['name'],
				'price_rule' => (array_key_exists('price_rule', $rateplan) ? ' ('.$rateplan['price_rule'].')' : ''),
				'occupancy' => (array_key_exists('min_occupancy', $rateplan) && array_key_exists('max_occupancy', $rateplan) ? $rateplan['min_occupancy'].' - '.$rateplan['min_occupancy'] : '')
			);
			if(array_key_exists('vcm_default', $rateplan)) {
				$rateplan_info['vcm_default'] = 1;
			}
			$rooms_rate_plans[$idroom][$rateplan['id']] = $rateplan_info;
			if(!$rate_plans_caught) {
				$rate_plans_sel .= '<option value="'.$rateplan['id'].'"'.(array_key_exists('vcm_default', $rateplan) ? ' selected="selected"' : '').' title="ID: '.$rateplan['id'].' - Occupancy: '.$rateplan_info['occupancy'].'">'.$rateplan['name'].(array_key_exists('price_rule', $rateplan_info) ? $rateplan_info['price_rule'] : '').'</option>'."\n";
			}
		}
		$rate_plans_caught = true;
	}
	...
	function vcmSetRoomRatePlans(rtype_id) {
		...
		if(rooms_rate_plans.hasOwnProperty(rtype_id)) {
			...
			for(var i in rooms_rate_plans[rtype_id]) {
				if(rooms_rate_plans[rtype_id].hasOwnProperty(i)) {
					allrateplans += "<option value=\""+rooms_rate_plans[rtype_id][i].id+"\" title=\"ID: "+rooms_rate_plans[rtype_id][i].id+" - Occupancy: "+rooms_rate_plans[rtype_id][i].occupancy+"\""+(rooms_rate_plans[rtype_id][i].hasOwnProperty('vcm_default') ? " selected" : "")+">"+rooms_rate_plans[rtype_id][i].name+rooms_rate_plans[rtype_id][i].price_rule+"</option>";
				}
			}
			jQuery("#ch_rateplanid").append(allrateplans);
		}
...
?>
		...
		<div id="vcmcopyrateshelper" style="display: none;">
			...
			<div class="vcm-copyrar-left">
				<div class="vcm-copyrar-new">
					<label for="vcm-copyrar-fromday"><?php echo (in_array($channel['uniquekey'], array(VikChannelManagerConfig::EXPEDIA, VikChannelManagerConfig::BOOKING, VikChannelManagerConfig::AGODA, VikChannelManagerConfig::YCS50)) ? JText::_('VCMCOPYRATESINVFROMOR') : JText::_('VCMCOPYRATESINVFROM')); ?></label>
				</div>
				<!--
	...
	jQuery("body").on("click", ".vcm-copy-ratesinv", function() {
		...
		if(in_array($channel['uniquekey'], array(VikChannelManagerConfig::BOOKING, VikChannelManagerConfig::EXPEDIA, VikChannelManagerConfig::AGODA, VikChannelManagerConfig::YCS50, VikChannelManagerConfig::BEDANDBREAKFASTIT, VikChannelManagerConfig::BEDANDBREAKFASTEU, VikChannelManagerConfig::BEDANDBREAKFASTNL))) {
			?>
