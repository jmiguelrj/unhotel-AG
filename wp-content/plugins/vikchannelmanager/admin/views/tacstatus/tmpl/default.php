...
<?php } else if( count($contents['data']['locations']) > 0 ) { 
	...
	<div class="vcmtacstatusdiv">
		...
		<?php if( !empty($status['business_listing_since']) && is_scalar($status['business_listing_since']) && strlen($status['business_listing_since']) > 0 ) { 
		...
		<?php if( !empty($status['tripconnect_since']) && is_scalar($status['tripconnect_since']) && strlen($status['tripconnect_since']) > 0 ) { 
		...
		<?php
		if (!empty($status['tripconnect_since']) && is_array($status['tripconnect_since']) && !empty($status['tripconnect_since']['year'])) { 
			$tripconnect_date = $status['tripconnect_since']['year'];
			$tripconnect_date .= !empty($status['tripconnect_since']['month']) ? '-' . $status['tripconnect_since']['month'] : '';
			$tripconnect_date .= !empty($status['tripconnect_since']['day']) ? '-' . $status['tripconnect_since']['day'] : '';
			?>
			<p class="vcmstatusinfo">
				<span class="vcmstatusinfolabel"><?php echo JText::_('VCMTACSTATUSTRIPDATE'); ?>:</span>
				<span class="vcmstatusinfoval"><?php echo $tripconnect_date; ?></span>
		...
		<?php if( !empty($status['tripconnect_last_active']) && is_scalar($status['tripconnect_last_active']) && strlen($status['tripconnect_last_active']) > 0 ) { 
