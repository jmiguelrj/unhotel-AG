				...
				<div class="vcm-custa-roomdetails">
					...
					foreach( $ca['details'] as $details ) { ?>
						<div class="vcm-custa-roomblock">
							...
							<?php } ?>
							<span class="vcm-custa-units-label"><i class="vboicn-stack"></i><?php echo JText::_('VCMOSUNITSONDATE'); ?></span>
				...
				<div class="vcm-custa-roomchannels">
					...
					<?php if( count($ca['channels']) > 0 ) { ?>
						<div class="vcm-custa-channelhead">
							<input type="button" class="btn" value="<?php echo JText::_('VCMOSCHECKALL'); ?>" onClick="jQuery('.check-<?php echo $idroom; ?>').prop('checked', true);"/>
							<input type="button" class="btn" value="<?php echo JText::_('VCMOSUNCHECKALL'); ?>" onClick="jQuery('.check-<?php echo $idroom; ?>').prop('checked', false);"/>
						</div>
					<?php } ?>
					...
					foreach ($ca['channels'] as $ch) {
						...
						if ($ch['uniquekey'] == VikChannelManagerConfig::AIRBNBAPI) {
							$ch['name'] = 'Airbnb';
						} elseif ($ch['uniquekey'] == VikChannelManagerConfig::GOOGLEHOTEL) {
							$ch['name'] = 'Google Hotel';
						} elseif ($ch['uniquekey'] == VikChannelManagerConfig::GOOGLEVR) {
							$ch['name'] = 'Google VR';
						}
						?>
