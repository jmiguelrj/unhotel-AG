...
class VikChannelManagerViewEgpromomanage extends JViewUI {
	...
	function display($tpl = null) {
		...
		if (!empty($cid[0])) {
			...
			if (!$totvars || !count($totvars)) {
				...
				$mainframe->redirect("index.php?option=com_vikbooking&task=egpromo");
				exit;
			}
		}
	...
	protected function addToolBar() {
		...
		if (!empty($cid[0])) {
			//edit
			JToolBarHelper::title(JText::sprintf('VCMMAINTCHEDITPROMOTION', 'Expedia'), 'vikchannelmanager');
			if (JFactory::getUser()->authorise('core.edit', 'com_vikchannelmanager')) {
				JToolBarHelper::save( 'egpromo.updatepromo', JText::_('SAVECLOSE'));
				JToolBarHelper::spacer();
			}
			JToolBarHelper::cancel( 'cancelegpromo', JText::_('CANCEL'));
			JToolBarHelper::spacer();
		} else {
			//new
			JToolBarHelper::title(JText::sprintf('VCMMAINTCHNEWPROMOTION', 'Expedia'), 'vikchannelmanager');
			if (JFactory::getUser()->authorise('core.create', 'com_vikchannelmanager')) {
				JToolBarHelper::save('egpromo.savepromo', JText::_('SAVE'));
				JToolBarHelper::spacer();
			}
			JToolBarHelper::cancel( 'cancelegpromo', JText::_('CANCEL'));
			JToolBarHelper::spacer();
