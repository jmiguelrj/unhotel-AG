	...
	function vcmValidateField (el, id) {
		if(jQuery.trim(el.value).length) {
			...
			if(jQuery.trim(xrefid).length && jQuery.trim(rdate).length) {
				jQuery("#esitvalidate"+id).html("<img src='<?php echo addslashes(VCM_ADMIN_URI.'assets/css/images/enabled.png'); ?>' style='cursor: pointer;' onclick='javascript: vcmHideEsit(this);'/>");
				if(parseInt(el.value) == 0) {
					jQuery("#esitvalidate"+id).append("<span class='vcmesitinfo'><?php echo addslashes(JText::_('VCMROOMWILLBECLOSED')); ?></span>");
				}
			}else {
				jQuery("#esitvalidate"+id).html("<img src='<?php echo addslashes(VCM_ADMIN_URI.'assets/css/images/error.png'); ?>' style='cursor: pointer;' onclick='javascript: vcmHideEsit(this);'/>");
			}
		}else {
		...
		<table class="adminform">
			...
			for($i = 1; $i <= 14; $i++) { ?>
				<tr>
					...
					<td width="150px" style="text-align: left;"><?php echo JText::_('VCMCUSTADATE'); ?> <?php echo JHTML::_('calendar', '', 'custdate[]', 'custdate'.$i, $df, array('class'=>'', 'size'=>'10',  'maxlength'=>'19', 'todayBtn' => 'true')); ?></td>
