			...
			foreach ($rooms[0]['RatePlan'] as $rateplan) {
				...
				if (array_key_exists('Rate', $rateplan) && count($rateplan['Rate']) > 0) {
					...
					foreach ($rateplan['Rate'] as $occkey => $rate) {
						...
						$occ = $occ_parts[1];
						$rate_plans .= '<span class="vcmrarratesp">'.JText::sprintf('VCMRARRATEPEROCCUPANCY', $occ).' <span class="vcmrarcurrency">'.$currency.'</span> <input type="text" size="5" name="rateplan_'.$day.'_'.$rooms[0]['id'].'_'.$rateplan['id'].'_'.$occ.'" value="'.$rate.'"/></span>'."\n";
					}
					$rate_plans .= '</div>'."\n";
					//Comparison with IBE
					if (@count($comparison[$day][$rooms[0]['id']]) > 0) {
			...
			foreach($rooms[$j]['RatePlan'] as $rateplan) {
				...
				if (array_key_exists('Rate', $rateplan) && count($rateplan['Rate']) > 0) {
					...
					foreach ($rateplan['Rate'] as $occkey => $rate) {
						$occ_parts = explode('_', $occkey);
						$occ = $occ_parts[1];
						$rate_plans .= '<span class="vcmrarratesp">'.JText::sprintf('VCMRARRATEPEROCCUPANCY', $occ).' <span class="vcmrarcurrency">'.$currency.'</span> <input type="text" size="5" name="rateplan_'.$day.'_'.$rooms[$j]['id'].'_'.$rateplan['id'].'_'.$occ.'" value="'.$rate.'"/></span>'."\n";
					}
					$rate_plans .= '</div>'."\n";
					//Comparison with IBE
					if (@count($comparison[$day][$rooms[$j]['id']]) > 0) {
