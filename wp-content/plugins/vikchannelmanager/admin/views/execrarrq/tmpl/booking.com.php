		...
		if (count($restrictions) > 0) {
			...
			foreach ($restrictions as $rpid => $restriction) {
				if ($was_updated) {
					...
					if (isset($rar_updates[$day][$rooms[0]['id']]['data']['RatePlan'][$rpid]['Restrictions']['minimumstay']) && strlen($rar_updates[$day][$rooms[0]['id']]['data']['RatePlan'][$rpid]['Restrictions']['minimumstay'])) {
						$restriction['minimumstay'] = $rar_updates[$day][$rooms[0]['id']]['data']['RatePlan'][$rpid]['Restrictions']['minimumstay'];
					}
					if (isset($rar_updates[$day][$rooms[0]['id']]['data']['RatePlan'][$rpid]['Restrictions']['maximumstay']) && strlen($rar_updates[$day][$rooms[0]['id']]['data']['RatePlan'][$rpid]['Restrictions']['maximumstay'])) {
	...
	<tr class="row<?php echo $k; ?>">
		...
		if (count($restrictions) > 0) {
			...
			foreach ($restrictions as $rpid => $restriction) {
				if ($was_updated) {
					//previously updated records
					if (isset($rar_updates[$day][$rooms[$j]['id']]['data']['RatePlan'][$rpid]['Restrictions']['minimumstay']) && strlen($rar_updates[$day][$rooms[$j]['id']]['data']['RatePlan'][$rpid]['Restrictions']['minimumstay'])) {
						$restriction['minimumstay'] = $rar_updates[$day][$rooms[$j]['id']]['data']['RatePlan'][$rpid]['Restrictions']['minimumstay'];
					}
					if (isset($rar_updates[$day][$rooms[$j]['id']]['data']['RatePlan'][$rpid]['Restrictions']['maximumstay']) && strlen($rar_updates[$day][$rooms[$j]['id']]['data']['RatePlan'][$rpid]['Restrictions']['maximumstay'])) {
...
if (tot_parent_rates > 0) {
	var ch_controls = "<select id=\"vcm-channel-controls-pmodeltype\" onchange=\"vcmChoosePricingModel(this.value);\"><option value=\"person\" title=\"<?php echo addslashes(JText::_('VCMRARBCOMPERSONPMODELTIP')); ?>\"><?php echo addslashes(JText::_('VCMRARBCOMPERSONPMODEL')); ?></option><option value=\"los\" title=\"<?php echo addslashes(JText::_('VCMRARBCOMLOSPMODELTIP')); ?>\"><?php echo addslashes(JText::_('VCMRARBCOMLOSPMODEL')); ?></option><option value=\"any\"><?php echo addslashes(JText::_('VCMRARBCOMANYPMODEL')); ?></option><select>";
...
if (!empty($cookie_ariprmodel)) {
	...
	//October 2016: always trigger the change because now the first value is "person" (Default Pricing)
	?>
	jQuery("#vcm-channel-controls-pmodeltype").trigger("change");
	<?php
}
?>
}
...
function vcmChoosePricingModel(pmodel) {
	...
	document.cookie = "vcmAriPrModel<?php echo $channel['uniquekey']; ?>=" + pmodel + "; expires=" + nd.toUTCString() + "; path=/; SameSite=Lax";
<?php
if (empty($cookie_ariprmodel)) {
	?>
...
jQuery("body").on("click", "button.vcm-addlos-rateplanibe-apply", function() {
	...
	if (rplan_id.length > 0 && last_ibe_los.length > 0) {
		var obj_res = JSON.parse(last_ibe_los);
