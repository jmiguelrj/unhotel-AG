		...
		if ($tot_rate_plans > 0) {
			foreach ($rooms[0]['RatePlan'] as $rateplan) {
				...
				if (array_key_exists('Rate', $rateplan) && count($rateplan['Rate']) > 0) {
					...
					//Double Occupancy
					if($inventory_loaded === false && (array_key_exists('RoomInfo', $ota_rate_plan) && array_key_exists('max_persons', $ota_rate_plan['RoomInfo']) && $ota_rate_plan['RoomInfo']['max_persons'] < 2)) {
					...
					//Extra Bed
					if(array_key_exists('extraBedRate', $rateplan['Rate'])) {
						$rate_plans .= '<span class="vcmrarratesp">'.JText::_('VCMAGODARARRATEEXTRABED').' <span class="vcmrarcurrency">'.$currency.'</span> <input type="text" size="5" name="rateplan_'.$day.'_'.$rooms[0]['id'].'_'.$rateplan['id'].'_ExtraBed" value="'.$rateplan['Rate']['extraBedRate'].'" placeholder="0.00"/></span>'."\n";
					}
					$rate_plans .= '</div>'."\n";
					//Comparison with IBE
					if (@count($comparison[$day][$rooms[0]['id']]) > 0) {
						...
						if (count($tp_tabs) > 0) {
							...
							foreach ($tp_tabs as $tpid => $fare) {
								...
								foreach ($rateplan['Rate'] as $kr => $rate) {
									...
									if(!in_array($rate_type, array('singleRate', 'doubleRate'))) {
										continue;
									}
									foreach ($comparison[$day][$rooms[0]['id']] as $nights => $prices) {
										if (is_numeric($nights)) {
		...
		if($tot_rate_plans > 0) {
			foreach($rooms[$j]['RatePlan'] as $rateplan) {
				...
				if (array_key_exists('Rate', $rateplan) && count($rateplan['Rate']) > 0) {
					...
					//Extra Bed
					if(array_key_exists('extraBedRate', $rateplan['Rate'])) {
						$rate_plans .= '<span class="vcmrarratesp">'.JText::_('VCMAGODARARRATEEXTRABED').' <span class="vcmrarcurrency">'.$currency.'</span> <input type="text" size="5" name="rateplan_'.$day.'_'.$rooms[$j]['id'].'_'.$rateplan['id'].'_ExtraBed" value="'.$rateplan['Rate']['extraBedRate'].'" placeholder="0.00"/></span>'."\n";
					}
					$rate_plans .= '</div>'."\n";
					//Comparison with IBE
					if (@count($comparison[$day][$rooms[$j]['id']]) > 0) {
