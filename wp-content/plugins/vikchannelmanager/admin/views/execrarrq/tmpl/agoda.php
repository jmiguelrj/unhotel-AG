...
foreach ($rars['AvailRate'] as $day => $rooms) {
	...
	?>
		...
		$rate_plans = '';
		$restrictions = array();
		//Promotion Blackout and Breakfast Included
		$extra_restrictions = array();
		$extra_restrictions['BreakfastIncluded'] = array_key_exists('BreakfastIncluded', $rooms[0]['Inventory']) && $rooms[0]['Inventory']['BreakfastIncluded'] == 'True' ? true : false;
		$extra_restrictions['PromotionBlackout'] = array_key_exists('PromotionBlackout', $rooms[0]['Inventory']) && $rooms[0]['Inventory']['PromotionBlackout'] == 'True' ? true : false;
		//
		...
		if($tot_rate_plans > 0) {
			foreach($rooms[0]['RatePlan'] as $rateplan) {
				...
				$rate_plans .= '<span class="'.(!empty($rate_plan_tip) ? 'vcmshowtip ' : '').'vcmrateplansp vcmrateplanon" id="rateplanstatus'.$day.$rateplan['id'].'" title="'.$rate_plan_tip.'">'.JText::sprintf('VCMRARRATEPLANTITLE', (array_key_exists($rateplan['id'], $ota_rate_plan['RatePlan']) ? $ota_rate_plan['RatePlan'][$rateplan['id']]['name'].' ' : ''), $rateplan['id']).'</span><span class="vcmrar-spacer"></span><input type="hidden" name="rateplanstatus'.$day.$rateplan['id'].'" value="" id="inprateplanstatus'.$day.$rateplan['id'].'"/>'."\n";
				...
				if (array_key_exists('Rate', $rateplan) && count($rateplan['Rate']) > 0) {
					...
					//Double Occupancy
					if($inventory_loaded === false && (array_key_exists('RoomInfo', $ota_rate_plan) && array_key_exists('MaxOccupancy', $ota_rate_plan['RoomInfo']) && $ota_rate_plan['RoomInfo']['MaxOccupancy'] < 2)) {
					...
					//Full Rate
					if($inventory_loaded === false && (array_key_exists('RoomInfo', $ota_rate_plan) && array_key_exists('MaxOccupancy', $ota_rate_plan['RoomInfo']) && $ota_rate_plan['RoomInfo']['MaxOccupancy'] < 3)) {
					...
					//Extra Person
					if($inventory_loaded === false && (array_key_exists('RoomInfo', $ota_rate_plan) && array_key_exists('MaxOccupancy', $ota_rate_plan['RoomInfo']) && $ota_rate_plan['RoomInfo']['MaxOccupancy'] <= 1)) {
					...
					//Extra Child
					if($inventory_loaded === false && (array_key_exists('RoomInfo', $ota_rate_plan) && array_key_exists('MaxChildren', $ota_rate_plan['RoomInfo']) && $ota_rate_plan['RoomInfo']['MaxChildren'] < 1)) {
					...
					//Extra Bed
					if($inventory_loaded === false && (array_key_exists('RoomInfo', $ota_rate_plan) && array_key_exists('MaxExtraBed', $ota_rate_plan['RoomInfo']) && $ota_rate_plan['RoomInfo']['MaxExtraBed'] < 1)) {
		...
		if(count($restrictions) > 0) {
			...
			foreach ($restrictions as $rpid => $restriction) {
				...
				<?php
				if(count($extra_restrictions) > 0) {
					?>
					<div class="vcmrarrestr-arrivdep">
						<span class="vcmrarrestr-tag <?php echo $extra_restrictions['BreakfastIncluded'] === true ? 'vcmtagenabled' : 'vcmtagdisabled'; ?>" onclick="toggleRestrBreakfast(<?php echo $extra_restrictions['BreakfastIncluded'] === true ? '1' : '0'; ?>, '<?php echo $day; ?>', '<?php echo $rpid; ?>');" id="restrplanbreakfast<?php echo $day.$rpid; ?>"><?php echo JText::_('VCMRARRESTRBREAKFASTINCLUDED'); ?></span><input type="hidden" name="restrplanbreakfast<?php echo $day.$rpid; ?>" value="" id="inprestrplanbreakfast<?php echo $day.$rpid; ?>"/>
						<span class="vcmrarrestr-tag <?php echo $extra_restrictions['PromotionBlackout'] === true ? 'vcmtagenabled' : 'vcmtagdisabled'; ?>" onclick="toggleRestrPromoblackout(<?php echo $extra_restrictions['PromotionBlackout'] === true ? '1' : '0'; ?>, '<?php echo $day; ?>', '<?php echo $rpid; ?>');" id="restrplanpromoblackout<?php echo $day.$rpid; ?>"><?php echo JText::_('VCMRARRESTRPROMOBLACKOUT'); ?></span><input type="hidden" name="restrplanpromoblackout<?php echo $day.$rpid; ?>" value="" id="inprestrplanpromoblackout<?php echo $day.$rpid; ?>"/>
					</div>
					<?php
				}
				?>
	...
	<tr class="row<?php echo $k; ?>">
		...
		<td class="center" align="center"><img src="<?php echo VCM_ADMIN_URI; ?>assets/css/images/<?php echo $rooms[$j]['closed'] == 'true' ? 'disabled' : 'enabled'; ?>.png" class="imgtoggle" onclick="toggleRoomStatus(<?php echo $rooms[$j]['closed'] == 'true' ? '0' : '1'; ?>, '<?php echo $day; ?>', '<?php echo $rooms[$j]['id'] ?>');"/><div class="vcmrar-newroomstatus" id="divroomstatus<?php echo $day.$rooms[$j]['id']; ?>"><input type="hidden" name="<?php echo 'roomstatus_'.$day.'_'.$rooms[$j]['id']; ?>" value="" id="roomstatus<?php echo $day.$rooms[$j]['id']; ?>"/></div></td>
		<td class="center" align="center">
		...
		//Promotion Blackout and Breakfast Included
		$extra_restrictions = array();
		$extra_restrictions['BreakfastIncluded'] = array_key_exists('BreakfastIncluded', $rooms[$j]['Inventory']) && $rooms[$j]['Inventory']['BreakfastIncluded'] == 'True' ? true : false;
		$extra_restrictions['PromotionBlackout'] = array_key_exists('PromotionBlackout', $rooms[$j]['Inventory']) && $rooms[$j]['Inventory']['PromotionBlackout'] == 'True' ? true : false;
		//
...
function toggleRestrBreakfast(status, date, rpid) {
	var setclass = status == 1 ? "vcmrarrestr-tag-todisabled" : "vcmrarrestr-tag-toenabled";
	var cur_status = jQuery("#inprestrplanbreakfast"+date+rpid).val();
	var opposite_status = status == 1 ? 0 : 1;
	var setstatus = cur_status.length == 0 ? opposite_status : '';
	if(cur_status.length == 0) {
		jQuery("#inprestrplanbreakfast"+date+rpid).val(setstatus);
		jQuery("#restrplanbreakfast"+date+rpid).removeClass("vcmrarrestr-tag-toenabled").removeClass("vcmrarrestr-tag-todisabled").addClass(setclass);
	}else {
		jQuery("#inprestrplanbreakfast"+date+rpid).val("");
		jQuery("#restrplanbreakfast"+date+rpid).removeClass("vcmrarrestr-tag-toenabled").removeClass("vcmrarrestr-tag-todisabled");
	}
	jQuery("#inprestrplanbreakfast"+date+rpid).trigger("change");
}
function toggleRestrPromoblackout(status, date, rpid) {
	var setclass = status == 1 ? "vcmrarrestr-tag-todisabled" : "vcmrarrestr-tag-toenabled";
	var cur_status = jQuery("#inprestrplanpromoblackout"+date+rpid).val();
	var opposite_status = status == 1 ? 0 : 1;
	var setstatus = cur_status.length == 0 ? opposite_status : '';
	if(cur_status.length == 0) {
		jQuery("#inprestrplanpromoblackout"+date+rpid).val(setstatus);
		jQuery("#restrplanpromoblackout"+date+rpid).removeClass("vcmrarrestr-tag-toenabled").removeClass("vcmrarrestr-tag-todisabled").addClass(setclass);
	}else {
		jQuery("#inprestrplanpromoblackout"+date+rpid).val("");
		jQuery("#restrplanpromoblackout"+date+rpid).removeClass("vcmrarrestr-tag-toenabled").removeClass("vcmrarrestr-tag-todisabled");
	}
	jQuery("#inprestrplanpromoblackout"+date+rpid).trigger("change");
}
