	...
	?>
		...
		if($tot_rate_plans > 0) {
			foreach($rooms[0]['RatePlan'] as $rateplan) {
				...
				$rate_plans .= '<span class="'.(!empty($rate_plan_tip) ? 'vcmshowtip ' : '').'vcmrateplansp '.(isset($rateplan['Restrictions']) && isset($rateplan['Restrictions']['closed']) && $rateplan['Restrictions']['closed'] == 'true' ? 'vcmrateplanoff' : 'vcmrateplanon').'" id="rateplanstatus'.$day.$rateplan['id'].'" title="'.$rate_plan_tip.'" onclick="toggleRatePlanStatus('.(isset($rateplan['Restrictions']) && isset($rateplan['Restrictions']['closed']) && $rateplan['Restrictions']['closed'] == 'true' ? '0' : '1').', \''.$day.'\', \''.$rateplan['id'].'\');">'.JText::sprintf('VCMRARRATEPLANTITLE', (array_key_exists($rateplan['id'], $ota_rate_plan['RatePlan']) ? $ota_rate_plan['RatePlan'][$rateplan['id']]['name'].' ' : ''), $rateplan['id']).'</span><span class="vcmrar-spacer"></span><input type="hidden" name="rateplanstatus'.$day.$rateplan['id'].'" value="" id="inprateplanstatus'.$day.$rateplan['id'].'"/>'."\n";
				...
				if (array_key_exists('Rate', $rateplan) && count($rateplan['Rate']) > 0) {
					...
					$currency = $inventory_loaded === true ? $rateplan['currency'] : $currency;
					//no matter if the pricing is PerRoomPerNight or PerPersonPerNight, we should loop over the Rate array and print a cost for every adult occupancy (if PerRoomPerNight there will be just the key 'price_n')
					$guests_rates_count = array();
					if ($inventory_loaded === true || !isset($rateplan['charge_type'])) {
						foreach ($rateplan['Rate'] as $pricek => $pricev) {
							...
							if ((int)$price_parts[1] > 1) {
								$price_name = JText::sprintf('VCMDESPEGARRARRATEGUESTS', (int)$price_parts[1]);
							} else {
								$price_name = JText::_('VCMDESPEGARRARRATEGUEST');
							}
							$rate_plans .= '<span class="vcmrarratesp">'.$price_name.' <span class="vcmrarcurrency">'.$currency.'</span> <input type="text" size="5" name="rateplan_'.$day.'_'.$rooms[0]['id'].'_'.$rateplan['id'].'_Price'.$price_parts[1].'" value="'.$pricev.'" placeholder="0.00"/></span>'."\n";
						}
					}
					//if not charge_type, probably because of no inventory, read it from the mapping data and set it
					if (!isset($rateplan['charge_type']) && isset($ota_rate_plan['RatePlan']) && isset($ota_rate_plan['RatePlan'][$rateplan['id']])) {
						if (isset($ota_rate_plan['RatePlan'][$rateplan['id']]['ChargeTypeCode']) && (int)$ota_rate_plan['RatePlan'][$rateplan['id']]['ChargeTypeCode'] == 19) {
							$rateplan['charge_type'] = 'PerRoomPerNight';
						} elseif (isset($ota_rate_plan['RatePlan'][$rateplan['id']]['ChargeTypeCode']) && (int)$ota_rate_plan['RatePlan'][$rateplan['id']]['ChargeTypeCode'] == 21) {
							$rateplan['charge_type'] = 'PerPersonPerNight';
						}
					}
					...
					if (isset($rateplan['charge_type']) && $rateplan['charge_type'] == 'PerRoomPerNight' && isset($ota_rate_plan['RoomInfo'])) {
						if (!in_array((int)$ota_rate_plan['RoomInfo']['StandardOccupancy'], $guests_rates_count)) {
							$price_name = '';
							if ((int)$ota_rate_plan['RoomInfo']['StandardOccupancy'] > 1) {
								$price_name = JText::sprintf('VCMDESPEGARRARRATEGUESTS', (int)$ota_rate_plan['RoomInfo']['StandardOccupancy']);
							} else {
								$price_name = JText::_('VCMDESPEGARRARRATEGUEST');
							}
							$rate_plans .= '<span class="vcmrarratesp">'.$price_name.' <span class="vcmrarcurrency">'.$currency.'</span> <input type="text" size="5" name="rateplan_'.$day.'_'.$rooms[0]['id'].'_'.$rateplan['id'].'_Price'.(int)$ota_rate_plan['RoomInfo']['StandardOccupancy'].'" value="" placeholder="0.00"/></span>'."\n";
						}
					}
					...
					if (isset($rateplan['charge_type']) && $rateplan['charge_type'] == 'PerPersonPerNight' && isset($ota_rate_plan['RoomInfo'])) {
						...
						if ((int)$ota_rate_plan['RoomInfo']['MaxAdultOccupancy'] > $max_guests_inp) {
							...
							for ($num = $max_guests_inp; $num <= (int)$ota_rate_plan['RoomInfo']['MaxAdultOccupancy']; $num++) {
								$price_name = '';
								if ($num > 1) {
									$price_name = JText::sprintf('VCMDESPEGARRARRATEGUESTS', $num);
								} else {
									$price_name = JText::_('VCMDESPEGARRARRATEGUEST');
								}
								$rate_plans .= '<span class="vcmrarratesp">'.$price_name.' <span class="vcmrarcurrency">'.$currency.'</span> <input type="text" size="5" name="rateplan_'.$day.'_'.$rooms[0]['id'].'_'.$rateplan['id'].'_Price'.$num.'" value="" placeholder="0.00"/></span>'."\n";
							}
						}
					}
					//always give the possibility to set the cost for an additional adult
					$rate_plans .= '<span class="vcmrarratesp">'.JText::_('VCMDESPEGARRARRATEEXTRAGUEST').' <span class="vcmrarcurrency">'.$currency.'</span> <input type="text" size="5" name="rateplan_'.$day.'_'.$rooms[0]['id'].'_'.$rateplan['id'].'_ExtraAdult" value="'.(isset($rateplan['Rate']['extra_adults']) ? $rateplan['Rate']['extra_adults'] : '').'" placeholder="0.00"/></span>'."\n";
					$rate_plans .= '</div>'."\n";
					//Comparison with IBE
					if (@count($comparison[$day][$rooms[0]['id']]) > 0) {
			...
			foreach ($restrictions as $rpid => $restriction) {
				...
				<div class="vcmrarrestr-block">
					...
					<div class="vcmrarrestr-arrivdep">
						<span class="vcmrarrestr-tag <?php echo $restriction['closedonarrival'] == 'true' ? 'vcmtagenabled' : 'vcmtagdisabled'; ?>" onclick="toggleRestrArrivalStatus(<?php echo $restriction['closedonarrival'] == 'true' ? '1' : '0'; ?>, '<?php echo $day; ?>', '<?php echo $rpid; ?>');" id="restrplanarrival<?php echo $day.$rpid; ?>"><?php echo JText::_('VCMRARRESTRCLOSEDARRIVAL'); ?></span><input type="hidden" name="restrplanarrival<?php echo $day.$rpid; ?>" value="" id="inprestrplanarrival<?php echo $day.$rpid; ?>"/>
						<span class="vcmrarrestr-tag <?php echo $restriction['closedondeparture'] == 'true' ? 'vcmtagenabled' : 'vcmtagdisabled'; ?>" onclick="toggleRestrDepartureStatus(<?php echo $restriction['closedondeparture'] == 'true' ? '1' : '0'; ?>, '<?php echo $day; ?>', '<?php echo $rpid; ?>');" id="restrplandeparture<?php echo $day.$rpid; ?>"><?php echo JText::_('VCMRARRESTRCLOSEDDEPARTURE'); ?></span><input type="hidden" name="restrplandeparture<?php echo $day.$rpid; ?>" value="" id="inprestrplandeparture<?php echo $day.$rpid; ?>"/>
					</div>
				</div>
				<?php
			}
		...
		if($tot_rate_plans > 0) {
			foreach($rooms[$j]['RatePlan'] as $rateplan) {
				...
				if (array_key_exists('Rate', $rateplan) && count($rateplan['Rate']) > 0) {
					...
					//if not charge_type, probably because of no inventory, read it from the mapping data
					if (!isset($rateplan['charge_type']) && isset($ota_rate_plan['RatePlan']) && isset($ota_rate_plan['RatePlan'][$rateplan['id']])) {
						if (isset($ota_rate_plan['RatePlan'][$rateplan['id']]['ChargeTypeCode']) && (int)$ota_rate_plan['RatePlan'][$rateplan['id']]['ChargeTypeCode'] == 19) {
							$rateplan['charge_type'] = 'PerRoomPerNight';
						} elseif (isset($ota_rate_plan['RatePlan'][$rateplan['id']]['ChargeTypeCode']) && (int)$ota_rate_plan['RatePlan'][$rateplan['id']]['ChargeTypeCode'] == 21) {
...
function toggleRoomStatus(status, date, roomid) {
	/* Despegar does not support closures at room level, only at room-rateplan level. */
