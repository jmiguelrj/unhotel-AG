		...
		if($tot_rate_plans > 0) {
			foreach($rooms[0]['RatePlan'] as $rateplan) {
				...
				if (array_key_exists('Rate', $rateplan) && count($rateplan['Rate']) > 0) {
					...
					//Single Occupancy
					$rate_plans .= '<span class="vcmrarratesp">'.JText::_('VCMAGODARARRATESINGLE').' <span class="vcmrarcurrency">'.$currency.'</span> <input type="text" size="5" name="rateplan_'.$day.'_'.$rooms[0]['id'].'_'.$rateplan['id'].'_SingleRate" value="'.$rateplan['Rate']['adults1'].'" placeholder="0.00"/></span>'."\n";
					//Full Rate
					$rate_plans .= '<span class="vcmrarratesp">'.JText::_('VCMAGODARARRATEFULL').' <span class="vcmrarcurrency">'.$currency.'</span> <input type="text" size="5" name="rateplan_'.$day.'_'.$rooms[0]['id'].'_'.$rateplan['id'].'_FullRate" value="'.$rateplan['Rate']['price'].'" placeholder="0.00"/></span>'."\n";
					$rate_plans .= '</div>'."\n";
					//Comparison with IBE
					if (@count($comparison[$day][$rooms[0]['id']]) > 0) {
						...
						if (count($tp_tabs) > 0) {
							...
							foreach ($tp_tabs as $tpid => $fare) {
								...
								foreach ($rateplan['Rate'] as $kr => $rate) {
									...
									$rate_type = $kr;
									$occ = 0;
									if ($rate_type == 'adults1') {
										$occ = 1;
									} elseif ($rate_type == 'price') {
										$occ = 2;
									}
									if(!in_array($rate_type, array('adults1', 'price'))) {
										continue;
									}
									foreach ($comparison[$day][$rooms[0]['id']] as $nights => $prices) {
										if (is_numeric($nights)) {
		...
		if (count($restrictions) > 0) {
			...
			foreach ($restrictions as $rpid => $restriction) {
				...
				<div class="vcmrarrestr-block">
					...
					<input type="hidden" name="<?php echo 'restrmax_'.$rooms[0]['id'].'_'.$day.'_'.$rpid; ?>" size="3" value="0"/>
					<div class="vcmrarrestr-arrivdep">
						...
						<input type="hidden" name="restrplandeparture<?php echo $day.$rpid; ?>" value="" id="inprestrplandeparture<?php echo $day.$rpid; ?>"/>
					</div>
				</div>
				<?php
			}
		...
		if ($tot_rate_plans > 0) {
			foreach ($rooms[$j]['RatePlan'] as $rateplan) {
				...
				if (array_key_exists('Rate', $rateplan) && count($rateplan['Rate']) > 0) {
					...
					$currency = $inventory_loaded === true ? $rateplan['Rate']['currency'] : $currency;
					//Single Occupancy
					$rate_plans .= '<span class="vcmrarratesp">'.JText::_('VCMAGODARARRATESINGLE').' <span class="vcmrarcurrency">'.$currency.'</span> <input type="text" size="5" name="rateplan_'.$day.'_'.$rooms[$j]['id'].'_'.$rateplan['id'].'_SingleRate" value="'.$rateplan['Rate']['adults1'].'" placeholder="0.00"/></span>'."\n";
					//Full Rate
					$rate_plans .= '<span class="vcmrarratesp">'.JText::_('VCMAGODARARRATEFULL').' <span class="vcmrarcurrency">'.$currency.'</span> <input type="text" size="5" name="rateplan_'.$day.'_'.$rooms[$j]['id'].'_'.$rateplan['id'].'_FullRate" value="'.$rateplan['Rate']['price'].'" placeholder="0.00"/></span>'."\n";
					$rate_plans .= '</div>'."\n";
					//Comparison with IBE
					if (@count($comparison[$day][$rooms[$j]['id']]) > 0) {
						//build tabs for each type of price
		...
		if(count($restrictions) > 0) {
			...
			foreach ($restrictions as $rpid => $restriction) {
				...
				<div class="vcmrarrestr-block">
					...
					<input type="hidden" name="<?php echo 'restrmax_'.$rooms[$j]['id'].'_'.$day.'_'.$rpid; ?>" size="3" value="0"/>
					<div class="vcmrarrestr-arrivdep">
						<span class="vcmrarrestr-tag <?php echo $restriction['closedToArrival'] == 'true' ? 'vcmtagenabled' : 'vcmtagdisabled'; ?>" onclick="toggleRestrArrivalStatus(<?php echo $restriction['closedToArrival'] == 'true' ? '1' : '0'; ?>, '<?php echo $day; ?>', '<?php echo $rpid; ?>');" id="restrplanarrival<?php echo $day.$rpid; ?>"><?php echo JText::_('VCMRARRESTRCLOSEDARRIVAL'); ?></span><input type="hidden" name="restrplanarrival<?php echo $day.$rpid; ?>" value="" id="inprestrplanarrival<?php echo $day.$rpid; ?>"/>
						<input type="hidden" name="restrplandeparture<?php echo $day.$rpid; ?>" value="" id="inprestrplandeparture<?php echo $day.$rpid; ?>"/>
					</div>
