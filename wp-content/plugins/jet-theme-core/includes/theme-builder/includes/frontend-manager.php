...
class Frontend_Manager {
	...
	public function get_matched_page_template_layouts( $single = false ) {
		...
		$page_template_id = $this->get_primary_page_template_id_by_conditions( $matched_page_template_conditions );
	...
	/**
	 * @param $page_template_conditions
	 * @return int|string|null
	 */
	public function get_primary_page_template_id_by_conditions( $page_template_conditions = [] ) {

		uasort($page_template_conditions, function ( $templateA, $templateB ) {
			...
			$templateALength = count( $templateA );
			$templateBLength = count( $templateB );

			$filteredTemplateA = array_filter( $templateA, function( $item ) {
				return filter_var( $item['include'], FILTER_VALIDATE_BOOLEAN );
			} );

			$filteredTemplateB = array_filter( $templateB, function( $item ) {
				return filter_var( $item['include'], FILTER_VALIDATE_BOOLEAN );
			} );

			$priorityA = round( array_sum( array_column( $filteredTemplateA, 'priority') ) / count( $filteredTemplateA ) );
			$priorityB = round( array_sum( array_column( $filteredTemplateB, 'priority') ) / count( $filteredTemplateB ) );
		...
		return array_key_first( $page_template_conditions );
	}
		...
		foreach ( $all_site_conditions as $page_template_id => $page_template_condition_data ) {
			...
			if ( 'and' === $page_template_relation_type ) {
				...
				$is_excluded = ( ! empty( $excludes_matchs ) &&
					apply_filters(
						'jet-theme-core/page-template-condition/is_excluded/and', ! in_array( false, $excludes_matchs ),
						$excludes_matchs,
						$page_template_id
					) ) ? true : false;
			} else {
				...
				$is_excluded = ( ! empty( $excludes_matchs ) &&
					apply_filters(
						'jet-theme-core/page-template-condition/is_excluded/or', in_array( true, $excludes_matchs ),
						$excludes_matchs,
						$page_template_id
					) ) ? true : false;
			}
