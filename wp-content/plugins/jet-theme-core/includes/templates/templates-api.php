...
if ( ! class_exists( 'Jet_Theme_Core_Templates_Api' ) ) {
	...
	class Jet_Theme_Core_Templates_Api {
		...
		public function get_template_tabs() {
			...
			$tabs = jet_theme_core()->structures->get_structures_for_library();
		...
		public function clone_template() {
			...
			if ( ! isset( $_REQUEST['nonce'] ) || ! wp_verify_nonce( $_REQUEST['nonce'], 'jet-theme-core-elementor-editor-nonce' ) ) {
				wp_send_json( [
					'status'  => 'error',
					'message' => __( 'Page has expired. Please reload this page.', 'jet-theme-core' ),
				] );
			}
